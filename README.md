...
a real shipped proof. The verification itself runs fully offline — only the
one-time clone and build touch the network. A cold `rust:1-bookworm` container did
the whole thing (clone → build → **VERIFIED**) in **58 seconds**:
...
```
✓ VERIFIED — this record's structure, identity binding, and post-quantum signature all verify. Scope: the record's own integrity — no seal or anchor chain was supplied, so no chain claim is made.
...
it did **not** — it will not fake a green. Tamper with the sample and it prints
`✗ FAILED`; leave out a required proof and it prints `⚠ UNPROVEN`.

### Or skip the clone — install from crates.io (fastest path)

`elara-verify` is published on crates.io, so you can skip the git clone: install the
verifier straight from the registry and fetch just the sample.

```bash
cargo install elara-verify --features cli
curl -fsSL -o sample-record.json https://raw.githubusercontent.com/navigatorbuilds/elara-mesh/main/examples/verify/sample-record.json
elara-verify sample-record.json
```

Same `VERDICT: VERIFIED` — **~30 seconds** in a cold container (the crates.io install is ~25s of it).
...
drand "not-before" time bound (BLS-verified against the pinned League-of-Entropy
key), hash-linked epoch seals, and sealed-state
...
layer for autonomous agents: a timestamp says *when* a hash existed and
...
op/zone scope is recorded but deferred to v1 — full head-to-head vs. sigstore
...
This codebase was built by a solo developer (Nenad Vasic) working with [Claude](https://claude.com) as an engineering collaborator — the AI helps write and adversarially audit the code under human direction. Since July 9, 2026 that role is also on-chain: the AI maintainer operates under a public, revocable mandate issued on the mesh it maintains (`605d8e66…`), and the commits and deploys it performs are emitted as signed acts under it, each row [checkable in your browser](https://navigatorbuilds.github.io/elara-mesh/receipts.html) (best-effort emission — the feed shows what it shows; it cannot prove completeness).
...
Dilithium3/SPHINCS+ signature + content hash), **anchor** (an external
*existed-by* upper bound — proof the seal was notarized by a given time),
**seal** (the epoch seal's signature plus its embedded **drand** "not-before"
pulse, BLS-verified against the pinned League-of-Entropy key), and **inclusion**
(record → sealed Merkle root). A tampered input
...
- **Trustless time — live today.** Epoch seals embed a **drand**
  ...
  League-of-Entropy threshold group (not on us), verifiable offline. The pulse
  rides *inside* the epoch seal itself on producers that enable the opt-in
  fetcher (`drand_pulse_enabled`, default off): the sample bundle in
...
- **Agent mandates — observational v0 shipped, enforcement next.** A principal
  ...
  revocation, sub-delegation chain-walk (leaf→root genealogy + per-hop scope
  narrowing + depth cap), and the recorded-and-flagged taxonomy for out-of-mandate
  acts (zero weight, never silently dropped — a truth ledger records what
  happened). Next (v1): op/zone scope enforcement for non-wildcard mandates and
  consensus-weight gating. [`docs/AGENT-DELEGATION.md`](docs/AGENT-DELEGATION.md)
...
├── elara-record                # Wire codec + record/receipt types — the wire-format source of truth
├── elara-verify                # Offline verifier (record/seal/anchor/mandate) — signing-incapable by design
...
| Mac — Intel (pre-2020) | build from source (below) — Apple Silicon is the only prebuilt Mac target |
...
(record-keeping, human oversight, log retention — applying from December 2027
under the Digital Omnibus timeline)?
[`docs/EU-AI-ACT-EVIDENCE.md`](docs/EU-AI-ACT-EVIDENCE.md) maps these
primitives onto Articles 12/14/19/26 — including, bluntly, what they do
**not** cover.
...
The code is open source and stays that way; the engineering behind it is for
hire — fixed-price conformance suites, integration days, and verification
services, with acceptance criteria that ship as executable runners. See
