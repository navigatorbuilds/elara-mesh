...
/// globally, ~8 MB/sec at ~500B/seal — comfortable on mainnet gossip budget.
/// Lowered 120 → 60 once 120-cap soak showed quiet-zone seal traffic well below
/// the headroom needed for the 2× cadence.
...
pub struct EpochState {
    ...
    /// Computed adaptive epoch duration per zone (seconds). ADVISORY / display
    /// only — does NOT currently gate sealing (the seal loop ticks at the fixed
...
impl EpochState {
    ...
    /// Highest epoch reached across this node's ACTIVE zones (`0..zone_count`),
    /// i.e. the live "current epoch" tip reported on `/status`. Stale zones left
    /// over from a superseded `zone_count` config are excluded so they can't
    /// inflate the reported tip. O(zones-on-this-node) — same bound and
    /// rationale as [`highest_seal_anchor`]; never iterates all 1M zones because
    /// `latest_epoch` only carries entries for zones this node has sealed.
    ///
    /// **Single source of truth for `current_epoch`.** The state_core snapshot
    /// caches this value, but its copy only refreshes on record ingest — on an
    /// idle node the empty seals keep advancing the real tip while the cached
    /// copy freezes at the last-ingest epoch, so a stale `/status` looks
    /// stalled. The `/status` handlers therefore call this method LIVE off an
    /// epoch guard they already hold; the snapshot builder calls it too so both
    /// paths agree by construction.
    pub fn active_zone_max_epoch(&self, zone_count: u64) -> u64 {
        self.latest_epoch
            .iter()
            .filter(|(z, _)| z.path().parse::<u64>().is_ok_and(|n| n < zone_count))
            .map(|(_, &v)| v)
            .max()
            .unwrap_or(0)
    }
...
///
/// An anchor in a *non-stuck* zone emits this record. The
/// 2/3-of-non-stuck-zones stake-weighted threshold governs SETTLEMENT-layer
/// finality of the escalation record only — the `latest_seal_hash` state
/// mutation in `register_global_seal` is first-seen-wins per node, with no
/// canonical selection between same-height competitors yet (the documented
/// designed-not-built N≥2 gap: DEEP-AUDIT-2026-07-09 §2, sweep-2026-07-12 B2).
///
...
/// KR-3 S2 wiring-(c) c3-ii-2a: the id a record is ROUTED by for seal-membership.
///
/// A rotation-class hop is filtered into the seal window under its **lineage**
/// zone (so every hop of a rotation chain co-locates), NOT under its own
/// record-id zone; every other record routes by its own id (byte-identical
/// legacy behaviour). Gated on the network-uniform S2 process global — flag-OFF
/// this is always `rec.id`, so the seal window is byte-for-byte the legacy set.
///
/// This is the SINGLE routing-id source shared by BOTH the seal producer
/// (`create_epoch_seal_with_balance`) and the witness re-check
/// (`scan_window_record_hashes`): calling one function guarantees the producer
/// and every verifier compute the SAME membership zone for the SAME record from
/// the SAME durable predecessor state — the anti-fork (R-2/R-8) invariant. The
/// lineage is derived from record CONTENT (via `rotation_finality::
/// rotation_routing_id`), never a local pin row, so a node that never admitted
/// the hop still agrees. `rotation_hop_fields` short-circuits non-rotation
/// records before any CF read, so the ≤`MAX_SEAL_RECORDS` window loop pays zero
/// per-record CF cost on the common path (SCALE).
fn rotation_route_id(storage: &dyn Storage, rec: &ValidationRecord) -> String {
    if super::consensus::s2_rotation_ordering_enabled() {
        if let Some(lineage) = super::rotation_finality::rotation_routing_id(rec, |pk| {
            storage.get_rotation_newkey_index(pk)
        }) {
            return lineage;
        }
    }
...
pub fn create_epoch_seal_with_balance(
    ...
    for rid in &ids {
        ...
        // KR-3 S2 (c3-ii-2a): route rotation-class hops by their lineage id so a
        // rotation chain seals under one zone. `route_id == rec.id` for every
        // non-rotation record and whenever the S2 flag is OFF ⇒ byte-identical
        // seal window. The witness re-check (`scan_window_record_hashes`) uses
        // the SAME `rotation_route_id`, so producer and verifier agree bit-for-bit.
        let route_id = rotation_route_id(storage, &rec);
...
pub fn scan_window_record_hashes(
    ...
    for rid in &ids {
        ...
        // KR-3 S2 (c3-ii-2a): mirror the producer's `rotation_route_id` filter —
        // a rotation hop is a member of its LINEAGE zone. Same helper, same
        // predecessor state ⇒ producer and witness derive the same membership
        // (anti-fork). `route_id == rec.id` for non-rotation records / flag-OFF.
        let route_id = rotation_route_id(storage, &rec);
        if ZoneId::for_record_dynamic(&route_id, seal_zc) != seal.zone {
            continue;
        }
        if rec.metadata.contains_key(EPOCH_OP_KEY) {
            continue;
...
pub fn attach_xzone_proofs_from_seal_with_finality(
    ...
    // O(seal_size) via the needs-proof index — NOT a scan over all pending
    // (that was O(total_pending) per applied seal on the live path, §4.2-D of
    // internal design notes). The index is derived
    // state, so each hit is re-checked against the live `pending` entry.
    let needs_proof: Vec<(String, [u8; 32], ZoneId)> = {
        let cz = &ledger.cross_zone;
        let mut v: Vec<(String, [u8; 32], ZoneId)> = Vec::new();
        for h in sealed_hashes {
            for tid in cz.needs_proof_ids(h) {
                if let Some(t) = cz.pending.get(tid) {
                    if t.status == crate::accounting::cross_zone::TransferStatus::Locked
                        && t.merkle_proof.is_empty()
                    {
                        v.push((tid.clone(), t.lock_record_hash, t.dest_zone.clone()));
                    }
                }
            }
        }
...
pub async fn epoch_seal_loop(
    ...
    // Tick at the fixed configured interval. NOTE: the per-zone adaptive
    // interval is computed + displayed but is NOT consulted here — every active
    // zone is evaluated each tick and gated only by should_propose_seal
    // (AlreadySealed / bootstrap / VRF-rank). Per-zone adaptive cadence is a
    // design-stage item (fusion-resolved 2026-07-21, verdict C).
    let tick_interval = Duration::from_secs(interval_secs);
    info!("epoch seal loop started (fixed interval {}s; per-zone adaptive interval is advisory/display-only)", interval_secs);
    ...
    loop {
        ...
        // Stamp liveness before any per-tick work (incl. the catchup-skip
        // `continue` below) so a hang in the tick body — not the expected sleep
        // — is what trips the registry's staleness detector.
        hb.heartbeat();
        ...
        for zone in zones_to_seal {
            // Per-zone liveness stamp. The tick-top heartbeat alone would let a
            // legitimately-busy multi-zone tick (per-zone Dilithium3 sign + SMT
            // flush + coop_yield backoff, all O(zones_to_seal)) exceed
            // stale_after and false-trip Stale on this consensus-critical loop.
            ...
            let our_rank = match should_propose_seal(&state, &zone, epoch_number).await {
                SealProposal::PerZone(r) => {
                    ...
                    // slashing punishes. Counted in the tick summary — an
                    // uncounted continue here reads as "evaluated zero
                    // zones" and masks a muted-genesis boot deadlock.
            ...
            let xzone_dest_committees_for_seal: Option<BTreeMap<String, ([u8; 32], u32)>> = {
                // read_recover (not raw read().ok()): std::sync poison is STICKY, so a
                // single poisoning of state.epoch would otherwise make this read skip
                // FOREVER — permanently unbinding cross-zone finality committees →
                // Locked transfers silently unclaimable (rust-reviewer 2026-07-19). The
                // `None` arm below is now defensive-only (read_recover cannot fail).
                let seal_epoch = Some(state.epoch.read_recover().next_epoch(&zone));
                match seal_epoch {
                    Some(seal_epoch) => {
            ...
            match super::gossip::insert_record_synced(&state, record.clone()).await {
                Ok(_) => {
                    // Update epoch state. write_recover (NOT raw write()): a poisoned
                    // epoch lock must NOT silently skip this in-memory seal
                    // registration. The old `if let Ok` skipped on poison, leaving
                    // `latest_epoch` STALE after the durable insert above — the next
                    // tick then reads already_sealed==false and re-proposes a
                    // content-different seal for the same (zone,epoch), which
                    // SlashingMonitor::check_seal_equivocation auto-slashes 25% of the
                    // anchor's own stake. Recover the poison and register the seal
                    // (loop-supervision verdict 2026-07-19; twin of memory_prune 94626494).
                    state.epoch.write_recover().register_seal(&parsed, &record_id, record_hash);
                    ...
                    let super_snapshot = {
                        // read_recover: sticky poison must not silently kill super-seal
                        // creation for the rest of the process (the `_` arm still handles
                        // the real should_create_super_seal==false case).
                        let epoch_r = Some(state.epoch.read_recover());
                        match epoch_r {
                        ...
                        match create_super_seal(SuperSealParams {
                            ...
                            Ok((ss_record, _ss_parsed)) => {
                                ...
                                match super::gossip::insert_record_synced(&state, ss_record).await {
                                    Ok(_) => {
                                        // write_recover: a poisoned epoch lock must not
                                        // silently skip super-seal bookkeeping (a stale
                                        // in-memory buffer would re-create it next cycle).
                                        state.epoch.write_recover().register_super_seal(parsed.zone.clone(), end_epoch, ss_id.clone(), ss_hash, committee_hash);
                                        info!(
                    ...
                    if let Ok(mut ledger) = state.ledger.try_write() {
                        let proofed = match state.consensus.try_lock() {
                            ...
                            Err(_) => {
                                // Same silent-minting fallback as the ingest
                                // site: proof without witnesses = post-Phase-5
                                // unclaimable. Count it (B1 audit 2026-07-12).
                                state
                                    .xzone_attach_finality_lock_contended_total
                                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                                attach_xzone_proofs_from_seal(&mut ledger, &parsed)
                            }
                        };
                        if proofed > 0 {
                            info!("epoch seal (local): attached merkle proofs to {proofed} pending xzone transfers");
                        }
                        // Refresh the fleet-divergence digest while the ledger
                        // write lock is already held (once per applied seal).
                        ...
                        state
                            ...
                            .store(parsed.epoch_number, std::sync::atomic::Ordering::Relaxed);
                    }
                    ...
                    if correct + wrong > 0 {
                        ...
                        // Recurrence wiring
                        {
                            ...
                            // write_recover: poison-consistency with the seal writes above.
                            state.epoch.write_recover().prediction_recurrence.insert(
                                parsed.zone.clone(),
                    ...
                    // NOTE: this creator-side registration is the SECONDARY
                    // site — insert_record_synced above already routed the
                    // seal through insert_record_inner's Phase-5 registration
                    // (ingest.rs), so R1's settle-check there normally wins
                    // and this call returns empty. Routed anyway: the
                    // edge-guard makes double-registration idempotent, and
                    // unusual timing can land promotions here.
                    let creator_member_rids = {
                        let (record_ids, unresolved_hashes) =
                            super::ingest::resolve_seal_record_ids_with_unresolved(
                                &state,
                        ...
                        let mut consensus = state.consensus.lock_recover();
                        let newly = consensus.register_seal_records(&record_id, record_ids);
                        consensus.register_unresolved_seal_members(&record_id, unresolved_hashes);
                    ...
                    super::reward::route_member_finality(&state, creator_member_rids).await;
...
mod tests {
    ...
    fn active_zone_max_epoch_is_live_active_zone_tip() {
        // Pins the canonical `current_epoch` definition the /status handlers
        // read LIVE (and the state_core snapshot caches): the max epoch across
        // ACTIVE zones (0..zone_count), with stale/superseded and non-numeric
        // zones excluded, and 0 on empty.
        let mut ep = EpochState::new();
        assert_eq!(ep.active_zone_max_epoch(4), 0, "empty → 0");

        ep.latest_epoch.insert(ZoneId::new("0"), 50_120);
        ep.latest_epoch.insert(ZoneId::new("1"), 50_128);
        assert_eq!(ep.active_zone_max_epoch(4), 50_128, "max over active zones");

        // A zone left over from a superseded, larger zone_count (index ≥
        // zone_count) must NOT inflate the reported tip.
        ep.latest_epoch.insert(ZoneId::new("9"), 999_999);
        assert_eq!(
            ep.active_zone_max_epoch(4),
            50_128,
            "stale zone index ≥ zone_count is excluded"
        );

        // A non-numeric zone label can't be an active zone index → excluded.
        ep.latest_epoch.insert(ZoneId::new("sandbox/x"), 777_777);
        assert_eq!(
            ep.active_zone_max_epoch(4),
            50_128,
            "non-numeric zone label excluded"
        );
    }

    // ── KR-3 S2 wiring-(c) c3-ii-2a: rotation_route_id (sealer+witness) ──────
    //
    // The S2 gate is a process global; these tests flip it, so they serialize on
    // one lock and each restores the prior value on drop (panic-safe) — a
    // parallel test never observes a leaked ON state.
    static S2_FLAG_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
    struct S2FlagGuard(bool);
    impl Drop for S2FlagGuard {
        fn drop(&mut self) {
            super::super::consensus::set_s2_rotation_ordering(self.0);
        }
    }
    fn rotation_test_record(old_key: &[u8], new_key: &[u8]) -> ValidationRecord {
    ...
    /// Flag OFF (default): a rotation-class hop routes by its OWN record id —
    /// byte-identical to the pre-S2 seal window.
    #[test]
    fn rotation_route_id_flag_off_is_record_id() {
        let _g = S2_FLAG_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _fg = S2FlagGuard(super::super::consensus::s2_rotation_ordering_enabled());
        super::super::consensus::set_s2_rotation_ordering(false);
        let dir = tempfile::tempdir().unwrap();
        let rocks = StorageEngine::open(dir.path().join("r")).unwrap();
        let rec = rotation_test_record(b"pk0", b"pk1");
        assert_eq!(
            rotation_route_id(&rocks, &rec),
            rec.id,
            "flag-OFF routes a rotation hop by its own id (byte-identical)"
        );
    }

    /// Flag ON + a rotation hop: routes by the LINEAGE id (root = sha3(pk₀) =
    /// creator identity), AND the sealer's resolution (`zone_for_record`) equals
    /// the witness's (`for_record_dynamic` at the same zone count) on that
    /// lineage-routed id — the anti-fork (R-2/R-8) bit-parity the two membership
    /// filters depend on. Both filters call the SAME `rotation_route_id`.
    #[test]
    fn rotation_route_id_flag_on_routes_lineage_sealer_eq_witness() {
        let _g = S2_FLAG_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _fg = S2FlagGuard(super::super::consensus::s2_rotation_ordering_enabled());
        super::super::consensus::set_s2_rotation_ordering(true);
        super::super::consensus::set_zone_count(4);
        let dir = tempfile::tempdir().unwrap();
        let rocks = StorageEngine::open(dir.path().join("r")).unwrap();
        let rec = rotation_test_record(b"pk0", b"pk1");
        let lineage = crate::accounting::types::creator_identity_hash(&rec);

        let route_id = rotation_route_id(&rocks, &rec);
        assert_eq!(route_id, format!("d:{lineage}"), "flag-ON rotation hop routes by lineage");
        assert_ne!(
            route_id, rec.id,
            "and the lineage route diverges from the naive record-id route"
        );

        // Anti-fork: seal producer vs witness compute the SAME membership zone.
        let seal_zc = super::super::consensus::get_zone_count();
        let sealer_zone = zone_for_record(&route_id);
        let witness_zone = ZoneId::for_record_dynamic(&route_id, seal_zc);
        assert_eq!(
            sealer_zone, witness_zone,
            "sealer==witness membership zone on the same rotation record (anti-fork)"
        );
    }

    /// Flag ON but the record is NOT a rotation hop: still routes by its own id.
    /// The routing change is scoped to rotation-class hops only, so ordinary
    /// traffic is byte-identical even with S2 enabled.
    #[test]
    fn rotation_route_id_flag_on_non_rotation_is_record_id() {
        let _g = S2_FLAG_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _fg = S2FlagGuard(super::super::consensus::s2_rotation_ordering_enabled());
        super::super::consensus::set_s2_rotation_ordering(true);
        let dir = tempfile::tempdir().unwrap();
        ...
        assert_eq!(rotation_route_id(&rocks, &rec), rec.id);
    }
