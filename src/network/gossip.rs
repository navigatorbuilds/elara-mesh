...
pub(crate) fn is_retryable_ingest_rejection(err_str: &str) -> bool {
    // Structural marker (KR-3 S2 v4 slice-0 / C2): any decline the ingest path
    // classified `Transient` at the reject site renders through
    // `ElaraError::TransientReject`, whose Display is `"transient reject: <why>"`.
    // Keying on the ONE typed prefix — rather than enumerating each transient
    // reason substring — means a future transient reason is retryable the moment it
    // uses the typed variant, and closes the shipped H3b interim's embargo bug (a
    // rotation hop whose predecessor had not finalized was permanent-cached, never
    // parked, because its reason string matched none of the clauses below).
    err_str.contains("transient reject:")
...
pub async fn pull_loop(state: Arc<NodeState>, mut shutdown: watch::Receiver<()>, hb: Arc<super::supervision::LoopStatus>) {
    ...
    let jitter_cap_ms = pull_jitter_cap_ms(base_interval);

    loop {
        let jitter = Duration::from_millis(pull_jitter_ms(
        ...
        if pull_targets.is_empty() {
            // No-targets is already WARN-reported (throttled, with suppression
            // counts) inside select_gossip_peers — see NO_TARGETS_WARN_THROTTLE_SECS.
            // A second unthrottled warn here fired every pull tick on a lone seed
            // (~5k lines/day for a legitimate permanent state), so keep only the
            // per-tick detail at debug.
            debug!("pull loop: no targets available (dag_len={dag_len}, cycle={pull_cycle})");
        } else {
...
pub async fn orphan_resolver_loop(state: Arc<NodeState>, mut shutdown: watch::Receiver<()>, hb: Arc<super::supervision::LoopStatus>) {
        ...
        tokio::spawn(async move {
            ...
            match pq_client.receive_attestation_gossip(
                ...
                Ok(resp) => {
                    ...
                    if n.is_multiple_of(50) {
                        tracing::warn!("att-push REJECTED pq-status={} to {} (#{n}) body={}", resp.status, &pq_addr[..pq_addr.len().min(60)], String::from_utf8_lossy(&resp.body[..resp.body.len().min(200)]));
                    }
                }
                Err(e) => {
...
async fn process_attestation_pull_batch(
    ...
    for att in atts {
        ...
        // Attestation preimage (domain-tagged v6+; bare v≤5) — shared helper, T63.
        let signable = match state.get_record(&rid).map(|rec| super::witness::witness_attestation_preimage(&rec)) {
            Ok(s) => s,
            Err(_) => {
    ...
    if !consensus_refeed.is_empty() {
        ...
        let mut refeed_member_rids: Vec<String> = Vec::new();
        {
            let ledger = state.ledger.read().await;
            let mut consensus = state.consensus.lock_recover();
            ...
            for (rid, wh, ts) in &consensus_refeed {
                ...
                if let Some(seal) = seal_infos.get(rid) {
                    if !phase6c_blocked.contains(&(rid.clone(), wh.clone())) {
                        refeed_member_rids.extend(consensus.add_seal_attestation(
        ...
        // Layer-2 fast-track promotions from the refeed's seal attestations —
        // durable write + exactly-once effects (chunked + counted inside).
        super::reward::route_member_finality(state, refeed_member_rids).await;
    }
    ...
    super::reward::finalization_effects(state, feed_outcome.seal_members_finalized);
            ...
            Ok(wire_list) => {
                ...
                if fetched > 0 {
                    ...
                    for att in &deferred_attestations {
                        ...
                        let signable = match state.get_record(record_id).map(|r| super::witness::witness_attestation_preimage(&r)) {
                            Ok(s) => s,
                    ...
                    if !retry_verified.is_empty() {
                        ...
                        super::reward::finalization_effects(state, retry_outcome.seal_members_finalized);
                    }
                }
...
/// Proportional cap for pull-loop jitter: a quarter of the base interval,
/// never more than the historical 5 s. The flat 0–5 s range swamped small
/// intervals (100% of the 5 s NAT floor); `.max(1)`-via-clamp keeps the
/// modulus non-zero for degenerate configs.
fn pull_jitter_cap_ms(base_interval: Duration) -> u64 {
    ((base_interval.as_millis() / 4) as u64).clamp(1, 5000)
}

/// Per-node, per-cycle pull-loop jitter in `[0, cap_ms)`.
///
/// sha3(identity_hash ‖ domain ‖ cycle): unique per node, advancing each
/// cycle, domain-separated between the record and attestation loops. The old
/// `(now() as u64).wrapping_mul(k) % 5000` derived from shared wall-clock
/// SECONDS — every node ticking in the same second computed the identical
/// offset, so the thundering herd the jitter existed to break stayed in step.
fn pull_jitter_ms(identity_hash: &str, domain: &[u8], cycle: u64, cap_ms: u64) -> u64 {
    let mut input = Vec::with_capacity(identity_hash.len() + domain.len() + 8);
    input.extend_from_slice(identity_hash.as_bytes());
    input.extend_from_slice(domain);
    input.extend_from_slice(&cycle.to_le_bytes());
    let digest = crate::crypto::hash::sha3_256(&input);
...
pub async fn attestation_pull_loop(state: Arc<NodeState>, mut shutdown: watch::Receiver<()>, hb: Arc<super::supervision::LoopStatus>) {
    ...
    loop {
        ...
        if pull_targets.is_empty() {
            // Lone-seed no-op cycle (~4k lines/day at info) — same
            // warning-blindness class as the pull-loop no-targets line; the
            // throttled WARN inside select_gossip_peers covers alerting.
            // Keep info when targets exist: useful serve-side telemetry.
            debug!("att-pull cycle={pull_cycle} force_full={force_full} targets=0");
        } else {
            ...
            // Scan recent local attestations and re-feed any not tracked in consensus.
            {
                ...
                if let Ok(recent) = scan {
                    ...
                    if !gap_feed.is_empty() {
                        ...
                        super::reward::finalization_effects(&state, outcome.seal_members_finalized);
                    }
                }
            }
...
mod tests {
    ...
    /// Pull-loop jitter must DECORRELATE nodes: distinct identities produce
    /// distinct offset sequences, the two loops on one node are domain-
    /// separated, and the offset moves as the cycle advances. The wall-clock
    /// derivation this replaced gave every node ticking in the same second
    /// the identical offset — the herd it existed to break stayed in step.
    #[test]
    fn pull_jitter_decorrelates_nodes_and_domains() {
        let seq = |id: &str, dom: &[u8]| -> Vec<u64> {
            (0..8).map(|c| pull_jitter_ms(id, dom, c, 5000)).collect()
        };
        let a = seq("node-a-identity-hash", b"record-pull");
        let b = seq("node-b-identity-hash", b"record-pull");
        assert_ne!(a, b, "distinct identities must not share a jitter sequence");
        let a_att = seq("node-a-identity-hash", b"att-pull");
        assert_ne!(a, a_att, "record and attestation loops must be domain-separated");
        assert!(
            a.windows(2).any(|w| w[0] != w[1]),
            "offset must move across cycles, not freeze"
        );
        // Deterministic per (node, domain, cycle): restarts resume the
        // persisted cycle counter, not a new random stream.
        assert_eq!(a, seq("node-a-identity-hash", b"record-pull"));
    }

    /// Jitter stays strictly under its cap for a run of cycles, and the
    /// degenerate caps (1, 0) can never panic on a zero modulus.
    #[test]
    fn pull_jitter_bounded_and_degenerate_cap_safe() {
        for c in 0..64 {
            assert!(pull_jitter_ms("node-a-identity-hash", b"record-pull", c, 1250) < 1250);
        }
        assert_eq!(pull_jitter_ms("node-a-identity-hash", b"record-pull", 7, 1), 0);
        assert_eq!(pull_jitter_ms("node-a-identity-hash", b"record-pull", 7, 0), 0);
    }

    /// The cap is a quarter of the base interval, ceilinged at the historical
    /// 5 s and floored at 1 ms: the default 30 s config keeps its 0–5 s
    /// spread, while the 5 s NAT floor is no longer 100%-swamped by jitter.
    #[test]
    fn pull_jitter_cap_is_proportional_with_floor_and_ceiling() {
    ...
    // P0 regression (2026-07-19): a state_core channel failure (worker
    // down/restarting) is TRANSIENT infra, not a content rejection. The insert
    // wrappers render it as `ElaraError::TransientReject`, whose Display must
    // classify as retryable so the record PARKS for retry — never lands in the
    // permanent `gossip_rejected` cache (consult-and-skip on every pull driver →
    // the record would be un-repullable on this node). Locks the end-to-end
    // classification the fix depends on: if a refactor drops the "transient
    // reject:" clause from is_retryable_ingest_rejection, or changes
    // TransientReject's Display, the P0 embargo silently returns.
    #[test]
    fn state_core_transient_error_classifies_retryable_not_permanent() {
        for msg in ["state core shut down", "state core dropped reply"] {
            let rendered =
                crate::errors::ElaraError::TransientReject(msg.to_string()).to_string();
            assert!(
                is_retryable_ingest_rejection(&rendered),
                "state_core transient error {msg:?} (rendered {rendered:?}) must be \
                 retryable — else it permanent-embargoes the record (P0 silent-loss)"
            );
        }
    }
    ...
    fn test_temporary_rejection_not_cached() {
        ...
        let retryable_errors = [
            ...
            "stake not found",
            "transfer not found",
            // KR-3 S2 v4 slice-0: any decline the ingest path tagged Transient renders
            // through the `transient reject:` prefix and must park, not embargo.
            "transient reject: rotation-class predecessor finality failed: reject-transient",
        ];
        ...
        // End-to-end anti-drift pin: the TYPED `ElaraError::TransientReject` variant
        // is the single source of the retry classification, and its Display must land
        // in the classifier's retryable set with no per-reason substring bridge — so a
        // future transient reason is covered the moment it uses the variant. This is
        // what closes C2 (the shipped H3b interim's `reject-transient` was silently
        // permanent-cached because its raw reason matched none of the string clauses).
        let typed = crate::errors::ElaraError::TransientReject(
            "no cited parent is durably finalized (H3 durable-parent leg)".to_string(),
        )
        .to_string();
        assert!(
            is_retryable_ingest_rejection(&typed),
            "TransientReject must classify retryable via its typed Display: {typed}"
        );
    }
    ...
    fn build_kind_test_record(meta: std::collections::BTreeMap<String, serde_json::Value>) -> ValidationRecord {
        {
            ...
            __r.id = "kind-classify-test".to_string();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0u8; 32];
            __r.creator_public_key = vec![0u8; 1952];
            __r.timestamp = 0.0;
            __r.parents = vec![];
