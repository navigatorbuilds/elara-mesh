...
// ── B5 act-permanence (verdict 2026-07-18): act-index entry deletions, by
//    reason. Under B5 the GC and ZONE_PURGE counters MUST stay 0 forever —
//    retention GC and zone resharding PRESERVE the act index; only a deliberate
//    operator evict or the budget evictor removes an entry. A non-zero GC/
//    ZONE_PURGE reading is the B5 regression alarm (the cascade was reintroduced
//    under a preserving intent). Read by the /metrics surface as
//    `elara_mandate_act_entries_deleted_total{reason=…}`. ──
pub static MANDATE_ACT_ENTRIES_DELETED_GC: AtomicU64 = AtomicU64::new(0);
pub static MANDATE_ACT_ENTRIES_DELETED_ZONE_PURGE: AtomicU64 = AtomicU64::new(0);
pub static MANDATE_ACT_ENTRIES_DELETED_ADMIN_EVICT: AtomicU64 = AtomicU64::new(0);
pub static MANDATE_ACT_ENTRIES_DELETED_BUDGET_EVICT: AtomicU64 = AtomicU64::new(0);
...
/// `pub(crate)` so the B5 STATE-2 endpoint path can reconstruct an act entry from
/// a record body byte-identically to what this ingest hook would have indexed.
pub(crate) fn amount_from_record(record: &ValidationRecord) -> Option<u64> {
    record
        .metadata
        .get("beat_amount")
