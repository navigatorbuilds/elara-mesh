...
pub struct NodeState {
    ...
    /// T65 D1 — submissions that arrived WITHOUT an `x-elara-network-id`
    /// header on the two checked ingress paths (HTTP + PQ submit). This
    /// number near zero is the precondition for flipping
    /// `require_network_id_header`. It is NOT a network-isolation proxy:
    /// sync/snapshot/probe ingress carries no network check at all (T74).
    pub submit_missing_network_header_total: AtomicU64,
    ...
    /// T63 R2 — records rejected at the ingest chokepoint because they carry a
    /// NON-EMPTY `network_id` that differs from ours (`config.network_id`).
    /// Empty = legacy-accept (pre-flip records; what makes the v6 flip
    /// non-forking). Sits in `insert_record_inner`, so it covers every ingress
    /// path — HTTP submit, PQ verbs, gossip, initial sync, snapshot sync,
    /// probe — including the T74/T75 paths that bypass the transport-header
    /// check. Growth = a foreign-network (or misconfigured) peer pushing
    /// records at us; the reject is cheap (string compare before PQ verify).
    pub ingest_network_id_mismatch_rejected_total: AtomicU64,
...
impl NodeState {
    ...
    pub fn new(
        ...
        Self {
            ...
            submit_missing_network_header_total: AtomicU64::new(0),
            ...
            ingest_network_id_mismatch_rejected_total: AtomicU64::new(0),
    ...
    pub fn create_self_ledger_record(
        ...
        // T65 D2 — network binding enters at construction (inside the content
        // preimage + signature), gated off by default; re-light flips the flag.
        let bind = self
            .config
            .bind_network_id_in_metadata
            .then_some(self.config.network_id.as_str());
        crate::accounting::types::create_ledger_record_with_nonce_bound(
            &self.identity, parents, metadata, nonce, bind,
        )
    }
...
mod tests {
    ...
    fn ops125_test_record(
        ...
        }
        {
            ...
            __r.id = id.into();
            __r.version = 5;
            __r.content_hash = vec![];
            __r.creator_public_key = vec![0; 32];
            __r.timestamp = timestamp;
            __r.parents = vec![];
            __r.classification = classification;
