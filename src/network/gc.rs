...
pub fn gc_pass_rocks_with(
    ...
    for id in &to_delete {
        if let Err(e) = rocks.delete_record(id) {
            warn!("gc: failed to delete record {}: {e}", &id[..id.len().min(16)]);
        } else {
            dag.remove(id);
        }
...
pub async fn gc_loop(
    ...
    loop {
        ...
        let live_bytes = if disk_cap > 0 { state.rocks.total_live_bytes() } else { 0 };
        let (retention_secs, size_pressure) = if disk_cap > 0 && live_bytes > disk_cap {
            let shrink = (disk_cap as f64) / (live_bytes as f64); // (0, 1)
        ...
        if size_pressure {
            info!(
                "gc: disk cap pressure — live_bytes={:.1}GB cap={:.1}GB → compressed retention={}h",
