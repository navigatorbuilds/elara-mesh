...
/// RAII guard: shuts the mDNS `ServiceDaemon`'s background thread down on ANY
/// drop — clean return, browse-error early-return, OR a panic unwind. `mdns_sd`'s
/// `ServiceDaemon` has no `Drop`, so without this a panic (or a spawn_supervised
/// restart) leaks one OS thread per occurrence — this guard is the prerequisite
/// for bringing the loop under supervision.
...
impl std::ops::Deref for DaemonGuard {
    type Target = mdns_sd::ServiceDaemon;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}

/// Max whole-setup attempts before cleanly disabling LAN discovery for this
/// process. A persistent `ServiceDaemon::new()`/`browse()` failure means the host
/// genuinely can't do mDNS (no multicast / sandbox) — retrying forever would just
/// spam. Attempts 1-5 back off 2+4+8+16+32s (~1 min of transient tolerance).
const MDNS_SETUP_MAX_ATTEMPTS: u32 = 6;

/// Exponential backoff (seconds) before the Nth setup retry; the shift is capped
/// so it cannot overflow and the value is capped at 60s. Never 0 (a 0s retry would
/// busy-spin).
fn mdns_setup_backoff_secs(attempt: u32) -> u64 {
    (1u64 << attempt.min(6)).min(60)
}

/// Announce this node on the LAN. Non-fatal: a failed `ServiceInfo`/`register`
/// still leaves `browse()` able to discover peers, so we warn and continue.
fn announce_mdns(daemon: &DaemonGuard, state: &NodeState) {
...
pub async fn mdns_loop(
    ...
    // Bounded-retry setup: now that this loop runs under `spawn_supervised`, a
    // TRANSIENT `ServiceDaemon::new()`/`browse()` failure must NOT permanently
    // disable LAN discovery. It used to `return` on the first failure → the
    // supervisor reads a clean exit as Disabled-forever (rust-review hazard 4).
    // Retry the WHOLE setup — a fresh daemon each attempt, the old one cleaned up
    // by `DaemonGuard`'s Drop — with capped backoff, and give up (clean Disable)
    // only after MDNS_SETUP_MAX_ATTEMPTS, at which point the host genuinely can't
    // do mDNS. Announcing stays non-fatal inside `announce_mdns`.
    // `_daemon` is the RAII guard: never referenced again, but MUST stay in scope
    // for the whole loop — dropping it shuts the ServiceDaemon thread down and the
    // `receiver` stops yielding. (`_daemon`, not bare `_`, keeps it alive to scope
    // end; bare `_` would drop it immediately and break discovery.)
    let (_daemon, receiver) = {
        let mut attempt: u32 = 0;
        loop {
            hb.heartbeat();
            let setup: std::result::Result<_, String> = (|| {
                let daemon = DaemonGuard(
                    mdns_sd::ServiceDaemon::new().map_err(|e| format!("daemon start: {e}"))?,
                );
                announce_mdns(&daemon, &state); // non-fatal (warns internally)
                let receiver = daemon
                    .browse(SERVICE_TYPE)
            ...
            match setup {
                ...
                Err(e) => {
                    attempt += 1;
                    if attempt >= MDNS_SETUP_MAX_ATTEMPTS {
                        warn!("mDNS: setup failed {attempt}× (last: {e}) — LAN discovery disabled for this process");
                        return;
                    }
                    let backoff = mdns_setup_backoff_secs(attempt);
                    warn!("mDNS: setup failed (attempt {attempt}/{MDNS_SETUP_MAX_ATTEMPTS}: {e}); retrying in {backoff}s");
                    tokio::select! {
    ...
    loop {
        hb.heartbeat(); // hang-detection: stamps each ≤2s recv_timeout iteration (« 60s stale_after)
        tokio::select! {
            _ = shutdown.changed() => {
                debug!("mDNS: shutting down");
                // DaemonGuard's Drop shuts the daemon down on return (covers the
                // panic + browse-error paths too) — no explicit shutdown needed.
                return;
            }
...
mod tests {
    ...
    #[test]
    fn mdns_setup_backoff_is_bounded_monotone_and_nonzero() {
        // Attempts 1-5 (the ones actually used before MDNS_SETUP_MAX_ATTEMPTS=6
        // gives up): exponential 2,4,8,16,32s.
        assert_eq!(mdns_setup_backoff_secs(1), 2);
        assert_eq!(mdns_setup_backoff_secs(2), 4);
        assert_eq!(mdns_setup_backoff_secs(3), 8);
        assert_eq!(mdns_setup_backoff_secs(4), 16);
        assert_eq!(mdns_setup_backoff_secs(5), 32);
        // Shift is capped at 6 and the value at 60s — a large attempt cannot
        // overflow the shift or grow unboundedly.
        assert_eq!(mdns_setup_backoff_secs(6), 60);
        assert_eq!(mdns_setup_backoff_secs(50), 60);
        // Never 0 (a 0s retry would busy-spin the setup loop).
        assert!((1..=6).all(|a| mdns_setup_backoff_secs(a) >= 1));
    }
