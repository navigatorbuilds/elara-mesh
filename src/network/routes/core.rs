...
/// T65 D1 — the network a header-less submission is attributed to. FROZEN
/// backward-compatibility constant: this must NEVER be made to track
/// `default_network_id()` / `DEFAULT_NETWORK_ID`. Today both spell "testnet",
/// which is exactly the trap — a frozen literal fails closed everywhere
/// (header-less records are foreign on any network not literally named
/// "testnet"), while a default-tracking value silently carries the fail-open
/// to whatever network becomes the compiled-in default, e.g. mainnet
/// (adversarial-verifier ruling, T65 verdict §D1).
pub(crate) const LEGACY_MISSING_HEADER_NETWORK: &str = "testnet";
...
pub async fn submit_record(
    ...
    // T65 D1: a MISSING header is distinguished from a present one — counted
    // always, rejected only behind `require_network_id_header` (default off).
    let peer_network_hdr = headers.get("x-elara-network-id").and_then(|v| v.to_str().ok());
    ...
    let peer_network: &str = peer_network_hdr.unwrap_or(LEGACY_MISSING_HEADER_NETWORK);
...
pub async fn witness_record(
    ...
    // Attestation is signed over the domain-tagged preimage (v6+; v≤5 bare —
    // helper dispatches on record.version) so this public endpoint is not an
    // oracle for creator-signature bytes (T63). Creator verify above stays on
    // the bare preimage.
    let attestation_preimage = crate::network::witness::witness_attestation_preimage(&record);
