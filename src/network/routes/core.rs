...
pub async fn status(
    ...
    let (dag_size, dag_tips, dag_roots, dag_edges, ledger_supply, ledger_staked, ledger_accounts,
         peers_connected, peers_total, finalized_count, current_epoch, conservation_pool) = if let Some(core) = state.state_core.get() {
    ...
    let latest_seal_anchor = state
        ...
        .read_recover()
        .highest_seal_anchor()
        .map(|(zone, epoch, hash)| serde_json::json!({
            "zone": zone.to_string(),
            "epoch": epoch,
    ...
    if let Some(core) = state.state_core.get() {
        ...
        match result {
            ...
            crate::network::state_core::InsertResult::Error { message } => {
                ...
                return Err(AppError(crate::errors::ElaraError::Storage(message)));
            }
        }
    } else {
...
pub async fn compute_validate_record(
    ...
    let sig_ok = match record.signature.as_ref() {
        Some(sig) => {
            match dilithium3_verify(&record.signable_bytes(), sig, &record.creator_public_key) {
                Ok(true) => true,
                Ok(false) => false,
                Err(_) => false,
            }
    ...
    // 6. DAG parent existence check (advisory, not blocking)
    let parents_found = {
        let dag = state.dag.read().await;
