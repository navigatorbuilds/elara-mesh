//! Token/ledger route handlers: /balances, /stakes, /ledger/summary, /history,
...
//
// The pure-read helper at `routes/token.rs:393` is the wire-source for the
...
mod batch_gggg_compute_tx_history {
    ...
    /// `routes/token.rs:500` must (a) clamp the ECHOED `limit` field
