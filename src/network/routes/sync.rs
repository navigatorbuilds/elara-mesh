...
pub async fn receive_attestation(
    ...
    // Verify Dilithium3 signature against the attestation preimage (T63
    // dispatch: bare signable_bytes for v≤5 records, domain-tagged for v6+ —
    // the witness helper is THE preimage authority, never hand-rolled). If the
...
pub async fn serve_snapshot_fast(
    ...
    let result = tokio::task::spawn_blocking(move || -> std::result::Result<serde_json::Value, ElaraError> {
        ...
        if meta_only {
            ...
            // Whole-node snapshot (merkle_root + record_count are node-wide), so the
            // epoch high-water is max across ALL zones, not zone-0 only — matches the
            // state-delta current_epoch semantics below. (Identical at zone_count==1.)
            let epoch_number = state2.epoch.read_recover()
                .latest_epoch.values().copied().max().unwrap_or(0);
...
pub async fn serve_state_delta(
    ...
    // HTTP twin of the PQ router's global heavy-read gate (2026-07-12 sweep
    // A9): `state_delta` is in `is_heavy_blocking_read` on the PQ side, so an
    // off-host HTTP caller must share the same cross-identity concurrency cap
    // — otherwise the HTTP route is the free lane around the PQ semaphore.
    // Same bounded-wait-then-shed semantics; loopback exempt (the PQ twin
    // exempts local same-process callers). Permit is held across the compute
    // and dropped at handler return — before the response body is written.
    let _heavy_permit = if connect_info.0.ip().is_loopback() {
        None
    } else {
...
pub async fn receive_conflict_proof(
    ...
    // Verify cryptographically — O(2) Dilithium3 verifications on caller
    // bytes, so run off the async worker (spawn_blocking), mirroring the PQ
    // twin (2026-07-12 sweep A7).
    ...
    fn ops124_test_record(timestamp: f64, id: &str) -> crate::record::ValidationRecord {
        {
            ...
            __r.id = id.to_string();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = crate::crypto::hash::sha3_256(id.as_bytes()).to_vec();
            __r.creator_public_key = vec![0u8; 32];
