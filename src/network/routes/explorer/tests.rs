...
// ─── B5 /mandate/status three-state honesty contract (verdict 2026-07-18) ───

#[tokio::test]
async fn b5_mandate_status_state1_pruned_body_still_authoritative() {
    // STATE 1: index entry present, body GC-pruned → still an authoritative found
    // verdict with record_present:false + a record_note.
    let state = test_state();
    let (mand, agent) = ("aa".repeat(32), "bb".repeat(32));
    let rid = "s1-pruned";
    state.rocks.put_record(rid, &stub_record(rid)).unwrap();
    state
        .rocks
        .put_mandate_act(rid, &crate::mandate::MandateActEntry::new(&mand, &agent, 500, None))
        .unwrap();
    state
        .rocks
        .delete_record(rid, crate::storage::rocks::DeleteIntent::GcPrune)
        .unwrap();
    let v = mandate_status(State(state.clone()), AxumPath(rid.to_string())).await.0;
    assert_eq!(v["is_mandate_act"], serde_json::json!(true));
    assert_eq!(v["record_present"], serde_json::json!(false));
    assert!(v["record_note"].is_string());
    assert_eq!(v["index_entry_present"], serde_json::json!(true));
    assert_eq!(v["authoritative_complete"], serde_json::json!(true));
}

#[tokio::test]
async fn b5_mandate_status_state2_record_only_contract() {
    let state = test_state();
    // 2a: body present with a valid non-empty mandate_ref, NO index entry →
    // reconstructed as an act (index_entry_present:false), authoritative.
    let rid_a = "s2a-rec";
    let mut rec_a = stub_record(rid_a);
    rec_a.metadata.insert(
        crate::mandate::MANDATE_REF_METADATA_KEY.into(),
        serde_json::json!("cc".repeat(32)),
    );
    state.rocks.put_record(rid_a, &rec_a).unwrap();
    let va = mandate_status(State(state.clone()), AxumPath(rid_a.to_string())).await.0;
    assert_eq!(va["is_mandate_act"], serde_json::json!(true));
    assert_eq!(va["index_entry_present"], serde_json::json!(false));
    assert_eq!(va["record_present"], serde_json::json!(true));
    assert_eq!(va["authoritative_complete"], serde_json::json!(true));

    // 2b: body present, no mandate_ref → the signed record is the refutation,
    // authoritative on every node.
    let rid_b = "s2b-rec";
    state.rocks.put_record(rid_b, &stub_record(rid_b)).unwrap();
    let vb = mandate_status(State(state.clone()), AxumPath(rid_b.to_string())).await.0;
    assert_eq!(vb["is_mandate_act"], serde_json::json!(false));
    assert_eq!(vb["absence_basis"], serde_json::json!("record_checked"));
    assert_eq!(vb["authoritative_complete"], serde_json::json!(true));
}

#[tokio::test]
async fn b5_mandate_status_state3_removed_by_operator() {
    // STATE 3 removed-marker: AdminEvict → deliberate un-index, authoritative.
    let state = test_state();
    let (mand, agent) = ("aa".repeat(32), "bb".repeat(32));
    let rid = "s3-removed";
    state.rocks.put_record(rid, &stub_record(rid)).unwrap();
    state
        .rocks
        .put_mandate_act(rid, &crate::mandate::MandateActEntry::new(&mand, &agent, 1, None))
        .unwrap();
    state
        .rocks
        .delete_record(rid, crate::storage::rocks::DeleteIntent::AdminEvict)
        .unwrap();
    let v = mandate_status(State(state.clone()), AxumPath(rid.to_string())).await.0;
    assert_eq!(v["is_mandate_act"], serde_json::json!(false));
    assert_eq!(v["absence_basis"], serde_json::json!("removed_by_operator"));
    assert_eq!(v["authoritative_complete"], serde_json::json!(true));
}

#[tokio::test]
async fn b5_mandate_status_state3_absent_coverage_window() {
    let state = test_state();
    // FRESH (uninitialized — test_state never runs boot init): the node has NOT
    // recorded its coverage boundary, so absence is FAIL-CLOSED to non-authoritative
    // (basis "uninitialized"). This pins the B5-review CRITICAL fix — an
    // uninitialized floor must NEVER read as authoritative full_coverage.
    let vu = mandate_status(State(state.clone()), AxumPath("nope-0".to_string())).await.0;
    assert_eq!(vu["absence_basis"], serde_json::json!("outside_coverage"));
    assert_eq!(vu["authoritative_complete"], serde_json::json!(false));
    let basis_u = vu["acts_coverage"]["basis"].as_array().unwrap();
    assert!(basis_u.iter().any(|b| b == "uninitialized"), "uninitialized floor must be tagged");

    // Genesis-0 (written 0) → authoritative full_coverage (distinct from uninitialized).
    state.rocks.force_acts_coverage_floor_genesis().unwrap();
    let v0 = mandate_status(State(state.clone()), AxumPath("nope-1".to_string())).await.0;
    assert_eq!(v0["absence_basis"], serde_json::json!("full_coverage"));
    assert_eq!(v0["authoritative_complete"], serde_json::json!(true));

    // Advance the floor → outside_coverage, non-authoritative, window surfaced.
    state.rocks.advance_acts_coverage_floor(5000).unwrap();
    let v1 = mandate_status(State(state.clone()), AxumPath("nope-2".to_string())).await.0;
    assert_eq!(v1["absence_basis"], serde_json::json!("outside_coverage"));
    assert_eq!(v1["authoritative_complete"], serde_json::json!(false));
    assert_eq!(v1["acts_coverage"]["complete_from_ms"], serde_json::json!(5000u64));
    assert!(v1["hint"].is_string());
}

#[tokio::test]
async fn b5_mandate_status_zone_scoped_never_authoritative_absent() {
    // B3: a zone-scoped node never claims authoritative absence, even at floor 0.
    let state = test_state();
    // Initialize the floor to a written 0 so this test exercises the ZONE_SUBSET
    // path specifically (an uninitialized floor would short-circuit to
    // basis:"uninitialized" first — see the state3 test for that case).
    state.rocks.force_acts_coverage_floor_genesis().unwrap();
    state.zone_manager.lock().unwrap().subscribe(&ZoneId::new("0"));
    let v = mandate_status(State(state.clone()), AxumPath("nope-z".to_string())).await.0;
    assert_eq!(v["is_mandate_act"], serde_json::json!(false));
    assert_eq!(v["absence_basis"], serde_json::json!("outside_coverage"));
    assert_eq!(v["authoritative_complete"], serde_json::json!(false));
    let basis = v["acts_coverage"]["basis"].as_array().unwrap();
    assert!(
        basis.iter().any(|b| b == "zone_subset"),
        "absent answer on a zone-scoped node must carry zone_subset basis"
    );
}

#[tokio::test]
async fn b5_list_endpoints_carry_coverage() {
    let state = test_state();
    // Initialize the floor so complete_from_ms is a number (an uninitialized node
    // honestly emits null there — covered by the state3 uninitialized assertion).
    state.rocks.force_acts_coverage_floor_genesis().unwrap();
    let mand = "aa".repeat(32);
    let v = mandate_acts(
        State(state.clone()),
        AxumPath(mand),
    ...
    .await
    .0;
    assert!(v["acts_coverage"].is_object());
    assert!(v["acts_coverage"]["complete_from_ms"].is_number());
    ...
    .await
    .0;
    assert!(v2["acts_coverage"].is_object());
}
...
fn batch_aaaa_compute_seal_debug_happy_path_emits_fifteen_field_envelope() {
    ...
    let zone = crate::ZoneId::from_legacy(0);
    {
        ...
        let _ = consensus.register_seal_records("seal-aaaa-envelope", vec!["rec-aaaa-1".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
...
fn batch_aaaa_compute_seal_debug_settlement_denominator_picks_committee_when_nonzero_else_zone() {
    ...
    let zone_with_committee = crate::ZoneId::from_legacy(2);
    {
        ...
        let _ = consensus.register_seal_records("seal-aaaa-fallback-zone", vec!["rec-fb-z".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
        ...
        let _ = consensus.register_seal_records("seal-aaaa-committee-wins", vec!["rec-cw".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
...
fn batch_aaaa_compute_seal_debug_stake_threshold_is_exactly_two_thirds_of_settlement_denominator() {
    ...
    let zone = crate::ZoneId::from_legacy(3);
    {
        ...
        let _ = consensus.register_seal_records("seal-aaaa-threshold", vec!["rec-thr".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
...
fn batch_aaaa_compute_seal_debug_committee_members_emitted_sorted_lexicographically() {
    ...
    let sorted_expected = ["a-id", "c-id", "m-id", "x-id", "z-id"];
    {
        ...
        let _ = consensus.register_seal_records("seal-aaaa-committee-sort", vec!["rec-cs".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
...
fn batch_aaaa_compute_seal_debug_attestors_carry_effective_stake_equals_stake_times_independence() {
    ...
    let zone = crate::ZoneId::from_legacy(5);
    {
        let mut consensus = state.consensus.lock_recover();
        consensus.register_zone_stake(zone.clone(), 1000);
        let _ = consensus.register_seal_records("seal-aaaa-attestor-math", vec!["rec-am".to_string()]);
...
fn batch_bbbb_compute_seal_debug_global_seal_flips_is_global_seal_wire_field_with_ordering_invariance(
    ...
    let non_stuck_zone = crate::ZoneId::from_legacy(8);
    {
        ...
        let _ = consensus.register_seal_records("seal-bbbb-pre-register", vec!["rec-bbbb-pre".to_string()]);
        let _ = consensus.add_seal_attestation(crate::network::consensus::SealAttestation {
...
fn batch_bbbb_compute_seal_debug_global_seal_is_settled_excludes_stuck_zone_attestations() {
    ...
    let non_stuck_zone = crate::ZoneId::from_legacy(10);
    {
        ...
        let _ = consensus.register_seal_records("seal-bbbb-safety", vec!["rec-bbbb-safety".to_string()]);
...
fn batch_bbbb_compute_seal_debug_global_seal_wire_denominator_reflects_first_attestation_zone_not_cross_zone_max(
    ...
    let non_stuck_b = crate::ZoneId::from_legacy(13);
    {
        ...
        let _ = consensus.register_seal_records(
...
async fn mandate_acts_coverage_authoritative_complete_tracks_snapshot_bootstrap() {
    let state = test_state();
    // A genesis-replay node with full history = a WRITTEN genesis-0 coverage floor
    // (B5-review fix: an *uninitialized* floor is fail-closed to non-authoritative,
    // so model full coverage explicitly rather than relying on the absent-key default).
    state.rocks.force_acts_coverage_floor_genesis().unwrap();
    let mid = "a".repeat(64);
...
async fn mandate_status_not_found_authoritative_complete_tracks_snapshot_bootstrap() {
    let state = test_state();
    // Genesis-replay node = full history = a WRITTEN genesis-0 coverage floor
    // (B5-review fix: an uninitialized floor is fail-closed to non-authoritative).
    state.rocks.force_acts_coverage_floor_genesis().unwrap();
