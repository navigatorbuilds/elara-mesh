...
/// Build the full `/mandate/status` FOUND-act body (B5 STATE 1 / STATE 2a): the
/// recomputed flag, anti-framing principal echo, and verified leaf→root lineage.
/// Shared by the indexed path and the record-only reconstruction so both render
/// identically. The caller stamps `record_present` (a CF_RECORDS point read) and
/// `index_entry_present` (true for STATE 1, false for STATE 2a). A found act is
/// judged from snapshot-carried mandate+revocation state and the found path never
/// reads the body, so `authoritative_complete` is always true here.
fn build_act_status_body(
    record_id: &str,
    entry: &crate::mandate::MandateActEntry,
    network_id: &str,
    resolver: &crate::network::mandate_node::StorageMandateResolver<'_>,
    record_present: bool,
    index_entry_present: bool,
) -> serde_json::Value {
    ...
    // A reference that isn't a well-formed mandate id is `malformed` (an honest
    // "this reference is not a valid mandate") — distinct from `no_chain` (a
    // well-formed id that simply doesn't resolve).
    ...
    let mut body = serde_json::json!({
        ...
        // Entry-driven verdict — authoritative regardless of how the node
        // bootstrapped and regardless of whether the body is still on disk.
        "authoritative_complete": true,
        "index_entry_present": index_entry_present,
        "record_present": record_present,
    });
    if !record_present {
        body["record_note"] = serde_json::json!(
            "record body pruned from this node; the offline receipt envelope / receipts \
             archive holds the full signed record"
        );
    }
    ...
    // verdict — a non-authorizing or unverifiable chain never names an ancestor
    // (anti-libel; the evaluator returns empty lineage for every other flag).
...
/// B5: compute the act-index coverage object + whether an ABSENT answer is
/// AUTHORITATIVE. Reads the durable coverage floor STATE (one bloom-filtered point
/// read) and the node's zone scope. Absence is authoritative only when the floor
/// is a WRITTEN 0 (genuine full coverage), the node accepts ALL zones (B3 — a
/// zone-scoped node never indexes out-of-zone acts), it did NOT bootstrap from a
/// snapshot, and no storage read faulted. An UNINITIALIZED floor (key absent or
/// malformed — e.g. a node whose one-time boot init write transiently failed) is
/// fail-CLOSED to non-authoritative, NOT collapsed to the `0` that means full
/// coverage (that collapse was a false-negative reopener found by the B5 post-ship
/// review). Each failed condition adds an honest `basis` tag. Returns
/// `(acts_coverage_json, authoritative, storage_error)`.
fn build_acts_coverage(state: &NodeState) -> (serde_json::Value, bool, bool) {
    let accepts_all = state
        .zone_manager
        .lock_recover()
        .subscribed_zones()
        .is_empty();
    ...
    let floor_state = match state.rocks.acts_coverage_floor_state() {
        Ok(s) => s,
        Err(_) => {
            // A storage fault reading the floor is NEVER a confident answer.
            let cov = serde_json::json!({
                "complete_from_ms": serde_json::Value::Null,
                "basis": ["storage_error"],
            });
            return (cov, false, true);
        }
    };
    let Some(floor) = floor_state else {
        // Uninitialized (key absent/malformed) → the node has NOT recorded its
        // coverage boundary, so it cannot vouch that an absent act never existed.
        // Fail-CLOSED: non-authoritative, distinct from a written genesis-0.
        let cov = serde_json::json!({
            "complete_from_ms": serde_json::Value::Null,
            "basis": ["uninitialized"],
        });
        return (cov, false, false);
    };
    let mut basis: Vec<&str> = Vec::new();
    if floor > 0 {
        basis.push("pruned_or_upgrade_baseline");
    }
    if snapshot {
        basis.push("snapshot_baseline");
    }
    if !accepts_all {
        basis.push("zone_subset");
    }
    let authoritative = floor == 0 && accepts_all && !snapshot;
    let cov = serde_json::json!({ "complete_from_ms": floor, "basis": basis });
    (cov, authoritative, false)
}

/// `GET /mandate/status/{record_id}` — B5 three-state honesty contract (verdict
/// 2026-07-18). Returns exactly one of: STATE 1 INDEXED (act entry present →
/// authoritative found verdict), STATE 2 RECORD-ONLY (no entry, body present →
/// reconstruct-or-refute from the signed record), STATE 3 ABSENT (both miss →
/// coverage-window answer that can be authoritative "not a mandate act" only
/// inside the covered window, never a confident lie over a pruned/zone/snapshot
/// gap). A pruned or absent act can no longer read as an authoritative
/// "not a mandate act".
pub async fn mandate_status(
    ...
    // ── STATE 1: the act index has an entry (checked getter — a read fault is
    //    surfaced as storage_error via STATE 3, never a confident miss). ──
    match state.rocks.get_mandate_act_checked(&record_id) {
        Ok(Some(entry)) => {
            let record_present = state
                .rocks
                .get_record(&record_id)
                .map(|o| o.is_some())
                .unwrap_or(false);
            return Json(build_act_status_body(
                &record_id,
                &entry,
                network_id,
                &resolver,
        ...
        Err(_) => {
            return Json(serde_json::json!({
                ...
                "absence_basis": "storage_error",
                "authoritative_complete": false,
                "hint": "a storage read failed; retry or query the receipts archive",
            }));
        }
    }

    // ── STATE 2: no index entry, but the record BODY is present — the signed
    //    record itself is the ground truth (strict upgrade over a blanket miss). ──
    match state.rocks.get_record(&record_id) {
        Ok(Some(rec)) => {
            // 2a: the record carries a non-empty string mandate_ref — the EXACT
            // ingest guard (mandate_node.rs), so nothing ingest declined to index
            // is served. Reconstruct the entry byte-identically and serve the
            // found shape (index_entry_present:false). Strictly read-pure — a
            // public GET never heals the index.
            if let Some(mref) = rec
                .metadata
                .get(crate::mandate::MANDATE_REF_METADATA_KEY)
                .and_then(|v| v.as_str())
                .filter(|s| !s.is_empty())
            {
                let entry = crate::mandate::MandateActEntry::new(
                    mref,
                    crate::crypto::hash::sha3_256_hex(&rec.creator_public_key),
                    crate::mandate::secs_f64_to_ms_saturating(rec.timestamp),
                    crate::network::mandate_node::amount_from_record(&rec),
                );
                return Json(build_act_status_body(
                    &record_id,
                    &entry,
            ...
            // 2b: the body exists but is not a mandate act (no/empty/non-string
            // ref) — the signed record itself is the refutation, authoritative on
            // EVERY node including snapshot followers.
            return Json(serde_json::json!({
                "record_id": record_id,
                "is_mandate_act": false,
                "absence_basis": "record_checked",
                "record_present": true,
                "authoritative_complete": true,
            }));
        }
    ...
    // ── STATE 3: ABSENT (no entry, no body). Removed-marker first (a deliberate
    //    forensic un-index), then the coverage-window verdict. ──
    match state.rocks.has_act_removed_marker(&record_id) {
        Ok(true) => {
            return Json(serde_json::json!({
                "record_id": record_id,
                "is_mandate_act": false,
                ...
                "authoritative_complete": true,
                "note": "an operator forensically un-indexed this act; its absence is deliberate, not a coverage gap",
            }));
        }
        Ok(false) => {}
        Err(_) => {
    ...
    let (cov, authoritative, storage_err) = build_acts_coverage(&state);
    let (absence_basis, hint): (&str, Option<&str>) = if storage_err {
        ("storage_error", Some("a storage read failed; retry or query the receipts archive"))
    } else if authoritative {
        ("full_coverage", None)
    } else {
        (
            "outside_coverage",
            Some("coverage window incomplete on this node — query the receipts archive for a definitive answer"),
        )
    };
    let mut body = serde_json::json!({
        "record_id": record_id,
        ...
        "acts_coverage": cov,
        "authoritative_complete": authoritative,
    });
    if let Some(h) = hint {
...
pub async fn mandate_acts(
    ...
    let acts: Vec<serde_json::Value> = record_ids
        ...
        .filter_map(|rid| {
            ...
            let mut v = render_act_compact(rid, &entry, network_id, &resolver);
            // B5: per-row record_present (one bounded point read, ≤page-size).
            let present = state.rocks.get_record(rid).map(|o| o.is_some()).unwrap_or(false);
            v["record_present"] = serde_json::json!(present);
            Some(v)
        })
        .collect();

    // B5 coverage honesty: the enumeration is authoritative only when the query
    // succeeded, the node did NOT bootstrap from a snapshot, the act-index
    // coverage floor is 0, AND the node accepts all zones (a zone-scoped node
    // omits out-of-zone acts). `acts_coverage` carries the exact window + basis so
    // a `{count:0}` is never misread as "this agent never acted".
    let (cov, cov_authoritative, _cov_storage_err) = build_acts_coverage(&state);
    let authoritative_complete = query_ok && cov_authoritative;
    Json(serde_json::json!({
        "mandate_id": mid,
        ...
        "acts_coverage": cov,
        // true only on a full-history, all-zones, in-window node.
...
pub async fn agent_acts(
    ...
    // B5 coverage honesty (same fold as mandate_acts): authoritative only on a
    // full-history, all-zones, in-window, fault-free enumeration.
    let (cov, cov_authoritative, _cov_storage_err) = build_acts_coverage(&state);
    let authoritative_complete = query_ok && cov_authoritative;
    Json(serde_json::json!({
        "agent_identity_hash": ah,
...
pub(crate) async fn compute_governance_params(state: Arc<NodeState>) -> serde_json::Value {
    ...
    // NOTE — enforcement split (see GovernableParams doc + design brief
    // GOVERNABLE-PARAMS-WIRING-SPLIT-2026-07-21): only propagation_rate_limit_per_hour
    // and stake_throughput_ratio are enforced at runtime. The other three are recorded
    // + displayed here as voted, but the runtime reads config.* and no governance→config
    // sync exists, so those values are advisory (a vote changes them here but not behaviour).
    serde_json::json!({
...
pub async fn compute_list_witness_profiles(
    ...
    let mut profiles: Vec<serde_json::Value> = consensus.profiles()
        ...
        .collect();
    // Sort OUTSIDE the consensus guard — seal ingest contends this mutex.
    drop(consensus);
