...
//! It is **not yet wired into seal production or consensus** — that is slice
//! (a2), done with the chain owner present, because seal metadata is
//! fork-sensitive. Nothing in the live path constructs a `DrandPulse` yet,
//! so this module is runtime-inert until then (mirrors how Gap-7's
//! `account_state_root` and REALMS (b)'s transport certs landed
//! type-first).
//!
...
//! gated behind the `verify-cli` feature only). So an anchor artifact's
//! not-before is trustless when checked by `elara-verify`; this in-protocol
//! module remains runtime-inert (no producer constructs a `DrandPulse` yet —
//! that is the fork-sensitive a2 slice). The OTS/TSA anchor braid (P1.5(b))
//! independently carries the existed-by leg.
//!
//! Spec references:
//!   @spec Protocol §11.12
