...
/// Domain-separation tag for v6+ witness attestations (T63 verdict 2026-08-18).
/// A witness counter-signature and a creator signature over the SAME
/// `signable_bytes()` were indistinguishable — the `/witness` public-oracle
/// class: any signature a node hands out over caller-supplied record bytes
/// could be replayed wherever a creator signature is expected. The tag makes
/// the attestation preimage a distinct domain. `V1` = the tag's own version,
/// decoupled from the wire version (same rule as `DOMAIN_TAG_RECORD_V1`).
pub const DOMAIN_TAG_WITNESS_ATTESTATION_V1: &[u8] = b"ELARA_WITNESS_ATTESTATION_V1";

/// THE witness-attestation preimage — every sign site and every verify site
/// MUST route through this one function so the two can never diverge (the
/// T63 enumeration table lists all twelve — 3 sign + 9 verify; a thirteenth
/// site must call this. The original 8-site table missed 4 verify sites —
/// sync.rs receive_attestation, state_core deferred drain, auto_witness
/// pull ×2 — caught 2026-08-18 by sweeping every `store_attestation_with_powas`
/// caller's verify leg; that sweep, not the sign-call grep, is the complete
/// enumeration method).
///
...
pub fn witness_attestation_preimage(record: &elara_record::record::ValidationRecord) -> Vec<u8> {
    ...
    if record.version >= 6 {
        let mut buf =
            Vec::with_capacity(DOMAIN_TAG_WITNESS_ATTESTATION_V1.len() + signable.len());
        buf.extend_from_slice(DOMAIN_TAG_WITNESS_ATTESTATION_V1);
        buf.extend_from_slice(&signable);
        buf
    } else {
        signable
    }
...
impl WitnessManager {
    ...
    pub fn store_attestation_with_powas(
        ...
        // Fail-closed timestamp guard: CF_IDX_ATT_TIME keys embed the
        // timestamp's big-endian bytes, and byte order == numeric order ONLY
        // for finite non-negative doubles. A wire-supplied NaN/Inf/negative
        // (incl. -0.0) would sort past every real entry — poisoning the
        // latest-timestamp cursor and hiding from the index-driven prune. No
        // honest witness produces one (SystemTime is finite non-negative).
        if !timestamp.is_finite() || timestamp.is_sign_negative() {
            return Err(ElaraError::Wire(format!(
                "attestation timestamp not indexable (finite non-negative required): {timestamp}"
            )));
        }
    ...
    /// the attestation preimage. This is the preferred entry point for
    /// attestations received from the network.
    ///
    /// `signable_bytes` MUST be the output of [`witness_attestation_preimage`]
    /// (bare `signable_bytes()` for v≤5 records, tag-prefixed for v6+) — never
    /// a hand-rolled preimage; the helper is the single authority (T63).
    ///
    /// Verification steps:
    /// 1. witness_public_key must be provided (reject otherwise).
    /// 2. SHA3-256(witness_public_key) must equal witness_hash.
    /// 3. dilithium3_verify(preimage, signature, witness_public_key) must succeed.
    ///
    ...
    /// Delete attestations older than the given timestamp (strict `<`).
    /// Returns the number of pruned index entries (== attestations pruned in
    /// the steady state — the boot-time `backfill_time_index` keeps the index
    /// complete, and the store path writes both sides).
    ///
    /// O(pruned), not O(all): CF_IDX_ATT_TIME keys start with the timestamp's
    /// 8 big-endian bytes, and finite non-negative doubles order identically
    /// as numbers and as bytes — so a forward scan from the start of the CF
    /// yields ascending timestamps and stops at the first entry at/past the
    /// cutoff. (The old implementation full-scanned CF_ATTESTATIONS with a
    /// JSON parse per entry on every hourly pass — O(all_attestations).)
    ///
    /// Deletes run in bounded chunks with each attestation and its index
    /// entry in ONE WriteBatch — a torn prune can no longer orphan one side
    /// (the old two-loop delete could). Memory is O(chunk), never O(pruned).
    pub fn prune_before(&self, cutoff_ts: f64) -> Result<usize> {
        let cutoff_be = cutoff_ts.to_be_bytes();
        let mut total = 0usize;

        // Phase 1: ascending index seek up to the cutoff. Restarting from the
        // CF start each chunk is correct (previous chunk's keys are deleted)
        // and keeps the loop cursor-free.
        loop {
            let mut idx_keys: Vec<Vec<u8>> = Vec::new();
            let mut att_keys: Vec<Vec<u8>> = Vec::new();
            self.rocks.range_scan_cf_total_order(CF_IDX_ATT_TIME, &[], |key, _value| {
                if key.len() < 8 || key[..8] >= cutoff_be[..] {
                    return Ok(false); // ascending order → nothing older remains
                }
                // Index key = ts_be(8B) + record_id + ':' + witness_hash; the
                // attestation key is `att:` + that same suffix — reconstruct
                // byte-for-byte, no JSON parse needed.
                let mut att = Vec::with_capacity(ATT_PREFIX.len() + key.len() - 8);
                att.extend_from_slice(ATT_PREFIX);
                att.extend_from_slice(&key[8..]);
                att_keys.push(att);
                idx_keys.push(key.to_vec());
                Ok(idx_keys.len() < PRUNE_CHUNK)
            })?;
            if idx_keys.is_empty() {
                break;
            }
            let full_chunk = idx_keys.len() == PRUNE_CHUNK;
            total += idx_keys.len();
            self.rocks
                .delete_pair_batch_cf(CF_ATTESTATIONS, &att_keys, CF_IDX_ATT_TIME, &idx_keys)?;
            if !full_chunk {
                break;
            }
        }

        // Phase 2: junk region. Keys whose first 8 bytes are ≥ +Inf's encoding
        // (0x7FF0…) decode to +Inf/+NaN/negative timestamps — rejected at
        // store time by the indexability guard, but pre-guard rows written by
        // a hostile peer would sort PAST every real timestamp and be invisible
        // to phase 1 forever. Everything in this region is
        // invalid-by-construction, so delete unconditionally (same chunking).
        const JUNK_REGION_START: [u8; 8] = f64::INFINITY.to_be_bytes(); // 0x7FF0…
        loop {
            let mut idx_keys: Vec<Vec<u8>> = Vec::new();
            let mut att_keys: Vec<Vec<u8>> = Vec::new();
            self.rocks.range_scan_cf_total_order(CF_IDX_ATT_TIME, &JUNK_REGION_START, |key, _value| {
                if key.len() >= 8 {
                    let mut att = Vec::with_capacity(ATT_PREFIX.len() + key.len() - 8);
                    att.extend_from_slice(ATT_PREFIX);
                    att.extend_from_slice(&key[8..]);
...
mod tests {
    ...
    /// Extractor-immune existence probe: CF_ATTESTATIONS carries a fixed
    /// 41-byte prefix extractor sized for real `att:{uuid}:` prefixes, so
    /// prefix reads with short TEST ids are out of domain — point GETs are
    /// unaffected and are what these prune tests assert on.
    fn att_exists(engine: &StorageEngine, record_id: &str, witness_hash: &str) -> bool {
        engine
            .get_cf_raw(CF_ATTESTATIONS, &WitnessManager::att_key(record_id, witness_hash))
            .unwrap()
            .is_some()
    }

    #[test]
    fn prune_boundary_is_strict_and_cleans_both_cfs() {
        let dir = tempfile::tempdir().unwrap();
        let engine = Arc::new(StorageEngine::open(dir.path()).unwrap());
        let mgr = WitnessManager::new(engine.clone());
        mgr.store_attestation("rec_50", "wit_1", b"sig", 50.0, None).unwrap();
        mgr.store_attestation("rec_100", "wit_2", b"sig", 100.0, None).unwrap();
        mgr.store_attestation("rec_200", "wit_3", b"sig", 200.0, None).unwrap();

        // Strict `<`: exactly the ts=50 entry goes; ts==cutoff stays.
        assert_eq!(mgr.prune_before(100.0).unwrap(), 1);
        assert!(!att_exists(&engine, "rec_50", "wit_1"));
        assert!(att_exists(&engine, "rec_100", "wit_2"));
        assert!(att_exists(&engine, "rec_200", "wit_3"));
        // Index side pruned in lockstep: the since-scan sees only survivors.
        let rows = mgr.get_attestations_since(0.0, 100).unwrap();
        assert_eq!(rows.len(), 2);
        // Idempotent: nothing older remains.
        assert_eq!(mgr.prune_before(100.0).unwrap(), 0);
    }

    #[test]
    fn prune_spans_multiple_chunks() {
        let dir = tempfile::tempdir().unwrap();
        let engine = Arc::new(StorageEngine::open(dir.path()).unwrap());
        let mgr = WitnessManager::new(engine.clone());
        // One full chunk plus a partial second one, all below the cutoff,
        // plus one survivor above it.
        let n = PRUNE_CHUNK + 8;
        for i in 0..n {
            mgr.store_attestation(&format!("rec_{i:05}"), "wit", b"sig", 10.0 + i as f64, None)
                .unwrap();
        }
        mgr.store_attestation("rec_keep", "wit", b"sig", 1_000_000.0, None).unwrap();

        assert_eq!(mgr.prune_before(500_000.0).unwrap(), n);
        assert!(att_exists(&engine, "rec_keep", "wit"));
        assert!(!att_exists(&engine, "rec_00000", "wit"));
        let rows = mgr.get_attestations_since(0.0, 10).unwrap();
        assert_eq!(rows.len(), 1);
    }

    #[test]
    fn prune_reaps_junk_region_index_entries() {
        // Pre-guard hostile rows: index keys encoding +Inf / NaN / negative
        // timestamps sort PAST every real key (invisible to the ascending
        // phase-1 seek) — phase 2 must reap them regardless of the cutoff.
        let dir = tempfile::tempdir().unwrap();
        let engine = Arc::new(StorageEngine::open(dir.path()).unwrap());
        let mgr = WitnessManager::new(engine.clone());

        mgr.store_attestation("rec_ok", "wit_ok", b"sig", 200.0, None).unwrap();
        for (i, ts) in [f64::INFINITY, f64::NAN, -1.0].into_iter().enumerate() {
            let rid = format!("rec_junk_{i}");
            let idx = WitnessManager::time_idx_key(ts, &rid, "wit_j");
            let att = WitnessManager::att_key(&rid, "wit_j");
            engine.put_cf_raw(CF_IDX_ATT_TIME, &idx, &[]).unwrap();
            engine.put_cf_raw(CF_ATTESTATIONS, &att, b"{}").unwrap();
        }

        // Cutoff below the legit entry: phase 1 prunes nothing, phase 2
        // reaps all three junk pairs.
        assert_eq!(mgr.prune_before(100.0).unwrap(), 3);
        assert!(att_exists(&engine, "rec_ok", "wit_ok"));
        assert!(!att_exists(&engine, "rec_junk_0", "wit_j"));
        assert!(!att_exists(&engine, "rec_junk_1", "wit_j"));
        assert!(!att_exists(&engine, "rec_junk_2", "wit_j"));
        // Latest-timestamp cursor is sane again (junk no longer sorts last).
        let latest = mgr.get_latest_timestamp().unwrap();
        assert!((latest - 200.0).abs() < 0.001);
    }

    #[test]
    fn store_rejects_unindexable_timestamps() {
        let (mgr, _dir) = test_mgr();
        for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY, -1.0, -0.0] {
            assert!(
                mgr.store_attestation("rec_bad", "wit", b"sig", bad, None).is_err(),
                "timestamp {bad} must be rejected"
            );
        }
        // +0.0 is finite and non-negative — indexable, sorts first.
        assert!(mgr.store_attestation("rec_zero", "wit", b"sig", 0.0, None).unwrap());
    }
    ...
    /// T63: the attestation-preimage dispatch. v≤5 = bare signable_bytes
    /// byte-identical (today's fleet behavior, unchanged until the flag day);
    /// v6+ = tag-prefixed AND domain-disjoint from the record's own v6
    /// preimage by leading bytes.
    #[test]
    fn attestation_preimage_dispatch_v5_identity_v6_tagged_disjoint() {
        use elara_record::record::{Classification, ValidationRecord, DOMAIN_TAG_RECORD_V1};

        let mut rec = ValidationRecord::create(
            b"attestation preimage test",
            vec![0xAA; 1952],
            vec![],
            Classification::Public,
            None,
        );
        // FLAG DAY 2026-08-19: create() now emits v6 — pin the v5 specimen
        // explicitly; the v5 arm guards the chain's HISTORY forever.
        rec.version = 5;
        rec.network_id = String::new();

        // v5: pass-through, byte-identical — a tagged v5 attestation would
        // break every historical attestation on the chain.
        assert_eq!(
            witness_attestation_preimage(&rec),
            rec.signable_bytes(),
            "v5 attestation preimage must remain the bare signable bytes"
        );

        // v6: tag-prefixed, and disjoint from the record's own preimage domain.
        rec.version = 6;
        rec.network_id = "testnet".into();
        let att = witness_attestation_preimage(&rec);
        let rec_pre = rec.signable_bytes();
        assert!(att.starts_with(DOMAIN_TAG_WITNESS_ATTESTATION_V1));
        assert!(rec_pre.starts_with(DOMAIN_TAG_RECORD_V1));
