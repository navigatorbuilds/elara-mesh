...
/// One-shot startup migration of `CF_ACCOUNT_SMT` from the legacy full-tail
/// layout to the v2 compact layout (R37, internal design notes
/// §3 Stage 2 / condition C4: migrate now, behind the format key, or not at
/// all — never half-migrated).
///
/// The leaf set is enumerated from the store's OWN `v:` rows (never the
/// ledger — an orphan SMT leaf must survive migration or the root would
/// change), and `elara_smt::migrate_legacy_to_v2` refuses with the store
/// untouched unless the v2 rebuild reproduces the committed root exactly.
/// A store past `MIGRATE_LEAF_CAP` leaves is refused here (`Err`) and simply
/// stays on the legacy walk — correct, just slower; migrate it in a
/// maintenance window instead. Call during single-threaded startup, before
/// any `account_smt_write_gate` holder can run.
pub fn migrate_account_smt_store(
    storage: &crate::storage::rocks::StorageEngine,
) -> Result<elara_smt::MigrateOutcome> {
    /// Startup-migration ceiling: at ~257 batch ops per leaf (tail tombstones
    /// plus v2 rows) this caps the one-shot atomic batch near ~1M ops. The
    /// live store is ~1 account today; a store that outgrew this cap gets an
    /// explicit refusal, never a partial migration.
    const MIGRATE_LEAF_CAP: usize = 4096;
    let leaves = storage.account_smt_value_leaves(MIGRATE_LEAF_CAP + 1)?;
    if leaves.len() > MIGRATE_LEAF_CAP {
        return Err(ElaraError::Storage(format!(
            "account-SMT v2 migration refused: >{MIGRATE_LEAF_CAP} leaves — store stays on the \
             legacy walk (correct, slower); migrate in a maintenance window"
        )));
    }
    let mut tree = AccountStateSMT::new(storage);
    tree.inner.migrate_legacy_to_v2(&leaves)
}
...
pub fn apply_snapshot(
    ...
    // R38 (audit stage 3): sharded parallel flush. Entries partition by the
    // top SHARD_BITS of their tree path (disjoint subtrees → owned delta per
    // shard, computed read-only in parallel), then one serial merge replays
    // boundary-crossing refusals and recomputes the crown. Byte-identical to
    // the serial walk (elara-smt battery pins root AND store content across
    // widths); worth the fan-out only past a batch-size floor, and only on a
    // v2 store (legacy stores refuse wholesale — the serial loop below is
    // then strictly less overhead).
    const SHARD_BITS: u8 = 4; // 16 shards ≈ 2× this box's 8 hardware threads
    const PARALLEL_MIN_ENTRIES: usize = 256;

    let mut tree = AccountStateSMT::new(storage);
    if snapshot.len() >= PARALLEL_MIN_ENTRIES && tree.inner.detected_format()? == 2 {
        use rayon::prelude::*;
        let mut groups: Vec<Vec<elara_smt::ShardEntry>> =
            (0..(1usize << SHARD_BITS)).map(|_| Vec::new()).collect();
        for (account_id, op) in snapshot {
            let path = elara_smt::account_path(account_id);
            let idx = (path[0] >> (8 - SHARD_BITS)) as usize;
            groups[idx].push((path, *account_id, *op));
        }
        let inner = &tree.inner;
        let deltas = groups
            .par_iter()
            .filter(|g| !g.is_empty())
            .map(|entries| inner.compute_shard_delta(SHARD_BITS, entries))
            .collect::<Result<Vec<_>>>()?;
        tree.inner.apply_shard_deltas(deltas)?;
    } else {
...
mod tests {
    ...
    /// R38 acceptance: the parallel sharded flush must produce a root
    /// byte-identical to the serial walk under thread pools of 1/2/4/8,
    /// REPEATED — on the real RocksDB store. (Store-content equality across
    /// shard widths is pinned crate-side; this pins the thread axis on the
    /// production path.)
    #[test]
    fn r38_parallel_flush_thread_count_invariance() {
        let seed_accounts: Vec<([u8; 32], [u8; 32])> = (0..1200u32)
            .map(|i| (acc(&i.to_be_bytes()), sha3_256(&[7, i as u8, (i >> 8) as u8])))
            .collect();
        // Mixed batch: 500 updates of existing, 200 deletes, 150 fresh inserts
        // (past the PARALLEL_MIN_ENTRIES=256 floor).
        let batch: Vec<([u8; 32], Option<[u8; 32]>)> = (0..500u32)
            .map(|i| (acc(&i.to_be_bytes()), Some(sha3_256(&[9, i as u8]))))
            .chain((500..700u32).map(|i| (acc(&i.to_be_bytes()), None)))
            .chain((0..150u32).map(|i| {
                (acc(&[0xC7, i as u8, (i >> 8) as u8]), Some(sha3_256(&[3, i as u8])))
            }))
            .collect();

        let seed_store = |storage: &StorageEngine| {
            let mut t = AccountStateSMT::new(storage);
            for (id, v) in &seed_accounts {
                t.update(id, v).unwrap();
            }
            t.commit().unwrap();
        };

        // Serial reference: per-entry walk, no sharding.
        let (ref_storage, _d0) = test_storage();
        seed_store(&ref_storage);
        let mut ref_tree = AccountStateSMT::new(&ref_storage);
        for (id, op) in &batch {
            match op {
                Some(v) => ref_tree.update(id, v).unwrap(),
                None => ref_tree.delete(id).unwrap(),
            }
        }
        ref_tree.commit().unwrap();
        let want_root = ref_tree.root().unwrap();
        drop(ref_tree);

        for threads in [1usize, 2, 4, 8] {
            for repeat in 0..2 {
                let (storage, _d) = test_storage();
                seed_store(&storage);
                let pool = rayon::ThreadPoolBuilder::new()
                    .num_threads(threads)
                    .build()
                    .unwrap();
                let (n, root) = pool.install(|| apply_snapshot(&storage, &batch)).unwrap();
                assert_eq!(n, batch.len());
                assert_eq!(
                    root, want_root,
                    "parallel flush diverged at {threads} threads (repeat {repeat})"
                );
            }
        }
    }

    /// R37 startup migration on the REAL RocksDB store: legacy full-tail
    /// content is migrated to v2 with the root preserved, orphan leaves
    /// survive (the leaf set comes from the store, not the ledger), the
    /// orphan scan still enumerates every `v:` leaf, and post-migration
    /// mutations run the compact walk.
    #[test]
    fn migrate_account_smt_store_preserves_root_on_rocks() {
        let (storage, _dir) = test_storage();
        // Write legacy-format content directly (forced legacy walk on the
        // rocks adapter — a fresh store would otherwise be born v2).
        let mut legacy =
            elara_smt::SparseMerkleTree::new_legacy_unchecked(RocksSmtStore { storage: &storage });
        for i in 0..5u8 {
            legacy.update(&acc(&[i, 0xA0]), &sha3_256(&[i, 0xB0])).unwrap();
        }
        // An SMT-ahead orphan (no ledger account) — must survive migration.
        legacy.update(&acc(b"rocks-orphan"), &sha3_256(b"ghost")).unwrap();
        legacy.commit().unwrap();
        let legacy_root = legacy.root().unwrap();
        drop(legacy);
        assert_eq!(AccountStateSMT::new(&storage).inner.detected_format().unwrap(), 1);

        match migrate_account_smt_store(&storage).unwrap() {
            elara_smt::MigrateOutcome::Migrated { keys } => assert_eq!(keys, 6),
            other => panic!("expected Migrated, got {other:?}"),
        }

        let tree = AccountStateSMT::new(&storage);
        assert_eq!(tree.inner.detected_format().unwrap(), 2);
        assert_eq!(tree.root().unwrap(), legacy_root, "migration must preserve the root");
        assert_eq!(tree.get(&acc(b"rocks-orphan")).unwrap(), Some(sha3_256(b"ghost")));
        // Second call is a recognized no-op.
        assert_eq!(
            migrate_account_smt_store(&storage).unwrap(),
            elara_smt::MigrateOutcome::AlreadyV2
        );
        // The orphan scan still sees every leaf (v: rows untouched by v2).
        let scan = scan_orphan_smt_leaves(&storage, &std::collections::HashSet::new(), 1000, 16);
        assert_eq!(scan.scanned_leaves, 6);
        // Post-migration mutations run the compact walk and stay proof-sound.
        let mut tree = AccountStateSMT::new(&storage);
        tree.update(&acc(b"post-migrate"), &sha3_256(b"pm")).unwrap();
        tree.commit().unwrap();
        let p = tree.proof(&acc(b"post-migrate")).unwrap().unwrap();
        assert!(elara_smt::verify_proof(&p));
    }
    ...
    fn make_record_with_creator(
    ...
    ) -> crate::record::ValidationRecord {
        {
            ...
            __r.id = "test-rec".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = sha3_256(b"scope-test").to_vec();
            __r.creator_public_key = creator_pk;
            __r.timestamp = 1700000000.0;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
    ...
    // ─── R35 soak: production-path flush throughput + incremental memory ───
    //
    // Rewritten 2026-07-27 per internal design notes §3
    // Stage 0. The old smt_soak_10k_records_per_sec_under_300mb_rss was doubly
    // invalid (audit finding 7): its rec/s was a decoupled generator/flush
    // ratio — a ~1.95× faster generator would pass the 9,000 gate with zero
    // SMT change — and its RSS assert compared current VmRSS, sampled AFTER
    // the flush tree was dropped, against a 300 MB cap in a test binary whose
    // baseline is ~1.7 GB (unconditionally false, masked only by the
    // throughput assert panicking first). It also exercised `flush_dirty`
    // (test-parity-only, zero production callers).
    //
    // This version times `snapshot_dirty` + `apply_snapshot` ALONE (the path
    // production runs at seal time), samples VmRSS from a sidecar thread
    // DURING the flush for the true incremental peak, and asserts honest
    // regression floors with the R36→R38 stage ladder documented at the
    // constants. Gated `#[ignore]`; invoke via:
    //   cargo test --features node --lib --release -- --ignored smt_soak --nocapture
    ...
    #[test]
    #[ignore]
    fn smt_soak_apply_snapshot_flush_throughput_and_memory() {
        use crate::accounting::ledger::{AccountState, LedgerState};
        use std::sync::atomic::{AtomicBool, AtomicU64, Ordering};
        use std::sync::Arc;
        use std::time::Instant;

        // Regression floors — RAISE as the audit stages land (C1 ladder).
        // Asserted on ROUND 1 ONLY: the first post-seed flush is the
        // compaction-clean, reproducible number (measured 2026-07-27 on the
        // reference box: 513/s). Later rounds run against L0 accumulation
        // from prior flushes (~2.4M new rows each, no compaction window) and
        // degrade ~2× per round — that trend is REPORTED, never asserted,
        // because it measures store accumulation, not SMT cost. Production
        // epochs are 120 s apart and get compaction windows the soak
        // deliberately denies itself. NOTE the bench's cold-insert-into-
        // empty-tree number (~993/s) is a DIFFERENT shape — do not compare.
        // Ladder: R36 LANDED 2026-07-27 — round-1 measured 601/s (was
        // 433–513). R37 LANDED 2026-07-28 (v2 compact store + divergence
        // walk): round-1 measured 5,664/s — inside the audit's adjudicated
        // 5.7–7.1k/s single-thread band — floor raised 420 → 3_900
        // (0.7 × measured). The R35 ~2×/round L0 degradation is GONE
        // (5664/5664/5592: ~250× fewer rows per flush removed its cause), so
        // aggregate ≈ round-1 now. R38 LANDED 2026-07-28 (sharded parallel
        // flush, 16 shards on the top 4 path bits): round-1 measured
        // 17,529/s — 3.1× core scaling on this 4-core/8-thread box — floor
        // raised 3_900 → 12_000 (0.68 × measured). The 20_000/s design
        // target is a mainnet-class-hardware number (≥8 physical cores) —
        // never this 4-core box's gate.
        const REGRESSION_FLOOR_ACCOUNTS_PER_SEC: f64 = 12_000.0;
        // Warm proof-gen: R36 measured 162/s (per-level store probing); R37's
        // divergence short-circuit measured 24,624/s (~154× — audit finding
        // 8: the read path has no hash floor, it was all lookups). Floor at
        // ~0.5 × measured, cache-variance slack included.
        const PROOF_FLOOR_PER_SEC: f64 = 12_000.0;
        // Incremental flush memory at 10k dirty: O(dirty × 256) ≈ 700 MB
        // pre-R37 (audit finding 6); the v2 store's O(dirty × log N)
        // measured 17 MB. Ceiling dropped 2 GiB → 128 MiB per the audit's
        // R37 line — generous slack over measured, an order under the old
        // wall.
        const FLUSH_MEM_CEILING_BYTES: u64 = 128 * 1024 * 1024;

        const ACCOUNT_POOL: u64 = 50_000;
        const DIRTY_PER_ROUND: u64 = 10_000;
        const ROUNDS: usize = 3;
        const WARM_PROOFS: u64 = 200;

        let (storage, _dir) = test_storage();
        let mut ledger = LedgerState::new();

        // Pre-seed and build the tree via the PRODUCTION path. Untimed setup;
        // this one 50k-dirty flush transiently allocates ~3.5 GB today (same
        // O(dirty × 256) finding, at 5× the measured-round size).
        ...
        let pairs = snapshot_dirty(&mut ledger);
        let (seeded, _root) = apply_snapshot(&storage, &pairs).unwrap();
        assert_eq!(seeded as u64, ACCOUNT_POOL);

        // Sidecar sampler: max VmRSS observed while armed, so the peak is
        // captured DURING apply_snapshot, not after its tree is dropped.
        let run = Arc::new(AtomicBool::new(true));
        let armed = Arc::new(AtomicBool::new(false));
        let max_rss = Arc::new(AtomicU64::new(0));
        let sampler = {
            let (run, armed, max_rss) =
                (Arc::clone(&run), Arc::clone(&armed), Arc::clone(&max_rss));
            std::thread::spawn(move || {
                while run.load(Ordering::Relaxed) {
                    if armed.load(Ordering::Relaxed) {
                        if let Some(rss) = read_rss_bytes() {
                            max_rss.fetch_max(rss, Ordering::Relaxed);
                        }
                    }
        ...
        let mut total_flush_secs: f64 = 0.0;
        let mut peak_incremental: u64 = 0;
        let mut cursor: u64 = 0;
        let mut round1_accounts_per_sec: f64 = 0.0;

        for round in 0..ROUNDS {
            // Steady state: mutate DIRTY_PER_ROUND existing accounts in place.
            for _ in 0..DIRTY_PER_ROUND {
                let idx = cursor % ACCOUNT_POOL;
                let hex_id = hex::encode(sha3_256(&idx.to_be_bytes()));
                let a = ledger.accounts.get_mut(&hex_id).unwrap();
                a.tx_count += 1;
                a.available = a.available.wrapping_add(1);
                ledger.smt_dirty.insert(hex_id);
                cursor += 1;
            }
            let pairs = snapshot_dirty(&mut ledger);
            let rss_before = read_rss_bytes().unwrap_or(0);
            max_rss.store(rss_before, Ordering::Relaxed);
            armed.store(true, Ordering::Relaxed);
            let t = Instant::now();
            let (flushed, _root) = apply_snapshot(&storage, &pairs).unwrap();
            let secs = t.elapsed().as_secs_f64();
            armed.store(false, Ordering::Relaxed);
            let peak = max_rss.load(Ordering::Relaxed).saturating_sub(rss_before);
            if peak > peak_incremental {
                peak_incremental = peak;
            }
            total_flushed += flushed as u64;
            total_flush_secs += secs;
            let aps = flushed as f64 / secs;
            if round == 0 {
                round1_accounts_per_sec = aps;
            }
            eprintln!(
                "  round {}: {} accounts in {:.2}s = {:.0}/s, incremental peak {:.1} MB",
                round + 1,
                flushed,
                secs,
                aps,
        ...
        // Warm proof-gen baseline (read path — audit finding 8: no hash
        // floor, so R37's short-circuit should move this ~10×).
        let tree = AccountStateSMT::new(&storage);
        let t = Instant::now();
        let mut got: u64 = 0;
        for i in 0..WARM_PROOFS {
            let id = sha3_256(&(i * 97 % ACCOUNT_POOL).to_be_bytes());
            if tree.proof(&id).unwrap().is_some() {
                got += 1;
            }
        }
        let proof_secs = t.elapsed().as_secs_f64();
        assert_eq!(got, WARM_PROOFS, "all seeded accounts must prove");

        let aggregate_accounts_per_sec = total_flushed as f64 / total_flush_secs;
        let proofs_per_sec = WARM_PROOFS as f64 / proof_secs;
        eprintln!(
            "SMT soak (production path):\n  flushed: {} accounts over {} rounds\n  round-1 (asserted): {:.0} accounts/s\n  aggregate (reported, accumulation-sensitive): {:.0} accounts/s\n  incremental flush peak: {:.1} MB\n  warm proof-gen: {:.0} proofs/s",
            total_flushed,
            ROUNDS,
            round1_accounts_per_sec,
            aggregate_accounts_per_sec,
            peak_incremental as f64 / 1_048_576.0,
            proofs_per_sec,
        );

        assert!(
            round1_accounts_per_sec >= REGRESSION_FLOOR_ACCOUNTS_PER_SEC,
            "flush throughput regression (round 1): {round1_accounts_per_sec:.0} accounts/s < floor {REGRESSION_FLOOR_ACCOUNTS_PER_SEC}",
        );
        assert!(
            proofs_per_sec >= PROOF_FLOOR_PER_SEC,
            "proof-gen regression: {proofs_per_sec:.0} proofs/s < floor {PROOF_FLOOR_PER_SEC}",
        );
        // Incremental ceiling — only enforced where the sampler read RSS.
        if peak_incremental > 0 {
            assert!(
                peak_incremental < FLUSH_MEM_CEILING_BYTES,
                "incremental flush memory regression: peak {:.1} MB >= ceiling {:.1} MB",
                peak_incremental as f64 / 1_048_576.0,
                FLUSH_MEM_CEILING_BYTES as f64 / 1_048_576.0,
            );
        }
    }
