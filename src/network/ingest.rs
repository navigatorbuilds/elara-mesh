    ...
    if let Some(core) = state.state_core.get() {
        return match core.insert_record(record, super::state_core::RecordSource::Local).await {
            ...
            super::state_core::InsertResult::Error { message } => {
                Err(crate::errors::ElaraError::Storage(message))
            }
        };
    }
...
async fn insert_record_inner(state: &Arc<NodeState>, mut record: ValidationRecord, origin_hint: Option<u64>, skip_timestamp_defense: bool) -> crate::errors::Result<String> {
    ...
    let (staked_micro, stake_ratio, gov_prop_rate) = {
        ...
        // Trust tier check — skip for synced records (already validated on source node)
        // Use try_read to avoid blocking if trust write lock is held by feed_attestation.
        // Uses continuity-gated tier evaluation: identities need continuous presence
        // (not just calendar age) to advance tiers (Protocol §11.35).
        if !skip_timestamp_defense {
            let cont_score = state.continuity.try_lock()
                .map(|c| c.score(effective_identity, now_ts))
                .unwrap_or(1.0); // no gate if lock contended

            if let Ok(trust) = state.trust.try_read() {
                trust.check_submission_with_continuity(
                    effective_identity,
                    &state.config.genesis_authority,
                    now_ts,
                    staked_micro,
            ...
            // If trust lock is contended, skip the rate check this time.
            // The record is still validated by signature, balance, and global rate limit.

            // Reincarnation penalty: suspected identity resets get Tier 0 limits
            // regardless of their current trust tier (Protocol §6.4).
            if let Ok(reinc) = state.reincarnation.try_lock() {
                let multiplier = reinc.trust_multiplier(effective_identity);
                if multiplier < 1.0 {
                    // Suspected reincarnation — enforce Tier 0 daily limit
                    if let Ok(trust) = state.trust.try_read() {
                        if let Some(profile) = trust.get_profile(effective_identity) {
                            let daily = profile.daily_count(now_ts);
                            let penalized_limit = crate::accounting::trust::TIER_0_DAILY;
                            if daily >= penalized_limit {
                                return Err(ElaraError::Ledger(format!(
                                    "reincarnation penalty: suspected identity reset, \
                                     limited to {}/day, already submitted {} today",
                                    penalized_limit, daily,
                                )));
                            }
                        }
                    }
    ...
    // ── Content safety: tombstone processing ─────────────────────────────
    {
        ...
        // Check if this record itself is tombstoned
        let tombstoned = state.is_tombstoned(&rec_id)?;

        if tombstoned {
            info!("tombstoned record stored but not propagated: {}", &record.id[..record.id.len().min(16)]);
            return Ok(hash);
        }
    }
    ...
    if record.metadata.contains_key(EPOCH_OP_KEY) {
        ...
        if let Ok(Some(mut seal)) = extracted {
            ...
            {
                let record_ids = resolve_seal_record_ids(state, &seal.record_hashes);
                ...
                let mut consensus = state.consensus.lock_recover();
                consensus.register_seal_records(&record.id, record_ids);
            ...
            if let Ok(mut ledger) = state.ledger.try_write() {
                ...
                let proofed = match state.consensus.try_lock() {
                    ...
                    Err(_) => epoch::attach_xzone_proofs_from_seal(&mut ledger, &seal),
                };
                if proofed > 0 {
