...
/// **Persisted since T61 (2026-07-29).** Before that this map was rebuilt empty
/// on every process start, which made `systemctl restart` a self-service quota
/// reset: measured against this node's own history, EVERY day that exceeded the
/// cap was a restart day (peak 77 records = 3.85× a 20/day cap over 12 restarts),
/// and every zero-restart day held at or below the cap. That is a bypass any node
/// operator could use against any identity, so the counter now rides the same
/// snapshot path as `trust`/`delegations`/`zone_state` — see
/// internal design notes §2.1.
...
/// T63 R2 admission decision, pure for unit-testability: a record is admitted
/// when its `network_id` is EMPTY (legacy — v≤5 wire never carries the field;
/// this acceptance is what keeps the v6 emission flip non-forking) or matches
/// our configured network EXACTLY (case-sensitive byte equality — the value is
/// bound into the v6 signed preimage, so any normalization here would create a
/// preimage/admission mismatch).
pub(crate) fn network_id_admits(record_network_id: &str, local_network_id: &str) -> bool {
...
async fn insert_record_inner(state: &Arc<NodeState>, mut record: ValidationRecord, origin_hint: Option<u64>, skip_timestamp_defense: bool) -> crate::errors::Result<String> {
    ...
    // ── T63 R2: network binding enforcement (flag-day verdict Q1/A6) ─────
    // A NON-EMPTY network_id that differs from ours is a foreign-network (or
...
mod tests {
    ...
    // ── T63 R2: network-binding admission (flag-day verdict Q1/A6) ──
    // The ingest gate calls THIS fn — testing it tests the gate's decision
    // (gates-must-derive: no parallel reimplementation to drift).

    #[test]
    fn t63_network_id_admission_decision() {
        use super::network_id_admits;
        // Empty = legacy-accept, regardless of local config (non-forking flip).
        assert!(network_id_admits("", "elara-mainnet-1"));
        assert!(network_id_admits("", ""));
        // Exact match admits.
        assert!(network_id_admits("elara-mainnet-1", "elara-mainnet-1"));
        // Mismatch rejects — including the default-collision shape T65 §0
        // warned about (foreign "testnet" vs our configured name).
        assert!(!network_id_admits("testnet", "elara-mainnet-1"));
        // Case-sensitive byte equality: the value is inside the SIGNED v6
        // preimage; normalizing here would split admission from the preimage.
        assert!(!network_id_admits("Elara-Mainnet-1", "elara-mainnet-1"));
        // A record claiming a network while WE are unconfigured (empty local)
        // is a mismatch — fail closed, never "probably ours".
        assert!(!network_id_admits("elara-mainnet-1", ""));
    }
    ...
    #[test]
    fn t61_daily_cap_counter_survives_serde_round_trip() {
        let mut c = DailyCapCounter::new();
        let now = 1_700_000_000.0;
        for _ in 0..7 {
            assert!(c.check_and_increment("ident-a", now, 20));
        }
        assert!(c.check_and_increment("ident-b", now, 20));

        // Round-trip exactly as save_snapshot/load_snapshot does.
        let bytes = serde_json::to_vec(&c).expect("serialize");
        let mut restored: DailyCapCounter = serde_json::from_slice(&bytes).expect("deserialize");
        assert_eq!(restored.tracked_identities(), 2);

        // The window continues where it left off: 13 remain, not 20.
        for i in 0..13 {
            assert!(restored.check_and_increment("ident-a", now, 20), "slot {i} should remain");
        }
        assert!(
            !restored.check_and_increment("ident-a", now, 20),
            "restored counter must WALL at the cap — a restart is no longer a reset"
        );
    }

    #[test]
    fn t61_restored_counter_still_rolls_over_and_prunes_stale_windows() {
        let mut c = DailyCapCounter::new();
        let t0 = 1_700_000_000.0;
        for _ in 0..20 { assert!(c.check_and_increment("ident", t0, 20)); }
        assert!(!c.check_and_increment("ident", t0, 20));

        let bytes = serde_json::to_vec(&c).unwrap();
        let mut restored: DailyCapCounter = serde_json::from_slice(&bytes).unwrap();

        // Anchor survives, so the window is still closed just before 24h.
        let almost = t0 + crate::accounting::trust::DAILY_WINDOW_SECS - 1.0;
        assert!(!restored.check_and_increment("ident", almost, 20));

        // ...and still rolls over on the far side.
        let after = t0 + crate::accounting::trust::DAILY_WINDOW_SECS + 1.0;
        assert!(restored.check_and_increment("ident", after, 20));

        // Boot-path cleanup drops fully-elapsed windows (what the loader calls).
        let mut stale: DailyCapCounter = serde_json::from_slice(&bytes).unwrap();
        stale.cleanup(t0 + crate::accounting::trust::DAILY_WINDOW_SECS + 1.0);
        assert_eq!(stale.tracked_identities(), 0, "elapsed window must not be resurrected");
    }
        ...
        let build = |n_parents: usize, n_refs: usize, meta: BTreeMap<String, serde_json::Value>| {
            {
                ...
                __r.timestamp = 1.0;
                __r.parents = (0..n_parents).map(|i| format!("{i:036}")).collect();
                __r.classification = crate::record::Classification::Public;
                __r.metadata = meta;
                __r.signature = Some(vec![0u8; 3309]);
                __r.sphincs_signature = Some(vec![0u8; 35664]);
                __r.zk_proof = None;
                __r.itc_stamp = None;
                __r.zone_refs = (0..n_refs).map(|_| vec![0u8; 17]).collect();
                __r.creator_sphincs_pk = Some(vec![0u8; 48]);
                __r.sig_algorithm = 1;
                __r.sphincs_algorithm = Some(2);
                __r.zone = None;
    ...
    fn store_ledger_record(
        ...
        let record = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = record_id.into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = content_hash.to_vec();
            __r.creator_public_key = creator_pk;
            __r.timestamp = timestamp;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
    ...
    async fn batch_k_compute_witness_smt_scope_uses_fallback_zone_when_record_zone_unset() {
        ...
        let record = {
            ...
            __r.timestamp = 5.0;
            __r.metadata = metadata;
            __r.zone = None; // the load-bearing detail
            __r
        };
        let record_hash = record.record_hash();
        state.rocks.put_record(record_id, &record).unwrap();
    ...
    fn store_seal_record(
        ...
        let record = {
            ...
            __r.id = seal_id.into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0xABu8; 32];
            __r.creator_public_key = b"seal-creator".to_vec();
            __r.timestamp = start + 120.0;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
            __r.metadata = metadata;
            __r.signature = None;
    ...
    fn recheck_dag_deficits_updates_then_closes_when_records_arrive() {
        ...
        let pending = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = "rec-heal-b".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0xBBu8; 32];
            __r.creator_public_key = b"creator_b".to_vec();
            __r.timestamp = 2_000.7;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
            __r.metadata = crate::accounting::types::transfer_metadata(2, &"bb".repeat(32), None);
            __r.signature = None;
            __r.sphincs_signature = None;
            __r.zk_proof = None;
            __r.itc_stamp = None;
    ...
    fn recheck_dag_deficits_rederives_instead_of_false_closing_on_empty_enum() {
        ...
        let pending = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = "rec-fc-c".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0xCCu8; 32];
            __r.creator_public_key = b"fc_c".to_vec();
            __r.timestamp = 11.5;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
            __r.metadata = crate::accounting::types::transfer_metadata(3, &"cc".repeat(32), None);
            __r.signature = None;
            __r.sphincs_signature = None;
            __r.zk_proof = None;
            __r.itc_stamp = None;
        ...
        let seal_rec = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = "seal-fc-1".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0xABu8; 32];
            __r.creator_public_key = b"fc-sealer".to_vec();
            __r.timestamp = 12.5;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
            __r.metadata = meta;
    ...
    fn synthetic_above_cap_seal_derives_and_feeds_consumers() {
        ...
        let seal_rec = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = "seal-cap-1".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = vec![0xAAu8; 32];
            __r.creator_public_key = b"cap-sealer".to_vec();
            __r.timestamp = 100.0 + n as f64 + 1.0;
            __r.parents = vec![];
            __r.classification = crate::record::Classification::Public;
            __r.metadata = meta;
            __r.signature = None;
