...
pub async fn drain_and_commit_pending(state: &Arc<NodeState>) -> CommitStats {
    ...
    for rid in finalized_ids {
        // ── KR-3 S2 W1: rotation-finality CF writer (§3-3), flag-gated. Placed
        // at the very top of the loop — before the is_applied guard and the
        // missing-delta early-exit — because rotation-class records are
        // non-ledger ops (extract_ledger_op → None) for which the loop below
        // exits early; running first also future-proofs a Step-2
        // authority_transfer that carries a ledger delta. Byte-identical when the
        // S2 flag is OFF: the whole block is skipped.
        if state.config.s2_rotation_ordering_enabled {
            w1_write_rotation_entry(state, &rid).await;
        }
...
/// KR-3 S2 W1 — the finalize-drain hook (§3-3). For a just-finalized rotation
/// *hop* whose covering seal has settled, write its rotation-CF entry. The entry
/// (with `state ≥ Final`) IS the durable finalization predicate for
/// rotation-class records — never `FinalizedIndex` / the 24 h-pruned attestation
/// map — so the effect machine and boot replay read the CF alone (§3-3/§4). W1
/// runs on the finalize tick with fresh in-memory seal state, so it writes
/// `evidence = Quorum`; W2's durable-marker sweep (a later slice) is the
/// backstop for every case W1 skips. Both writers produce byte-identical entries
/// (idempotent keyed insert), so any crash interleaving reconverges.
///
/// No-op (writes nothing) in exactly the design's fail-closed cases:
/// - the record body is gone, or is not a clean prev→new rotation hop —
///   revocations return `None` from [`rotation_hop_fields`] (their CF-entry
///   shape is the §6.3 resolver's), as do all ordinary records;
/// - no covering seal has finalized yet — the **Layer-1-first race**, where a
///   record reaches consensus-Finalized via pure attestation before any seal
///   covers it (`covering_seal_finality` → `None`); the CF entry then arrives via
///   the W2 sweep once the sweep-duty seal settles (W2 is THE writer for this
///   race, by specification);
/// - the covering seal carries no drand pulse → no coordinate → non-effective.
///
/// Runs ONLY under `s2_rotation_ordering_enabled` (caller-gated), so flag-OFF
/// behaviour is byte-identical: the whole hook is skipped.
async fn w1_write_rotation_entry(state: &Arc<NodeState>, rid: &str) {
    use crate::network::rotation_finality as rf;

    // One point lookup to tell rotation-class from ordinary (only reached with
    // the flag ON). A missing body is nothing to finalize here — the ledger
    // drain below handles/records missing ordinary records.
    let record = match state.rocks.get_record(rid) {
        Ok(Some(r)) => r,
        _ => return,
    };
    // Rotation *hop* only (key_rotation | sphincs_key_rotation).
    if rf::rotation_hop_fields(&record).is_none() {
        return;
    }
    // Covering seal + its set-once monotonic finality — never the demotable
    // `latest_seal_*` pointers (B4). `None` = the Layer-1-first race → W2.
    let seal_record_id = {
        let consensus = state.consensus.lock_recover();
        match consensus.covering_seal_finality(rid) {
            Some((seal_id, _finalized_at)) => seal_id,
            None => return,
        }
    };
    // Load + parse the covering seal for its coordinate inputs.
    let seal_record = match state.rocks.get_record(&seal_record_id) {
        Ok(Some(r)) => r,
        _ => return,
    };
    let parsed = match crate::network::epoch::extract_epoch_seal(&seal_record) {
        Ok(Some(p)) => p,
        _ => return,
    };
    let pulse = match parsed.drand_pulse {
        Some(p) => p,
        None => return, // pulse-less seal → no coordinate (§4 fail-closed)
    };
    // Compose the entry; lineage resolves via the durable O(1) predecessor index
    // (`get_rotation_newkey_index`), so no O(all-rotations) scan (SCALE rule).
    let entry = rf::build_rotation_entry(
        &record,
        pulse.chain_hash.clone(),
        pulse.round,
        parsed.zone.path().to_string(),
        parsed.epoch_number,
        seal_record_id,
        pulse,
        rf::FinalityEvidence::Quorum,
        |prev_key_hash| state.rocks.get_rotation_newkey_index(prev_key_hash),
    );
    let Some(entry) = entry else { return };
    // Persist entry + forward index atomically. Idempotent with W2.
    match state.rocks.put_rotation_entry(&entry) {
        Ok(()) => {
            state
                .rotation_cf_write_drain_total
                .fetch_add(1, Ordering::Relaxed);
            // Display parity (FIN-1): surface the rotation as finalized in the
            // API even though it seal-fast-tracked with zero Layer-1
            // attestations. Explicitly NOT the effect predicate — the CF entry is.
            state.finalized.write().await.insert(rid.to_string());
        }
        Err(e) => {
            warn!(
                "W1 rotation-CF write failed for {}: {e}",
                &rid[..rid.len().min(16)]
            );
            state
...
mod tests {
    ...
    fn mk_record(
    ...
    ) -> ValidationRecord {
        {
            ...
            __r.creator_public_key = creator_pk.to_vec();
            __r.timestamp = ts;
            __r.parents = vec![];
    ...
    // ── KR-3 S2 W1 — finalize-drain rotation-CF writer (§3-3, §9 named races) ──
    //
    // These drive the REAL `drain_and_commit_pending` path (not the hook in
    // isolation) so they pin the in-situ wiring: the flag gate at the top of the
    // finalized-id loop, the Layer-1-first race deferral to W2, and the happy
    // path where a settled covering seal assigns the coordinate. `flag OFF` is
    // the deploy-safety property — byte-identical, no CF write — proven with the
    // SAME settled-seal state present, so it is the flag (not missing state) that
    // suppresses the write.

    /// NodeState with `s2_rotation_ordering_enabled = flag_on`. Mirrors
    /// `state_with_genesis`; the flag must be set at construction (config is not
    /// mutable through the `Arc<NodeState>` the hook reads).
    fn rotation_state(genesis_pk: &[u8], flag_on: bool) -> (Arc<NodeState>, tempfile::TempDir) {
        let tmp = tempfile::tempdir().expect("tempdir");
        let data_dir = tmp.path().to_path_buf();
        let config = NodeConfig {
            data_dir: data_dir.clone(),
            ...
            db_path: data_dir.join("elara.db"),
            admin_token: "test-admin".into(),
            network_id: "kr3-s2-w1-test".into(),
            mdns_enabled: false,
            health_check_interval_secs: 0,
            min_pow_difficulty: 0,
            genesis_authority: identity_hash_of(genesis_pk),
        ...
        let identity = Identity::generate(EntityType::Device, CryptoProfile::ProfileB)
            .expect("generate identity");
        let rocks =
            Arc::new(StorageEngine::open(data_dir.join("rocksdb")).expect("open rocksdb"));
        let wmgr = Arc::new(WitnessManager::new(rocks.clone()));
        let state = Arc::new(NodeState::new(config, identity, rocks, wmgr));
        (state, tmp)
    }

    /// A drand pulse carrying a `chain_hash` → a well-formed `FinalityCoord`.
    fn w1_pulse(round: u64) -> crate::network::time_bracket::DrandPulse {
    ...
    /// Store a key-rotation record (creator_pk = old signing key `pk_old`,
    /// metadata new key `pk_new`); return its id. The root hop's coordinate is
    /// keyed `(sha3(pk_old), 0)`.
    fn store_rotation(state: &Arc<NodeState>, old: &[u8], new: &[u8]) -> String {
        ...
        state
            ...
            .expect("put rotation record");
        rec.id
    }

    /// Base epoch-seal metadata (`extract_epoch_seal` parses it). Caller embeds a
    /// pulse (or not) before building the record.
    fn seal_meta(zone: &str, epoch: u64) -> BTreeMap<String, serde_json::Value> {
        ...
        m.insert("epoch_record_count".into(), serde_json::json!(0u64));
        m.insert(
    ...
    /// Build + store a covering epoch-seal record with an embedded pulse; return
    /// its id (the id W1 reads via `covering_seal_finality` → `get_record`).
    fn store_covering_seal(
        state: &Arc<NodeState>,
        zone: &str,
        epoch: u64,
        pulse: &crate::network::time_bracket::DrandPulse,
    ) -> String {
        let mut m = seal_meta(zone, epoch);
        ...
        state.rocks.put_record(&rec.id, &rec).expect("put seal record");
        rec.id
    }

    /// Seed a *settled* covering seal for `rid` and promote `rid` to Finalized so
    /// the next drain tick collects it.
    fn seed_and_finalize(state: &Arc<NodeState>, rid: &str, seal_id: &str, finalized_at: f64) {
        let mut c = state.consensus.lock_recover();
        c.test_seed_covering_seal(rid, seal_id, finalized_at);
        c.force_finalized(rid);
    }

    /// FLAG OFF ⇒ hook skipped ⇒ byte-identical: no CF entry, counter stays 0 —
    /// proven with a settled covering seal PRESENT (so it is the flag, not
    /// missing state, that suppresses the write). This is the deploy-safety
    /// invariant for landing the slice flag-default-OFF.
    #[tokio::test]
    async fn w1_flag_off_is_byte_identical_no_cf_write() {
        let genesis = pk(0x01);
        let (state, _t) = rotation_state(&genesis, false);
        let (old, new) = (pk(0x10), pk(0x11));
        let rid = store_rotation(&state, &old, &new);
        let seal_id = store_covering_seal(&state, "default", 5, &w1_pulse(100));
        seed_and_finalize(&state, &rid, &seal_id, 123.0);

        let _ = drain_and_commit_pending(&state).await;

        let lineage = sha3_256_hex(&old);
        assert!(
            state.rocks.get_rotation_entry(&lineage, 0).is_none(),
            "flag OFF must write no rotation CF entry"
        );
        assert_eq!(
            state.rotation_cf_write_drain_total.load(Ordering::Relaxed),
            0
        );
        assert!(
            !state.finalized.read().await.contains(&rid),
            "flag OFF must not force the display-parity finalized insert"
        );
    }

    /// §9 Layer-1-first race: FLAG ON, rotation record reaches consensus-Finalized
    /// via pure attestation BEFORE any seal covers it (no covering-seal finality
    /// seeded). W1 must write NOTHING — the CF entry is W2's to write once the
    /// sweep-duty seal settles.
    #[tokio::test]
    async fn w1_layer1_first_race_defers_to_w2() {
        let genesis = pk(0x01);
        let (state, _t) = rotation_state(&genesis, true);
        let (old, new) = (pk(0x20), pk(0x21));
        let rid = store_rotation(&state, &old, &new);
        // No covering seal → `covering_seal_finality` returns None.
        {
            let mut c = state.consensus.lock_recover();
            c.force_finalized(&rid);
        }
        ...
        assert!(
            state.rocks.get_rotation_entry(&sha3_256_hex(&old), 0).is_none(),
            "Layer-1-first race: W1 defers to W2, writes nothing"
        );
        assert_eq!(
            state.rotation_cf_write_drain_total.load(Ordering::Relaxed),
            0
    ...
    /// FLAG ON, rotation record + settled covering seal bearing a pulse ⇒ W1
    /// writes the root-hop CF entry, the coordinate is taken entirely from the
    /// covering seal, the drain counter increments, the display-parity finalized
    /// insert lands, and the forward predecessor index is populated for the next
    /// hop. The flag-flips-effect proof.
    #[tokio::test]
    async fn w1_writes_entry_on_settled_covering_seal() {
        use crate::network::rotation_finality::{FinalityEvidence, RotationKind, RotationState};
        let genesis = pk(0x01);
        let (state, _t) = rotation_state(&genesis, true);
        let (old, new) = (pk(0x30), pk(0x31));
        let rid = store_rotation(&state, &old, &new);
        let pulse = w1_pulse(200);
        let seal_id = store_covering_seal(&state, "default", 7, &pulse);
        seed_and_finalize(&state, &rid, &seal_id, 456.0);

        let _ = drain_and_commit_pending(&state).await;

        let prev_hash = sha3_256_hex(&old);
        let lineage = format!("d:{prev_hash}");
        let entry = state
            .rocks
            .get_rotation_entry(&lineage, 0)
            .expect("W1 wrote the root-hop rotation CF entry");
        assert_eq!(entry.hop_index, 0);
        assert_eq!(entry.record_id, rid);
        assert_eq!(entry.prev_key_hash, prev_hash);
        assert_eq!(entry.new_key_hash, sha3_256_hex(&new));
        assert_eq!(entry.kind, RotationKind::Rotation);
        assert_eq!(entry.state, RotationState::Final);
        assert_eq!(entry.evidence, FinalityEvidence::Quorum);
        // Coordinate assigned by the covering seal, never the record.
        assert_eq!(entry.coord.chain_hash, "cA");
        assert_eq!(entry.coord.round, 200);
        assert_eq!(entry.coord.zone_path, "default");
        assert_eq!(entry.coord.epoch, 7);
        assert_eq!(entry.coord.record_id, rid);
        assert_eq!(entry.seal_record_id, seal_id);
        ...
        assert!(
            state.finalized.read().await.contains(&rid),
            "FIN-1 display parity: rotation surfaces as finalized"
        );
        // Forward predecessor index: this hop's new key → its position, so the
        // NEXT hop resolves lineage O(1).
        assert_eq!(
            state.rocks.get_rotation_newkey_index(&sha3_256_hex(&new)),
            Some((lineage, 0))
        );
    }

    /// FLAG ON, rotation record but the covering seal carries NO drand pulse ⇒ no
    /// coordinate ⇒ non-effective (§4 fail-closed): W1 writes nothing.
    #[tokio::test]
    async fn w1_pulseless_covering_seal_writes_nothing() {
        let genesis = pk(0x01);
        let (state, _t) = rotation_state(&genesis, true);
        let (old, new) = (pk(0x40), pk(0x41));
        let rid = store_rotation(&state, &old, &new);
        // Covering seal with no pulse embedded.
        let seal = ValidationRecord::create(
            b"seal",
            pk(0x55),
            vec![],
            Classification::Public,
            Some(seal_meta("default", 9)),
        );
        state.rocks.put_record(&seal.id, &seal).expect("put seal");
        seed_and_finalize(&state, &rid, &seal.id, 789.0);

        let _ = drain_and_commit_pending(&state).await;

        assert!(
            state.rocks.get_rotation_entry(&sha3_256_hex(&old), 0).is_none(),
            "pulse-less covering seal must not finalize a rotation-class record"
        );
        assert_eq!(
            state.rotation_cf_write_drain_total.load(Ordering::Relaxed),
            0
    ...
    /// FLAG ON, an ordinary (non-rotation) finalized record ⇒ `rotation_hop_fields`
    /// returns None ⇒ W1 no-ops. Pins the branch that keeps the hook off the
    /// ledger-record hot path.
    #[tokio::test]
    async fn w1_non_rotation_record_no_ops() {
        let genesis = pk(0x01);
        let (state, _t) = rotation_state(&genesis, true);
        let rec = ValidationRecord::create(
            b"ordinary",
        ...
        state.rocks.put_record(&rec.id, &rec).expect("put record");
        {
            let mut c = state.consensus.lock_recover();
            c.force_finalized(&rec.id);
        }
        ...
        assert_eq!(
            state.rotation_cf_write_drain_total.load(Ordering::Relaxed),
            0,
            "non-rotation record must not write a rotation CF entry"
        );
    }
}
