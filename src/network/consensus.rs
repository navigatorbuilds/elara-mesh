...
///
/// ## ⚠ SUPERSEDED IN PRODUCTION — RETAINED DELIBERATELY. DO NOT DELETE AS DEAD CODE.
///
/// The consensus hot path calls [`select_epoch_committee_scoped_indexed`]
/// (`epoch.rs:5509`); this non-indexed variant's only remaining caller is
/// [`is_in_epoch_committee_scoped`], which is itself reached only from the
/// `#[cfg(test)]` module. A dead-code sweep will therefore flag it — **that is
/// expected, and removing it would break `benches/bench_committee_selection.rs`**,
/// which measures this as the labelled BASELINE against `_indexed` (production)
/// and `select_committee_v2` (the migration target). Those three side-by-side
/// numbers are how the v1→v2 migration is judged; the baseline is the point.
/// Verified 2026-07-26 (queue R34) while checking whether the `bench_account_smt`
/// dead-path defect generalised — it did not, precisely because this bench
/// measures all three variants rather than only the superseded one.
///
...
pub struct AWCConsensus {
    ...
    cross_zone_registered_total: u64,
...
impl AWCConsensus {
    ...
    pub fn register_cross_zone_parents(&mut self, record_id: &str, parent_ids: Vec<String>) {
        if !parent_ids.is_empty()
            && self
                .cross_zone_parents
                .insert(record_id.to_string(), parent_ids)
                .is_none()
        {
            self.cross_zone_registered_total =
                self.cross_zone_registered_total.saturating_add(1);
        }
    }

    /// Strictly monotonic count of records that ever registered cross-zone
    /// parents. Unlike the `cross_zone_stats()` gauges, this survives
    /// `prune_finalized` — the pruner deletes a record's map entry precisely
    /// BECAUSE the record succeeded, so any "did cross-zone linking happen?"
    /// check must read this counter, never the live map size (the 2026-08-02
    /// public-CI cross-zone-finality failure was exactly that misread: 40
    /// boosts fired, then every entry pruned, then the sim asserted len()>0).
    /// Re-registering parents on a live key does not grow it; a key
    /// re-registered after pruning counts as a fresh registration event.
    pub fn cross_zone_registered_total(&self) -> u64 {
        self.cross_zone_registered_total
    }
...
mod tests {
    ...
    /// Pins the 2026-08-02 public-CI cross-zone-finality failure shape:
    /// a HEALTHY run finalizes everything, `prune_finalized` deletes the
    /// `cross_zone_parents` entries (success erases the gauge), and any
    /// "did cross-zone linking happen?" check reading the live map sees 0.
    /// `cross_zone_registered_total` is the monotonic record of the event
    /// and MUST survive every prune path.
    #[test]
    fn cross_zone_registered_total_survives_prune() {
        let mut awc = AWCConsensus::new();
        assert_eq!(awc.cross_zone_registered_total(), 0);

        awc.register_cross_zone_parents("r1", vec!["p1".to_string()]);
        awc.register_cross_zone_parents("r2", vec!["q1".to_string(), "q2".to_string()]);
        // Re-register on a live key: gauge refs change, monotonic total does NOT.
        awc.register_cross_zone_parents("r1", vec!["p1".to_string(), "p2".to_string()]);
        assert_eq!(awc.cross_zone_registered_total(), 2);

        // Finalized-prune both records — the exact CI-run lifecycle.
        let finalized: std::collections::HashSet<String> =
            ["r1", "r2"].iter().map(|s| s.to_string()).collect();
        awc.prune_finalized(&finalized);

        let (live_records, live_refs, _) = awc.cross_zone_stats();
        assert_eq!((live_records, live_refs), (0, 0),
            "prune correctly empties the live gauges");
        assert_eq!(awc.cross_zone_registered_total(), 2,
            "monotonic registration count MUST survive prune_finalized — \
             asserting on the live map after a healthy run reads 0 (the CI red)");

        // Attestation-lifecycle prune path must not roll it back either.
        awc.prune_older_than(1e12);
        assert_eq!(awc.cross_zone_registered_total(), 2);
    }
