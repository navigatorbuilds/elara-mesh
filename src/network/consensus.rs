...
pub struct AWCConsensus {
    ...
    /// entries are evicted to keep memory O(active_window). Drops are non-
    /// fatal: a transfer whose seal predates the eviction falls back to
    /// the legacy inclusion-only path (`committee_size == 0`) and is
    /// replayable from chain history.
...
/// the oldest seal-id entries are evicted; any pending transfer whose
/// seal predates the eviction falls back to the legacy inclusion-only
/// path (committee_size = 0) which remains correct (just unenforced)
/// until Phase 2c flips the kill switch.
...
impl AWCConsensus {
    ...
    pub fn add_seal_attestation(&mut self, attestation: SealAttestation) {
        ...
        if self.is_seal_settled(&seal_id) {
            ...
            if let Some(record_ids) = record_ids_in_seal {
                for rid in &record_ids {
                    ...
                    // Only fire the commit callback on the pre-finality →
                    // Finalized edge. A seal that re-lands its own records (already
                    // at Finalized/Anchored via recompute or force) must not
                    // double-fire — that would double-commit the pending delta.
                    if !matches!(
                        old,
                        Some(ConfirmationLevel::Finalized) | Some(ConfirmationLevel::Anchored)
                    ) {
    ...
    pub fn register_seal_records(&mut self, seal_id: &str, record_ids: Vec<String>) {
        ...
        self.seal_record_ids.insert(seal_id.to_string(), record_ids);
    ...
    ///
    /// Returns: (promoted_seals, promoted_records)
    pub fn promote_anchored(&mut self) -> (usize, usize) {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_secs_f64())
            .unwrap_or(0.0);
...
mod tests {
    ...
    fn test_seal_attestation_dedup_by_witness() {
        ...
        awc.zone_stakes.insert(zone.clone(), 300);
        awc.register_profile("w1", profile("org-a", "10.0.1", "earth-us"));
        awc.register_seal_records("seal-0", vec!["r1".to_string()]);
    ...
    fn test_seal_settlement_excludes_creator_stake() {
        ...
        awc.register_seal_records("seal-creator", vec!["r1".to_string()]);
    ...
    fn h1_seal_creator_self_attestation_rejected() {
        ...
        awc.register_seal_records(seal_id, vec!["r1".to_string()]);
    ...
    fn test_seal_settlement_without_creator_stake_unchanged() {
        ...
        awc.register_seal_records("seal-no-creator", vec!["r1".to_string()]);
        for (i, wh) in ["w1", "w2", "w3"].iter().enumerate() {
            awc.add_seal_attestation(SealAttestation {
    ...
    fn attestation_weight_for_seal_is_insertion_order_independent() {
        ...
        for (w, s) in attesters.iter() {
            fwd.add_seal_attestation(SealAttestation {
        ...
        for (w, s) in attesters.iter().rev() {
            rev.add_seal_attestation(SealAttestation {
    ...
    fn test_reverse_index_populated_by_register() {
        let mut awc = AWCConsensus::new();
        awc.register_seal_records("seal-0", vec!["r1".to_string(), "r2".to_string()]);
        awc.register_seal_records("seal-1", vec!["r3".to_string()]);
    ...
    fn test_seal_first_attested_at_pruned_on_all_paths() {
        ...
        awc.register_seal_records("seal-1", vec!["r1".to_string()]);
        awc.add_seal_attestation(SealAttestation {
    ...
    fn test_promote_anchored_respects_challenge_window() {
        ...
        // Now promote should anchor both records
        let (seals, records) = awc.promote_anchored();
        assert_eq!(seals, 1);
        assert_eq!(records, 2);
        ...
        let (seals, records) = awc.promote_anchored();
        assert_eq!(seals, 1);
        assert_eq!(records, 0); // already anchored
    }
    ...
    fn test_epoch_prune_shadow_eligible_only_counts_settled_records_past_horizon() {
        ...
        awc.register_seal_records("seal-old", vec!["r-old-1".to_string(), "r-old-2".to_string(), "r-old-3".to_string()]);
        ...
        awc.register_seal_records("seal-recent", vec!["r-recent-1".to_string()]);
        ...
        awc.register_seal_records("seal-old-unsettled", vec!["r-unsettled".to_string()]);
        ...
        awc.register_seal_records("seal-no-epoch", vec!["r-no-epoch".to_string()]);
        for (i, wh) in ["w1", "w2", "w3"].iter().enumerate() {
            awc.add_seal_attestation(SealAttestation {
    ...
    fn test_epoch_prune_shadow_eligible_seals_orders_by_lag_and_truncates() {
        ...
        for (sid, epoch, recs) in [
            ...
            awc.register_seal_records(sid, record_ids);
