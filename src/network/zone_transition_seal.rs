...
pub struct TransitionSeal {
    ...
    /// Signing-domain selector (T63 verdict 2026-08-18). `None` = legacy
    /// bare-hash anchor signatures — and because this field is omitted from
    /// JSON when `None`, every existing seal's `canonical_encode_for_sig`
    /// bytes (and therefore `seal_hash_for_sig`, and therefore every veto's
    /// `seal_hash` binding) are UNCHANGED. `Some(1)` = anchors sign
    /// `DOMAIN_TAG_ZONE_TRANSITION_SEAL_V1 ‖ seal_hash`; the selector then
    /// sits INSIDE the canonical bytes, so it is authenticated — stripping it
    /// changes the hash and every signature fails (downgrade = reject, never
    /// accept). Emission stays `None` until the CURRENT_SIGNING_VERSION flag
    /// day; unknown values fail closed in [`TransitionSeal::signing_preimage`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sig_domain: Option<u8>,
}

/// Domain-separation tag for `sig_domain = Some(1)` seal signatures (T63).
pub const DOMAIN_TAG_ZONE_TRANSITION_SEAL_V1: &[u8] = b"ELARA_ZONE_TRANSITION_SEAL_V1";
...
impl TransitionSeal {
    ...
    /// SHA3-256 of the canonical-for-sig bytes. This is the seal's IDENTITY
    /// hash (what vetoes bind to) — do not change its inputs casually.
    pub fn seal_hash_for_sig(&self) -> Result<[u8; 32]> {
        let bytes = self.canonical_encode_for_sig()?;
        Ok(sha3_256(&bytes))
    }

    /// The bytes anchor signatures actually cover — THE preimage authority
    /// for both sign and verify (T63; never hand-roll either side).
    /// `sig_domain: None` = the bare seal hash (legacy, all current seals);
    /// `Some(1)` = domain-tagged; anything else fails closed (a peer claiming
    /// a domain we don't speak must be rejected, not bare-verified).
    pub fn signing_preimage(&self) -> Result<Vec<u8>> {
        let hash = self.seal_hash_for_sig()?;
        match self.sig_domain {
            None => Ok(hash.to_vec()),
            Some(1) => {
                let mut buf =
                    Vec::with_capacity(DOMAIN_TAG_ZONE_TRANSITION_SEAL_V1.len() + hash.len());
                buf.extend_from_slice(DOMAIN_TAG_ZONE_TRANSITION_SEAL_V1);
                buf.extend_from_slice(&hash);
                Ok(buf)
            }
            Some(other) => Err(ElaraError::Wire(format!(
                "transition seal: unknown sig_domain {other} (supported: absent, 1)"
            ))),
        }
    }
    ...
    pub fn verify_sigs(
        ...
        // Domain-dispatched preimage (T63): the seal's authenticated
        // sig_domain selector decides bare-hash vs tagged; unknown domains
        // already failed closed inside signing_preimage().
        ...
        for sig in &self.proposer_sigs {
            ...
            match pqc::dilithium3_verify(&preimage, &sig.dilithium3_sig, pk) {
                Ok(true) => valid += 1,
                _ => continue,
            }
        }
...
mod tests {
    ...
    fn batch_b_zts_zone_snapshot_anchorsig_transition_seal_field_shape_and_serde_roundtrip() {
        ...
        let _sd: Option<u8> = seal.sig_domain;
    ...
    /// T63: seal signing-preimage dispatch. None = bare hash byte-identical
    /// (every existing seal signature keeps verifying); Some(1) = tagged;
    /// unknown fails closed; and the selector is AUTHENTICATED — flipping it
    /// changes seal_hash_for_sig, so a downgrade can only reject, never accept.
    #[test]
    fn t63_seal_signing_preimage_dispatch_and_selector_authentication() {
        let seal = make_split_seal();
        assert_eq!(seal.sig_domain, None);
        let bare = seal.signing_preimage().unwrap();
        assert_eq!(bare, seal.seal_hash_for_sig().unwrap().to_vec(),
            "None must remain the bare hash — legacy sigs unchanged");

        let mut tagged = seal.clone();
        tagged.sig_domain = Some(1);
        let pre = tagged.signing_preimage().unwrap();
        assert!(pre.starts_with(DOMAIN_TAG_ZONE_TRANSITION_SEAL_V1));
        // Selector is inside canonical bytes → the hash itself moves.
        assert_ne!(tagged.seal_hash_for_sig().unwrap(), seal.seal_hash_for_sig().unwrap(),
            "sig_domain must be authenticated (inside the canonical encode)");

        let mut unknown = seal.clone();
        unknown.sig_domain = Some(2);
