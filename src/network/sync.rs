...
async fn apply_bootstrap_snapshot_full(
    ...
    if let Some(ref expected_hex) = snapshot.account_state_root {
        // Mark every loaded account dirty so the SMT is rebuilt over the full
        // set, via snapshot_dirty + apply_snapshot (the offloaded path this
        // code actually calls — `flush_dirty` is test-only and holds the write
        // lock for the whole walk). O(accounts) — bootstrap is one-time work;
        // the SCALE RULE excludes one-shot bootstrap from "runtime hot paths".
        let pairs = {
            let mut ledger = state.ledger.write().await;
    ...
    fn ops123_test_record(timestamp: f64, id_seed: &str) -> crate::record::ValidationRecord {
        {
            ...
            __r.id = id_seed.to_string();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = sha3_256(id_seed.as_bytes()).to_vec();
