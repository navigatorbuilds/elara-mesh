...
///
/// `prev_sphincs_pk` is the SPHINCS+ key this hop rotates AWAY from (the
/// identity's original SPHINCS key for the first hop, or the prior hop's
/// introduced key thereafter). It is carried so the rotation-CF lineage can
/// chain on `sha3(prev_sphincs_pk)` — a SPHINCS record is signed by the
/// Dilithium PRIMARY, so without this the chaining hash would fall back to the
/// Dilithium identity and every SPHINCS hop would collapse to one root slot
/// (KR-3 origin-key audit F4). See
/// [`crate::network::rotation_finality::rotation_hop_fields`].
pub fn sphincs_rotation_metadata(
    ...
    m.insert("prev_sphincs_public_key".into(), serde_json::json!(hex::encode(prev_sphincs_pk)));
    m.insert("rotation_reason".into(), serde_json::json!(reason));
    m
}
...
/// KR-3 S2 §5-H5 [R-7]: documented reason vocabulary for rotation records —
/// both Dilithium (`key_rotation`) and SPHINCS+ (`sphincs_key_rotation`) read
/// `rotation_reason` and accept exactly these three.
pub const ROTATION_REASON_VOCAB: [&str; 3] = ["periodic", "compromise", "upgrade"];

/// KR-3 S2 §5-H5 [R-7]: documented reason vocabulary for key **revocations**
/// (`revocation_reason`).
pub const REVOCATION_REASON_VOCAB: [&str; 3] = ["compromise", "decommission", "superseded"];

/// KR-3 S2 §5: is this a rotation-CLASS record — a Dilithium (`key_rotation`) or
/// SPHINCS+ (`sphincs_key_rotation`) key rotation, or a key revocation
/// (`key_revocation`)? Detected via the same bool op-keys the `extract_*` parsers
/// use, so the H1/H2/H3/H5 admission gates and the parsers agree by construction
/// on what "rotation-class" means — one source of truth, no drift. Pure: reads
/// only the record's own metadata.
pub fn is_rotation_class_record(record: &ValidationRecord) -> bool {
    let is_set = |k: &str| {
        record
            .metadata
            .get(k)
    ...
    is_set(KEY_ROTATION_KEY) || is_set(SPHINCS_ROTATION_KEY) || is_set(REVOCATION_OP_KEY)
}

/// KR-3 S2 §5-H5 [R-7]: validate a rotation-class record's `reason` against its
/// class vocabulary — **exact match, no trim, no case-fold**. The §6.2/§6.3
/// cascade triggers on `reason == "compromise"`, so an unvalidated free-text
/// field lets `"Compromise"`, `"compromise "`, or a typo half-fire (or silently
/// fail to fire) the cascade; validating at admission forces the trigger input
/// into the exact documented set.
///
/// Op-class is detected EXACTLY as the `extract_*` parsers do — the bool op-key
/// (`KEY_ROTATION_KEY` / `SPHINCS_ROTATION_KEY` → `rotation_reason`;
/// `REVOCATION_OP_KEY` → `revocation_reason`) — and a missing reason field reads
/// as the same `"unspecified"` sentinel those parsers use, which is outside every
/// vocab ⇒ rejected (the author re-signs with a documented reason).
///
/// Returns `Ok(())` for a NON-rotation-class record (the gate does not apply) and
/// for a valid reason; `Err(reason)` carrying the offending string otherwise.
/// Pure: reads only the record's own metadata — no I/O, no node state — so every
/// node agrees, and it is cheap enough for the admission hot path behind the flag.
pub fn validate_rotation_class_reason(record: &ValidationRecord) -> Result<(), String> {
    let is_set = |k: &str| {
        record
            .metadata
            .get(k)
    ...
    let (reason, vocab): (&str, &[&str]) =
        if is_set(KEY_ROTATION_KEY) || is_set(SPHINCS_ROTATION_KEY) {
            (read("rotation_reason"), &ROTATION_REASON_VOCAB)
        } else if is_set(REVOCATION_OP_KEY) {
            (read("revocation_reason"), &REVOCATION_REASON_VOCAB)
        } else {
            return Ok(()); // not a rotation-class record — H5 does not apply
        };
    if vocab.contains(&reason) {
        Ok(())
    } else {
...
/// KR-3 S2 §5-H1 [R-4]: absolute recent-floor for rotation-class records — reject
/// at admission when `record.timestamp < node_now − ROTATION_CLASS_MAX_PAST_SECS`
/// (`3600` s, a FIXED protocol constant). The round-2 re-verify killed the
/// amendment-1 adaptive `clamp(2 × adaptive_interval, 120, 3600)` formula: feeding
/// per-node EMA state (empty on restart, operator-settable fallback) into a hard
/// gate made the sealer's bound ≠ the witness's bound under heterogeneous config or
/// cold restart → a consensus fork. One fixed constant is fork-free by construction.
///
/// This is honest-user protection, NOT an ordering input. The general ingest path
/// deliberately WARNS-but-accepts arbitrarily old records (offline-first submitters);
/// for a rotation-class record that leniency lets a stale rotation sit Pending
/// indefinitely (zombie-pending spam) and forces the §3-3 sweep / witness-coverage
/// machinery to carry it. Rejecting loud at the door means the author simply re-signs
/// with a fresh timestamp. Ordering security never rests on this gate — the
/// deterministic witness-side coverage bound (§4, evaluated against seal-committed
/// `start`/`end`, never `node_now`) carries that weight — so admission's use of a
/// per-node clock is correct here. The FUTURE side is already bounded upstream by the
/// general `MAX_FUTURE_DRIFT_SECS` gate, so H1 adds only the PAST floor.
///
/// Op-class is detected by the same bool op-keys the `extract_*` parsers use, so a
/// NON-rotation-class record returns `Ok(())` (gate n/a — ordinary offline-first
/// traffic keeps its warn-but-accept leniency). Returns `Err(detail)` describing the
/// overshoot otherwise. Pure: reads only the record's own timestamp/metadata against
/// the `node_now` the caller supplies and the fixed constant — no I/O, no node state.
pub fn validate_rotation_recent_floor(
    record: &ValidationRecord,
    node_now: f64,
) -> Result<(), String> {
    if !is_rotation_class_record(record) {
        return Ok(()); // not a rotation-class record — H1 does not apply
    }
    let floor = crate::network::rotation_finality::ROTATION_CLASS_MAX_PAST_SECS as f64;
    let age = node_now - record.timestamp;
    if age > floor {
        Err(format!(
            "timestamp {:.0} is {age:.0}s past the {floor:.0}s recent-floor (node_now {node_now:.0})",
            record.timestamp
        ))
    } else {
        Ok(())
...
mod tests {
    ...
    fn test_extract_sphincs_rotation() {
        let meta = sphincs_rotation_metadata(b"new_sphincs_pk", b"prev_sphincs_pk", "compromise");
        let record = ValidationRecord::create(
            b"test",
            vec![1, 2, 3],
            vec![],
    ...
    fn test_sphincs_rotation_metadata_roundtrip() {
        let meta = sphincs_rotation_metadata(b"\x01\x02\x03", b"\x0a\x0b", "upgrade");
        ...
        assert_eq!(meta.get("prev_sphincs_public_key").unwrap(), &serde_json::json!("0a0b"));
    ...
    fn batch_b_metadata_builders_exact_3_key_shape_lowercase_hex_and_shared_reason_audit() {
        ...
        // sphincs_rotation_metadata: 4 keys exactly — adds prev_sphincs_public_key
        // (the slice-1 lineage-chaining field) on top of the rotation trio.
        let sph = sphincs_rotation_metadata(b"\xAB\xCD", b"\x12\x34", "upgrade");
        assert_eq!(sph.len(), 4);
        ...
        assert_eq!(sph.get("prev_sphincs_public_key"), Some(&serde_json::json!("1234")));
        ...
        let empty_sph = sphincs_rotation_metadata(b"", b"", "x");
        assert_eq!(empty_sph.get("new_sphincs_public_key"), Some(&serde_json::json!("")));
        assert_eq!(empty_sph.get("prev_sphincs_public_key"), Some(&serde_json::json!("")));
    ...
    fn batch_b_extract_mutual_exclusivity_flag_type_strictness_and_reason_default() {
        ...
        let sph_rec = rec_with(sphincs_rotation_metadata(b"\x03\x04", b"\x01\x02", "upgrade"));
    ...
    // ── KR-3 S2 §5-H5 [R-7]: reason-vocabulary validation ────────────────────
    fn h5_rec(
    ...
    #[test]
    fn h5_valid_rotation_reasons_pass() {
        for r in ROTATION_REASON_VOCAB {
            assert!(
                validate_rotation_class_reason(&h5_rec(rotation_metadata(b"newpk", r))).is_ok(),
                "rotation reason {r:?} must pass"
            );
        }
    }

    #[test]
    fn h5_valid_revocation_and_sphincs_reasons_pass() {
        for r in REVOCATION_REASON_VOCAB {
            assert!(
                validate_rotation_class_reason(&h5_rec(revocation_metadata(b"rk", r))).is_ok(),
                "revocation reason {r:?} must pass"
            );
        }
        assert!(validate_rotation_class_reason(
            &h5_rec(sphincs_rotation_metadata(b"spk", b"prev-spk", "upgrade"))
        )
        .is_ok());
    }

    #[test]
    fn h5_case_and_whitespace_variants_rejected_verbatim() {
        // The exact-match guard is the whole point of R-7: these must NOT pass,
        // and the error carries the offending string unchanged (attributable).
        for bad in ["Compromise", "compromise ", " compromise", "COMPROMISE", "periodic\t"] {
            assert_eq!(
                validate_rotation_class_reason(&h5_rec(rotation_metadata(b"newpk", bad)))
                    .err()
                    .as_deref(),
                Some(bad),
                "variant {bad:?} must be rejected verbatim"
            );
        }
    }

    #[test]
    fn h5_out_of_vocab_and_cross_class_reasons_rejected() {
        // Free-text nonsense.
        assert!(
            validate_rotation_class_reason(&h5_rec(rotation_metadata(b"n", "because"))).is_err()
        );
        // A revocation-only reason on a rotation is out-of-vocab for rotations…
        assert!(
            validate_rotation_class_reason(&h5_rec(rotation_metadata(b"n", "decommission")))
                .is_err()
        );
        // …and a rotation-only reason on a revocation is out-of-vocab for revocations.
        assert!(
            validate_rotation_class_reason(&h5_rec(revocation_metadata(b"r", "periodic"))).is_err()
        );
    }

    #[test]
    fn h5_missing_reason_reads_unspecified_and_is_rejected() {
        // Rotation op-key present, no `rotation_reason` ⇒ the extractor sentinel
        // "unspecified" ⇒ rejected (author re-signs with a documented reason).
        let mut meta = std::collections::BTreeMap::new();
        meta.insert(KEY_ROTATION_KEY.to_string(), serde_json::json!(true));
        assert_eq!(
            validate_rotation_class_reason(&h5_rec(meta)).err().as_deref(),
            Some("unspecified")
        );
    }

    #[test]
    fn h5_non_rotation_class_is_not_applicable() {
        // A stray `rotation_reason` on a non-op record does NOT trigger the gate.
        let mut meta = std::collections::BTreeMap::new();
        meta.insert("rotation_reason".to_string(), serde_json::json!("bogus"));
        assert!(validate_rotation_class_reason(&h5_rec(meta)).is_ok());
        // A plain record with no metadata is likewise n/a.
        let plain = crate::record::ValidationRecord::create(
            b"x",
            b"k".to_vec(),
            vec![],
            crate::record::Classification::Public,
        ...
        assert!(validate_rotation_class_reason(&plain).is_ok());
    }

    // ── KR-3 S2 §5-H1 [R-4]: absolute recent-floor ───────────────────────────
    // Deterministic: `node_now` is set relative to the record's own `timestamp`,
    // so no test depends on the wall clock. floor = ROTATION_CLASS_MAX_PAST_SECS.
    fn h1_floor() -> f64 {
        crate::network::rotation_finality::ROTATION_CLASS_MAX_PAST_SECS as f64
    }

    #[test]
    fn h1_fresh_rotation_class_records_pass() {
        // All three op-classes, comfortably inside the floor (age = floor − 1).
        let rot = h5_rec(rotation_metadata(b"npk", "periodic"));
        let rev = h5_rec(revocation_metadata(b"rk", "compromise"));
        let sph = h5_rec(sphincs_rotation_metadata(b"spk", b"prev-spk", "upgrade"));
        for rec in [&rot, &rev, &sph] {
            assert!(
                validate_rotation_recent_floor(rec, rec.timestamp + h1_floor() - 1.0).is_ok(),
                "in-window rotation-class record must pass"
            );
        }
    }

    #[test]
    fn h1_stale_rotation_class_records_rejected() {
        // All three op-classes, one second past the floor (age = floor + 1).
        let rot = h5_rec(rotation_metadata(b"npk", "periodic"));
        let rev = h5_rec(revocation_metadata(b"rk", "compromise"));
        let sph = h5_rec(sphincs_rotation_metadata(b"spk", b"prev-spk", "upgrade"));
        for rec in [&rot, &rev, &sph] {
            assert!(
                validate_rotation_recent_floor(rec, rec.timestamp + h1_floor() + 1.0).is_err(),
                "past-floor rotation-class record must be rejected"
            );
        }
    }

    #[test]
    fn h1_floor_boundary_is_inclusive() {
        // age == floor passes (`>`, not `>=`); the first instant past it rejects.
        let rec = h5_rec(rotation_metadata(b"npk", "periodic"));
        assert!(validate_rotation_recent_floor(&rec, rec.timestamp + h1_floor()).is_ok());
        assert!(validate_rotation_recent_floor(&rec, rec.timestamp + h1_floor() + 0.001).is_err());
    }

    #[test]
    fn h1_future_dated_within_skew_passes() {
        // node_now < record.timestamp (negative age) ⇒ never past the PAST-floor.
        // The FUTURE bound is the general MAX_FUTURE_DRIFT_SECS gate, not H1.
        let rec = h5_rec(rotation_metadata(b"npk", "periodic"));
        assert!(validate_rotation_recent_floor(&rec, rec.timestamp - 50.0).is_ok());
    }

    #[test]
    fn h1_non_rotation_class_is_not_applicable() {
        // An ancient NON-rotation record keeps its warn-but-accept leniency — H1
        // must not touch ordinary offline-first traffic.
        let plain = crate::record::ValidationRecord::create(
            b"x",
            b"k".to_vec(),
            vec![],
        ...
        assert!(validate_rotation_recent_floor(&plain, plain.timestamp + 10_000_000.0).is_ok());
        // A stray `rotation_reason` on a non-op record does NOT trigger the gate.
        let mut meta = std::collections::BTreeMap::new();
        meta.insert("rotation_reason".to_string(), serde_json::json!("periodic"));
        let stray = h5_rec(meta);
        assert!(validate_rotation_recent_floor(&stray, stray.timestamp + 10_000.0).is_ok());
    }
}
