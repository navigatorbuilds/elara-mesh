...
pub fn spawn_state_core(
    ...
    // P1 (loop-supervision design-C — observability ONLY): register the worker
    // pool in the loop registry so a dead/hung worker surfaces on /health as
    // Stale. Today this loop is a bare tokio::spawn with no registry entry, so a
    // panic leaves /health GREEN — only the per-record watchdog logs (30/60s),
    // never a health signal. Shared LoopStatus across the pool: exact for the
    // default 1 worker; N>1 is a coarse "≥1 worker alive" signal (per-worker
    // registration + per-worker phase gauges are a follow-up). This is VISIBILITY
    // only — the restart mechanism (ArcSwap channel-swap + the torn-insert /
    // fallback-exactly-once prereqs) is separate, harder work.
    let worker_hb = state.loop_registry.register("state_core_worker", 120);
...
/// Panic-abort guard for `state_core_worker_loop`. Audit verdict: process-abort
/// is preferred over an in-place ArcSwap restart — the 1-worker default makes
/// them equivalent for in-flight loss, and abort sidesteps the
/// direct-apply-fallback double-apply an in-memory-preserving restart would
/// cause. A panic in the worker would otherwise silently wedge ALL record
/// ingestion (P1 surfaces it as `/health` Stale after 120s; abort AUTO-recovers
/// in ~10s). Dropped during a panic unwind, it calls `process::abort()` →
/// SIGABRT → systemd `Restart=on-failure` restart + boot-reconcile. A CLEAN
/// shutdown drops this with the thread NOT panicking, so it never fires on the
/// normal teardown path. Crash-loop bound: the systemd drop-in sets
/// `StartLimitIntervalSec=30` with `StartLimitBurst=5` (interval MUST exceed
/// `(burst-1)*RestartSec` = 20s, else the window resets and a deterministic panic
/// crash-loops forever instead of failing) — so a persistent worker bug hits the
/// limit in ~25s and systemd stops the unit (page), rather than spinning.
///
/// NOTE: each worker gets its own guard; blast radius is the WHOLE process. At
/// the default `state_core_worker_count=1` a worker panic already wedges all
/// ingestion, so abort is a clean win. If worker-count is ever raised for
/// zone-parallel throughput, revisit — one shard's panic would abort all N.
struct WorkerPanicAbort;
impl Drop for WorkerPanicAbort {
    fn drop(&mut self) {
        if std::thread::panicking() {
            tracing::error!(
                "STATE_CORE worker loop PANICKED — aborting process for systemd \
                 restart (Restart=on-failure); boot-reconcile heals the wedge. \
                 Repeated aborts hit StartLimitBurst=5 → systemd stops (page)."
            );
            std::process::abort();
        }
    }
...
async fn state_core_worker_loop(
    ...
    // Process-abort-on-panic (audit verdict): a panic in this loop would
    // otherwise silently wedge all record ingestion (default 1 worker). This
    // guard aborts on the unwind → systemd restart + boot-reconcile. It does
    // NOT fire on a clean shutdown return (thread not panicking).
    let _panic_abort = WorkerPanicAbort;
    tracing::info!(
    ...
    while running {
        // P1: stamp liveness each iteration. A panic or a hang INSIDE
        // process_message stops this stamping → the registry flips the worker to
        // Stale on /health (stale_after=120s > the 60s watchdog deadlock
        // threshold, so a legitimately-slow record does not false-trip).
        hb.heartbeat();
        ...
        while let Ok(msg) = priority_rx.try_recv() {
            ...
            // P1: stamp per drained message — a long priority backlog can drain
            // for many records without returning to the loop top, and the worker
            // is BUSY (not hung) throughout, so bound the inter-stamp gap to one
            // record's processing time rather than the whole drain.
            hb.heartbeat();
        ...
        tokio::select! {
            ...
            // P1: idle-wake. When both channels are empty (quiet node) the loop
            // would otherwise park here indefinitely and its heartbeat would go
            // stale → a false Stale on /health. This timer wakes the loop every
            // 10s to re-stamp. A genuine hang IN process_message never reaches
            // this select again, so it still (correctly) ages out to Stale.
...
async fn process_deferred_attestations(
    ...
    for da in atts {
        ...
        // T63 dispatch: witness attestations verify over the attestation
        // preimage (bare signable_bytes v≤5, domain-tagged v6+) — helper is
        // THE authority; a bare signable here would reject all v6 attestations.
        let signable = match state.get_record(record_id) {
            Ok(rec) => crate::network::witness::witness_attestation_preimage(&rec),
            Err(_) => continue,
        };
...
async fn update_snapshot(state: &Arc<NodeState>, snap: &Arc<ArcSwap<StateSnapshot>>) {
    ...
    let new_snap = StateSnapshot {
        ...
        current_epoch: {
            // Canonical definition lives on EpochState so the /status handlers
            // can call it LIVE — this cached copy only refreshes on record
            // ingest, so it lags on an idle node (empty seals still advance the
            // real tip). See EpochState::active_zone_max_epoch.
            let ep = state.epoch.read_recover();
            ep.active_zone_max_epoch(super::consensus::get_zone_count())
        },
...
pub fn spawn_pending_sweep_loop(state: Arc<NodeState>) {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // KR-3 S2 (W2-C): rotation durable-marker catch-up sweep, folded
            // onto this 60s tick. Self-gated on `s2_rotation_ordering_enabled`
            // — a byte-identical early return (no scan, no write) while the
            // flag is OFF, so this is a no-op on every node until S2 is enabled.
            //
            // A tokio interval's FIRST tick fires immediately, so this call
            // doubles as the §3-3 boot sweep for the boot-replay corner: a
            // rotation finalized via `recompute_confirmation(suppress_events=
            // true)` during boot replay, where the enqueue that drives the W1
            // drain hook never fired, leaving no W1 entry. Running before the
            // async consensus stake-rebuild completes is SAFE — a not-yet-
            // rebuilt zone has denominator 0 and `two_thirds_stake_met` returns
            // false on a zero denominator (Leg-A defers, marker kept), while
            // Leg-B canonical burial needs no stake and finalizes immediately;
            // quorum-only cases self-heal on a later tick once the rebuild
            // lands. It never over-finalizes.
            let rot = super::rotation_sweep::run_rotation_marker_sweep(&state).await;
            if rot.entries_written > 0 || rot.write_failures > 0 {
                tracing::debug!(
                    "KR-3 S2 W2 rotation sweep: scanned={} written={} discharged={} failed={} orphan={}",
                    rot.markers_scanned,
                    rot.entries_written,
                    rot.markers_discharged,
                    rot.write_failures,
                    rot.orphan_markers,
                );
            }
...
mod tests {
    ...
    /// P1 (design-C observability): `spawn_state_core` must register the worker
    /// pool in the loop registry EXACTLY ONCE (a shared `LoopStatus`), regardless
    /// of worker count. A future regression that moves `register()` INSIDE the
    /// per-worker loop would emit N duplicate `state_core_worker` label-sets —
    /// which Prometheus cannot validly carry for one metric — so pin register-once
    /// here. Also pins that a freshly-spawned pool reads `Running` (never-ticked
    /// entries are Running by design, not a false Stale).
    #[tokio::test]
    async fn p1_spawn_registers_state_core_worker_once_and_running() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let data_dir = tmp.path().to_path_buf();
        let config = NodeConfig {
            data_dir: data_dir.clone(),
            identity_path: data_dir.join("identity.json"),
            db_path: data_dir.join("elara.db"),
            admin_token: "p1-hb-test".into(),
            network_id: "p1-hb-test".into(),
            mdns_enabled: false,
            health_check_interval_secs: 0,
            min_pow_difficulty: 0,
            state_core_worker_count: 3, // N>1 must STILL register exactly once
            ..Default::default()
        };
        let identity = Identity::generate(EntityType::Device, CryptoProfile::ProfileB)
            .expect("generate identity");
        ...
        let entries: Vec<_> = state
            ...
            .snapshot()
            .into_iter()
            .filter(|(name, ..)| *name == "state_core_worker")
            .collect();
        assert_eq!(
            entries.len(),
            1,
            "exactly one state_core_worker registration regardless of worker count (got {entries:?})"
        );
        assert!(
            matches!(entries[0].1, crate::network::supervision::LoopState::Running),
            "freshly-spawned worker pool must read Running, got {:?}",
            entries[0].1
        );
    ...
    /// The `WorkerPanicAbort` guard must ONLY abort during a panic unwind. A
    /// NORMAL drop (clean loop exit / shutdown) must NOT abort — else every
    /// graceful shutdown would crash the process. If the guard were buggy and
    /// aborted on a clean drop, THIS test process would die on the drop below;
    /// reaching the assert proves it drops cleanly. (The fires-on-panic path is
    /// the standard `thread::panicking()` Drop idiom, verified by construction —
    /// testing the actual `process::abort()` would kill the test runner.)
    #[test]
    fn worker_panic_abort_does_not_fire_on_clean_drop() {
        {
            let _g = WorkerPanicAbort;
        } // dropped here with the thread NOT panicking → must NOT abort
        assert!(!std::thread::panicking(), "clean drop must not abort");
    }

    /// Proves the OTHER half: a panic in the guard's scope MUST `process::abort()`
    /// (SIGABRT), so the audit's process-abort mechanism actually fires — and a
    /// future refactor (moving the guard, wrapping the loop in `catch_unwind`)
    /// can't silently regress it. Uses the standard subprocess pattern: the child
    /// (env-flagged) creates the guard + panics → guard aborts; the parent asserts
    /// the child died with signal 6 (SIGABRT), NOT a normal test-failure exit
    /// (which would mean the guard did NOT fire). Unix-only (the fleet is Linux).
    #[cfg(unix)]
    #[test]
    fn worker_panic_abort_fires_on_panic_via_subprocess() {
        const CHILD_ENV: &str = "ELARA_TEST_WORKER_PANIC_ABORT_CHILD";
        if std::env::var(CHILD_ENV).is_ok() {
            // CHILD: panic inside the guard's scope. Its Drop runs during the
            // unwind with `thread::panicking()==true` → process::abort() → SIGABRT.
            let _g = WorkerPanicAbort;
            panic!("induced state_core worker panic for the abort test");
        }
        // PARENT: re-exec this exact test with the child flag set. Spawn via
        // /proc/self/exe where it exists (Linux): a concurrent `cargo` build
        // in the same target dir can rewrite this test binary mid-run, and
        // then `current_exe()`'s resolved path dangles (spawn → NotFound,
        // observed 2026-08-07) — the magic link execs the original inode
        // even after replacement.
        use std::os::unix::process::ExitStatusExt;
        let proc_self = std::path::Path::new("/proc/self/exe");
        let exe = if proc_self.exists() {
            proc_self.to_path_buf()
        } else {
            std::env::current_exe().expect("current_exe")
        };
        let status = std::process::Command::new(exe)
            .args([
                "network::state_core::tests::worker_panic_abort_fires_on_panic_via_subprocess",
                "--exact",
                "--nocapture",
            ])
            .env(CHILD_ENV, "1")
        ...
        assert_eq!(
            status.signal(),
            Some(6),
            "worker panic must abort the process (SIGABRT=6); got {status:?} — \
             a non-signal exit means the WorkerPanicAbort guard did NOT fire"
        );
    }
