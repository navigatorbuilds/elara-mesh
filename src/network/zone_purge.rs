...
pub async fn run_purge_tick(state: &Arc<NodeState>) -> usize {
    ...
    for rid in &record_ids {
        ...
        // ZonePurge (B5): resharding preserves the mandate-act index (a routing
        // change, not a takedown) — receipts survive the zone moving, and the
        // preserved entries stay bounded by the acts budget evictor.
        if let Err(e) = state.rocks.delete_record(rid, crate::storage::rocks::DeleteIntent::ZonePurge) {
