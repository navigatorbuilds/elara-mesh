...
pub async fn auto_witness_loop(
    ...
    loop {
        ...
        // Hang-detection: a deadlocked auto_witness silently stops this node
        // attesting → degrades finality with no other direct signal.
        hb.heartbeat();
        ...
        for record_id in &to_witness {
            ...
            // Counter-sign the attestation preimage (domain-tagged for v6+
            // records, bare signable_bytes for v≤5 — the shared helper is THE
            // preimage authority for all twelve attestation sites, T63).
            let signable = super::witness::witness_attestation_preimage(&record);
