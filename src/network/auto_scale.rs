...
mod tests {
    ...
    fn propose_split_builds_signed_seal() {
        ...
        // Signature verifies — over the DISPATCHED preimage (flag day
        // 2026-08-19: production seals carry sig_domain=Some(1), so the signed
        // bytes are the domain-selected preimage, never the bare hash; the
        // test verifies exactly the way any verifier must).
        seal.validate_structure().expect("structure");
        assert_eq!(seal.sig_domain, Some(1), "flag day: seals emit domain-selected");
        let preimage = seal.signing_preimage().expect("preimage");
