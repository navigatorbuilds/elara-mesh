...
/// Domain-separation tags for `sig_domain = Some(1)` snapshot-family
/// signatures (T63 verdict 2026-08-18). Distinct per type — a NodeSnapshot
/// signature must never verify as a StateDelta signature even though both
/// sign checksum strings of similar shape.
pub const DOMAIN_TAG_SNAPSHOT_V1: &[u8] = b"ELARA_SNAPSHOT_V1";
pub const DOMAIN_TAG_STATE_DELTA_V1: &[u8] = b"ELARA_STATE_DELTA_V1";

/// THE sign/verify preimage for checksum-signing types (T63): `None` = the
/// bare checksum bytes (all current emissions — byte-identical to before),
/// `Some(1)` = tag ‖ checksum bytes, anything else fails closed. The
/// `sig_domain` selector is authenticated on both types (bound into the
/// checksum body via the conditional-suffix pattern), so a stripped or
/// flipped selector breaks the checksum match before signatures are even
/// consulted — downgrade = reject, never accept.
fn checksum_sig_preimage(
    tag: &'static [u8],
    sig_domain: Option<u8>,
    checksum: &str,
) -> Result<Vec<u8>> {
    match sig_domain {
        None => Ok(checksum.as_bytes().to_vec()),
        Some(1) => {
            let mut buf = Vec::with_capacity(tag.len() + checksum.len());
            buf.extend_from_slice(tag);
            buf.extend_from_slice(checksum.as_bytes());
            Ok(buf)
        }
        Some(other) => Err(ElaraError::Wire(format!(
            "unknown sig_domain {other} (supported: absent, 1)"
        ))),
    }
}
...
fn compute_checksum(snapshot: &NodeSnapshot) -> String {
    ...
    // KR-3 S2 [B7]: bind carried rotation-class CF entries by CONTENT — the §4
    // `rotation_chain_root` (SHA3 over the entries in deterministic per-anchor
    // chain order, injective per-field), not by count, so a trusted-but-tampered
    // archive cannot swap a hop's frozen coordinate under a valid signature
    // (SNAP-1). Conditional (only when non-empty) → a legacy / flag-OFF snapshot
    // carries an empty Vec and reproduces the identical checksum — the same
    // back-compat pattern as account_state_root/mandates/emergency above.
    ...
    // signature (same back-compat pattern as account_state_root above).
    if let Some(d) = snapshot.sig_domain {
        data.push_str(&format!("|sig_domain={d}"));
    }
...
pub struct NodeSnapshot {
    ...
    /// KR-3 S2 [B7]: rotation-class CF entries carried for bootstrap, bound by
    /// CONTENT into `compute_checksum` via `rotation_chain_root` (SHA3 over the
    /// entries in deterministic per-anchor chain order) — a count-only or absent
    /// bind would let a trusted-but-tampered archive swap one hop's frozen
    /// coordinate under a still-valid Dilithium3 signature (the SNAP-1 footgun,
    /// §4). `#[serde(default)]` → legacy / flag-OFF snapshots load as empty and
    /// reproduce the identical pre-feature checksum. Populated only when
    /// `s2_rotation_ordering_enabled` (producer + apply wiring is the follow-up
    ...
    /// checksum`. Bound into `compute_checksum` via the house conditional-
    /// suffix pattern, so the selector is authenticated. Emission stays `None`
    /// until the CURRENT_SIGNING_VERSION flag day.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sig_domain: Option<u8>,
}
...
pub fn create_signed_snapshot(
    ...
    // Sign the domain-dispatched preimage (T63; sig_domain=None today = the
    // bare checksum bytes, byte-identical to before — the tagged form arrives
    // with the CURRENT_SIGNING_VERSION flag day).
    let preimage = checksum_sig_preimage(DOMAIN_TAG_SNAPSHOT_V1, snapshot.sig_domain, &checksum)?;
    if let Ok(sig) = identity.sign(&preimage) {
...
pub fn verify_signed_snapshot(snapshot: &NodeSnapshot) -> Result<String> {
    ...
    // T63 domain dispatch: the selector was authenticated by the checksum
    // recompute in step 1 (a stripped/flipped sig_domain already failed there).
    let preimage = checksum_sig_preimage(DOMAIN_TAG_SNAPSHOT_V1, snapshot.sig_domain, checksum)?;
    match crate::crypto::pqc::dilithium3_verify(&preimage, &sig, &pk) {
        Ok(true) => {}
...
pub struct StateDelta {
    ...
    /// Signing-domain selector (T63) — same contract as
    /// `NodeSnapshot::sig_domain`; tag = `DOMAIN_TAG_STATE_DELTA_V1`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sig_domain: Option<u8>,
}
...
fn compute_state_delta_checksum(delta: &StateDelta) -> String {
    ...
    // T63: bind the signing-domain selector, conditional — same contract and
    // back-compat rationale as the NodeSnapshot checksum suffix.
    let data = match delta.sig_domain {
        Some(d) => format!("{data}|sig_domain={d}"),
        None => data,
    };
    hex::encode(crate::crypto::hash::sha3_256(data.as_bytes()))
}
...
pub fn verify_signed_state_delta(delta: &StateDelta) -> Result<String> {
    ...
    // T63 domain dispatch — selector authenticated by the checksum recompute above.
...
mod tests {
    ...
    #[test]
    fn kr3_s2_rotation_chain_root_binds_into_checksum_and_detects_tamper() {
        ...
        let mk = |hop: u32| RotationChainEntry {
        ...
        let mut snap = make_signed_snapshot(7);

        // Baseline: the default snapshot carries an empty `rotations` Vec.
        // `compute_checksum` returns sha3(data), so the bind is asserted by
        // checksum DIVERGENCE, not substring.
        let c_empty = compute_checksum(&snap);

        // Populated: the content-bind fires and the checksum diverges — the
        // carried rotation state is now under the Dilithium3 signature.
        snap.rotations = vec![mk(0), mk(1)];
        let c_full = compute_checksum(&snap);
        assert_ne!(
            c_empty, c_full,
            "populating rotations must change the signed checksum"
        );

        // SNAP-1 tamper (§4/B7): mutating ONE hop field of an already-carried
        // entry, without re-signing, changes rotation_chain_root → changes the
        // checksum → Dilithium3 verify fails → snapshot rejected. A count-only
        // bind would miss this (cardinality unchanged). Mutate in place; c_full
        // is an owned String and is unaffected.
        snap.rotations[0].new_key_hash = "TAMPERED".into();
        assert_ne!(
            c_full,
            compute_checksum(&snap),
            "mutating a carried rotation hop must change the content-bound checksum"
        );

        // Cardinality-preserving reorder is also caught (the root sorts by
        // (lineage, hop) then absorbs per-field, so swapping a hop_index changes
        // the canonical bytes).
        snap.rotations[0].new_key_hash = "nk".into();
        snap.rotations[0].hop_index = 99;
        assert_ne!(
            c_full,
            compute_checksum(&snap),
            "reordering a hop must change the content-bound checksum"
        );

        // Flag-OFF / legacy parity (S5): clearing `rotations` back to empty
        // reproduces the EXACT baseline checksum — the conditional bind
        // contributes nothing when empty, so a node that never populates the
        // field (and a legacy peer that omits the serde-default field) computes
        // byte-identical checksums to today.
        snap.rotations.clear();
        assert_eq!(
            c_empty,
            compute_checksum(&snap),
            "empty rotations must reproduce the pre-feature checksum"
        );

        // WIRE LEGACY SAFETY (network-critical, mirrors the mandate-carry test):
        // a JSON literally MISSING the `rotations` key — a snapshot serialized by
        // a pre-B7 binary, e.g. a follower still on the prior release — must
        // deserialize via serde(default) to an empty Vec and recompute the
        // byte-identical checksum, so its original Dilithium3 signature still
        // verifies. The empty-parity assert above only clears an in-memory Vec;
        // this exercises the real absent-field wire path.
        let mut lv: serde_json::Value =
            serde_json::from_str(&serde_json::to_string(&make_signed_snapshot(7)).unwrap())
                .unwrap();
        lv.as_object_mut().unwrap().remove("rotations");
        assert!(!lv.as_object().unwrap().contains_key("rotations"));
        let s_legacy: NodeSnapshot = serde_json::from_value(lv).unwrap();
        assert!(
            s_legacy.rotations.is_empty(),
            "a snapshot JSON with no `rotations` key must serde-default to empty"
        );
        assert_eq!(
            compute_checksum(&s_legacy),
            c_empty,
            "a pre-B7 snapshot (absent `rotations` field) must reproduce the identical checksum"
        );

        // ROUND-TRIP (flag-ON wire correctness): a POPULATED snapshot survives
        // serialize→deserialize with every carried hop intact and the
        // content-bound checksum preserved across the wire.
        let mut s_pop = make_signed_snapshot(7);
        s_pop.rotations = vec![mk(0), mk(1)];
        let want = compute_checksum(&s_pop);
        let loaded: NodeSnapshot =
            serde_json::from_str(&serde_json::to_string(&s_pop).unwrap()).unwrap();
        assert_eq!(loaded.rotations.len(), 2, "carried rotations must survive the wire");
        assert_eq!(
            compute_checksum(&loaded),
            want,
            "round-tripped populated snapshot must preserve the content-bound checksum"
        );
    }
    ...
    /// T63: snapshot/delta domain dispatch end-to-end. FLAG DAY 2026-08-19:
    /// the production path now emits sig_domain=Some(1) (tagged), so the
    /// helper's output IS the tagged arm; the legacy None arm is constructed
    /// explicitly here — it guards every pre-flip snapshot forever. Stripping
    /// the selector breaks the (authenticated) checksum; unknown domains fail
    /// closed at the preimage authority.
    #[test]
    fn t63_snapshot_sig_domain_roundtrip_tamper_and_fail_closed() {
        let (snap, identity) = sign_test_snapshot(crate::identity::CryptoProfile::ProfileB);
        assert_eq!(snap.sig_domain, Some(1), "flag day: production snapshots emit tagged");
        verify_signed_snapshot(&snap).expect("tagged production path must verify");

        // Legacy None arm, built explicitly: same content, bare checksum+sig.
        let mut legacy = snap.clone();
        legacy.sig_domain = None;
        legacy.checksum = None;
        legacy.signature = None;
        legacy.sphincs_signature = None;
        let legacy_checksum = compute_checksum(&legacy);
        assert_ne!(Some(&legacy_checksum), snap.checksum.as_ref(),
            "sig_domain must be bound into the checksum body");
        legacy.checksum = Some(legacy_checksum.clone());
        let legacy_pre =
            checksum_sig_preimage(DOMAIN_TAG_SNAPSHOT_V1, None, &legacy_checksum).unwrap();
        assert_eq!(legacy_pre, legacy_checksum.as_bytes(),
            "None arm stays byte-identical bare (pre-flip snapshots verify forever)");
        legacy.signature = Some(hex::encode(identity.sign(&legacy_pre).unwrap()));
        verify_signed_snapshot(&legacy).expect("legacy bare path must verify");

        let checksum = snap.checksum.clone().unwrap();
        let preimage = checksum_sig_preimage(DOMAIN_TAG_SNAPSHOT_V1, snap.sig_domain, &checksum).unwrap();
        assert!(preimage.starts_with(DOMAIN_TAG_SNAPSHOT_V1));

        // Tamper: strip the selector under the same checksum+signature.
        let mut stripped = snap.clone();
        stripped.sig_domain = None;
        let err = verify_signed_snapshot(&stripped).unwrap_err();
        assert!(err.to_string().contains("checksum mismatch"),
            "stripping the selector must break the (authenticated) checksum, got: {err}");

        // Unknown domain fails closed at the preimage authority.
        assert!(checksum_sig_preimage(DOMAIN_TAG_SNAPSHOT_V1, Some(9), &checksum).is_err());
    }
