...
pub struct TransitionVeto {
    ...
    /// bare-hash signature; omitted from JSON when `None`, so existing veto
    /// hashes/signatures are unchanged. `Some(1)` = vetoer signs
    /// `DOMAIN_TAG_TRANSITION_VETO_V1 ‖ veto_hash`; the selector is inside
    /// the canonical bytes when present, hence authenticated (downgrade =
    /// reject). Emission stays `None` until the flag day; unknown values fail
    /// closed in [`TransitionVeto::signing_preimage`]. The production signer
    /// is EXTERNAL (client/SDK) — this node only verifies.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sig_domain: Option<u8>,
}

/// Domain-separation tag for `sig_domain = Some(1)` veto signatures (T63).
pub const DOMAIN_TAG_TRANSITION_VETO_V1: &[u8] = b"ELARA_TRANSITION_VETO_V1";
...
impl TransitionVeto {
    ...
    /// The bytes the vetoer's signature covers — THE preimage authority for
    /// both (external) sign and (node-side) verify (T63). `None` = bare hash;
    /// `Some(1)` = tagged; anything else fails closed.
    pub fn signing_preimage(&self) -> Result<Vec<u8>> {
        ...
        match self.sig_domain {
            ...
            Some(1) => {
                let mut buf =
                    Vec::with_capacity(DOMAIN_TAG_TRANSITION_VETO_V1.len() + hash.len());
                buf.extend_from_slice(DOMAIN_TAG_TRANSITION_VETO_V1);
                buf.extend_from_slice(&hash);
                Ok(buf)
            }
            Some(other) => Err(ElaraError::Wire(format!(
                "transition veto: unknown sig_domain {other} (supported: absent, 1)"
            ))),
        }
    }
    ...
    pub fn verify_sig(&self, vetoer_pubkey: &[u8]) -> Result<()> {
        ...
        match pqc::dilithium3_verify(&preimage, &self.dilithium3_sig, vetoer_pubkey) {
            Ok(true) => Ok(()),
...
mod tests {
    ...
    /// T63: veto signing-preimage dispatch — same contract as the seal's.
    #[test]
    fn t63_veto_signing_preimage_dispatch_and_selector_authentication() {
        let veto = unsigned_veto_for([7u8; 32], 998);
        assert_eq!(veto.sig_domain, None);
        assert_eq!(veto.signing_preimage().unwrap(), veto.veto_hash().unwrap().to_vec());

        let mut tagged = veto.clone();
        tagged.sig_domain = Some(1);
        assert!(tagged.signing_preimage().unwrap().starts_with(DOMAIN_TAG_TRANSITION_VETO_V1));
        assert_ne!(tagged.veto_hash().unwrap(), veto.veto_hash().unwrap(),
            "selector must be authenticated (inside canonical bytes)");

        let mut unknown = veto.clone();
        unknown.sig_domain = Some(3);
