...
// (crate::ZoneId is referenced fully-qualified below; no bare import needed)
...
async fn dispatch(state: Arc<NodeState>, req: PqRequest) -> PqResponse {
    ...
    let result: std::result::Result<Option<Vec<u8>>, ElaraError> = match req.method.as_str() {
        ...
        // Extracted `compute_<verb>` helpers in routes/{core,ledger,explorer}
...
fn error_status(e: &ElaraError) -> u16 {
    match e {
        ...
        // Transient decline (not-yet-present durable dependency) is retryable, not a
        // server fault: 503 so the peer re-tries once the dependency lands (v4 slice-0).
        ElaraError::TransientReject(_) => pq_status::SERVICE_UNAVAILABLE,
...
/// `validate_record` runs a full Dilithium3 verify on caller-supplied bytes;
/// `receive_conflict_proof` runs TWO — and as a write verb it also skips the
/// read-admission bucket, so before 2026-07-12 (sweep A7) it had NO gate at
/// all: an Open-realm peer streaming unique proofs was a direct async-worker
/// starvation lever.
...
fn pq_read_cost(method: &str) -> f64 {
    match method {
        ...
        // (merkle.rs:714). list_witness_profiles / vrf_registry /
        // peer_reputation collect+sort their WHOLE table (profile table, VRF
        // registry, peer table — each grows with the network) before the page
        // truncate — the same materialize-everything shape as `stakes`.
        // NB: `ledger_summary` is deliberately NOT here —
        // validate::summarize() reads maintained O(1) counters (B10), not an
        // account scan, so it stays at the default cost 1.
        "snapshot_full" | "state_delta" | "merkle_root" | "stakes"
        | "list_witness_profiles" | "vrf_registry" | "peer_reputation" => 10.0,
...
/// list_witness_profiles / vrf_registry / peer_reputation are gated too: they
/// run INLINE on the async worker (not spawn_blocking) but do the same
/// materialize-the-whole-table collect+sort as `stakes`, so the cap bounds how
/// many full-table serializes run at once all the same.
...
async fn handle_status(state: &Arc<NodeState>) -> Result<Vec<u8>> {
    ...
    // `_current_epoch_snap` is discarded — current_epoch is reported LIVE below
    // (parallels the axum /status fix): the state_core snapshot only refreshes
    // on record ingest, so its cached epoch freezes on an idle node while empty
    // seals keep advancing the real tip.
    let (dag_size, dag_tips, dag_roots, dag_edges, ledger_supply, ledger_staked,
         ...
         _current_epoch_snap, conservation_pool) = if let Some(core) = state.state_core.get() {
    ...
    // current_epoch read from the SAME live epoch guard as the anchor (not the
    // state_core snapshot) so the PQ /status tip stays truthful on idle nodes —
    // same active-zone max the snapshot caches, one guard for both.
    let (current_epoch, latest_seal_anchor) = {
        ...
        let anchor = ep.highest_seal_anchor().map(|(zone, epoch, hash)| json!({
            "zone": zone.to_string(),
            "epoch": epoch,
...
/// configured seed peer, or an identity staked at >= MIN_STAKE. Each check is
/// O(1) and the cheap lock-free branches short-circuit before any read lock is
/// taken.
async fn push_is_trusted(state: &Arc<NodeState>, peer_identity_hash: &[u8; 32]) -> bool {
    ...
    // A MEANINGFUL stake (>= MIN_STAKE = 100 beat), not merely > 0: a token stake
    // must not buy wholesale rate-exemption for third-party record pushes (the
    // anti-spam bypass hole flagged by the 2026-07-19 tokenomics/spam audit). The
    // fleet backbone stays trusted regardless — genesis + seed peers short-circuit
    // above, and staked witnesses hold >= MIN_STAKE (== the MIN_WITNESS_STAKE
    // floor). A sub-floor peer's pushes aren't rejected, just policed like a fresh
    // submit (rate-limited), so honest low-stake relay still propagates.
    state.ledger.read().await.staked(&hex) >= crate::accounting::types::MIN_STAKE
}
...
async fn handle_submit_record(
    ...
    // T65 D1 — missing header counted always, rejected only behind the knob;
    // fallback is the FROZEN constant, never the compiled-in default.
    let peer_network_hdr = headers.get("x-elara-network-id").map(|s| s.as_str());
    if peer_network_hdr.is_none() {
        state
            .submit_missing_network_header_total
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        if state.config.require_network_id_header {
            return to_body(&json!({
                "accepted": false,
                "reason": "network_id_required",
                "our_network": state.config.network_id,
            }));
        }
    }
    let peer_network: &str =
        peer_network_hdr.unwrap_or(crate::network::routes::core::LEGACY_MISSING_HEADER_NETWORK);
    ...
    if let Some(core) = state.state_core.get() {
        ...
        match result {
            ...
            crate::network::state_core::InsertResult::Error { message } => {
                ...
                // permanent-cache (gossip_rejected is consult-and-skip on pull, so
                // embargoing an infra failure makes the record un-repullable).
                if !crate::network::gossip::dispose_seal_ingest_failure_probed(
                    state,
                    &seal_probe,
                    0,
                ...
                // P0 parity: transient state-core failure → 503, not 500.
...
async fn handle_witness(state: &Arc<NodeState>, body: &[u8]) -> Result<Vec<u8>> {
    ...
    let attestation_sig = tokio::task::spawn_blocking(move || -> Result<Vec<u8>> {
        // Creator verify runs over the bare record preimage; the attestation we
        // hand back is signed over the DOMAIN-TAGGED preimage (v6+) so this
        // public endpoint can never be an oracle for creator-signature bytes
        // (T63; helper dispatches on record.version — v≤5 unchanged).
        ...
        let attestation_preimage =
            crate::network::witness::witness_attestation_preimage(&record);
        state2
            .identity
            .sign(&attestation_preimage)
...
async fn handle_snapshot_fast_meta(
    ...
    let meta = tokio::task::spawn_blocking(move || -> Result<SnapshotFastMeta> {
        ...
        // Whole-node snapshot → epoch high-water = max across ALL zones, not zone-0.
...
async fn handle_receive_attestation(
    ...
    let signable_result = state.get_record(&submit.record_id)
        .map(|rec| crate::network::witness::witness_attestation_preimage(&rec));
...
async fn handle_receive_conflict_proof(
    ...
    // Two Dilithium3 verifies on caller bytes — off the async worker
    // (spawn_blocking), the site the 2026-07-05 inbound-verify hardening
    // missed on this path.
    let (proof, verify_res) = tokio::task::spawn_blocking(move || {
        let r = proof.verify();
        (proof, r)
    })
    .await
...
async fn handle_balances(
    ...
    let body = crate::network::routes::ledger::compute_balances(
        Arc::clone(state),
        identity,
        with_recent,
    )
...
async fn handle_tx_history(
    ...
    let offset = headers
        ...
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(0);
    let body = crate::network::routes::ledger::compute_tx_history(
        Arc::clone(state),
        identity,
        limit,
        offset,
...
async fn handle_recent_transactions(
    ...
    let limit = headers
        ...
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(20);
    let body = crate::network::routes::ledger::compute_recent_transactions(
        Arc::clone(state),
        limit,
    )
    .await?;
...
async fn handle_ledger_summary(state: &Arc<NodeState>) -> Result<Vec<u8>> {
    let body = crate::network::routes::ledger::compute_ledger_summary(state).await;
    to_body(&body)
}
...
async fn handle_stakes(
    ...
    let body = crate::network::routes::ledger::compute_stakes(
        Arc::clone(state),
        identity,
        None,
    )
...
async fn handle_token_enforcement(state: &Arc<NodeState>) -> Result<Vec<u8>> {
    let body = crate::network::routes::ledger::compute_token_enforcement(
        Arc::clone(state),
    )
    .await;
    to_body(&body)
...
async fn handle_supply_circulating(state: &Arc<NodeState>) -> Result<Vec<u8>> {
    let (micros, beat) =
        crate::network::routes::ledger::compute_supply_circulating(Arc::clone(state)).await;
    to_body(&serde_json::json!({ "micros": micros, "beat": beat }))
}

async fn handle_supply_total(state: &Arc<NodeState>) -> Result<Vec<u8>> {
    let (micros, beat) =
        crate::network::routes::ledger::compute_supply_total(Arc::clone(state)).await;
    to_body(&serde_json::json!({ "micros": micros, "beat": beat }))
}

async fn handle_supply_max() -> Result<Vec<u8>> {
    let (micros, beat) = crate::network::routes::ledger::compute_supply_max();
    to_body(&serde_json::json!({ "micros": micros, "beat": beat }))
}
...
recent_bad_sig_record_ids: std::collections::VecDeque::new(),
    ...
    async fn router_fetch_records_byte_caps_fat_response_under_deadline_budget() {
        ...
        for i in 0..N {
            let rec = {
                ...
                __r.id = format!("rec-fr-budget-{i}");
                __r.version = crate::wire::CURRENT_SIGNING_VERSION;
                __r.content_hash = [i as u8; 32].to_vec();
                __r.creator_public_key = vec![0xAA; 1952];
                __r.timestamp = 1700000000.0 + i as f64;
                __r.parents = vec![];
                __r.classification = crate::record::Classification::Public;
                __r.metadata = std::collections::BTreeMap::new();
                __r.signature = Some(vec![0xBB; FAT]);
    ...
    async fn router_delta_sync_pages_stay_under_pq_frame_budget() {
        ...
        for i in 0..N {
            let rec = {
                let mut __r = ValidationRecord::fixture_shell();
                __r.id = format!("rec-frame-budget-{i}");
                __r.version = crate::wire::CURRENT_SIGNING_VERSION;
                __r.content_hash = [i as u8; 32].to_vec();
                __r.creator_public_key = vec![0xAA; 1952];
                __r.timestamp = 1700000000.0 + i as f64;
    ...
    async fn router_query_records_untruncated_page_keeps_legacy_array_shape() {
        ...
        for i in 0..5 {
            let rec = {
                ...
                __r.classification = crate::record::Classification::Public;
                __r.metadata = std::collections::BTreeMap::new();
                __r.signature = Some(vec![0xBB; 64]);
                __r.sphincs_signature = None;
                __r.zk_proof = None;
                __r.itc_stamp = None;
                __r.zone_refs = vec![];
    ...
    async fn router_witness_signed_record_returns_identity_hash_and_dilithium_sig() {
        ...
        // Re-parse and derive the exact ATTESTATION preimage the handler signs
        // (the dispatch authority — flag day 2026-08-19: records emit v6, so
        // this is the witness-tagged preimage; a client verifying the /witness
        // response must do exactly this, never bare signable_bytes).
        let rec = ValidationRecord::from_bytes(&body).expect("re-parse");
        let signable = crate::network::witness::witness_attestation_preimage(&rec);
    ...
    async fn s1123_la1_router_resolve_content_hash_local_hit_returns_record_detail() {
        ...
        let rec = {
            let mut __r = ValidationRecord::fixture_shell();
            __r.id = "rec-pq-resolve-hit".to_string();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = hash_bytes.to_vec();
            __r.creator_public_key = vec![0xAA; 1952];
...
mod pq_read_admit_tests {
    ...
    fn cost_table_charges_whole_ledger_ops_most() {
        ...
        // Full-table collect+sort verbs added by the 2026-07-12 sweep (A1) —
        // same materialize-everything shape as `stakes`.
        for m in ["list_witness_profiles", "vrf_registry", "peer_reputation"] {
            assert_eq!(pq_read_cost(m), 10.0, "{m} materializes its whole table before truncate");
        }
