...
/// (`node-core` vs `verify-cli`), so a shared const is impossible; the
/// hermetic verifier-leg test plus the live acceptance run catch divergence.
...
pub async fn drand_fetch_loop(state: Arc<NodeState>) {
