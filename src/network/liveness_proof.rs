...
mod tests {
    ...
    fn batch_b_liveness_domain_tag_byte_exact_pin_with_versioning_and_namespace_invariants() {
        ...
        // Namespace prefix invariant for THIS tag: it begins with "ELARA/" —
        // a regression that dropped the prefix would collide with any other
        // crate using a bare "LIVENESS_FAIL" tag. NOTE (T63 audit 2026-08-18):
        // the earlier claim here that ALL Elara domain tags share the "ELARA/"
        // slash style was FALSE codebase-wide (a comment-vs-code divergence, the
        // exact class the 2026-07-29 uuid7 collapse taught) — the ratified
        // project convention is upper-snake `ELARA_<NAME>_V1`
        // (`DOMAIN_TAG_RECORD_V1` etc.); this tag keeps its historical slash
        // form because changing it would break every existing liveness sig.
        assert!(
            LIVENESS_DOMAIN_TAG.starts_with(b"ELARA/"),
            "this tag's historical ELARA/ prefix is load-bearing for existing sigs"
        );
