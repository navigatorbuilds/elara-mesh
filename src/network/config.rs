...
pub struct NodeConfig {
    ...
    /// Default `false`: emission is deferred to re-light BY RULING — flipping
    /// it changes the signed content the authority seed produces, which is a
    /// watched-operator decision, not a freeze-window one. Flip requires
    /// nothing but this config + restart; the construction path is already
    /// wired and tested.
    #[serde(default)]
