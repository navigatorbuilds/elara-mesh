...
pub struct NodeConfig {
    ...
    /// Garbage collection interval in seconds (0 = disabled). Default 300 (5 min).
    pub gc_interval_secs: u64,
    /// Retention period for finalized records in seconds. Default 7 days
    /// (node_profile retention ceilings go up to 90 days on FullZone/Archive).
    ...
    /// B5 act permanence: hard cap on the on-disk mass of the GC-EXEMPT mandate-act
    /// index (forward + 3 reverse/time indexes). When exceeded, the GC evictor
    /// (`evict_acts_over_budget`) trims the OLDEST acts and advances the coverage
    /// floor — NEVER by rejecting ingest (the documented fleet-trap shape). 0 =
    /// unlimited (archive profile — the permanence anchor). Profile-aware default
    /// (full_zone 2 GiB, light 256 MiB, archive 0), clamped to ≤25% of
    /// `disk_cap_bytes` by `clamp_acts_budget` so the exempt mass can never
    /// saturate the size governor. Env override: `ELARA_ACTS_BUDGET_BYTES`.
    #[serde(default = "default_acts_budget_bytes")]
    pub acts_budget_bytes: u64,
    ...
    /// Public-HTTP twin of `pq_serve_concurrency` (design:
    /// internal design notes): max concurrent accepted
    /// connections on the public classical listener. Over-budget connections
    /// are shed at accept (dropped, `elara_http_conn_shed_total` bumped),
    /// never queued. Loopback is exempt. 1024 ≈ 1/64 of LimitNOFILE=65536 —
    /// deliberately tighter than the PQ tier: raw HTTP fan-in is the
    /// least-trusted surface (a TCP SYN is the only cost). Min 1.
    #[serde(default = "default_http_conn_cap")]
    pub http_conn_cap: u64,
    /// Per-remote-IP sub-cap within `http_conn_cap`. A global-only cap lets
    /// ONE address hold every slot and starve all other callers (the exact
    /// SSE bug class fixed 2026-07-12); 64 is generous for a CGNAT household
    /// while keeping a single-source slowloris to 1/16 of the global budget.
    /// Loopback exempt. Min 1.
    #[serde(default = "default_http_conn_per_ip_cap")]
    pub http_conn_per_ip_cap: u64,
    /// hyper http1 `header_read_timeout` (seconds) on the public listener —
    /// the wall-clock deadline for a client to finish sending request-line +
    /// headers. Arms from the first poll (pure-h1 builder, no untimed sniff
    /// phase), so a zero-byte connection is reaped too. 30 is hyper's own
    /// stock default; it only never armed before because axum::serve sets no
    /// Timer. Min 1.
    ...
    /// RESERVED / NOT IMPLEMENTED: the `/pq-ws` handler sends no keep-alive pings
    /// (idle clients are quiet-dropped on `WS_IDLE_READ_TIMEOUT` and reconnect), so
    /// this value is currently unused — no code reads it. Kept for a future
    /// server-initiated ping. Default 30.
    pub ws_heartbeat_secs: u64,
    /// Bearer token for `/rpc/*` endpoints ONLY — admin endpoints are PQ-signed
    /// (`X-PQ-Admin`, Dilithium3; bearer auth was removed from `/admin/*` in
    /// PQ-R7). Loopback RPC callers with no proxy headers need no token;
    /// proxied callers must present `Authorization: Bearer <token>`.
    /// Empty = a random token is auto-generated at boot (first 8 chars logged),
    /// so proxied RPC stays unusable until an explicit token is set.
    ...
    /// T65 D1 — require the `x-elara-network-id` header on record submission.
    /// Default `false` = today's behaviour: a missing header falls back to the
    /// frozen legacy constant (see `LEGACY_MISSING_HEADER_NETWORK` at the
    /// enforcement site) and is COUNTED, not rejected. Flip only after
    /// `elara_submit_missing_network_header_total` has been observed near zero
    /// — the counter is the precondition for the policy. Note the honest
    /// scope: this guards the two checked ingress paths (HTTP + PQ submit)
    /// only; sync/snapshot/probe ingress carries no network check (T74).
    #[serde(default)]
    pub require_network_id_header: bool,

    /// T65 D2 — bind `network_id` into the SIGNED metadata of records this
    /// node constructs (the bridge toward the v6 intrinsic wire field, D4).
    ///
    /// **DEPRECATED 2026-08-18 (T63 flag-day verdict R3) — never flip this
    /// on.** The v6 intrinsic field SUPERSEDES this bridge: the emission
    /// binding is armed at boot (`set_emission_network_id`) and stamps
    /// automatically at the Phase E flip; the record-level ingest check
    /// enforces it on ALL ingress paths. This flag predates that machinery
    /// and duplicates the binding through a different preimage (metadata →
    /// content_hash) with NO cross-check that the two agree. Turning it on
    /// now would put two independently-encoded network bindings on the same
    /// records. It stays default-off through the flip and is REMOVED after
    /// the v6 burn-in (validate() warns if set). Kept only so existing
    /// config files keep parsing until removal.
    #[serde(default)]
    pub bind_network_id_in_metadata: bool,
    ...
    /// derives from the record's frozen `FinalityCoord` (drand round of the
    /// covering FINAL seal) via the rotation CF — never from author-claimed
    /// timestamps. Governing design:
    /// internal design notes (GATE-PASS
    /// 2026-07-11).
    ///
    /// DEFAULT OFF everywhere and byte-identical to today at every read point
    /// while OFF (the S5 flag-scoping discipline). Step-1 wiring (W1/W2
    /// writers → routing pin → seal gates → resolver) lands behind this gate
    /// incrementally; the flag stays OFF until the whole surface ships and,
    /// for mainnet, live-pulse coverage is health-gated (§4b). Env override:
    /// `ELARA_S2_ROTATION_ORDERING_ENABLED`.
    #[serde(default)]
    pub s2_rotation_ordering_enabled: bool,
...
/// B5: default act-index budget = 2 GiB (the full_zone value; ~3.5–6M
/// per-rid-queryable acts). `clamp_acts_budget` adjusts per profile (archive→0
/// unlimited, light→≤256 MiB) and clamps to ≤25% of disk_cap at boot.
fn default_acts_budget_bytes() -> u64 { 2 * 1024 * 1024 * 1024 }
/// Compiled-in default network name. The CLI's header-resolution chain ends
/// here (T65 D1 item 4) so a bare `elara_cli` keeps working if
/// `require_network_id_header` is ever flipped on. The missing-header
/// ENFORCEMENT fallback deliberately does NOT route through this — see
/// `LEGACY_MISSING_HEADER_NETWORK` in `routes/core.rs`: a frozen constant
/// fails closed everywhere, a default-tracking one follows the default to
/// mainnet the day someone changes it (T65 adversarial-verifier ruling).
pub const DEFAULT_NETWORK_ID: &str = "testnet";
fn default_network_id() -> String { DEFAULT_NETWORK_ID.to_string() }
...
fn default_http_conn_cap() -> u64 { 1024 }
fn default_http_conn_per_ip_cap() -> u64 { 64 }
fn default_http_header_read_timeout_secs() -> u64 { 30 }
...
impl Default for NodeConfig {
    fn default() -> Self {
        Self {
            ...
            admin_listen_addr: String::new(), // Empty = no DEDICATED admin listener; /admin/* still rides the loopback data-plane listener (data_plane_listen_addr, default 127.0.0.1:9472). Set only when the data plane is moved/disabled.
            ...
            acts_budget_bytes: default_acts_budget_bytes(), // B5: 2 GiB (profile-adjusted at boot)
            ...
            http_conn_cap: default_http_conn_cap(),
            http_conn_per_ip_cap: default_http_conn_per_ip_cap(),
            ...
            require_network_id_header: false,
            bind_network_id_in_metadata: false,
...
impl NodeConfig {
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Ok(v) = std::env::var("ELARA_ACTS_BUDGET_BYTES") {
            if let Ok(n) = v.parse() {
                self.acts_budget_bytes = n; // B5; still profile/25%-clamped at boot
            }
        }
        ...
        if let Ok(v) = std::env::var("ELARA_HTTP_CONN_CAP") {
            if let Ok(n) = v.parse::<u64>() {
                self.http_conn_cap = n.max(1);
            }
        }
        if let Ok(v) = std::env::var("ELARA_HTTP_CONN_PER_IP_CAP") {
            if let Ok(n) = v.parse::<u64>() {
                self.http_conn_per_ip_cap = n.max(1);
            }
        }
        if let Ok(v) = std::env::var("ELARA_HTTP_HEADER_READ_TIMEOUT_SECS") {
            if let Ok(n) = v.parse::<u64>() {
                self.http_header_read_timeout_secs = n.max(1);
            }
        }
        ...
        // live-pulse coverage be health-gated) before this is flipped ON.
        if let Ok(v) = std::env::var("ELARA_S2_ROTATION_ORDERING_ENABLED") {
            self.s2_rotation_ordering_enabled =
                matches!(v.as_str(), "1" | "true" | "TRUE" | "on" | "yes");
        }
    ...
    pub fn validate(&self) -> Vec<String> {
        ...
        // T63 R1 — network_id must satisfy the wire rules (≤64 bytes, ASCII)
        // or a release node could emit records its own decoder rejects (the
        // self-brick path). The HARD stop is at boot (`set_emission_network_id`
        // refuses and the node exits); this warning is the visible twin for
        // config consumers that never reach that call.
        if let Err(e) = elara_record::record::validate_network_id(&self.network_id) {
            warnings.push(format!(
                "network_id invalid — the node will REFUSE to boot with this value: {e}"
            ));
        }
        // T63 R3 — the D2 metadata bridge is deprecated (superseded by the v6
        // intrinsic field + boot-armed emission binding). An operator flipping
        // it on now would double-encode the network binding with no agreement
        // cross-check between the two mechanisms.
        if self.bind_network_id_in_metadata {
            warnings.push(
                "bind_network_id_in_metadata=true is DEPRECATED (T63 flag-day verdict R3): the \
                 v6 intrinsic network_id field supersedes this metadata bridge — leave it false; \
                 the flag is removed after the v6 burn-in"
                    .into(),
            );
        }
        ...
        if self.admin_token.is_empty() {
            warnings.push("admin_token is empty — a random token will be auto-generated at boot for /rpc/* bearer auth (admin endpoints use PQ signatures, not this token). Loopback RPC callers are unaffected; proxied RPC callers need an explicit token (ELARA_ADMIN_TOKEN or admin_token in config).".into());
        } else if self.admin_token.len() < 32 {
            warnings.push("admin_token is too short (< 32 chars) — use `openssl rand -hex 32` for a secure token (used for /rpc/* bearer auth only; admin endpoints are PQ-signed)".into());
        }
    ...
    /// B5: apply profile-aware + 25%-of-disk_cap clamps to `acts_budget_bytes`.
    /// Call at boot AFTER `clamp_disk_cap_to_fs` so the 25% is computed on the
    /// final disk cap. Returns `Some((pre, post))` iff the value changed.
    ///
    /// - archive → 0 (unlimited): the permanence anchor never evicts acts.
    /// - light   → ≤256 MiB: resource-constrained (phone-tier) floor.
    /// - all non-archive with a disk cap → ≤25% of `disk_cap_bytes`, so the exempt
    ///   act mass can never saturate the GC size governor (leaves ≥75% compressible
    ///   headroom, well above the governor's 1 GiB minimum — MINOR-1 coupling).
    pub fn clamp_acts_budget(&mut self) -> Option<(u64, u64)> {
        let pre = self.acts_budget_bytes;
        let mut post = pre;
        match self.node_profile.as_str() {
            "archive" => {
                post = 0; // unlimited — never evict, this node IS the permanence anchor
            }
            "light" => {
                const LIGHT_MAX: u64 = 256 * 1024 * 1024; // 256 MiB
                post = post.min(LIGHT_MAX);
            }
            _ => {}
        }
        // 25%-of-disk_cap clamp (non-archive only; archive already 0/unlimited).
        if post > 0 && self.disk_cap_bytes > 0 {
            let quarter = self.disk_cap_bytes / 4;
            post = post.min(quarter);
        }
        if post != pre {
            self.acts_budget_bytes = post;
            Some((pre, post))
        } else {
            None
        }
...
mod tests {
    ...
    #[test]
    #[allow(clippy::field_reassign_with_default)] // incremental field-setting reads clearer for these profile cases
    fn b5_clamp_acts_budget_profile_and_quarter_cap() {
        const GIB: u64 = 1024 * 1024 * 1024;
        // archive → unlimited (0), regardless of prior value.
        let mut c = NodeConfig::default();
        c.node_profile = "archive".to_string();
        c.acts_budget_bytes = 2 * GIB;
        assert!(c.clamp_acts_budget().is_some());
        assert_eq!(c.acts_budget_bytes, 0);

        // light → ≤256 MiB (disk cap large enough not to bind).
        let mut c = NodeConfig::default();
        c.node_profile = "light".to_string();
        c.acts_budget_bytes = 2 * GIB;
        c.disk_cap_bytes = 200 * GIB;
        c.clamp_acts_budget();
        assert_eq!(c.acts_budget_bytes, 256 * 1024 * 1024);

        // full_zone with a small disk cap → ≤25% of disk_cap binds.
        let mut c = NodeConfig::default();
        c.node_profile = "full_zone".to_string();
        c.acts_budget_bytes = 2 * GIB;
        c.disk_cap_bytes = 4 * GIB; // 25% = 1 GiB
        c.clamp_acts_budget();
        assert_eq!(c.acts_budget_bytes, GIB);

        // full_zone with a big disk cap → default 2 GiB unchanged (no-op).
        let mut c = NodeConfig::default();
        c.acts_budget_bytes = 2 * GIB;
        c.disk_cap_bytes = 200 * GIB;
        assert!(c.clamp_acts_budget().is_none());
        assert_eq!(c.acts_budget_bytes, 2 * GIB);
    }
    ...
    #[test]
    fn s2_rotation_ordering_defaults_off_and_parses_opt_in() {
        // S5 flag-scoping discipline — KR-3 S2 must be OFF by default so the
        // node stays byte-identical to today at every rotation read point until
        // the whole W1/W2 + routing + seal-gate surface ships (and, for
        // mainnet, live-pulse coverage is health-gated, §4b).
        assert!(!NodeConfig::default().s2_rotation_ordering_enabled);
        // Omitted from an operator's config → false (container serde(default)),
        // so adding this field never bricks an existing config that omits it.
        let omitted: NodeConfig =
            serde_json::from_str("{}").expect("empty config parses via serde(default)");
        assert!(!omitted.s2_rotation_ordering_enabled);
        // Explicit opt-in deserializes true.
        let on: NodeConfig = serde_json::from_str(r#"{"s2_rotation_ordering_enabled": true}"#)
            .expect("opt-in config parses");
        assert!(
            on.s2_rotation_ordering_enabled,
            "explicit true must deserialize"
        );
    }
