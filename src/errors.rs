...
pub enum ElaraError {
    ...
    /// A record was declined not because it is structurally invalid but because a
    /// durable dependency it needs is not present HERE yet — e.g. a rotation hop
    /// whose introducing predecessor has not finalized on this node, or a record
    /// whose cited parent has not synced. The gossip/sync driver parks + re-fetches
    /// such a record (bounded, attempt-capped) instead of writing it to the permanent
    /// `gossip_rejected` embargo, and the HTTP/PQ surfaces answer 503 (retry later)
    /// rather than 400 (malformed) or 500 (server fault). The `"transient reject: "`
    /// Display prefix is the single stable marker
    /// `network::gossip::is_retryable_ingest_rejection` keys on, so the retry class is
    /// decided at the reject site rather than guessed from a per-reason substring
    /// (KR-3 S2 v4 slice-0 / C2). Every emitter is flag-gated ⇒ flag-OFF
    /// byte-identical.
    #[error("transient reject: {0}")]
    TransientReject(String),
...
mod tests {
    ...
    //
    //  (1) 18-variant Display strict-prose pin — every variant rendered
    ...
    #[test]
    fn batch_b_eighteen_variant_display_strict_prose_pin() {
        ...
        //
        // 14 String-carrying variants — sentinel "X" payload makes the
        ...
        assert_eq!(ElaraError::TransientReject("X".into()).to_string(),"transient reject: X");
        ...
        for s in [
            ...
            ElaraError::TransientReject("X".into()).to_string(),
    ...
    fn batch_b_source_chain_matrix_flat_none_vs_from_downcast() {
        ...
        // (A) 16 flat variants — source() must be None on each.
        let flats: Vec<ElaraError> = vec![
            ...
            ElaraError::TransientReject("X".into()),
            ElaraError::RateLimited,
        ];
        assert_eq!(flats.len(), 16, "ElaraError flat-variant count must be 16");
        for err in flats {
            assert!(
                err.source().is_none(),
