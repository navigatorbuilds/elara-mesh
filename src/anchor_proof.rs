...
fn require_str<'m>(
    ...
    meta.get(key)
        .and_then(|v| v.as_str())
        .ok_or_else(|| ElaraError::Wire(format!("anchor_proof record missing '{key}'")))
}
...
pub fn parse_anchor_proof_metadata(
    ...
    match meta.get("kind").and_then(|v| v.as_str()) {
        ...
        other => {
            return Err(ElaraError::Wire(format!(
                "not an anchor_proof record (kind={other:?})"
            )))
        }
    }
    ...
    if anchor_kind != ANCHOR_KIND_ELARA_SEAL && anchor_kind != ANCHOR_KIND_EXTERNAL_ROOT {
        return Err(ElaraError::Wire(format!(
            "unknown anchor_kind '{anchor_kind}'"
        )));
    }
    ...
    if anchor_digest.len() != 64 || hex::decode(&anchor_digest).is_err() {
        return Err(ElaraError::Wire(
            "anchor_digest is not 64 hex chars".into(),
        ));
    }
    ...
    let anchor_epoch = meta
        .get("anchor_epoch")
        .and_then(|v| v.as_u64())
        .ok_or_else(|| ElaraError::Wire("anchor_epoch missing or not a u64".into()))?;
    let artifact_bytes = b64()
        .decode(require_str(meta, "anchor_artifact_b64")?)
        .map_err(|e| ElaraError::Wire(format!("anchor_artifact_b64 invalid base64: {e}")))?;
    let ots_bytes = b64()
        .decode(require_str(meta, "anchor_ots_b64")?)
        .map_err(|e| ElaraError::Wire(format!("anchor_ots_b64 invalid base64: {e}")))?;
    if artifact_bytes.len() > MAX_ANCHOR_ARTIFACT_RAW {
        return Err(ElaraError::Wire("anchor artifact exceeds raw cap".into()));
    }
    if ots_bytes.len() > MAX_ANCHOR_OTS_RAW {
        return Err(ElaraError::Wire("OTS proof exceeds raw cap".into()));
    }
