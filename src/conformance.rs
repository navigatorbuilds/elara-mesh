...
//! primitive is bound to the actual epoch-8219 seal whose `account_smt_root` the
...
/// `seal_wire` is `examples/verify/epoch-8219-zone-0.seal.wire` (a real
...
pub fn generate_vector_set(
    ...
    // epoch-8219 seal whose `account_smt_root` the `account-binding` vector
    ...
    let seal_rec = ValidationRecord::from_bytes(seal_wire).map_err(|e| {
        format!("examples/verify/epoch-8219-zone-0.seal.wire failed to decode: {e:?}")
    })?;
    ...
    .is_ok();
    vectors.push(ConformanceVector {
        name: "seal-anchor-sig/epoch-8219-zone-0".into(),
        ...
        input: serde_json::json!({
            ...
            "seal_wire_file": "examples/verify/epoch-8219-zone-0.seal.wire",
...
mod tests {
    ...
    fn seal_wire() -> Vec<u8> {
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/examples/verify/epoch-8219-zone-0.seal.wire"
        );
        std::fs::read(path).expect("read examples/verify/epoch-8219-zone-0.seal.wire")
    }
    ...
    /// committed inputs (sample record wire, epoch-8219 seal wire, zone-0 anchor
    ...
    fn merkle_inclusion_vector_folds_to_pinned_root_and_verifies() {
        ...
        const PIN_INCL_ROOT: &str =
            "d9b70c95e85a2ea74df513f8cda6faaf7dabebc5ee1ff7547d3ab4a9c513fc5f";
        let set = derive();
        let v = set
            .vectors
            .iter()
    ...
    fn seal_anchor_sig_vector_verifies_and_reject_is_rejected() {
        ...
        const SEAL_HASH: &str =
            "d0742b057c228305f1ae36e60230dd68fdbbef65ac339eea2963ca44ad3e6f04";
        let set = derive();
        let seal = seal_wire();
