...
pub fn generate_vector_set(
    ...
    // ── v6 vectors (T63 flag-day Phase C — APPENDED per the R5 policy: v5
    // vectors above are frozen forever; never mutate them in place) ─────────
    let record_v6 = ValidationRecord::from_bytes(sample_record_v6_wire)
        .map_err(|e| format!("sample v6 record wire does not decode: {e}"))?;
    vectors.push(ConformanceVector {
        name: "record-hash-v6".into(),
        primitive: "record-hash".into(),
        spec_ref: "§4.4 v6 arm / WIRE-FORMAT §7.5".into(),
        note: "Same recipe as record-hash, against the v6 fixture: parse the wire \
               (v6 appends a u16-prefixed ASCII network_id AFTER the v5 nonce), \
               build signable_bytes with the v6 prefix (ELARA_RECORD_V1 tag, then \
               u16-BE network_id length, then network_id bytes, BEFORE the §4.4 \
               body), then record_hash = SHA3-256(signable_bytes). Reproducing \
               this proves your v6 decode + tagged canonicalization end-to-end."
            .into(),
        input: serde_json::json!({
            "wire_file": "examples/verify/sample-record-v6.wire",
            "wire_sha3_256": hex::encode(sha3_256(sample_record_v6_wire)),
            "record_id": record_v6.id,
            "network_id": record_v6.network_id,
        }),
        expected: hex::encode(record_v6.record_hash()),
    });
    let tag = crate::record::DOMAIN_TAG_RECORD_V1;
    let mut v6_prefix = Vec::with_capacity(tag.len() + 2 + record_v6.network_id.len());
    v6_prefix.extend_from_slice(tag);
    v6_prefix.extend_from_slice(&(record_v6.network_id.len() as u16).to_be_bytes());
    v6_prefix.extend_from_slice(record_v6.network_id.as_bytes());
    let signable_v6 = record_v6.signable_bytes();
    if !signable_v6.starts_with(&v6_prefix) {
        return Err("v6 signable_bytes does not start with the domain prefix — \
                    generator and record.rs disagree"
            .into());
    }
    vectors.push(ConformanceVector {
        name: "signable-prefix-v6".into(),
        primitive: "domain-separation".into(),
        spec_ref: "§4.4 v6 arm (T63)".into(),
        note: "The exact byte prefix that LEADS every v6 signing preimage: the \
               constant ASCII tag ELARA_RECORD_V1, then the u16-BE length of \
               network_id, then network_id. v<=5 preimages carry NO prefix \
               (byte-identical to the historical form). A verifier that omits \
               or misplaces this prefix computes a wrong record_hash for every \
               v6 record while remaining correct for v5 — test both."
            .into(),
        input: serde_json::json!({
            "network_id": record_v6.network_id,
            "tag_ascii": "ELARA_RECORD_V1",
        }),
        expected: hex::encode(&v6_prefix),
    });
...
mod tests {
    ...
    fn sample_wire_v6() -> Vec<u8> {
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/examples/verify/sample-record-v6.wire"
        );
        std::fs::read(path).expect("read examples/verify/sample-record-v6.wire")
    }

    /// The single derivation entry point every test shares: feeds all four
    /// committed inputs (sample record wire v5 + v6, bundled epoch seal wire,
    /// zone-0 anchor pubkey) to the authoritative generator.
    fn derive() -> ConformanceVectorSet {
        generate_vector_set(&sample_wire(), &seal_wire(), &anchor_pubkey(), &sample_wire_v6())
