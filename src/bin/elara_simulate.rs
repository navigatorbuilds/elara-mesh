...
async fn activate_node(node: &mut SimNode) {
    ...
    tokio::spawn(discovery::heartbeat_loop(
        node.state.clone(), rx,
        node.state.loop_registry.register("heartbeat", 300),
    ));
    ...
    // Gossip pull
    let (tx, rx) = watch::channel(()); node.shutdown_txs.push(tx);
    tokio::spawn(gossip::pull_loop(node.state.clone(), rx, node.state.loop_registry.register("pull", 120)));

    // Attestation pull
    let (tx, rx) = watch::channel(()); node.shutdown_txs.push(tx);
    tokio::spawn(gossip::attestation_pull_loop(node.state.clone(), rx, node.state.loop_registry.register("attestation_pull", 120)));

    // Auto-witness
    let (tx, rx) = watch::channel(()); node.shutdown_txs.push(tx);
    tokio::spawn(auto_witness::auto_witness_loop(
        node.state.clone(), node.witness_mgr.clone(), rx,
        node.state.loop_registry.register("auto_witness", 1200),
    ));

    // Epoch seal
    let (tx, rx) = watch::channel(()); node.shutdown_txs.push(tx);
    tokio::spawn(epoch::epoch_seal_loop(
        node.state.clone(), rx,
        node.state.loop_registry.register("epoch_seal", 240),
    ));
}

async fn shutdown_node(node: &SimNode) {
    for tx in &node.shutdown_txs { let _ = tx.send(()); }
    tokio::time::sleep(Duration::from_millis(50)).await;
}
...
async fn scenario_cross_zone_finality(cli: &Cli) -> Result<()> {
    ...
    // Check cross-zone metrics on all nodes. Detection MUST read the
    // monotonic registered_total, not the live map gauges: prune_finalized
    // deletes a record's cross_zone_parents entry precisely because the
    // record succeeded, so on a healthy fast run the gauges legitimately
    // read 0 at scenario end (observed 2026-08-02 on public CI — 40 boosts
    // fired, all entries pruned, len()==0, scenario red).
    let mut total_xzone_registered = 0u64;
    ...
    for (i, n) in nodes.iter().enumerate() {
        ...
        let xz_registered = consensus.cross_zone_registered_total();
        let finalized = n.state.finalized.try_read()
            .map(|f| f.len()).unwrap_or(0);
        info!(
            "  Node {i}: xzone_registered={xz_registered} xzone_live={xz_records} xzone_refs={xz_refs} boosts={xz_boosts} finalized={finalized}"
        );
        total_xzone_registered += xz_registered;
        total_xzone_records += xz_records as u64;
        total_boosts += xz_boosts;
    }

    let xzone_detected = total_xzone_registered > 0;
    let boosts_fired = total_boosts > 0;

    info!(
        "Cross-zone summary: total_xzone_registered={total_xzone_registered} live_map={total_xzone_records} total_boosts={total_boosts}"
    );

    if !xzone_detected {
        warn!("No cross-zone parents were EVER registered — records may all be in same zone");
    }
    if !boosts_fired {
