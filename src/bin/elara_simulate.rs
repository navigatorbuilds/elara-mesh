...
async fn scenario_basic(cli: &Cli) -> Result<()> {
    ...
    for i in 0..cli.nodes {
        ...
        dirs.push(dir.clone());
        let config = make_config(ports[i], &genesis, &seeds, cli.epoch_interval, cli.zone_count, &dir, i);
    ...
    for (i, n) in nodes.iter().enumerate() {
        ...
        if !s_ok { ok = false; }
        info!(
            "  Node {i}: supply={} dag={dag} peers={peers} atts={atts} max_epoch={node_max_epoch} settled={settled}",
    ...
    if total_settled == 0 {
        error!("  Cluster settled NO records (0 reached 2/3 quorum across all {} nodes) — attestation path stalled!", nodes.len());
        ok = false;
    }
    ...
    if ok {
        info!("=== PASSED (max_epoch={max_epoch}, total_settled={total_settled}) ===");
        Ok(())
    } else {
    ...
    F: FnMut(usize, &mut NodeConfig),
{
    ...
    for i in 0..cli.nodes {
        ...
        let dir = make_temp_dir(i);
        dirs.push(dir.clone());
        let mut config = make_config(ports[i], &genesis, &seeds, cli.epoch_interval, cli.zone_count, &dir, i);
...
async fn scenario_byzantine_all_ranks_malicious(cli: &Cli) -> Result<()> {
    ...
    for i in 0..cli_adj.nodes {
        ...
        let dir = make_temp_dir(i);
        dirs.push(dir.clone());
        let mut config = make_config(ports[i], &genesis, &seeds, cli_adj.epoch_interval, cli_adj.zone_count, &dir, i);
