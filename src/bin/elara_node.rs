...
async fn run() -> Result<()> {
    ...
    info!("rebuilding DAG index and ledger...");
    {
        ...
        let checkpoint_ledger: Option<(elara_runtime::accounting::ledger::LedgerState, f64)> = {
            let cp_ts = node_state.rocks.load_snapshot::<f64>("checkpoint_timestamp").ok().flatten();
            let cp_ledger = node_state.rocks.load_snapshot::<elara_runtime::accounting::ledger::LedgerState>("ledger").ok().flatten();
            match (cp_ts, cp_ledger) {
                (Some(ts), Some(ledger)) if ledger.total_supply == elara_runtime::accounting::types::MAX_SUPPLY => {
                    info!(
        ...
        let result: std::result::Result<std::result::Result<(
            ...
            state2.rocks.for_each_record_ordered_bounded(rebuild_cap, |rec| {
                ...
                if f_chal {
                    ...
                    if rec.metadata.contains_key(fisherman::CHALLENGE_OP_KEY) {
    ...
    let (orphan_tx, orphan_rx) = mpsc::channel(1);
    let (witness_tx, witness_rx) = mpsc::channel(1);
    let (att_gossip_tx, att_gossip_rx) = mpsc::channel(1);
    let (prune_tx, prune_rx) = mpsc::channel(1);
    ...
    let (system_load_tx, system_load_rx) = mpsc::channel(1);
    let (fork_tx, fork_rx) = mpsc::channel(1);
    let (probe_tx, probe_rx) = mpsc::channel(1);
    let (tip_merge_tx, tip_merge_rx) = mpsc::channel(1);
    let (mdns_tx, mdns_rx) = mpsc::channel(1);
    ...
    tokio::spawn(async move {
        discovery::heartbeat_loop(hb_state, heartbeat_rx).await;
    });
    ...
    if config.network_realm.discovery_enabled() {
        ...
        tokio::spawn(async move {
            discovery::pex_loop(pex_state, pex_rx).await;
        });
    } else {
    ...
    if config.mdns_enabled && config.network_realm.discovery_enabled() {
        ...
        tokio::spawn(async move {
            elara_runtime::network::mdns::mdns_loop(mdns_state, mdns_rx).await;
        });
    ...
    tokio::spawn(async move {
        gossip::pull_loop(gs_state, gossip_rx).await;
    });
    ...
    tokio::spawn(async move {
        gossip::orphan_resolver_loop(or_state, orphan_rx).await;
    });
    ...
    tokio::spawn(async move {
        auto_witness::auto_witness_loop(aw_state, aw_mgr, witness_rx).await;
    });

    // Attestation gossip pull loop
    let ag_state = node_state.clone();
    tokio::spawn(async move {
        gossip::attestation_pull_loop(ag_state, att_gossip_rx).await;
    });

    // Attestation pruning loop (hourly, keep 30 days)
    let prune_mgr = witness_mgr.clone();
    tokio::spawn(async move {
        attestation_prune_loop(prune_mgr, prune_rx).await;
    });
    ...
    // Trust engine pruning loop (hourly, prune identities inactive >30 days)
    {
        ...
        tokio::spawn(async move {
            trust_prune_loop(trust_prune_state).await;
        });
    }
    ...
    // Memory pruning loop (every 10 min — consensus, reputation, rate limiter)
    {
        ...
        tokio::spawn(async move {
            memory_prune_loop(mem_prune_state).await;
        });
    }
    ...
    if config.snapshot_interval_secs > 0 {
        ...
        tokio::spawn(async move {
            snapshot_loop(snap_state, snap_path, snap_interval, snapshot_rx).await;
        });
    }
    ...
    // nodes can bootstrap from the latest epoch rather than replaying genesis.
    {
        ...
        if node_type.is_archival() && config.archive_snapshot_every_n_epochs > 0 {
            ...
            tokio::spawn(async move {
                archive_snapshot_loop(snap_state, snap_dir).await;
            });
        }
    }
    ...
    // RocksDB TTL compaction loop (every 6 hours — prune old reputation/disputes from disk)
    {
        ...
        tokio::spawn(async move {
            ttl_compaction_loop(ttl_state).await;
        });
    }
    ...
    // Epoch seal loop (genesis authority only, auto-skips on non-genesis nodes)
    {
        ...
        tokio::spawn(async move {
            epoch::epoch_seal_loop(ep_state, epoch_rx).await;
        });
    }
    ...
    if node_state.config.drand_pulse_enabled && node_state.config.allow_public_https {
        ...
        tokio::spawn(async move {
            drand_fetch::drand_fetch_loop(drand_state).await;
        });
    }
    ...
    // Finality monitor + consensus attestation pruning
    {
        ...
        tokio::spawn(async move {
            finality_monitor_loop(fm_state, finality_rx).await;
        });
    }
    ...
    // every 5s so gossip/ingest hot loops can back off when the host is busy.
    {
        ...
        tokio::spawn(async move {
            elara_runtime::network::system_load::run_refresh_loop(sl_sensor, system_load_rx).await;
        });
    }
    ...
    if config.network_realm.discovery_enabled() {
        ...
        tokio::spawn(async move {
            discovery::seed_reconnect_loop(rc_state, reconnect_rx).await;
        });
    } else {
    ...
    // GC loop (prune expired + old finalized records)
    {
        ...
        tokio::spawn(async move {
            gc::gc_loop(gc_state, gc_rx).await;
        });
    }
    ...
    // Fork monitor loop (detect divergence, auto-heal)
    {
        ...
        tokio::spawn(async move {
            elara_runtime::network::fork::fork_monitor_loop(fork_state, fork_rx).await;
        });
    }
    ...
    // Peer liveness probe loop (all nodes — keeps network alive with zero user traffic)
    {
        ...
        tokio::spawn(async move {
            elara_runtime::network::probe::probe_loop(probe_state, probe_rx).await;
        });
    }
    ...
    // Tip-merge loop (every 60s — compress DAG frontier into actual mesh)
    {
        ...
        tokio::spawn(async move {
            elara_runtime::network::tip_merge::tip_merge_loop(tm_state, tip_merge_rx).await;
        });
    }
    ...
    axum::serve(
        ...
        .await
        .map_err(|e| ElaraError::Network(format!("server error: {e}")))?;
...
async fn attestation_prune_loop(
    ...
    loop {
        ...
        match witness_mgr.prune_before(cutoff) {
            Ok(0) => {}
            Ok(n) => info!("pruned {n} old attestations"),
            Err(e) => warn!("attestation prune failed: {e}"),
        }
    }
}
...
async fn trust_prune_loop(state: Arc<NodeState>) {
    let interval = std::time::Duration::from_secs(3600); // 1 hour
...
async fn memory_prune_loop(state: Arc<NodeState>) {
    ...
    loop {
        ...
        match state.reputation.lock() {
            ...
            Err(e) => warn!("memory prune: reputation lock failed: {e}"),
        }
        ...
        match state.propagation_limiter.lock() {
            Ok(mut limiter) => {
            ...
            Err(e) => warn!("memory prune: propagation limiter lock failed: {e}"),
        }
...
async fn ttl_compaction_loop(state: Arc<NodeState>) {
    ...
    loop {
        ...
        // Reputation CF
        match state.rocks.ttl_cleanup_cf_by_timestamp(CF_REPUTATION, cutoff) {
            Ok(0) => {}
        ...
        // Disputes CF
        match state.rocks.ttl_cleanup_cf_by_timestamp(CF_DISPUTES, cutoff) {
            Ok(0) => {}
...
async fn finality_monitor_loop(
    ...
    loop {
        ...
        if cycles.is_multiple_of(10) {
            let (promoted_seals, promoted_records) = {
                let mut consensus = state.consensus.lock_recover();
                consensus.promote_anchored()
            };
            if promoted_records > 0 {
                // Mark in finalized index (anchored ⊃ finalized)
                // Records promoted to Anchored are the strongest confirmation level.
                info!(
                    "finality monitor: anchored {} records across {} seals (24h challenge window passed)",
                    promoted_records, promoted_seals
                );
            }
        }
