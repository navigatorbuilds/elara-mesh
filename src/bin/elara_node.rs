...
async fn run() -> Result<()> {
    ...
    // T63 R1 — arm the emission network binding at the record-creation
    ...
    // R37: one-shot account-SMT store-format migration (legacy full-tail →
    // v2 compact), root-gated and atomic. Every refusal shape leaves the
    // store on the legacy walk, which stays byte-for-byte correct — so any
    // failure here is a warning, never a boot blocker.
    match elara_runtime::network::account_merkle::migrate_account_smt_store(&rocks) {
        Ok(elara_smt::MigrateOutcome::Migrated { keys }) => {
            info!("account-SMT store migrated to v2 compact format ({keys} leaves, root preserved)");
        }
        Ok(elara_smt::MigrateOutcome::AlreadyV2) => {
            debug!("account-SMT store already v2");
        }
        Ok(elara_smt::MigrateOutcome::RootMismatch { expected, rebuilt }) => {
            warn!(
                "account-SMT v2 migration REFUSED (root mismatch: committed {} vs rebuild {}) — \
                 store stays legacy; run the divergence diagnostics before retrying",
                hex::encode(expected),
                hex::encode(rebuilt)
            );
        }
        Err(e) => {
            warn!("account-SMT v2 migration skipped (continuing on legacy walk): {e}");
        }
    }
    ...
    // T61 (2026-07-29): continuity + daily-cap counters are now durable.
    //
    // Both were rebuilt EMPTY on every process start, and both failures compounded
    // in opposite directions (internal design notes):
    //   • daily_caps empty  ⇒ `systemctl restart` handed every identity a fresh
    //     full quota — a bypass available to any node operator.
    //   • continuity empty  ⇒ `score()` returns 0.0 for every unseen identity and
    //     `first_seen` re-anchors to process start, so the score tracked NODE
    //     UPTIME instead of identity history. Tier 1 needs continuity >= 0.2,
    //     i.e. ~10.7 days of unbroken uptime — structurally unreachable, and the
    //     live logs showed continuity=0.00 on 96 of 96 samples.
    // `need_continuity` drives the same rebuild-from-records fallback the trust
    // engine uses when no snapshot exists.
    let need_continuity = match node_state.rocks.load_snapshot::<elara_runtime::continuity::ContinuityState>("continuity") {
        Ok(Some(cont)) => {
            let tracked = cont.identity_count();
            *node_state.continuity.lock_recover() = cont;
            if tracked > 0 {
                info!("continuity state loaded from RocksDB: {} tracked identities", tracked);
            }
            false
        }
        Ok(None) => true,
        Err(e) => { warn!("continuity snapshot load failed, falling back to record rebuild: {e}"); true }
    };

    match node_state.rocks.load_snapshot::<elara_runtime::network::ingest::DailyCapCounter>("daily_caps") {
        Ok(Some(mut caps)) => {
            // Drop windows that fully elapsed while the process was down, so a
            // long downtime never resurrects a stale window (O(active), same
            // call the prune loop already makes).
            caps.cleanup(elara_runtime::network::ingest::now());
            let tracked = caps.tracked_identities();
            *node_state.daily_caps.lock_recover() = caps;
            if tracked > 0 {
                info!("daily-cap counters restored from RocksDB: {} identities still inside their 24h window", tracked);
            }
        }
        // No snapshot (fresh node / first upgrade) ⇒ start empty. Deliberately NOT
        // rebuilt from records: the count is a 24h admission window, not chain
        // state, and replaying it would be an O(all-records) scan (SCALE RULE).
        Ok(None) => {}
        Err(e) => warn!("daily-cap snapshot load failed, starting empty this window: {e}"),
    }
    ...
    if any_need_rebuild {
        ...
        let result: std::result::Result<std::result::Result<(
            ...
            // T61: continuity rides the shared state rather than the result tuple —
            // its rebuild is a single `record_activity` per record with no
            // post-stream processing, so widening the 9-tuple (and eight unrelated
            // write-back sites) would buy nothing.
            ...
            state2.rocks.for_each_record_ordered_bounded(rebuild_cap, |rec| {
                ...
                // Continuity: same per-record feed as live ingest (ingest.rs
                // `cont.record_activity(&creator_hash, record.timestamp)`), so a
                // rebuilt score is identical to one accumulated online. Anchoring
                // `first_seen` to the identity's OLDEST record — not to process
                // start — is the whole point of T61.
                if let Some(ref mut cs) = continuity_rebuild {
                    cs.record_activity(
                        &elara_runtime::accounting::types::creator_identity_hash(rec),
                        rec.timestamp,
                    );
                }
                ...
                // epoch / continuity (all fed for EVERY record since T61 wired the
                // continuity feed here — before that this comment named a subsystem the
                // loop never actually touched) are deliberately NOT gated —
            ...
            // Overwrite (not merge): live ingest may have fed a handful of records
            // during the rebuild, but those identities are all present in the
            // rebuilt state with an OLDER, truer `first_seen` — keeping the live
            // map would re-anchor them to process start, which is the bug.
            if let Some(cs) = continuity_rebuild.take() {
                let tracked = cs.identity_count();
                *state2.continuity.lock_recover() = cs;
                info!("continuity rebuilt from records: {} tracked identities", tracked);
            }
    ...
    // RocksDB checkpoint loop (RAM-tiered: >4GB = every 6h keep 2; see checkpoint_loop)
    {
    ...
    let shutdown_signal = async move {
        ...
        //    Acquire tokio RwLocks with timeout; std locks are fast.
        {
            ...
            // T61: the anti-spam pair. A clean shutdown is the common case for
            // this fleet (122 deliberate restarts since 2026-07-05, zero crash
            // loops), so saving here is what actually closes the restart-reset
            // window — the periodic loop covers the hard-kill tail.
            {
                let cont_clone = shutdown_state.continuity.lock_recover().clone();
                snapshots.push(("continuity", shutdown_state.rocks.save_snapshot("continuity", &cont_clone)));
                let caps_clone = shutdown_state.daily_caps.lock_recover().clone();
                snapshots.push(("daily_caps", shutdown_state.rocks.save_snapshot("daily_caps", &caps_clone)));
            }
...
async fn snapshot_loop(
    ...
    loop {
        ...
        let (d_dag, d_trust, d_subsystem) = {
            ...
            // T61: anti-spam pair, cloned UNCONDITIONALLY (not behind
            // `full_snapshots`) — see the essential-tier note below.
            let continuity_snap = state.continuity.lock_recover().clone();
            let daily_caps_snap = state.daily_caps.lock_recover().clone();
...
///
/// Cadence and retention are RAM-tiered (see the match below): >4GB RAM =
/// every 6h keeping 2 checkpoints; <=4GB = every 12h keeping 1; <=2GB = off.
/// Creates a point-in-time snapshot in
/// `{data_dir}/checkpoints/checkpoint_{timestamp}` using RocksDB's built-in
/// Checkpoint API (fast, hard-link based) and rotates older ones out
/// (pre-rotate-before-create; see the incident note inside).
...
fn prune_stale_checkpoints(
    ...
    for entry in std::fs::read_dir(dir).map_err(|e| format!("read checkpoints dir: {e}"))? {
        ...
        // Accept real checkpoints (`checkpoint_<ts>`) AND orphaned RocksDB
        // staging dirs (`checkpoint_<ts>.tmp`): librocksdb never reclaims a
        // PRIOR cycle's `.tmp` (each create stages under a fresh timestamped
        // name), and an old orphan pins compacted-away SSTs forever — the same
        // unbounded-growth class as the wedged-checkpoint incident above. The
        // age bound keeps this safe: an in-flight staging dir is seconds old,
        // never older than `max_age_secs` (2× the checkpoint interval).
        let suffix = &name["checkpoint_".len()..];
        let ts: u64 = match suffix.strip_suffix(".tmp").unwrap_or(suffix).parse() {
            Ok(t) => t,
            Err(_) => continue,
        };
...
///
/// Only exact `checkpoint_<unix_ts>` names participate. A leftover RocksDB
/// staging dir (`checkpoint_<ts>.tmp` — librocksdb stages at `<target>.tmp`
/// and its failure cleanup is best-effort, skipped on SIGKILL/power loss)
/// previously matched the old `starts_with` filter AND won the descending
/// name sort (`"…​.tmp"` sorts above its digits-only sibling), so rotation
/// deleted the REAL newest checkpoint and kept the orphan — which
/// `prune_stale_checkpoints` skipped by design (u64 parse failed). Repeated
/// create failure then converges on zero real checkpoints plus a growing
/// prune-proof orphan: the wedged-checkpoint incident class, resurrected.
/// Orphaned `.tmp` dirs are instead reaped, age-bounded, by
/// `prune_stale_checkpoints`.
fn rotate_checkpoints(dir: &std::path::Path, keep: usize) -> std::result::Result<(), String> {
    let mut entries: Vec<(u64, String, std::path::PathBuf)> = std::fs::read_dir(dir)
        .map_err(|e| format!("read checkpoints dir: {e}"))?
        .filter_map(|entry| {
            let entry = entry.ok()?;
            let name = entry.file_name().into_string().ok()?;
            let ts: u64 = name.strip_prefix("checkpoint_")?.parse().ok()?;
            if entry.path().is_dir() {
                Some((ts, name, entry.path()))
            } else {
                None
            }
        })
    ...
    // Newest first — numeric on the parsed timestamp (name-lexicographic happens
    // to agree for equal-width unix timestamps, but don't depend on it).
    entries.sort_by_key(|e| std::cmp::Reverse(e.0));

    // Remove entries beyond the keep limit
    for (_, name, path) in entries.iter().skip(keep) {
    ...
    if rocks_path.exists() {
        match StorageEngine::open(&rocks_path) {
            Ok(engine) => {
                ...
                // T63 flag-day verdict Q5/A2 precondition tool: exact wire-version
                // census over the RAW stored bytes (header peek — never from_bytes,
                // which would MIN-gate away exactly the sub-MIN rows this exists to
                // count). Run against a STOPPED data dir or a checkpoint copy; the
                // MIN 4→5 raise ships only when the below-MIN(5) line reads 0 here.
                match engine.wire_version_histogram() {
                    Ok((hist, malformed_keys)) => {
                        use elara_runtime::wire::{WIRE_VERSION, WIRE_VERSION_MIN};
                        let total: u64 = hist.values().sum();
                        let rows: Vec<String> = hist
                            .iter()
                            .map(|(v, n)| format!("v{v}={n}"))
                            .collect();
                        println!("  wire-version census (exact, raw-header): total={total} [{}] malformed={}",
                                 rows.join(" "), malformed_keys.len());
                        for mk in &malformed_keys {
                            println!("    malformed row: {mk}");
                        }
                        let below_min: u64 = hist.range(..WIRE_VERSION_MIN).map(|(_, n)| n).sum();
                        let above_max: u64 = hist.range(WIRE_VERSION + 1..).map(|(_, n)| n).sum();
                        let below_5: u64 = hist.range(..5u16).map(|(_, n)| n).sum();
                        println!("    below current MIN({WIRE_VERSION_MIN}): {below_min}  |  above ceiling({WIRE_VERSION}): {above_max}  |  below v5 (the queued MIN raise): {below_5}");
                        if below_min > 0 || above_max > 0 || !malformed_keys.is_empty() {
                            println!("    WARNING: rows outside the decodable range exist — they are INVISIBLE to every from_bytes read path (silent skips); investigate before any migration");
                        }
                    }
                    Err(e) => println!("  wire-version census FAILED: {e}"),
                }
...
mod tests {
    ...
    #[test]
    fn rotate_checkpoints_ignores_tmp_staging_dir() {
        // The 2026-08-18 audit's E2 finding: a leftover RocksDB staging DIRECTORY
        // (`checkpoint_<ts>.tmp`) used to match the prefix filter AND win the
        // descending name sort, so rotate(keep=1) deleted the REAL newest
        // checkpoint and kept the orphan. Pin the fix: .tmp dirs are invisible
        // to rotation; only exact `checkpoint_<digits>` names participate.
        let dir = tempdir().unwrap();
        std::fs::create_dir(dir.path().join("checkpoint_1000")).unwrap();
        std::fs::create_dir(dir.path().join("checkpoint_2000")).unwrap();
        std::fs::create_dir(dir.path().join("checkpoint_2000.tmp")).unwrap();
        ...
        let names: std::collections::BTreeSet<String> = std::fs::read_dir(dir.path())
            ...
            .collect();
        assert!(
            names.contains("checkpoint_2000"),
            "REAL newest checkpoint survives (pre-fix it was deleted in favor of the .tmp)"
        );
        assert!(!names.contains("checkpoint_1000"), "older real checkpoint rotated out");
        assert!(
            names.contains("checkpoint_2000.tmp"),
            ".tmp staging dir is not rotation's to touch (prune reaps it, age-bounded)"
        );
    }

    #[test]
    fn prune_stale_checkpoints_reaps_old_tmp_orphan_keeps_fresh() {
        // Companion pin for the E2 fix: prune now also reaps ORPHANED RocksDB
        // staging dirs (`checkpoint_<ts>.tmp`) past the age bound, while an
        // in-flight (fresh) staging dir survives untouched.
        let dir = tempdir().unwrap();
        let now: u64 = 1_000_000;
        let max_age: u64 = 100;
        std::fs::create_dir(dir.path().join("checkpoint_999000.tmp")).unwrap(); // 1000s old orphan
        std::fs::create_dir(dir.path().join(format!("checkpoint_{}.tmp", now - 5))).unwrap(); // in-flight
        std::fs::create_dir(dir.path().join(format!("checkpoint_{}", now - 10))).unwrap(); // fresh real

        let removed = prune_stale_checkpoints(dir.path(), now, max_age).unwrap();
        assert_eq!(removed, 1, "exactly the old orphan reaped");

        let names: std::collections::BTreeSet<String> = std::fs::read_dir(dir.path())
            .unwrap()
            .filter_map(|e| e.ok().and_then(|e| e.file_name().into_string().ok()))
            .collect();
        assert!(!names.contains("checkpoint_999000.tmp"), "old .tmp orphan reaped");
        assert!(
            names.contains(&format!("checkpoint_{}.tmp", now - 5)),
            "in-flight staging dir untouched"
        );
        assert!(
            names.contains(&format!("checkpoint_{}", now - 10)),
            "fresh real checkpoint untouched"
        );
    }
