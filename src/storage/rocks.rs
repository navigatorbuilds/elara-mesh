...
/// the db-v5→db-v6 migration fixed the WRITER format consistency but never
/// repaired the call-site mismatch. This index closes that gap.
///
/// Migration db-v6→db-v7 backfills it from CF_RECORDS.
///
/// ⚠ NAMING (T63 flag-day verdict R8): every "v5"/"v6"/"v7" in this file's
/// migration story is the STORAGE SCHEMA version (`__db_version__`, a u32 —
/// see `run_migrations`), a number line wholly unrelated to the record WIRE
/// versions that happen to occupy the same small integers. Log strings say
/// "db-vN" for this reason; keep that convention.
...
/// Time-ordered act index (B5 act permanence, verdict 2026-07-18): a global,
/// mandate-agnostic ordering of every act entry by its `act_timestamp_ms`,
/// powering the OLDEST-FIRST budget evictor (`evict_acts_over_budget`) without an
/// O(all-entries) scan. Key:
///     `act_timestamp_ms(8 BE) ++ record_id(ascii)`
/// value empty. Written as the 4th key of `put_mandate_act`'s atomic batch and
/// deleted in lockstep with the other three on `AdminEvict` / budget eviction, so
/// its coverage is byte-identical to `CF_MANDATE_ACT`. DERIVED, NOT
/// snapshot-carried, consensus-INERT (same class as the other act CFs).
/// Rebuildable from `CF_MANDATE_ACT` (`migrate_9_to_10` backfill). READ ONLY as
/// an ascending range scan from `IteratorMode::Start` — no prefix extractor (the
/// evictor walks the whole ordering oldest-first, bounded per tick).
pub const CF_MANDATE_ACTS_BY_TIME: &str = "mandate_acts_by_time";

/// `CF_METADATA` key holding the durable, monotone act-index coverage FLOOR in
/// milliseconds (B5). Absence of an act with `claimed_ts >= floor` is an
/// AUTHORITATIVE refutation; below the floor coverage is genuinely unknown on
/// this node. Init policy: `now_ms` on ANY DB missing the key (upgraded live seed
/// that already GC'd acts, or a virgin pull-join to an existing chain), and 0
/// only when the genesis-ceremony creation path proves chain birth
...
pub const ALL_CF_NAMES: &[&str] = &[
    ...
    CF_MANDATE_ACTS_BY_TIME,
    CF_EMERGENCY,
    // KR-3 S2 (§4): rotation-chain finality state, keyed by `lineage_id`.
    // Registered here so the CF is created (empty) at open; the §3-3 W1/W2
    // writers land in a later slice. Forward-safe via
    // `create_missing_column_families(true)` — see the const's own doc.
    crate::network::rotation_finality::CF_KEY_ROTATION_CHAIN,
];
...
/// KR-3 S2 (W2-A): a rotation-finality side-write to batch atomically with a
/// record's Phase-2 store (§3-3). Threaded through [`StorageEngine::put_record_with_pk_zone`]
/// exactly like `disc5_epoch_key`, so the side-write shares the record's crash
/// fate (seal-durable ⟹ marker-durable). Mutually exclusive — a record is either
/// a seal or a rotation-class hop, never both. Only ever `Some` under
/// `s2_rotation_ordering_enabled` (caller-gated), so a flag-OFF batch is
/// byte-identical to a pre-feature one.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum RotationBatchOp {
    /// This record is a seal covering ≥1 rotation-class record: arm the durable
    /// `rotation_seal_pending:{zone}:{epoch}` marker in the seal's own batch so
    /// the W2 sweep will check the slot for rotation finality.
    ArmSealMarker { zone: String, epoch: u64 },
    /// This record IS a rotation-class hop. Under flag-ON it triggers TWO atomic
    /// side-writes in the record's own admission `WriteBatch` (§3-3 / §6.1), so
    /// both share the record's crash fate:
    ///  1. `record_hash` → `rot_pending_hash:` CF row, so a later seal's
    ///     marker-arm decision can detect coverage cheaply (an index point-lookup,
    ///     never a member record load); and
    ///  2. `rotation_zone_pin:{record_id}` → `lineage_id` (wiring-(c) c2), so the
    ///     hop's storage zone resolves via the rotation-stable lineage id — the
    ///     whole chain stays co-zoned across key changes for the W2 per-zone sweep,
    ///     instead of scattering as the creator key rotates.
    RotationHopAdmit {
...
/// the derived mandate-act index (B5 act permanence, verdict 2026-07-18).
///
/// The act index (`CF_MANDATE_ACT` + its two reverse indexes + the by-time
/// index) is tiny (~0.6 KB/act) and consensus-inert. Preserving it across
/// retention GC lets a pruned act still answer an AUTHORITATIVE
/// `/mandate/status` verdict (`record_present:false`) instead of degrading to a
/// false "not a mandate act". Only a deliberate operator evict removes it — and
/// then it leaves a rid-keyed removed-marker so the absence reads as
/// `removed_by_operator`, never as a coverage gap.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DeleteIntent {
    /// Retention GC prune of the record body. The act-index block is SKIPPED —
    /// all act-CF rows survive so the receipt verdict outlives the body.
    GcPrune,
    /// Zone-resharding purge. Same preservation as [`DeleteIntent::GcPrune`]:
    /// purge is a routing change, not a takedown, and the preserved entries stay
    /// budget-governed by the evictor (avoids a global absence-floor slam).
    ZonePurge,
    /// Operator forensic evict / unverifiable-record takedown. Full cleanup
    /// INCLUDING the act index, plus a `__act_removed__:{rid}` marker in
    /// `CF_METADATA` (same batch) so the un-index is auditable and the absence
    /// answer is `removed_by_operator`.
    AdminEvict,
}

/// `CF_METADATA` key prefix for the rid-keyed act removed-marker written by an
/// [`DeleteIntent::AdminEvict`] that actually removed an act entry. Its presence
/// makes `/mandate/status` return `absence_basis:"removed_by_operator"` with
/// `authoritative_complete:true` — a deliberate un-index, distinct from a
/// coverage-window gap. Bounded by admin-evict rarity.
pub const ACT_REMOVED_MARKER_PREFIX: &str = "__act_removed__:";
...
pub struct StorageEngine {
    ...
    /// snapshot-bootstrap advance; `WriteBatch` gives atomicity but not isolation,
    /// so without this lock two interleaved max-writes could regress the durable
    /// floor below a concurrent deletion's timestamp — re-creating a confident
    /// false negative after restart. Writers are structurally few (boot init,
    /// snapshot bootstrap, the single evictor), so contention is negligible.
    acts_floor_lock: std::sync::Mutex<()>,
}
...
impl StorageEngine {
        ...
        let cf_descriptors: Vec<ColumnFamilyDescriptor> = ALL_CF_NAMES
            ...
            .map(|name| {
                ...
                match *name {
                    ...
                    | CF_MANDATE | CF_REVOCATION | CF_MANDATE_ACT | CF_MANDATE_ACTS_BY_MANDATE | CF_MANDATE_ACTS_BY_AGENT | CF_MANDATE_ACTS_BY_TIME => {
                    ...
                    // Rotation-chain (KR-3 S2, §4): key is
                    //     {64-char lineage_id}{0x00}{hop_index_be(4)}
                    // Access = point-read for the H3b predecessor check (a
                    // missing-hop lookup is the common negative case) + a short
                    // (≤ ROTATION_MAX_CHAIN_DEPTH = 16) prefix scan by lineage_id
                    // for chain-order replay. 10-bit WHOLE-KEY bloom + shared
                    // cache so negative point lookups short-circuit SSTs at
                    // 1M-lineage scale — the same pathology the mandate/IDX blooms
                    // fix. Deliberately NO fixed-prefix extractor: unlike the
                    // zone/attestation CFs (huge per-prefix result sets) the
                    // per-lineage scan is ≤16 hops — an already-indexed seek — so
                    // a prefix bloom buys almost nothing AND would couple scan
                    // correctness to an exact key length (a fixed-N extractor
                    // silently returns empty for any key shorter than N). Same
                    // config + `prefix_iterator_cf` + `starts_with` guard as the
                    // extractor-less CF_WITNESS_REGISTRY scan. Tuned at
                    // create-time (empty until the §3-3 writers land) to avoid a
                    // later migration.
                    crate::network::rotation_finality::CF_KEY_ROTATION_CHAIN => {
                        cf_opts.set_write_buffer_size(index_buf);
                        let mut rot_block_opts = rocksdb::BlockBasedOptions::default();
                        rot_block_opts.set_bloom_filter(10.0, false);
                        rot_block_opts.set_block_size(4 * 1024);
                        rot_block_opts.set_block_cache(&cache);
                        rot_block_opts.set_cache_index_and_filter_blocks(true);
                        rot_block_opts.set_pin_l0_filter_and_index_blocks_in_cache(true);
                        cf_opts.set_block_based_table_factory(&rot_block_opts);
                    }
    ...
    pub fn put_record_with_pk_zone(
        ...
        // under `s2_rotation_ordering_enabled` (caller-gated) ⇒ flag-OFF batches
        // are byte-identical.
        if let Some(op) = rotation_op {
            match op {
                RotationBatchOp::ArmSealMarker { zone, epoch } => {
                    self.put_rotation_seal_marker_to_batch(&mut batch, &zone, epoch)?;
                }
                RotationBatchOp::RotationHopAdmit {
                    record_hash_hex,
                    record_id: pin_record_id,
                    lineage_id,
                } => {
                    // (1) pending-CF hash — the W2-A seal marker-arm gate.
                    batch.put_cf(
                        &self.try_cf(CF_METADATA)?,
                        Self::rotation_pending_hash_key(&record_hash_hex),
                        b"",
                    );
                    // (2) routing pin (wiring-(c) c2) — crash-atomic with the
                    // record store so the hop can never be durable under a naive
                    // zone index while its lineage pin is missing.
                    self.put_rotation_zone_pin_to_batch(&mut batch, &pin_record_id, &lineage_id)?;
                }
            }
        }
    ...
    /// Every seal `record_id` durably stored at `(zone_path, epoch)` — the
    /// DISC-5 rival set the KR-3 S2 W2 sweep enumerates per armed marker. Phase-2
    /// writes EVERY parsed seal at a slot unconditionally, before the C2 lex-min
    /// canonicality decision, so a slot can hold >1 seal (an honest seal and a
    /// crash-before-broadcast phantom / partition rival); the sweep must consider
    /// all of them and let the durable evidence legs pick the winner
    /// (`rotation_finality::plan_marker_slot_sweep`). Returns them in DISC-5 key
    /// order (record_id ascending within the slot); an empty vec means no seal is
    /// stored there (an orphaned marker — the sweep keeps it, never a write).
    ///
    /// Cost: O(seals at (zone,epoch)) DISC-5 keys — normally 1–2, bounded by the
    /// per-epoch competing-sealer count. One `From`-seek + `starts_with` boundary
    /// guard, never a full-CF scan (SCALE).
    pub fn seal_record_ids_at_zone_epoch(&self, epoch: u64, zone_path: &str) -> Vec<String> {
        let zone_bytes = zone_path.as_bytes();
        let mut prefix = Vec::with_capacity(8 + zone_bytes.len() + 1);
        prefix.extend_from_slice(&epoch.to_be_bytes());
        prefix.extend_from_slice(zone_bytes);
        prefix.push(0u8);
        let Ok(epochs_cf) = self.try_cf(CF_EPOCHS) else {
            return Vec::new();
        };
        let mode = rocksdb::IteratorMode::From(&prefix, rocksdb::Direction::Forward);
        let mut out = Vec::new();
        for kv in self.db.iterator_cf(&epochs_cf, mode) {
            let Ok((key, _)) = kv else { continue };
            if !key.starts_with(&prefix) {
                break; // left the (zone, epoch) slot — DISC-5 keys are contiguous
            }
            if let Some((_, _, record_id)) = crate::network::epoch::parse_disc5_index_key(&key) {
                out.push(record_id.to_string());
            }
        }
        out
    }
    ...
    /// Delete a record.
    pub fn delete_record(&self, record_id: &str, intent: DeleteIntent) -> Result<()> {
        let cf = self.try_cf(CF_RECORDS)?;
        ...
        // B5 act permanence (verdict 2026-07-18): the mandate-act index
        // (CF_MANDATE_ACT + both reverse indexes) is preserved across retention
        // GC and zone purge and removed ONLY on an operator forensic evict. Act
        // records carry `mandate_ref` (not the GC-exempt `mandate_op`), so their
        // BODIES are ordinary GC-eligible records — but the tiny derived entry
        // (~0.6 KB) is what powers the always-authoritative `/mandate/status`
        // receipt verdict after the body ages out, so GcPrune/ZonePurge keep it.
        // (CF_MANDATE / CF_REVOCATION are never cleaned here under any intent —
        // their carriers are GC-exempt, so a mandate/revocation record reaching
        // delete_record is an explicit admin action and the registry entry must
        // outlive any single carrier copy.)
        match intent {
            // Preserve the entire act-CF block: the body is mortal, the receipt
            // index is not. Growth is bounded by the per-profile acts budget
            // evictor (`evict_acts_over_budget`), never by ingest rejection.
            DeleteIntent::GcPrune | DeleteIntent::ZonePurge => {}
            // Operator forensic evict: full cleanup + a rid-keyed removed-marker.
            //
            // The reverse-index keys embed the act's (mandate_ref, ts) /
            // (signer_hash, ts) which only the FORWARD entry carries — so read it
            // via the Result-returning getter FIRST (a storage read error ABORTS
            // the delete before the forward key is touched, so we never leak the
            // two reverse keys — MINOR-2) and reconstruct each reverse key with
            // the SAME builder used on write. On a double-delete the entry is
            // already gone → `None`, which is correct (idempotent), not a leak.
            DeleteIntent::AdminEvict => {
                if let Some(act) = self.get_mandate_act_checked(record_id)? {
                    ...
                    // B5 4th key: the time index, keyed by the entry's own ts.
                    let tkey = Self::mandate_acts_time_key(act.act_timestamp_ms, record_id);
                    batch.delete_cf(&self.try_cf(CF_MANDATE_ACTS_BY_TIME)?, &tkey);
                    // Rid-keyed removed-marker in the SAME batch: the absence
                    // answer becomes `removed_by_operator` (a deliberate un-index,
                    // authoritative) rather than a coverage-window gap.
                    let marker = format!("{ACT_REMOVED_MARKER_PREFIX}{record_id}");
                    batch.put_cf(&self.try_cf(CF_METADATA)?, marker.as_bytes(), b"");
                    crate::network::mandate_node::MANDATE_ACT_ENTRIES_DELETED_ADMIN_EVICT
    ...
    // ── Rotation-chain CF (KR-3 S2, §4) ──────────────────────────────────────
    //
    // Typed accessors for `CF_KEY_ROTATION_CHAIN`. The §3-3 W1 (finalize-drain
    // hook) and W2 (durable-marker sweep) writers have no live caller yet, so the
    // CF stays empty — these are the persistence layer they will build on. Keyed
    // `lineage_id || 0x00 || hop_index_be`, so a per-lineage prefix scan reads
    // the chain in `hop_index` order.

    /// Rotation-chain hop key: `lineage_id || 0x00 || hop_index_be(4)`. The
    /// big-endian hop suffix makes a `prefix_iterator_cf(lineage_id || 0x00)`
    /// yield hops in ascending `hop_index` order (§4 chain-order replay).
    fn rotation_chain_key(lineage_id: &str, hop_index: u32) -> Vec<u8> {
        let mut k = lineage_id.as_bytes().to_vec();
        k.push(0u8);
        k.extend_from_slice(&hop_index.to_be_bytes());
        k
    }

    /// Persist one rotation-chain hop (§3-3 W1/W2 writer target). Idempotent
    /// keyed insert — W1 and W2 write identical content for the same
    /// `(lineage_id, hop_index)`, so any crash interleaving re-converges (§3-3).
    pub fn put_rotation_entry(
        &self,
        entry: &crate::network::rotation_finality::RotationChainEntry,
    ) -> Result<()> {
        let cf = self.try_cf(crate::network::rotation_finality::CF_KEY_ROTATION_CHAIN)?;
        let meta_cf = self.try_cf(CF_METADATA)?;
        let key = Self::rotation_chain_key(&entry.lineage_id, entry.hop_index);
        let bytes = serde_json::to_vec(entry)
            .map_err(|e| ElaraError::Storage(format!("put_rotation_entry encode: {e}")))?;
        // The entry and its forward predecessor-index row commit in ONE batch so
        // they can never diverge across a crash (§6.1): the index maps this hop's
        // introduced key → this hop's `(lineage_id, hop_index)`, so the NEXT hop
        // resolves its lineage with an O(1) point lookup
        // (`get_rotation_newkey_index`), never an O(all-rotations) CF scan (the
        // SCALE rule). Idempotent — W1 (drain hook) and W2 (marker sweep) write
        // byte-identical content for the same hop, so any crash interleaving
        // re-converges.
        let mut batch = rocksdb::WriteBatch::default();
        batch.put_cf(&cf, &key, &bytes);
        batch.put_cf(
            &meta_cf,
            Self::rotation_newkey_idx_key(&entry.new_key_hash),
            Self::encode_lineage_pos(&entry.lineage_id, entry.hop_index),
        );
        // W2-A: this hop's CF entry has landed — clear its pending-CF marker so
        // the seal-arm gate's emptiness short-circuit stays effective. Idempotent
        // (deleting an absent key is a no-op); same batch ⇒ crash-atomic with the
        // entry, so a hop can never be both durably-final AND still pending.
        batch.delete_cf(
            &meta_cf,
            Self::rotation_pending_hash_key(&entry.record_hash),
        );
        self.db
            .write(batch)
            .map_err(|e| ElaraError::Storage(format!("put_rotation_entry: {e}")))
    }

    /// Point-lookup one hop (the §5-H3b predecessor-finality check). `None` = no
    /// such finalized hop → pending at best, never effective (§4 fail-closed).
    pub fn get_rotation_entry(
        &self,
        lineage_id: &str,
        hop_index: u32,
    ) -> Option<crate::network::rotation_finality::RotationChainEntry> {
        let cf = self
            .try_cf(crate::network::rotation_finality::CF_KEY_ROTATION_CHAIN)
            .ok()?;
        let key = Self::rotation_chain_key(lineage_id, hop_index);
        let bytes = self.db.get_cf(&cf, &key).ok().flatten()?;
        serde_json::from_slice(&bytes).ok()
    }

    /// All hops of one lineage in ascending `hop_index` order (§4 chain-order
    /// replay). Bounded by `ROTATION_MAX_CHAIN_DEPTH` per anchor — a per-lineage
    /// prefix scan, never a full-CF scan.
    pub fn get_rotation_chain(
        &self,
        lineage_id: &str,
    ) -> Vec<crate::network::rotation_finality::RotationChainEntry> {
        let Ok(cf) = self.try_cf(crate::network::rotation_finality::CF_KEY_ROTATION_CHAIN) else {
            return Vec::new();
        };
        let mut prefix = lineage_id.as_bytes().to_vec();
        prefix.push(0u8);
        let iter = self.db.prefix_iterator_cf(&cf, &prefix);
        let mut out = Vec::new();
        for item in iter {
            let Ok((k, v)) = item else { continue };
            // A prefix extractor is set on this CF; the explicit boundary guard
            // is what actually stops the scan at the lineage (house pattern).
            if !k.starts_with(&prefix) {
                break;
            }
            if let Ok(entry) = serde_json::from_slice::<
                crate::network::rotation_finality::RotationChainEntry,
            >(&v)
            {
                out.push(entry);
            }
    ...
    // ---- W1/W2 forward predecessor-index (§6.1 lineage derivation) -----------
    //
    // Maps an INTRODUCED key hash → the position of the hop that introduced it:
    // `new_key_hash → (lineage_id, hop_index)`. `derive_lineage_position`'s
    // `introducing_hop(prev_key_hash)` is exactly a point lookup into this index,
    // so lineage resolution is O(1) per hop — never an O(all-rotations) CF scan
    // (the SCALE rule). Written atomically with each rotation CF entry by
    // `put_rotation_entry` above, so entry and index cannot diverge; the §5-H3b
    // admission gate (later slice) reads the same index. Rows live in CF_METADATA
    // under a text prefix (like the W2 markers), keeping CF_KEY_ROTATION_CHAIN
    // pure for per-lineage prefix scans and full-CF snapshot enumeration.

    /// Index namespace prefix (CF_METADATA). A `new_key_hash` is lowercase hex,
    /// so it can never collide with this ASCII prefix.
    const ROTATION_NEWKEY_IDX_PREFIX: &'static [u8] = b"rotation_newkey:";

    /// Index key: `rotation_newkey:` ‖ `new_key_hash`.
    fn rotation_newkey_idx_key(new_key_hash: &str) -> Vec<u8> {
        let mut k = Self::ROTATION_NEWKEY_IDX_PREFIX.to_vec();
        k.extend_from_slice(new_key_hash.as_bytes());
        k
    }

    /// Index value: `lineage_id` ‖ `0x00` ‖ `hop_index_be(4)`. `lineage_id` is
    /// lowercase hex (no interior `0x00`), so the separator is unambiguous.
    fn encode_lineage_pos(lineage_id: &str, hop_index: u32) -> Vec<u8> {
        let mut v = lineage_id.as_bytes().to_vec();
        v.push(0u8);
        v.extend_from_slice(&hop_index.to_be_bytes());
        v
    }

    fn decode_lineage_pos(bytes: &[u8]) -> Option<(String, u32)> {
        if bytes.len() < 5 {
            return None; // need ≥ 1 lineage byte + sep + 4 hop bytes
        }
        let split = bytes.len() - 4;
        if bytes[split - 1] != 0u8 {
            return None; // malformed separator
        }
        let lineage = std::str::from_utf8(&bytes[..split - 1]).ok()?.to_string();
        let hop = u32::from_be_bytes(bytes[split..].try_into().ok()?);
        Some((lineage, hop))
    }

    /// The hop that introduced `key_hash` → its `(lineage_id, hop_index)`, or
    /// `None` when no finalized hop ever introduced `key_hash` (it is a lineage
    /// root `pk₀`). This IS `derive_lineage_position`'s `introducing_hop` lookup
    /// — a single point read, so per-hop lineage derivation stays O(1).
    pub fn get_rotation_newkey_index(&self, key_hash: &str) -> Option<(String, u32)> {
        let cf = self.try_cf(CF_METADATA).ok()?;
        let bytes = self
            .db
            .get_cf(&cf, Self::rotation_newkey_idx_key(key_hash))
            .ok()
            .flatten()?;
        Self::decode_lineage_pos(&bytes)
    }

    // ---- KR-3 v4 slice-2: durable origin/deep fact layer (UNWIRED) ----------
    //
    // The GC-durable, content-addressed facts the KR-3 v4 origin-key predicate
    // (internal design notes §3) branches on at
    // the three rotation surfaces. Landed here as storage primitives ONLY — NO
    // production write/read site wires them yet. The consuming classifier
    // (`classify_predecessor`) and its write-arbitration (a CAS-aware
    // `put_rotation_entry` + the §5 same-slot resolver) are BLOCKED on the
    // v4→v5 redesign (audit `kr3-origin-key-predicate-v4-an internal audit,
    // DEFER verdict) and `s2_rotation_ordering_enabled` stays OFF network-wide,
    // so landing the fact layer now is flag-OFF byte-identical (nothing calls
    // these) and lets the eventual wiring be a pure consumer.
    //
    // GC-durability (the B1 close, §3.4): both rows live in CF_METADATA keyed by
    // an IDENTITY hash / a RECORD_HASH — never a record id — so `delete_record`
    // (which deletes only `record_id`-keyed rows, this file) never removes them.
    // The predicate outlives the GC of its carrier record.

    /// Root-fact namespace prefix (CF_METADATA) — realizes the brief's
    /// `origin:{identity}` fact. An identity hash is lowercase hex, so it never
    /// collides with this ASCII prefix nor with the sibling `rotation_*:` rows.
    const ORIGIN_KEY_PREFIX: &'static [u8] = b"origin_key:";

    /// Root-fact key: `origin_key:` ‖ `identity_hash`.
    fn origin_key_row_key(identity_hash: &str) -> Vec<u8> {
        let mut k = Self::ORIGIN_KEY_PREFIX.to_vec();
        k.extend_from_slice(identity_hash.as_bytes());
        k
    }

    /// Audit value `u32-BE len(record_hash) ‖ record_hash ‖ coord`. Length-framing
    /// the first field makes the split unambiguous for ANY byte content of either
    /// field: a future wiring that passes a raw 32-byte digest (≈11.6% carry an
    /// interior `0x00`) instead of its hex, or a `coord` that ever carries a NUL,
    /// still round-trips exactly — the decode can never mis-split, so the durable
    /// fact can never silently corrupt. (The prior `record_hash ‖ 0x00 ‖ coord`
    /// framing, split on the FIRST NUL, relied on an unenforced "record_hash is
    /// NUL-free hex" caller convention — a footgun for the deferred wiring step.
    /// UNWIRED ⇒ no on-disk rows use the old framing, so this is a pure hardening
    /// with no migration.)
    fn encode_origin_fact(record_hash: &str, coord: &str) -> Vec<u8> {
        let rh = record_hash.as_bytes();
        let mut v = Vec::with_capacity(4 + rh.len() + coord.len());
        v.extend_from_slice(&(rh.len() as u32).to_be_bytes());
        v.extend_from_slice(rh);
        v.extend_from_slice(coord.as_bytes());
        v
    }

    fn decode_origin_fact(bytes: &[u8]) -> Option<(String, String)> {
        let rh_len = u32::from_be_bytes(bytes.get(..4)?.try_into().ok()?) as usize;
        let end = 4usize.checked_add(rh_len)?;
        let record_hash = std::str::from_utf8(bytes.get(4..end)?).ok()?.to_string();
        let coord = std::str::from_utf8(bytes.get(end..)?).ok()?.to_string();
        Some((record_hash, coord))
    }

    /// Stage the durable root fact for `identity_hash` into `batch` (the eventual
    /// origin_reg-finalize hook commits it atomically with the finalize writes).
    /// Written UNCONDITIONALLY on origin_reg finalize (brief §3.1, monotonic) — a
    /// presence-only predicate, so re-writing the same identity is a benign
    /// idempotent overwrite of equal-or-fresher audit metadata.
    pub fn put_origin_key_to_batch(
        &self,
        ...
        batch.put_cf(
            &cf,
            Self::origin_key_row_key(identity_hash),
            Self::encode_origin_fact(record_hash, coord),
        );
        Ok(())
    }

    /// Presence predicate the v4 classifier's Root branch reads (brief §3.1/§4):
    /// true iff `identity_hash` has a durable origin-key fact.
    pub fn has_origin_key(&self, identity_hash: &str) -> bool {
        self.try_cf(CF_METADATA)
            .ok()
            .and_then(|cf| {
                self.db
                    .get_cf(&cf, Self::origin_key_row_key(identity_hash))
                    .ok()
                    .flatten()
            })
            .is_some()
    ...
    /// The audit payload `(record_hash, coord)` behind an origin-key fact, or
    /// `None` when `identity_hash` is not a durable root.
    pub fn get_origin_key(&self, identity_hash: &str) -> Option<(String, String)> {
        let cf = self.try_cf(CF_METADATA).ok()?;
        let bytes = self
            .db
            .get_cf(&cf, Self::origin_key_row_key(identity_hash))
            .ok()
            .flatten()?;
        Self::decode_origin_fact(&bytes)
    }

    /// Deep-fact namespace prefix (CF_METADATA) — the content-addressed forward
    /// index `rotation_by_hash:{record_hash} → (lineage_id, hop_index)` (brief
    /// §3.2). A record_hash is lowercase hex, never colliding with the prefix or
    /// the sibling `rotation_newkey:` / `rotation_seal_pending:` rows.
    const ROTATION_BY_HASH_PREFIX: &'static [u8] = b"rotation_by_hash:";

    fn rotation_by_hash_key(record_hash: &str) -> Vec<u8> {
        let mut k = Self::ROTATION_BY_HASH_PREFIX.to_vec();
        k.extend_from_slice(record_hash.as_bytes());
        k
    }

    /// Stage the content-addressed `record_hash → (lineage_id, hop_index)` row
    /// into `batch` (brief §3.2: the eventual wiring writes it inside
    /// `put_rotation_entry`'s batch, so entry and index commit atomically).
    /// Unlike `rotation_newkey:` (LWW by new_key_hash — the backward collision
    /// B2), this is keyed by the hop's OWN record_hash: distinct hops never
    /// collide (a collision would be a sha3 second-preimage) and the same hop
    /// re-writes byte-identical content — the collision-free classification
    /// lookup the v4 Deep branch resolves through (B3/B4).
    pub fn put_rotation_by_hash_to_batch(
        &self,
        batch: &mut rocksdb::WriteBatch,
        record_hash: &str,
        lineage_id: &str,
        ...
        batch.put_cf(
            &cf,
            Self::rotation_by_hash_key(record_hash),
            Self::encode_lineage_pos(lineage_id, hop_index),
        );
        Ok(())
    }

    /// Resolve a rotation hop's `(lineage_id, hop_index)` by its `record_hash`,
    /// or `None` when the hop's durable row has not landed here yet (the v4 Deep
    /// branch treats absence as RejectTransient — a timing artifact, never a
    /// permanent reject, brief §4).
    pub fn get_rotation_by_hash(&self, record_hash: &str) -> Option<(String, u32)> {
        let cf = self.try_cf(CF_METADATA).ok()?;
        let bytes = self
            .db
            .get_cf(&cf, Self::rotation_by_hash_key(record_hash))
            .ok()
            .flatten()?;
        Self::decode_lineage_pos(&bytes)
    }

    // ---- W2-A rotation-pending-hash index (§3-3 marker-arm gate) -------------
    //
    // Records the `record_hash` of every ingested rotation-class hop that has
    // not yet had its rotation-CF entry written. A seal's Phase-2 marker-arm
    // decision ("does this seal cover ≥1 rotation-class record?") is then
    // computable from the seal's OWN `record_hashes` alone — an O(1) emptiness
    // short-circuit (`has_any_rotation_pending`) for the common no-rotation seal,
    // then point lookups (`rotation_hash_pending`), never a member record load
    // (SCALE rule). A row is added in-batch when the rotation record is stored
    // (`RotationBatchOp::RotationHopAdmit`) and cleared in-batch by
    // `put_rotation_entry` once the CF entry lands, so the set stays near-empty
    // (rotation hops are rare per S5). Rows live in CF_METADATA under a text
    // prefix; a `record_hash` is lowercase hex, never colliding with the prefix.

    const ROTATION_PENDING_HASH_PREFIX: &'static [u8] = b"rot_pending_hash:";

    fn rotation_pending_hash_key(record_hash_hex: &str) -> Vec<u8> {
        let mut k = Self::ROTATION_PENDING_HASH_PREFIX.to_vec();
        k.extend_from_slice(record_hash_hex.as_bytes());
        k
    }

    /// True iff ≥1 rotation-class record is still pending its CF entry. One
    /// prefix seek: the common (no rotation activity) case short-circuits every
    /// seal's marker-arm decision to O(1). The first key at/after the prefix is
    /// in-namespace iff a pending row exists.
    pub fn has_any_rotation_pending(&self) -> bool {
        let Ok(cf) = self.try_cf(CF_METADATA) else {
            return false;
        };
        let prefix = Self::ROTATION_PENDING_HASH_PREFIX;
        let mut it = self
            .db
    ...
    /// True iff `record_hash_hex` names a rotation-class record still pending its
    /// CF entry — the per-member arm check over a seal's `record_hashes`.
    pub fn rotation_hash_pending(&self, record_hash_hex: &str) -> bool {
        let Ok(cf) = self.try_cf(CF_METADATA) else {
            return false;
        };
        self.db
            .get_cf(&cf, Self::rotation_pending_hash_key(record_hash_hex))
            .ok()
            .flatten()
            .is_some()
    }
    ...
    //
    // A seal covering ≥1 rotation-class record writes a durable
    // `rotation_seal_pending:{zone}:{epoch}` marker row in the SAME WriteBatch
    // as the seal store — crash-atomicity: a seal can never land durable
    // without its marker, else the W2 sweep never fires for that slot and the
    // rotation stalls Pending forever (§3-3 R-1). The marker names a SEALED
    // SLOT — a historical object whose `(zone, epoch)` identity is immutable
    // once sealed — so the key never goes stale under later zone-count
    // transitions. The W2 sweep (boot + periodic tick) scans the prefix in
    // ascending epoch order per zone, establishes the seal leg from durable
    // evidence, writes the rotation CF entries for the member rids, and deletes
    // the marker. No live caller yet — these are the persistence layer W2 will
    // build on.

    /// Marker namespace. All rows live in [`CF_METADATA`] under this prefix so
    /// the sweep's `From`-seek reads only markers — bounded, since
    /// rotation-bearing seals are rare by S5 and markers are deleted once
    /// processed (never a full-CF scan).
    const ROTATION_SEAL_MARKER_PREFIX: &'static [u8] = b"rotation_seal_pending:";

    /// Marker key: `rotation_seal_pending:` ‖ `zone` ‖ `0x00` ‖ `epoch_be(8)`.
    /// The big-endian epoch suffix makes a forward scan yield, within each zone
    /// (zone strings are ASCII — no interior `0x00`), ascending `epoch` order —
    /// the deterministic forward build the sweep requires (§3-3: descendants'
    /// seals processed after their predecessors').
    fn rotation_seal_marker_key(zone: &str, epoch: u64) -> Vec<u8> {
        let mut k = Self::ROTATION_SEAL_MARKER_PREFIX.to_vec();
        k.extend_from_slice(zone.as_bytes());
        k.push(0u8);
        k.extend_from_slice(&epoch.to_be_bytes());
        k
    }

    /// Arm the W2 sweep for a sealed slot by adding the marker to `batch` — the
    /// caller MUST commit `batch` in the same write as the seal store so the two
    /// are crash-atomic (§3-3). Value is empty: existence IS the signal.
    pub fn put_rotation_seal_marker_to_batch(
        &self,
        batch: &mut rocksdb::WriteBatch,
        zone: &str,
        epoch: u64,
        ...
        let cf = self.try_cf(CF_METADATA)?;
        batch.put_cf(&cf, Self::rotation_seal_marker_key(zone, epoch), b"");
        Ok(())
    }

    /// Clear a marker once the W2 sweep has written its rotation CF entries.
    /// Idempotent — deleting an absent marker is a no-op (a rival-seal slot may
    /// be swept and cleared by whichever candidate finalizes first).
    pub fn delete_rotation_seal_marker(&self, zone: &str, epoch: u64) -> Result<()> {
        let cf = self.try_cf(CF_METADATA)?;
        self.db
            .delete_cf(&cf, Self::rotation_seal_marker_key(zone, epoch))
            .map_err(|e| ElaraError::Storage(format!("delete_rotation_seal_marker: {e}")))
    }

    /// Test-only: arm a W2 sweep marker OUTSIDE a seal-store batch. Production
    /// arms markers only *inside* the seal's Phase-2 `WriteBatch`
    /// ([`put_rotation_seal_marker_to_batch`](Self::put_rotation_seal_marker_to_batch))
    /// so the marker and the seal are crash-atomic (§3-3); this stand-alone put
    /// deliberately breaks that coupling and exists ONLY so the W2 sweep's
    /// cross-module unit tests can present a marker without re-driving the whole
    /// ingest → Phase-2-store path. `#[cfg(test)]` so no production caller can
    /// reach it and arm a marker without its covering seal.
    #[cfg(test)]
    pub fn arm_rotation_seal_marker_for_test(&self, zone: &str, epoch: u64) -> Result<()> {
        let mut batch = rocksdb::WriteBatch::default();
        self.put_rotation_seal_marker_to_batch(&mut batch, zone, epoch)?;
        self.db
            .write(batch)
            .map_err(|e| ElaraError::Storage(format!("arm_rotation_seal_marker_for_test: {e}")))
    }

    /// All armed markers as `(zone, epoch)`, grouped by zone and ascending in
    /// `epoch` within each zone (§3-3 sweep order). A total-order `From`-seek +
    /// `starts_with` boundary guard — extractor-independent, bounded to the
    /// marker prefix, never a full-CF scan.
    pub fn scan_rotation_seal_markers(&self) -> Vec<(String, u64)> {
        let Ok(cf) = self.try_cf(CF_METADATA) else {
            return Vec::new();
        };
        let prefix = Self::ROTATION_SEAL_MARKER_PREFIX;
        let mode = rocksdb::IteratorMode::From(prefix, rocksdb::Direction::Forward);
        let mut out = Vec::new();
        for kv in self.db.iterator_cf(&cf, mode) {
            let Ok((key, _)) = kv else { continue };
            if !key.starts_with(prefix) {
                break; // left the marker namespace — stop
            }
            let rest = &key[prefix.len()..];
            // rest = zone ‖ 0x00 ‖ epoch_be(8): need ≥ 1 sep + 8 epoch bytes.
            if rest.len() < 9 {
                continue; // malformed, skip
            }
            let split = rest.len() - 8;
            if rest[split - 1] != 0u8 {
                continue; // malformed separator, skip
            }
            let Ok(zone) = std::str::from_utf8(&rest[..split - 1]) else {
                continue; // non-UTF8 zone, skip
            };
            let mut eb = [0u8; 8];
            eb.copy_from_slice(&rest[split..]);
            out.push((zone.to_string(), u64::from_be_bytes(eb)));
        }
        out
    }

    // ---- §6.1 rotation zone-routing pin (write-once at admission) ------------
    //
    // A rotation-class record's storage / seal-membership / proof routing must
    // key off its rotation-stable `lineage_id` (`sha3(pk₀)`), NOT its own record
    // id — else a rotation re-routes the chain on every hop and the feature is
    // non-functional (§6.1, R-2/R-8). At admission the record's `lineage_id` is
    // derived (§6.1 rule) and pinned in a durable `rotation_zone_pin:{record_id}
    // → lineage_id` row committed in the SAME WriteBatch as the record store —
    // read-your-write for every downstream consumer (the storage-zone insert key,
    // seal membership, proof paths). The row stores the ROUTING KEY (`lineage_id`,
    // a 32-byte hash in hex), never a resolved zone literal, so zone-registry
    // splits and zone-count transitions re-route the pinned chain deterministically
    // for everyone — exactly like identity-keyed account state. Rotation-class is
    // rare (S5), so the pin set is tiny and boot-loads into an in-memory set for
    // an O(1) negative-hit fall-through at the central resolver. Rows live in
    // CF_METADATA under a text prefix; a `record_id`/`lineage_id` never collides
    // with the prefix. No live caller yet — this is the persistence layer the
    // admission pin write and the `resolve_record_zone` hook build on.

    const ROTATION_ZONE_PIN_PREFIX: &'static [u8] = b"rotation_zone_pin:";

    fn rotation_zone_pin_key(record_id: &str) -> Vec<u8> {
        let mut k = Self::ROTATION_ZONE_PIN_PREFIX.to_vec();
        k.extend_from_slice(record_id.as_bytes());
        k
    }

    /// Pin `record_id`'s routing key (`lineage_id`) by adding the row to `batch` —
    /// the caller MUST commit `batch` in the same write as the record store so the
    /// pin and the record are crash-atomic (§6.1). A record durable without its
    /// pin would resolve naively and store under the wrong zone's index, breaking
    /// `query_zone_ids` for the rotation chain. Idempotent by key (a re-ingest of
    /// the same record re-pins the identical `lineage_id`).
    pub fn put_rotation_zone_pin_to_batch(
        &self,
        batch: &mut rocksdb::WriteBatch,
        record_id: &str,
        lineage_id: &str,
        ...
        batch.put_cf(
            &cf,
            Self::rotation_zone_pin_key(record_id),
            lineage_id.as_bytes(),
        );
        Ok(())
    }

    /// The pinned routing key (`lineage_id`) for `record_id`, or `None` when the
    /// record is not rotation-class / not pinned (a non-UTF8 value is treated as
    /// absent — pins are always written as hex). One point read.
    pub fn get_rotation_zone_pin(&self, record_id: &str) -> Option<String> {
        let cf = self.try_cf(CF_METADATA).ok()?;
        let bytes = self
            .db
            .get_cf(&cf, Self::rotation_zone_pin_key(record_id))
            .ok()
            .flatten()?;
        String::from_utf8(bytes).ok()
    }

    /// All pins as `(record_id, lineage_id)` — the boot-load source for the
    /// in-memory pinned set. A total-order `From`-seek + `starts_with` boundary
    /// guard, bounded to the pin prefix (rotation-class rare per S5, deterministic
    /// order, never a full-CF scan). Rows with a non-UTF8 key-suffix or value are
    /// skipped (defensive — production only ever writes UTF-8 ids/hex).
    pub fn scan_rotation_zone_pins(&self) -> Vec<(String, String)> {
        let Ok(cf) = self.try_cf(CF_METADATA) else {
            return Vec::new();
        };
        let prefix = Self::ROTATION_ZONE_PIN_PREFIX;
        let mode = rocksdb::IteratorMode::From(prefix, rocksdb::Direction::Forward);
        let mut out = Vec::new();
        for kv in self.db.iterator_cf(&cf, mode) {
            let Ok((key, val)) = kv else { continue };
            if !key.starts_with(prefix) {
                break; // left the pin namespace — stop
            }
            let Ok(record_id) = std::str::from_utf8(&key[prefix.len()..]) else {
                continue; // non-UTF8 record id, skip
            };
            if record_id.is_empty() {
                continue; // malformed empty-id row, skip
            }
            let Ok(lineage_id) = std::str::from_utf8(&val) else {
                continue; // non-UTF8 lineage value, skip
            };
            out.push((record_id.to_string(), lineage_id.to_string()));
        }
        out
    }

    /// Test-only: pin a record's routing key OUTSIDE a record-store batch.
    /// Production pins rides the record's admission `WriteBatch`
    /// ([`put_rotation_zone_pin_to_batch`](Self::put_rotation_zone_pin_to_batch))
    /// so pin and record are crash-atomic (§6.1); this stand-alone put
    /// deliberately breaks that coupling so the resolver's cross-module tests can
    /// present a pin without re-driving the whole admission path. `#[cfg(test)]`
    /// so no production caller can pin a routing key without its covering record.
    #[cfg(test)]
    pub fn pin_rotation_zone_for_test(&self, record_id: &str, lineage_id: &str) -> Result<()> {
        let mut batch = rocksdb::WriteBatch::default();
        self.put_rotation_zone_pin_to_batch(&mut batch, record_id, lineage_id)?;
        self.db
            .write(batch)
            .map_err(|e| ElaraError::Storage(format!("pin_rotation_zone_for_test: {e}")))
    }
    ...
    /// Build the `CF_MANDATE_ACTS_BY_TIME` key for an act: `ts(8 BE) ++ record_id`
    /// (B5). Big-endian timestamp makes a byte-lexicographic scan from
    /// `IteratorMode::Start` an ascending-time (oldest-first) order — exactly what
    /// the budget evictor needs. Unlike the reverse indexes this NEVER returns
    /// `None`: every act has a timestamp, and the record_id suffix disambiguates
    /// ties. One encoding shared by put / delete / evict / scan so they can't drift.
    fn mandate_acts_time_key(act_timestamp_ms: u64, record_id: &str) -> Vec<u8> {
        let mut key = Vec::with_capacity(8 + record_id.len());
        key.extend_from_slice(&act_timestamp_ms.to_be_bytes());
        key.extend_from_slice(record_id.as_bytes());
        key
    }
    ...
    /// id), the by-agent reverse index (`signer_hash ++ ts ++ record_id → ()`),
    /// AND the by-time index (`ts ++ record_id → ()`, B5) — all in a SINGLE atomic
    /// `WriteBatch`, so the four indexes can never diverge across a crash.
    pub fn put_mandate_act(
        ...
        // B5 4th key: the time-ordered index (never None — every act has a ts).
        let tkey = Self::mandate_acts_time_key(entry.act_timestamp_ms, record_id);
        batch.put_cf(&self.try_cf(CF_MANDATE_ACTS_BY_TIME)?, &tkey, b"");
        self.db
            .write(batch)
    ...
    ///
    /// Convenience over [`Self::get_mandate_act_checked`] that COLLAPSES a storage
    /// read/decode error to `None`. Safe for callers that treat "not an act" and
    /// "couldn't tell" the same way, but NEVER use it where a false negative is a
    /// correctness/honesty bug — `delete_record`'s AdminEvict path and the
    /// `/mandate/status` absence contract both require the checked variant so a
    /// transient fault can't read as a confident "not a mandate act".
    pub fn get_mandate_act(&self, record_id: &str) -> Option<crate::mandate::MandateActEntry> {
        self.get_mandate_act_checked(record_id).ok().flatten()
    }

    /// Fetch the act-index entry for a record_id, distinguishing a genuine miss
    /// (`Ok(None)`) from a storage read or decode fault (`Err`). B5 (verdict
    /// 2026-07-18): the `.ok()?` swallow in the old getter turned a transient CF
    /// read error into a confident "not a mandate act" — the exact lie-shape B5
    /// exists to kill. Used inside `delete_record` (Err aborts the evict before
    /// any key is touched) and by the honest absence contract.
    pub fn get_mandate_act_checked(
        &self,
        record_id: &str,
    ) -> Result<Option<crate::mandate::MandateActEntry>> {
        let cf = self.try_cf(CF_MANDATE_ACT)?;
        match self
            .db
            .get_cf(&cf, record_id.as_bytes())
            .map_err(|e| ElaraError::Storage(format!("get_mandate_act read: {e}")))?
        {
            Some(bytes) => serde_json::from_slice(&bytes)
                .map(Some)
                .map_err(|e| ElaraError::Storage(format!("get_mandate_act decode: {e}"))),
            None => Ok(None),
        }
    }

    // ─── B5 act-index coverage floor + budget evictor ─────────────────────

    /// Read the durable act-index coverage floor (ms). 0 = full authoritative
    /// coverage claimed (no gap). A single bloom-filtered `CF_METADATA` point read
    /// — cheap enough to call per `/mandate/status` query at any scale, which is
    /// why the absence contract reads it DIRECTLY rather than through a
    /// stale-prone in-memory cache (the storage-layer evictor advances the durable
    /// floor with no path to a `NodeState` atomic; a lagging cache would under-
    /// report the floor and re-create the exact false-negative B5 kills).
    pub fn acts_coverage_floor_ms(&self) -> Result<u64> {
        Ok(self.acts_coverage_floor_state()?.unwrap_or(0))
    }

    /// The HONEST coverage-floor state: `Some(v)` iff the key is present AND a
    /// well-formed 8-byte value (v==0 is genuine genesis full-coverage), `None`
    /// when the key is ABSENT or malformed (uninitialized — this node has not yet
    /// recorded its coverage boundary). The absence contract MUST use this, not
    /// [`Self::acts_coverage_floor_ms`]: collapsing "uninitialized" to the same `0`
    /// that means "authoritative full coverage" is fail-OPEN — a node whose one-time
    /// boot init write transiently failed would otherwise serve a confident
    /// `authoritative_complete:true` "not a mandate act" for every absent query (the
    /// exact false-negative B5 exists to kill). `None` → non-authoritative
    /// (fail-CLOSED). Found 2026-07-19 by the B5 post-ship adversarial review.
    pub fn acts_coverage_floor_state(&self) -> Result<Option<u64>> {
        match self.get_cf_raw(CF_METADATA, ACTS_COVERAGE_FLOOR_KEY.as_bytes())? {
            Some(b) if b.len() == 8 => {
                let mut buf = [0u8; 8];
                buf.copy_from_slice(&b[..8]);
                Ok(Some(u64::from_be_bytes(buf)))
            }
            _ => Ok(None), // absent or malformed — uninitialized, NOT genesis-0
        }
    }

    /// Advance the durable coverage floor to `max(current, ts_ms)`, returning the
    /// post-advance value. Monotone (never regresses) and serialized by
    /// `acts_floor_lock` so concurrent evictor / snapshot-bootstrap advances can't
    /// interleave a regressing write (B4). Advancing to `ts_ms <= current` is a
    /// no-op.
    pub fn advance_acts_coverage_floor(&self, ts_ms: u64) -> Result<u64> {
        let _guard = self.acts_floor_lock.lock().unwrap_or_else(|e| e.into_inner());
        let current = self.acts_coverage_floor_ms()?;
        if ts_ms <= current {
            return Ok(current);
        }
        self.put_cf_raw(
            CF_METADATA,
            ACTS_COVERAGE_FLOOR_KEY.as_bytes(),
            &ts_ms.to_be_bytes(),
        )?;
        Ok(ts_ms)
    }

    /// Ensure the floor key EXISTS, initializing to `now_ms` if absent (B5 init
    /// policy). Called once at boot. A DB missing the key is either the upgraded
    /// live seed (already GC'd acts → honest window from now) or a virgin pull-join
    /// to an existing chain (can never re-ingest already-pruned act carriers → must
    /// not claim authoritative coverage over pre-join history). Both want `now_ms`.
    /// A fresh genesis-ceremony chain would want 0, via
    /// `force_acts_coverage_floor_genesis` — but that path is NOT yet wired into
    /// any ceremony (a future re-genesis must call it), so TODAY every DB inits to
    /// `now_ms` (the safe under-claim). Serialized RMW (can't race a boot advance).
    /// The durable write is RETRIED + synced: a single transient failure would
    /// leave the node uninitialized, which `acts_coverage_floor_state` renders
    /// fail-CLOSED (non-authoritative) — safe, but a retry shrinks that window.
    pub fn ensure_acts_coverage_floor_initialized(&self) -> Result<()> {
        let _guard = self.acts_floor_lock.lock().unwrap_or_else(|e| e.into_inner());
        if self
            .get_cf_raw(CF_METADATA, ACTS_COVERAGE_FLOOR_KEY.as_bytes())?
            .is_some()
        {
            return Ok(());
        }
        let now_ms = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        // Synced + up-to-3 attempts: this one-time write gates absence-authority, so
        // it must actually reach disk (a crash right after a non-synced write would
        // re-leave the key absent → non-authoritative).
        let mut last_err = None;
        for attempt in 0..3 {
            match self.put_cf_raw_synced(
                CF_METADATA,
                ACTS_COVERAGE_FLOOR_KEY.as_bytes(),
                &now_ms.to_be_bytes(),
            ) {
                Ok(()) => {
                    tracing::info!(
                        "B5: acts coverage floor initialized to now_ms={now_ms} (no prior key)"
                    );
                    return Ok(());
                }
                Err(e) => {
                    tracing::warn!("B5: floor init write attempt {} failed: {e}", attempt + 1);
                    last_err = Some(e);
        ...
        Err(last_err.unwrap_or_else(|| {
            ElaraError::Storage("acts coverage floor init failed (no error captured)".into())
        }))
    }

    /// Genesis-ceremony ONLY: force the coverage floor to 0 (full authoritative
    /// coverage from chain birth). Bypasses the monotone guard because it must set
    /// a value BELOW the `now_ms` a plain boot init would write. Call from the
    /// genesis creation path on a freshly-minted chain — NEVER on an existing DB.
    /// NOTE (2026-07-19): NO production caller wires this yet — a future re-genesis
    /// (G5) must call it, else the fresh chain inits to `now_ms` (safe under-claim,
    /// but the archive anchor can't prove authoritative full-genesis coverage).
    pub fn force_acts_coverage_floor_genesis(&self) -> Result<()> {
        let _guard = self.acts_floor_lock.lock().unwrap_or_else(|e| e.into_inner());
        self.put_cf_raw(
            CF_METADATA,
            ACTS_COVERAGE_FLOOR_KEY.as_bytes(),
        ...
        tracing::info!("B5: acts coverage floor forced to 0 (genesis-ceremony chain birth)");
        Ok(())
    }

    /// Whether a record carries an operator removed-marker (B5 AdminEvict). A hit
    /// means the act was deliberately un-indexed — the absence is
    /// `removed_by_operator` (authoritative), distinct from a coverage-window gap.
    pub fn has_act_removed_marker(&self, record_id: &str) -> Result<bool> {
        let key = format!("{ACT_REMOVED_MARKER_PREFIX}{record_id}");
        Ok(self.get_cf_raw(CF_METADATA, key.as_bytes())?.is_some())
    }

    /// Oldest-first budget evictor (B5). When the act-CF mass exceeds
    /// `budget_bytes`, walk `CF_MANDATE_ACTS_BY_TIME` from the oldest key and
    /// delete ALL FOUR keys of each act (forward + 2 reverse + time), advancing the
    /// coverage floor to `last_evicted_ts + 1` in the SAME operation, until back
    /// under budget or `max_per_tick` acts are evicted. NEVER rejects ingest (the
    /// documented fleet-trap shape, B2) — over-budget nodes still accept sealed act
    /// records; the evictor trims the oldest tail. Returns the count evicted.
    ///
    /// O(evicted ≤ max_per_tick) via the time index — never a full-CF scan; the
    /// budget check is one O(1) property read. `budget_bytes == 0` disables
    /// eviction (unlimited — e.g. the archive profile).
    pub fn evict_acts_over_budget(&self, budget_bytes: u64, max_per_tick: usize) -> Result<usize> {
        if budget_bytes == 0 || max_per_tick == 0 {
            return Ok(0); // unlimited / disabled
        }
        if self.act_cf_live_bytes() <= budget_bytes {
            return Ok(0); // under budget — O(1) exit
        }
        self.evict_oldest_acts(max_per_tick)
    }

    /// Delete the OLDEST ≤`limit` acts (all four keys each), advance the coverage
    /// floor past the newest evicted ts, and bump the budget-evict counter.
    /// Returns the count evicted. Split out from the byte-budget gate in
    /// [`Self::evict_acts_over_budget`] so the eviction MECHANICS are unit-testable
    /// without materializing live-SST bytes (the gate's `live-sst-files-size` read
    /// is 0 until a flush). Not a public API — only the gate should call it.
    fn evict_oldest_acts(&self, max_per_tick: usize) -> Result<usize> {
        if max_per_tick == 0 {
            return Ok(0);
        }
        // Take the oldest ≤max_per_tick time keys (a contiguous ascending prefix).
        let cf_time = self.try_cf(CF_MANDATE_ACTS_BY_TIME)?;
        let mut victims: Vec<(Vec<u8>, u64, String)> = Vec::with_capacity(max_per_tick.min(4096));
        {
            let iter = self.db.iterator_cf(&cf_time, rocksdb::IteratorMode::Start);
            for item in iter {
                if victims.len() >= max_per_tick {
                    break;
                }
                let (key, _v) =
                    item.map_err(|e| ElaraError::Storage(format!("evict scan: {e}")))?;
                if key.len() < 8 {
                    continue; // malformed key — skip (never index-scanned again)
                }
                let mut tb = [0u8; 8];
                tb.copy_from_slice(&key[..8]);
                let ts = u64::from_be_bytes(tb);
                let rid = match std::str::from_utf8(&key[8..]) {
                    Ok(s) => s.to_string(),
                    Err(_) => continue,
                };
                victims.push((key.to_vec(), ts, rid));
            }
        }
        if victims.is_empty() {
            return Ok(0);
        ...
        let mut batch = rocksdb::WriteBatch::default();
        let mut max_evicted_ts = 0u64;
        let cf_act = self.try_cf(CF_MANDATE_ACT)?;
        let cf_by_m = self.try_cf(CF_MANDATE_ACTS_BY_MANDATE)?;
        let cf_by_a = self.try_cf(CF_MANDATE_ACTS_BY_AGENT)?;
        for (tkey, ts, rid) in &victims {
            // Reconstruct reverse keys from the forward entry when decodable; a
            // decode error just skips them — we still drop the forward + time keys
            // so the evictor always makes progress (never stalls on a bad row).
            if let Ok(Some(act)) = self.get_mandate_act_checked(rid) {
                if let Some(rk) =
                    Self::mandate_acts_reverse_key(&act.mandate_ref, act.act_timestamp_ms, rid)
                {
                    batch.delete_cf(&cf_by_m, &rk);
                }
                if let Some(ak) = Self::mandate_acts_agent_reverse_key(
                    &act.signer_identity_hash,
                    act.act_timestamp_ms,
                    rid,
                ) {
                    batch.delete_cf(&cf_by_a, &ak);
                }
            }
            batch.delete_cf(&cf_act, rid.as_bytes());
            batch.delete_cf(&cf_time, tkey);
            if *ts > max_evicted_ts {
                max_evicted_ts = *ts;
            }
        }
        self.db
            .write(batch)
            .map_err(|e| ElaraError::Storage(format!("evict batch: {e}")))?;
        // Everything ≤ the newest evicted act may now be incomplete on this node,
        // so absence below `max_evicted_ts + 1` is no longer authoritative. The
        // evicted set is a contiguous oldest prefix, so this floor is exact.
        self.advance_acts_coverage_floor(max_evicted_ts.saturating_add(1))?;
        crate::network::mandate_node::MANDATE_ACT_ENTRIES_DELETED_BUDGET_EVICT
    ...
    /// Delete a subsystem snapshot (checkpoint invalidation). F2 R2: tombstoning a
    /// ledger/governance-op-carrying record deletes the "ledger" checkpoint so the
    /// next boot takes the full-rebuild path (which honors the is_tombstoned gov
    /// skip) instead of fast-booting from a checkpoint that baked in the now-
    /// tombstoned op's effect. Scale-safe: a full rebuild ONCE on the rare
    /// (genesis-only) tombstone event, never an O(all_records) gov rescan per boot.
    ...
    pub fn delete_snapshot(&self, name: &str) -> Result<()> {
        ...
        self.db
            ...
            .map_err(|e| ElaraError::Storage(format!("snapshot delete({name}): {e}")))?;
        Ok(())
    }

    /// F2 PSR-1: persistent "governance rebuild required" marker key. A
    /// governance-op tombstone sets it; boot reads it FAIL-CLOSED and, if present,
    /// forces the full ledger rebuild (which honors the gov fold-skip) instead of
    /// the checkpoint fast-path. It is a SEPARATE CF_METADATA key from the "ledger"
    /// snapshot, so the unconditional periodic/shutdown snapshot re-save — which
    /// re-persists the stale in-memory governance tally and thereby defeats
    /// `delete_snapshot("ledger")` alone — cannot clobber it. See the F2 brief
    /// POST-SHIP REVIEW (PSR-1); no CF_METADATA prefix-delete touches this key
    /// (only `ban:`/`blocked_term:`/`__snapshot__:` are prefix-swept).
    const GOV_REBUILD_MARKER: &'static [u8] = b"gov_rebuild_required";

    /// Set the gov-rebuild marker (a governance-op record was tombstoned → next
    /// boot must full-rebuild to drop it from the out-of-seal governance tally).
    pub fn set_gov_rebuild_required(&self) -> Result<()> {
        self.put_cf_raw(CF_METADATA, Self::GOV_REBUILD_MARKER, b"1")
    }

    /// Read the gov-rebuild marker FAIL-CLOSED: a read error returns `true`
    /// (force the full rebuild) rather than silently keeping a possibly-stale
    /// checkpoint. The safe polarity is the OPPOSITE of the checkpoint load
    /// (`.ok().flatten()` → None → rebuild, safe there): here "absent" means keep
    /// the checkpoint, so a transient read error must NOT read as absent.
    pub fn gov_rebuild_required(&self) -> bool {
        self.get_cf_raw(CF_METADATA, Self::GOV_REBUILD_MARKER)
            .map(|o| o.is_some())
            .unwrap_or(true)
    }

    /// Clear the gov-rebuild marker (best-effort). A failed clear only costs one
    /// redundant full rebuild next boot — never crash boot over it (`let _ =`).
    pub fn clear_gov_rebuild_required(&self) {
        let _ = self.delete_cf_raw(CF_METADATA, Self::GOV_REBUILD_MARKER);
    }
    ...
    /// Live on-disk bytes across the GC-EXEMPT mandate CFs (B5). `total_live_bytes`
    /// deliberately lists none of these — they are preserved across retention GC —
    /// so the size governor must subtract this mass from `disk_cap_bytes` before
    /// deciding record-retention pressure, or it would try to reclaim bytes it can
    /// never compress and livelock record retention at the 1h floor (MINOR-1). The
    /// act sub-mass is separately bounded by `acts_budget_bytes` via the evictor.
    /// O(CFs) — a few O(1) RocksDB property reads, no I/O.
    pub fn exempt_live_bytes(&self) -> u64 {
        const EXEMPT_CFS: &[&str] = &[
            CF_MANDATE,
            CF_REVOCATION,
            CF_MANDATE_ACT,
            CF_MANDATE_ACTS_BY_MANDATE,
        ...
        let mut total: u64 = 0;
        for cf in EXEMPT_CFS {
            total = total.saturating_add(self.cf_live_bytes(cf));
        }
        total
    }

    /// Live on-disk bytes across just the ACT CFs (forward + 3 indexes) — the
    /// mass the budget evictor governs (excludes the mandate/revocation registry,
    /// which is never evicted). O(CFs), O(1) each.
    pub fn act_cf_live_bytes(&self) -> u64 {
        const ACT_CFS: &[&str] = &[
            CF_MANDATE_ACT,
            CF_MANDATE_ACTS_BY_MANDATE,
            CF_MANDATE_ACTS_BY_AGENT,
    ...
    /// Range scan from `start_key` forward with `total_order_seek=true` — the
    /// early-stop twin of `full_scan_cf` for CFs that carry a
    /// `prefix_extractor` (a raw `From` seek there follows prefix-same-as-start
    /// logic and can return nothing for out-of-domain seek keys, e.g. an empty
    /// key or a bare 8-byte timestamp against a 41-byte extractor). Callback
    /// returns `Ok(true)` to continue, `Ok(false)` to stop.
    pub fn range_scan_cf_total_order<F>(&self, cf_name: &str, start_key: &[u8], mut f: F) -> Result<()>
    where
        F: FnMut(&[u8], &[u8]) -> Result<bool>,
    {
        let cf = self.try_cf(cf_name)?;
        ...
        for item in iter {
            let (key, value) = item
                .map_err(|e| ElaraError::Storage(format!("range_scan_total({cf_name}): {e}")))?;
            if !f(&key, &value)? {
                break;
            }
        }
    ...
    /// Atomically delete `keys_a` from `cf_a` AND `keys_b` from `cf_b` in one
    /// WriteBatch. For primary+index lockstep deletes (e.g. attestation +
    /// its time-index entry) where a torn two-loop delete would orphan one
    /// side. Deleting a missing key is a RocksDB no-op, so callers may pass
    /// reconstructed keys without existence checks.
    pub fn delete_pair_batch_cf(
        &self,
        cf_a: &str,
        keys_a: &[Vec<u8>],
        cf_b: &str,
        ...
        let ha = self.try_cf(cf_a)?;
        let hb = self.try_cf(cf_b)?;
        let mut batch = WriteBatch::default();
        for k in keys_a {
            batch.delete_cf(&ha, k);
        }
        for k in keys_b {
            batch.delete_cf(&hb, k);
        }
        self.db
            .write(batch)
            .map_err(|e| ElaraError::Storage(format!("delete_pair_batch({cf_a},{cf_b}): {e}")))
    }
    ...
    pub fn delete_touched_cfs() -> &'static [&'static str] {
        &[
            ...
            CF_ATTESTATIONS,
            CF_DAG,
            // B5: the budget evictor (and AdminEvict) tombstone the time index;
            // compact it in the same post-GC pass so budget-evict churn at scale
            // doesn't leave stale SSTs. The forward + two reverse act CFs are
            // touched only by the rare AdminEvict and the bounded evictor trickle,
            // so periodic/boot compaction still covers them (unchanged from today).
            CF_MANDATE_ACTS_BY_TIME,
        ]
    }
        ...
        let actual_count = {
            ...
            for (key, value) in iter.flatten() {
                if !key.starts_with(b"finalized:")
                    ...
                    && &*key != b"__record_count__"
                    && &*key != b"__full_pull_cursor__"
                    // ⚠ WIRE_VERSION_MIN COUPLING (T63 flag-day verdict RR4):
                    // from_bytes() enforces the MIN..=WIRE_VERSION range, so a
                    // MIN raise silently DROPS sub-MIN records from this count
                    // — no error surfaces, the number just moves. Never use
                    // this count as a before/after invariant across a MIN
                    // change; the offline version histogram is the tool there.
    ...
    /// Wire-version histogram over CF_RECORDS raw stored bytes — the offline
    /// precondition tool for any `WIRE_VERSION_MIN` raise (T63 flag-day verdict
    /// Q5/A2: "no stored byte sequence below the chosen MIN", proven by scan,
    /// never believed).
    ///
    /// CRUCIAL: this peeks the RAW header bytes (magic at [0..4], version u16
    /// BE at [4..6]) and NEVER calls `from_bytes()` — the decoder enforces the
    /// current MIN range, so a decode-based scan would silently skip exactly
    /// the sub-MIN records this tool exists to count (the RR4 trap, one
    /// function up). Control keys are skipped with the same prefix set as the
    /// record count above; rows shorter than 6 bytes or with a wrong magic are
    /// counted under `malformed` — a non-zero malformed count is itself a
    /// finding, never silently dropped.
    ///
    /// ADMIN/RECOVERY CLASS: full CF scan, operator-invoked only (via
    /// `elara-node --verify-data` on a STOPPED data dir or a checkpoint copy)
    /// — never wired into any runtime path (SCALE RULE: recovery-only full
    /// scans, like the rebuild).
    ///
    /// Returns (version → count, malformed keys — first 8, lossy-printed, so a
    /// non-zero malformed count NAMES its rows instead of sending the operator
    /// back in with a debugger).
    pub fn wire_version_histogram(
        &self,
    ) -> Result<(std::collections::BTreeMap<u16, u64>, Vec<String>)> {
        let cf = self.try_cf(CF_RECORDS)?;
        let mut hist: std::collections::BTreeMap<u16, u64> = std::collections::BTreeMap::new();
        ...
        for (key, value) in iter.flatten() {
            if key.starts_with(b"finalized:")
                ...
                || &*key == b"__record_count__"
                || &*key == b"__full_pull_cursor__"
                // storage-schema marker (db-vN, u32 LE) — lives in CF_RECORDS;
                // named by the census's own first live run (2026-08-18: the
                // single "malformed" row on the real chain was exactly this)
                || &*key == b"__db_version__"
            {
                continue;
            }
            if value.len() < 6 || &value[0..4] != b"ELRA" {
                if malformed.len() < 8 {
                    malformed.push(format!(
                        "{} ({} bytes, head {:02x?})",
                        String::from_utf8_lossy(&key.iter().copied().take(48).collect::<Vec<u8>>()),
                        value.len(),
                        &value[..value.len().min(8)]
                    ));
                } else if malformed.len() == 8 {
                    malformed.push("… (further malformed rows elided)".into());
                }
                continue;
            }
            let version = u16::from_be_bytes([value[4], value[5]]);
            *hist.entry(version).or_insert(0) += 1;
        }
        Ok((hist, malformed))
    }
...
impl super::Storage for StorageEngine {
    ...
    /// KR-3 S2: delegate the trait lookup to the inherent `CF_METADATA` point
    /// read (fully-qualified to make the non-recursion explicit — inherent
    /// methods win resolution, but the path spelling removes all doubt).
    fn get_rotation_newkey_index(&self, key_hash: &str) -> Option<(String, u32)> {
        StorageEngine::get_rotation_newkey_index(self, key_hash)
    }
    ...
    fn delete(&mut self, record_id: &str) -> Result<()> {
        ...
        // GcPrune: the sole caller is the GC compaction path (gc.rs), which must
        // preserve the mandate-act index (B5) — the body ages out, the receipt
        // verdict does not.
        self.delete_record(record_id, DeleteIntent::GcPrune)
    }
...
impl StorageEngine {
    ...
    pub fn gc_scan_and_delete(
        ...
        // GcPrune: this is retention GC, so the mandate-act index is preserved
        // (B5) — pruned act bodies still answer an authoritative /mandate/status.
        for id in &deleted_ids {
            if let Err(e) = self.delete_record(id, DeleteIntent::GcPrune) {
    ...
    pub fn rebuild_ledger_streaming(
        ...
        for record in &gov_records {
            // F2 R2 (degenerate-B): a tombstoned governance op is EXCLUDED from the
            // tally — a moderated Vote/Propose shouldn't count. Governance has no live
            // incremental apply, so skip-in-fold is the single authoritative, order-
            // independent choice. (Ledger ops above apply UNCONDITIONALLY — R1/semantic
            // A — so this skip is governance-only.) See TOMBSTONE-LEDGER-REDERIVE brief.
            if self
                .get_cf_raw(CF_METADATA, format!("tombstone:{}", record.id).as_bytes())
                .ok()
                .flatten()
                .is_some()
            {
    ...
    pub fn incremental_ledger_replay(
        ...
        for record in &gov_records {
            // F2 R2 (degenerate-B): skip a tombstoned governance op — see the same
            // note in rebuild_ledger_streaming. Belt-and-braces with the mark_tombstoned
            // checkpoint invalidation (which routes the pre-checkpoint case to a full
            // rebuild); this covers a post-checkpoint (ts > since_ts) gov op tombstoned
            // before this replay runs. Ledger ops above apply unconditionally (R1/A).
            if self
                .get_cf_raw(CF_METADATA, format!("tombstone:{}", record.id).as_bytes())
                .ok()
                .flatten()
    ...
    pub fn run_migrations(&self, data_dir: &Path) -> Result<()> {
        ...
        // "db-v" prefix, NOT bare "v" (T63 flag-day verdict R8): the storage
        // schema version and the record WIRE version are unrelated number
        // lines that both live around 5-7; during a wire-version flag day an
        // operator grepping logs for "v5"/"v6" must never land here first.
        tracing::info!(
            "Database schema migration: db-v{current} → db-v{CURRENT_DB_VERSION} ({} pending; unrelated to record wire versions)",
        ...
        for v in current..CURRENT_DB_VERSION {
            ...
            match v {
                ...
                9 => self.migrate_9_to_10()?,
                other => {
                    return Err(ElaraError::Storage(format!(
                        "unknown schema migration db-v{other} → db-v{}",
                        other + 1
                    )));
                }
            }
    ...
    /// Migration 9→10 (B5): backfill `CF_MANDATE_ACTS_BY_TIME` (ts → act) from the
    /// EXISTING forward act index `CF_MANDATE_ACT`, so the budget evictor's
    /// oldest-first ordering covers acts ingested before the time index existed.
    /// Bounded source (O(mandate_acts), not O(all_records) — SCALE-RULE-safe
    /// boot-only path), idempotent write-if-missing, crash-safe (the version is set
    /// LAST by the caller, so a crash mid-run re-runs from the start). Mirrors
    /// `migrate_8_to_9`; the time key never fails to build (every act has a ts), so
    /// there is no malformed-skip class here beyond an unparseable stored row.
    fn migrate_9_to_10(&self) -> Result<()> {
        use rocksdb::IteratorMode;

        tracing::info!(
            "Migration 9→10: backfilling CF_MANDATE_ACTS_BY_TIME (ts → acts) from CF_MANDATE_ACT"
        );

        let cf_act = self.try_cf(CF_MANDATE_ACT)?;
        let cf_by_time = self.try_cf(CF_MANDATE_ACTS_BY_TIME)?;
        ...
        for item in iter {
            let (key, value) =
                item.map_err(|e| ElaraError::Storage(format!("migration 9→10 iter: {e}")))?;
            let record_id = match std::str::from_utf8(&key) {
                Ok(s) => s,
                Err(_) => {
                    skipped += 1;
            ...
            let tkey = Self::mandate_acts_time_key(entry.act_timestamp_ms, record_id);
            // Idempotent: skip if already indexed (covers re-entry after partial run).
            if self.db.get_cf(&cf_by_time, &tkey).ok().flatten().is_some() {
                already_present += 1;
                continue;
            }
            batch.put_cf(&cf_by_time, &tkey, b"");
            indexed += 1;
            batch_count += 1;
            if batch_count >= BATCH_FLUSH_SIZE {
                self.db
                    .write(batch)
                    .map_err(|e| ElaraError::Storage(format!("migration 9→10 batch: {e}")))?;
                batch = WriteBatch::default();
                batch_count = 0;
            }
        }
        if batch_count > 0 {
            self.db
                .write(batch)
                .map_err(|e| ElaraError::Storage(format!("migration 9→10 final: {e}")))?;
        }

        tracing::info!(
            "Migration 9→10 complete: indexed {indexed} acts into CF_MANDATE_ACTS_BY_TIME, \
             {already_present} already present, skipped {skipped} unparseable rows"
        );
        Ok(())
    }
}
...
mod tests {
    ...
    /// T63 Q5/A2 — the wire-version census counts what `from_bytes` can no
    /// longer see. Stores real v5 records via the normal path, then hand-plants
    /// RAW rows a decoder would reject: a v1-headered blob (below MIN — the
    /// exact class the MIN 4→5 precondition must count), a v9 (above ceiling),
    /// a wrong-magic blob and a 3-byte stub (malformed). The histogram must
    /// report ALL of them; control keys must stay excluded.
    #[test]
    fn t63_wire_version_census_counts_sub_min_raw_rows() {
        let (engine, _dir) = test_engine();
        // two real records through the normal store path (stamp v5)
        for i in 0..2 {
            let rec = ValidationRecord::create(
                format!("census {i}").as_bytes(),
                vec![0xAA; 32],
                vec![],
                Classification::Public,
                None,
            );
            engine.put_record(&rec.id.clone(), &rec).unwrap();
        }
        // hand-planted raw rows, invisible to any from_bytes-based scan:
        let mut v1 = b"ELRA".to_vec();
        v1.extend_from_slice(&1u16.to_be_bytes());
        v1.extend_from_slice(&[0x01, 0x00, 0xde, 0xad]);
        engine.put_cf_raw(CF_RECORDS, b"census-v1-row", &v1).unwrap();
        let mut v9 = b"ELRA".to_vec();
        v9.extend_from_slice(&9u16.to_be_bytes());
        v9.extend_from_slice(&[0x01, 0x00]);
        engine.put_cf_raw(CF_RECORDS, b"census-v9-row", &v9).unwrap();
        engine.put_cf_raw(CF_RECORDS, b"census-bad-magic", b"NOPE\x00\x05xx").unwrap();
        engine.put_cf_raw(CF_RECORDS, b"census-stub", b"EL").unwrap();
        // control keys must not be counted
        engine.put_cf_raw(CF_RECORDS, b"__full_pull_cursor__", b"cursor").unwrap();
        engine.put_cf_raw(CF_RECORDS, b"tombstone:x", b"t").unwrap();
        // the db-vN storage marker (named by the census's first live run)
        engine.put_cf_raw(CF_RECORDS, b"__db_version__", &10u32.to_le_bytes()).unwrap();
        ...
        let below_min: u64 = hist.range(..crate::wire::WIRE_VERSION_MIN).map(|(_, n)| n).sum();
        assert_eq!(below_min, 1, "the below-MIN summary the raise decision reads");
    }
    ...
    // Pins the durable mechanism that survives the unconditional ledger re-save
    // (which defeats delete_snapshot("ledger") alone). The fail-closed-on-read-error
    // polarity is documented on `gov_rebuild_required` and not unit-testable without
    // injecting a rocksdb error; this pins the absent/set/cleared states.
    #[test]
    fn f2_psr1_gov_rebuild_marker_roundtrip() {
        let (engine, _dir) = test_engine();
        // fresh db: no marker → false (checkpoint fast-path allowed)
        assert!(!engine.gov_rebuild_required(), "fresh db → false (no forced rebuild)");
        // set → true (force full rebuild next boot)
        engine.set_gov_rebuild_required().unwrap();
        assert!(engine.gov_rebuild_required(), "after set → true (force full rebuild)");
        // clear → false again
        engine.clear_gov_rebuild_required();
        assert!(!engine.gov_rebuild_required(), "after clear → false");
        // clear is idempotent (deleting a missing key is Ok)
        engine.clear_gov_rebuild_required();
        assert!(!engine.gov_rebuild_required(), "double clear stays false");
    }

    // KR-3 v4 slice-2 durable-fact layer (UNWIRED). Realistic 64-char hex
    // ids/hashes — the production key shape.
    #[test]
    fn kr3_v4_origin_key_fact_roundtrip_and_absence() {
        let (engine, _dir) = test_engine();
        let id = "a".repeat(64);
        let rh = "b".repeat(64);

        // Fail-closed before any write.
        assert!(!engine.has_origin_key(&id));
        assert!(engine.get_origin_key(&id).is_none());

        // Stage + commit via the batch primitive (the wiring path).
        let mut batch = rocksdb::WriteBatch::default();
        engine
            .put_origin_key_to_batch(&mut batch, &id, &rh, "5:default")
            .unwrap();
        engine.db.write(batch).unwrap();

        // Presence + audit round-trip exactly; a different identity stays absent.
        assert!(engine.has_origin_key(&id));
        assert_eq!(
            engine.get_origin_key(&id),
            Some((rh.clone(), "5:default".into()))
        );
        assert!(!engine.has_origin_key(&"c".repeat(64)));

        // Empty coord is a valid audit value (the length-framed split still parses).
        let id2 = "d".repeat(64);
        let mut b2 = rocksdb::WriteBatch::default();
        engine.put_origin_key_to_batch(&mut b2, &id2, &rh, "").unwrap();
        engine.db.write(b2).unwrap();
        assert_eq!(engine.get_origin_key(&id2), Some((rh.clone(), String::new())));

        // Idempotent monotonic re-write (brief §3.1): overwrite is benign.
        let mut b3 = rocksdb::WriteBatch::default();
        engine
            .put_origin_key_to_batch(&mut b3, &id, &rh, "9:default")
            .unwrap();
        engine.db.write(b3).unwrap();
        assert_eq!(engine.get_origin_key(&id), Some((rh, "9:default".into())));
    }

    /// Hardening pin (KR-3 v4 slice-2): the origin-fact value framing must round-trip
    /// for ANY byte content of either field and must never let two distinct
    /// `(record_hash, coord)` inputs encode to the same bytes. The prior
    /// `record_hash ‖ 0x00 ‖ coord` framing (split on the FIRST NUL) aliased
    /// `("A\0B","C")` and `("A","B\0C")` to identical bytes and silently mis-split a
    /// `record_hash` carrying an interior NUL — the ≈11.6% failure a future wiring
    /// that passed a raw 32-byte digest instead of its hex would have hit. Length-
    /// framing the first field makes the split unambiguous; this test would fail on
    /// the old encoder.
    #[test]
    fn kr3_v4_origin_fact_framing_is_collision_proof() {
        // The exact adversarial pair that aliased under first-NUL splitting.
        let a = StorageEngine::encode_origin_fact("A\0B", "C");
        let b = StorageEngine::encode_origin_fact("A", "B\0C");
        assert_ne!(a, b, "distinct (record_hash, coord) inputs must not encode alike");
        assert_eq!(
            StorageEngine::decode_origin_fact(&a),
            Some(("A\0B".to_string(), "C".to_string())),
            "an interior-NUL record_hash must decode to itself, not a mis-split"
        );
        assert_eq!(
            StorageEngine::decode_origin_fact(&b),
            Some(("A".to_string(), "B\0C".to_string())),
            "a coord carrying a NUL must survive the round-trip verbatim"
        );

        // A raw 32-byte digest with an interior NUL (the wiring footgun) round-trips.
        let raw = String::from_utf8_lossy(&[0x00u8; 32]).to_string();
        let enc = StorageEngine::encode_origin_fact(&raw, "5:default");
        assert_eq!(
            StorageEngine::decode_origin_fact(&enc),
            Some((raw, "5:default".to_string()))
        );

        // Malformed/short stored bytes fail closed to None, never panic.
        assert_eq!(StorageEngine::decode_origin_fact(&[]), None);
        assert_eq!(StorageEngine::decode_origin_fact(&[0, 0, 0]), None); // <4-byte length header
        assert_eq!(StorageEngine::decode_origin_fact(&[0, 0, 0, 9, b'x']), None); // len past end
    }

    #[test]
    fn kr3_v4_rotation_by_hash_roundtrip_and_absence() {
        let (engine, _dir) = test_engine();
        let rh_a = "a".repeat(64);
        let rh_b = "b".repeat(64);
        let lineage = "e".repeat(64);

        assert!(engine.get_rotation_by_hash(&rh_a).is_none());

        let mut batch = rocksdb::WriteBatch::default();
        engine
            .put_rotation_by_hash_to_batch(&mut batch, &rh_a, &lineage, 3)
            .unwrap();
        engine
            .put_rotation_by_hash_to_batch(&mut batch, &rh_b, &lineage, 4)
            .unwrap();
        engine.db.write(batch).unwrap();

        // Content-addressed: distinct record_hashes resolve to distinct
        // positions, never LWW-colliding (the B2 backward-collision the newkey
        // index has).
        assert_eq!(engine.get_rotation_by_hash(&rh_a), Some((lineage.clone(), 3)));
        assert_eq!(engine.get_rotation_by_hash(&rh_b), Some((lineage.clone(), 4)));
        assert!(engine.get_rotation_by_hash(&"f".repeat(64)).is_none());
    }

    #[test]
    fn kr3_v4_durable_facts_survive_record_gc() {
        // §3.4 B1 close: the GC primitive `delete_record` (gc.rs calls it per
        // record) must NEVER remove an identity/record_hash-keyed CF_METADATA
        // fact. Write both facts, drive delete_record over their carrier record
        // ids, assert both survive.
        let (engine, _dir) = test_engine();
        let id = "a".repeat(64);
        let rh = "b".repeat(64);
        let lineage = "c".repeat(64);
        ...
        engine
            ...
            .unwrap();
        engine
            .put_rotation_by_hash_to_batch(&mut batch, &rh, &lineage, 0)
            .unwrap();
        engine.db.write(batch).unwrap();

        // GC the carrier records (ids need not exist — delete_record runs its
        // full record_id-keyed deletion path regardless). The last delete uses
        // an id equal to the fact's hash-portion: proves the `origin_key:` /
        // `rotation_by_hash:` PREFIX is what protects the row, not id disjointness.
        engine.delete_record("origin-reg-carrier", DeleteIntent::GcPrune).unwrap();
        engine.delete_record("rotation-hop-carrier", DeleteIntent::GcPrune).unwrap();
        engine.delete_record(&rh, DeleteIntent::GcPrune).unwrap();

        // Facts outlive their carriers.
        assert!(engine.has_origin_key(&id));
        assert_eq!(
            engine.get_origin_key(&id),
            Some((rh.clone(), "5:default".into()))
        );
        assert_eq!(engine.get_rotation_by_hash(&rh), Some((lineage, 0)));
    }

    #[test]
    fn rotation_cf_put_get_roundtrip_and_ordering() {
        use crate::network::rotation_finality::{
            rotation_chain_root, FinalityCoord, FinalityEvidence, RotationChainEntry, RotationKind,
            RotationState,
        };
        use crate::network::time_bracket::DrandPulse;
        let (engine, _dir) = test_engine();

        let mk = |lineage: &str, hop: u32| RotationChainEntry {
            lineage_id: lineage.into(),
            hop_index: hop,
            record_id: format!("rec-{lineage}-{hop}"),
            record_hash: "rh".into(),
        ...
        // Realistic 64-char lineage ids (sha3_256_hex width) — the production
        // key shape; also guards against re-introducing a length-coupled prefix
        // extractor (a short id would then silently break the scan).
        let la = "a".repeat(64);
        let lb = "b".repeat(64);

        // Missing → None / empty before any write (fail-closed reads).
        assert!(engine.get_rotation_entry(&la, 0).is_none());
        assert!(engine.get_rotation_chain(&la).is_empty());

        // Two lineages, hops inserted OUT of order → proves ordered read.
        engine.put_rotation_entry(&mk(&la, 2)).unwrap();
        engine.put_rotation_entry(&mk(&la, 0)).unwrap();
        engine.put_rotation_entry(&mk(&la, 1)).unwrap();
        engine.put_rotation_entry(&mk(&lb, 0)).unwrap();

        // Point lookup round-trips exactly.
        assert_eq!(engine.get_rotation_entry(&la, 1), Some(mk(&la, 1)));
        assert!(engine.get_rotation_entry(&la, 9).is_none());

        // Chain read is ascending hop order, lineage-scoped (no bleed from lb).
        let chain = engine.get_rotation_chain(&la);
        assert_eq!(
            chain.iter().map(|e| e.hop_index).collect::<Vec<_>>(),
            vec![0, 1, 2]
        );
        assert_eq!(engine.get_rotation_chain(&lb).len(), 1);

        // Idempotent re-write (the W1/W2 convergence property) — still 3 hops.
        engine.put_rotation_entry(&mk(&la, 1)).unwrap();
        assert_eq!(engine.get_rotation_chain(&la).len(), 3);

        // The chain-root primitive agrees on a freshly re-read chain.
        assert_eq!(
            rotation_chain_root(&chain),
            rotation_chain_root(&engine.get_rotation_chain(&la))
        );
    }

    #[test]
    fn rotation_newkey_index_resolves_predecessor_positions() {
        ...
        // Absent before any write (fail-closed).
        assert!(engine.get_rotation_newkey_index("deadbeef").is_none());

        // Finalize a lineage pk0→pk1→pk2→pk3 exactly as W1 will: derive each
        // hop's position via the DURABLE index (populated by prior puts), then
        // persist. `put_rotation_entry` writes the index forward atomically.
        let keys: Vec<Vec<u8>> = (0..=3).map(|i| format!("pk{i}").into_bytes()).collect();
        // Root lineage_id carries the RotationKind tag ("d:" for Dilithium).
        let lineage = format!("d:{}", crate::crypto::hash::sha3_256_hex(&keys[0]));
        for i in 0..(keys.len() - 1) {
            let rec = rot(&keys[i], &keys[i + 1]);
            let entry = build_rotation_entry(
                ...
                |k| engine.get_rotation_newkey_index(k), // the durable predecessor index
            )
            .expect("hop builds");
            assert_eq!(entry.lineage_id, lineage, "hop {i} shares lineage");
            assert_eq!(entry.hop_index, i as u32, "hop {i} index");
            engine.put_rotation_entry(&entry).unwrap();
        }

        // The index now resolves each INTRODUCED key to its introducing hop.
        let h = |k: &[u8]| crate::crypto::hash::sha3_256_hex(k);
        assert_eq!(
            engine.get_rotation_newkey_index(&h(b"pk1")),
            Some((lineage.clone(), 0))
        );
        assert_eq!(
            engine.get_rotation_newkey_index(&h(b"pk2")),
            Some((lineage.clone(), 1))
        );
        assert_eq!(
            engine.get_rotation_newkey_index(&h(b"pk3")),
            Some((lineage.clone(), 2))
        );
        // pk0 is the root — never *introduced* by a hop → no index row.
        assert!(engine.get_rotation_newkey_index(&h(b"pk0")).is_none());

        // Malformed index bytes decode to None (defensive parse, never panic).
        assert!(StorageEngine::decode_lineage_pos(b"").is_none());
        assert!(StorageEngine::decode_lineage_pos(b"ab").is_none());
        assert!(StorageEngine::decode_lineage_pos(b"abcdefg").is_none());
    }

    #[test]
    fn rotation_seal_markers_batch_scan_ordered_and_delete() {
        let (engine, _dir) = test_engine();

        // No markers before any write.
        assert!(engine.scan_rotation_seal_markers().is_empty());

        // Arm markers across two zones, epochs inserted OUT of order, via the
        // crash-atomic batched write site (the real W2 marker path).
        let mut batch = rocksdb::WriteBatch::default();
        engine.put_rotation_seal_marker_to_batch(&mut batch, "0", 30).unwrap();
        engine.put_rotation_seal_marker_to_batch(&mut batch, "0", 5).unwrap();
        engine.put_rotation_seal_marker_to_batch(&mut batch, "0", 12).unwrap();
        engine.put_rotation_seal_marker_to_batch(&mut batch, "1", 7).unwrap();
        engine.db.write(batch).unwrap();

        // Scan yields ascending epoch WITHIN each zone (§3-3 sweep order). The
        // 5-before-12 pair proves big-endian epoch encoding — a decimal-string
        // key would sort "12" before "5" and break the forward build.
        assert_eq!(
            engine.scan_rotation_seal_markers(),
            vec![
                ("0".to_string(), 5),
                ("0".to_string(), 12),
                ("0".to_string(), 30),
        ...
        // Deleting one slot removes exactly it; delete is idempotent.
        engine.delete_rotation_seal_marker("0", 12).unwrap();
        engine.delete_rotation_seal_marker("0", 12).unwrap(); // no-op, no error
        assert_eq!(
            engine.scan_rotation_seal_markers(),
            vec![
                ("0".to_string(), 5),
        ...
        engine.delete_rotation_seal_marker("9", 999).unwrap();
        assert_eq!(engine.scan_rotation_seal_markers().len(), 3);
    }

    // ── W2-A rotation-finality side-writes (marker-arm + pending-hash index) ──

    fn w2a_rotation_record(old: &[u8], new: &[u8]) -> crate::record::ValidationRecord {
    ...
    fn w2a_pulse() -> crate::network::time_bracket::DrandPulse {
        crate::network::time_bracket::DrandPulse {
            round: 7,
            randomness: "beef".into(),
            genesis_unix: 1,
            period_secs: 30,
    ...
    /// The pending-CF index: a `RotationHopAdmit` side-write (batched with the
    /// rotation record's store) makes the hash pending; `put_rotation_entry`
    /// clears it in the entry's own batch once the CF entry lands. The emptiness
    /// short-circuit flips accordingly. The same side-write also lands the
    /// permanent routing pin, which `put_rotation_entry` does NOT clear (asserted
    /// below) — the pin outlives the transient pending-hash.
    #[test]
    fn rotation_pending_hash_index_arm_then_clear_lifecycle() {
        use crate::network::rotation_finality::{build_rotation_entry, FinalityEvidence};
        let (engine, _dir) = test_engine();

        let (old, new) = (b"pk0".as_slice(), b"pk1".as_slice());
        let rot = w2a_rotation_record(old, new);
        let hash_hex = hex::encode(rot.record_hash());
        let lineage = crate::accounting::types::creator_identity_hash(&rot);

        // Pre-state: nothing pending, short-circuit reads false, no pin.
        assert!(!engine.has_any_rotation_pending());
        assert!(!engine.rotation_hash_pending(&hash_hex));
        assert!(engine.get_rotation_zone_pin(&rot.id).is_none());

        // Store the rotation record WITH its hop-admit side-writes (atomic):
        // both the pending-hash row AND the routing pin land in the record batch.
        engine
            ...
            .unwrap();
        assert!(engine.get_record(&rot.id).unwrap().is_some(), "record persisted");
        assert!(engine.has_any_rotation_pending(), "hash now pending");
        assert!(engine.rotation_hash_pending(&hash_hex));
        assert!(
            !engine.rotation_hash_pending(&hex::encode([0xabu8; 32])),
            "an unrelated hash is not pending"
        );
        assert_eq!(
            engine.get_rotation_zone_pin(&rot.id).as_deref(),
            Some(lineage.as_str()),
            "routing pin landed in the same admission batch"
        );

        // The CF entry lands → put_rotation_entry clears the pending-hash row in
        // the same batch. entry.record_hash == hash_hex (rotation_hop_fields).
        let entry = build_rotation_entry(
            &rot,
            Some("cA".into()),
            100,
        ...
        .expect("rotation hop builds");
        assert_eq!(entry.record_hash, hash_hex);
        engine.put_rotation_entry(&entry).unwrap();

        assert!(!engine.rotation_hash_pending(&hash_hex), "cleared once CF entry landed");
        assert!(!engine.has_any_rotation_pending(), "short-circuit empty again");
        // The entry itself is durable.
        assert!(engine.get_rotation_entry(&entry.lineage_id, 0).is_some());
        // The routing pin is PERMANENT — put_rotation_entry clears only the
        // transient pending-hash, never the pin. The hop must keep routing via its
        // lineage for the rest of its life (a chain later than its own finality).
        assert_eq!(
            engine.get_rotation_zone_pin(&rot.id).as_deref(),
            Some(lineage.as_str()),
            "pin survives finalization"
        );
    }

    /// A seal's `ArmSealMarker` side-write rides the SAME batch as the seal
    /// record's store (crash-atomic §3-3): both the record and the `(zone,epoch)`
    /// marker are durable after one `put_record_with_pk_zone`.
    #[test]
    fn arm_seal_marker_rides_seal_store_batch() {
        let (engine, _dir) = test_engine();
        let seal = test_record("seal-rec");
        ...
        assert!(engine.get_record(&seal.id).unwrap().is_some(), "seal record persisted");
        assert_eq!(
            engine.scan_rotation_seal_markers(),
            vec![("medical/eu".to_string(), 9)],
            "marker armed atomically with the seal store"
        );
    }

    /// `rotation_op = None` (flag-OFF path) writes ZERO side rows — byte-identical
    /// to a pre-feature store: no marker, no pending-hash.
    #[test]
    fn rotation_op_none_writes_no_side_rows() {
        let (engine, _dir) = test_engine();
        ...
        engine
            ...
            .unwrap();
        assert!(engine.get_record(&rec.id).unwrap().is_some());
        assert!(engine.scan_rotation_seal_markers().is_empty());
        assert!(!engine.has_any_rotation_pending());
    }

    #[test]
    fn rotation_seal_marker_scan_skips_malformed_and_stops_at_boundary() {
        let (engine, _dir) = test_engine();
        let cf = engine.try_cf(CF_METADATA).unwrap();

        let mut batch = rocksdb::WriteBatch::default();
        // One well-formed marker.
        engine.put_rotation_seal_marker_to_batch(&mut batch, "0", 42).unwrap();
        // Prefix-sharing but too short to carry zone‖0x00‖epoch(8) → skipped.
        batch.put_cf(&cf, b"rotation_seal_pending:short", b"");
        // Just past the prefix range (':' 0x3A < 'Z' 0x5A) → scan must break,
        // never reading foreign CF_METADATA rows.
        batch.put_cf(&cf, b"rotation_seal_pendingZZZ", b"x");
        engine.db.write(batch).unwrap();
    ...
    #[test]
    fn rotation_zone_pin_roundtrip_and_scan() {
        // §6.1 c1: pin write-once/read-everywhere. A pin written in a batch is
        // point-readable by record id, an absent id resolves to None, and the
        // boot-load scan returns every pin as (record_id, lineage_id).
        let (engine, _dir) = test_engine();
        let lin_a = "aa".repeat(32); // 64-hex, sha3(pk₀) shape
        let lin_b = "bb".repeat(32);

        let mut batch = rocksdb::WriteBatch::default();
        engine
            .put_rotation_zone_pin_to_batch(&mut batch, "rec-alpha", &lin_a)
            .unwrap();
        engine
            .put_rotation_zone_pin_to_batch(&mut batch, "rec-beta", &lin_b)
            .unwrap();
        engine.db.write(batch).unwrap();

        assert_eq!(engine.get_rotation_zone_pin("rec-alpha").as_deref(), Some(lin_a.as_str()));
        assert_eq!(engine.get_rotation_zone_pin("rec-beta").as_deref(), Some(lin_b.as_str()));
        assert_eq!(engine.get_rotation_zone_pin("rec-absent"), None);

        let mut pins = engine.scan_rotation_zone_pins();
        pins.sort();
        assert_eq!(
            pins,
            vec![
                ("rec-alpha".to_string(), lin_a.clone()),
                ("rec-beta".to_string(), lin_b),
            ]
        );

        // Test-only stand-alone pin path (used by the resolver tests) round-trips.
        engine.pin_rotation_zone_for_test("rec-gamma", &lin_a).unwrap();
        assert_eq!(engine.get_rotation_zone_pin("rec-gamma").as_deref(), Some(lin_a.as_str()));
    }

    #[test]
    fn rotation_zone_pin_scan_stops_at_boundary() {
        // The `From`-seek + `starts_with` guard must never read a foreign
        // CF_METADATA row that merely shares the `rotation_` stem. ':' (0x3A)
        // sorts before 'Z' (0x5A), so a `rotation_zone_pinZZZ` row lands just
        // past the pin namespace and the scan must break, not absorb it.
        let (engine, _dir) = test_engine();
        let cf = engine.try_cf(CF_METADATA).unwrap();

        let lin = "cc".repeat(32);
        let mut batch = rocksdb::WriteBatch::default();
        engine
            .put_rotation_zone_pin_to_batch(&mut batch, "rec-in-namespace", &lin)
            .unwrap();
        // Empty-id row inside the namespace → skipped (malformed).
        batch.put_cf(&cf, b"rotation_zone_pin:", b"x");
        // Just past the prefix range → scan must break.
        batch.put_cf(&cf, b"rotation_zone_pinZZZ", b"foreign");
        engine.db.write(batch).unwrap();

        assert_eq!(
            engine.scan_rotation_zone_pins(),
            vec![("rec-in-namespace".to_string(), lin)]
        );
    }
    ...
    fn mandate_storage_roundtrip_authz_and_act_cleanup() {
        ...
        // act index put/get + AdminEvict delete_record cleanup (B5: GcPrune
        // PRESERVES the entry; only an operator evict removes it — see the
        // dedicated gc_prune_preserves_act_index_rows / admin_evict_* pins).
        ...
        engine.delete_record("act-rec-1", DeleteIntent::AdminEvict).unwrap();
        assert!(engine.get_mandate_act("act-rec-1").is_none()); // evicted entry gone
    ...
    fn mandate_acts_reverse_index_enumeration() {
        ...
        // (5) AdminEvict removes the reverse key (no orphan); double-delete is a no-op.
        engine.delete_record("rec-a-200", DeleteIntent::AdminEvict).unwrap();
        let (after_del, _) = engine.list_acts_for_mandate(&a, None, 10).unwrap();
        assert_eq!(after_del, vec!["rec-a-100", "rec-a-300"]);
        engine.delete_record("rec-a-200", DeleteIntent::AdminEvict).unwrap(); // idempotent, must not panic/re-orphan
    ...
    fn mandate_acts_by_agent_reverse_index_enumeration() {
        ...
        // (5) AdminEvict removes the agent reverse key (no orphan); idempotent.
        engine.delete_record("rec-200", DeleteIntent::AdminEvict).unwrap();
        let (after_del, _) = engine.list_acts_for_agent(&agent1, None, 10).unwrap();
        assert_eq!(after_del, vec!["rec-100", "rec-300"]);
        engine.delete_record("rec-200", DeleteIntent::AdminEvict).unwrap(); // double-delete must not panic/re-orphan
        let (after_del2, _) = engine.list_acts_for_agent(&agent1, None, 10).unwrap();
        assert_eq!(after_del2, vec!["rec-100", "rec-300"]);
    }

    #[test]
    fn gc_prune_preserves_act_index_rows() {
        // B5 (verdict 2026-07-18): retention GC of an act record's BODY must
        // PRESERVE every act-CF row (forward + both reverse) so the pruned act
        // still answers an authoritative /mandate/status verdict. Also pins the
        // GC regression-alarm counter at 0 (a non-zero reading means the cascade
        // was reintroduced under a preserving intent).
        use crate::mandate::MandateActEntry;
        use std::sync::atomic::Ordering;
        let (engine, _dir) = test_engine();
        let mand = "a".repeat(64);
        let agent = "cc".repeat(32);
        engine.put_record("gc-act-1", &test_record("gc-act-1")).unwrap();
        engine
            .put_mandate_act("gc-act-1", &MandateActEntry::new(&mand, &agent, 500, None))
            .unwrap();
        engine
            .delete_record("gc-act-1", DeleteIntent::GcPrune)
            .unwrap();
        // Body gone…
        assert!(engine.get_record("gc-act-1").unwrap().is_none());
        // …forward entry + both reverse indexes survive.
        assert_eq!(
            engine.get_mandate_act("gc-act-1").expect("forward preserved").mandate_ref,
            mand
        );
        assert_eq!(
            engine.list_acts_for_mandate(&mand, None, 10).unwrap().0,
            vec!["gc-act-1"]
        );
        assert_eq!(
        ...
        assert_eq!(
            crate::network::mandate_node::MANDATE_ACT_ENTRIES_DELETED_GC.load(Ordering::Relaxed),
            0,
            "B5 alarm: GcPrune must never delete an act-index entry"
        );
    }

    #[test]
    fn zone_purge_preserves_act_entries() {
        // B5: zone resharding purge preserves act entries too (a routing change,
        // not a takedown) — receipts survive a zone moving. ZONE_PURGE alarm = 0.
        use crate::mandate::MandateActEntry;
        use std::sync::atomic::Ordering;
        let (engine, _dir) = test_engine();
        let mand = "e".repeat(64);
        let agent = "ff".repeat(32);
        engine.put_record("zp-act-1", &test_record("zp-act-1")).unwrap();
        engine
            .put_mandate_act("zp-act-1", &MandateActEntry::new(&mand, &agent, 900, None))
            .unwrap();
        engine
            .delete_record("zp-act-1", DeleteIntent::ZonePurge)
            .unwrap();
        assert!(engine.get_record("zp-act-1").unwrap().is_none()); // body purged
        assert!(engine.get_mandate_act("zp-act-1").is_some()); // entry preserved
        assert_eq!(
            engine.list_acts_for_mandate(&mand, None, 10).unwrap().0,
            vec!["zp-act-1"]
        );
        assert_eq!(
            crate::network::mandate_node::MANDATE_ACT_ENTRIES_DELETED_ZONE_PURGE
                .load(Ordering::Relaxed),
            0,
            "B5 alarm: ZonePurge must never delete an act-index entry"
        );
    }

    #[test]
    fn admin_evict_full_cleanup_writes_removed_marker() {
        // B5: an operator forensic evict is the ONLY intent that removes the act
        // index — full cleanup of all three CF rows PLUS a rid-keyed
        // __act_removed__ marker so the absence reads as removed_by_operator.
        use crate::mandate::MandateActEntry;
        let (engine, _dir) = test_engine();
        let mand = "b".repeat(64);
        let agent = "dd".repeat(32);
        engine.put_record("ev-act-1", &test_record("ev-act-1")).unwrap();
        engine
            .put_mandate_act("ev-act-1", &MandateActEntry::new(&mand, &agent, 700, None))
            .unwrap();
        engine
            .delete_record("ev-act-1", DeleteIntent::AdminEvict)
            .unwrap();
        // Forward entry + both reverse indexes gone.
        assert!(engine.get_mandate_act("ev-act-1").is_none());
        assert!(engine.list_acts_for_mandate(&mand, None, 10).unwrap().0.is_empty());
        assert!(engine.list_acts_for_agent(&agent, None, 10).unwrap().0.is_empty());
        // Removed-marker present.
        let marker = format!("{ACT_REMOVED_MARKER_PREFIX}ev-act-1");
        assert!(
            engine.get_cf_raw(CF_METADATA, marker.as_bytes()).unwrap().is_some(),
            "AdminEvict must write the rid-keyed removed-marker"
        );
    }

    #[test]
    fn delete_record_evict_errs_on_entry_read_failure() {
        // MINOR-2: a corrupt/undecodable act entry must ABORT an AdminEvict
        // BEFORE the batch is written — a swallowed error would leak the reverse
        // keys and silently drop the entry. The checked getter surfaces Err and
        // the `?` in delete_record aborts before `db.write`.
        let (engine, _dir) = test_engine();
        engine.put_record("bad-act", &test_record("bad-act")).unwrap();
        // Garbage bytes → JSON decode fails (Err, not None) in the checked getter.
        engine.put_cf_raw(CF_MANDATE_ACT, b"bad-act", b"{not json").unwrap();
        assert!(engine.get_mandate_act_checked("bad-act").is_err());
        assert!(engine
            .delete_record("bad-act", DeleteIntent::AdminEvict)
            .is_err());
        // The aborted evict wrote nothing: forward act key + record body intact.
        assert!(
            engine.get_cf_raw(CF_MANDATE_ACT, b"bad-act").unwrap().is_some(),
            "aborted evict must not delete the forward act key"
        );
        assert!(engine.get_record("bad-act").unwrap().is_some());
    }

    #[test]
    fn put_mandate_act_four_key_atomicity() {
        // B5: put_mandate_act writes forward + by-mandate + by-agent + by-time in
        // one atomic batch — all four must be present after a single put.
        use crate::mandate::MandateActEntry;
        let (engine, _dir) = test_engine();
        let mand = "a".repeat(64);
        let agent = "cc".repeat(32);
        let ts = 12_345u64;
        engine.put_record("four-key", &test_record("four-key")).unwrap();
        engine
            .put_mandate_act("four-key", &MandateActEntry::new(&mand, &agent, ts, None))
            .unwrap();
        assert!(engine.get_mandate_act("four-key").is_some()); // (1) forward
        assert_eq!(
            engine.list_acts_for_mandate(&mand, None, 10).unwrap().0,
            vec!["four-key"]
        ); // (2) by-mandate
        ...
        let tk = StorageEngine::mandate_acts_time_key(ts, "four-key");
        assert!(
            engine.get_cf_raw(CF_MANDATE_ACTS_BY_TIME, &tk).unwrap().is_some(),
            "(4) by-time key must be present"
        );
    }

    #[test]
    fn time_index_backfill_migration() {
        // B5 migrate_9_to_10: backfill CF_MANDATE_ACTS_BY_TIME from the forward
        // act CF, bounded + idempotent. Simulate a pre-v10 DB by writing forward
        // entries directly (bypassing put_mandate_act's 4-key batch).
        use crate::mandate::MandateActEntry;
        let (engine, _dir) = test_engine();
        let mand = "b".repeat(64);
        let agent = "dd".repeat(32);
        let rows = [("bf-1", 100u64), ("bf-2", 300), ("bf-3", 200)];
        for (rid, ts) in rows {
            let entry = MandateActEntry::new(&mand, &agent, ts, None);
            engine
                .put_cf_raw(CF_MANDATE_ACT, rid.as_bytes(), &serde_json::to_vec(&entry).unwrap())
                .unwrap();
        }
        // No time keys yet.
        for (rid, ts) in rows {
            let tk = StorageEngine::mandate_acts_time_key(ts, rid);
            assert!(engine.get_cf_raw(CF_MANDATE_ACTS_BY_TIME, &tk).unwrap().is_none());
        }
        engine.migrate_9_to_10().unwrap();
        // Post-migration: every forward entry has a time key (count parity).
        for (rid, ts) in rows {
            let tk = StorageEngine::mandate_acts_time_key(ts, rid);
            assert!(
                engine.get_cf_raw(CF_MANDATE_ACTS_BY_TIME, &tk).unwrap().is_some(),
        ...
        engine.migrate_9_to_10().unwrap(); // idempotent — must not panic or double-write
    }

    #[test]
    fn budget_evictor_oldest_first_bounded() {
        // B5: evict strictly oldest-first via CF_MANDATE_ACTS_BY_TIME, delete all
        // four keys per act, advance the floor to last_evicted_ts+1, respect the
        // per-tick cap. Exercised via evict_oldest_acts (the mechanics behind the
        // live-SST byte gate, which reads 0 until flush).
        use crate::mandate::MandateActEntry;
        let (engine, _dir) = test_engine();
        let mand = "e".repeat(64);
        let agent = "ff".repeat(32);
        for (rid, ts) in [("ev-1", 100u64), ("ev-2", 200), ("ev-3", 300), ("ev-4", 400), ("ev-5", 500)] {
            engine.put_record(rid, &test_record(rid)).unwrap();
            engine
                .put_mandate_act(rid, &MandateActEntry::new(&mand, &agent, ts, None))
                .unwrap();
        }
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 0); // fresh: absent = 0
        // Tick 1: evict the oldest 2 (ev-1@100, ev-2@200).
        assert_eq!(engine.evict_oldest_acts(2).unwrap(), 2);
        assert!(engine.get_mandate_act("ev-1").is_none());
        assert!(engine.get_mandate_act("ev-2").is_none());
        assert!(engine.get_mandate_act("ev-3").is_some());
        assert_eq!(
            engine.list_acts_for_mandate(&mand, None, 10).unwrap().0,
            vec!["ev-3", "ev-4", "ev-5"]
        );
        assert!(engine
            .get_cf_raw(CF_MANDATE_ACTS_BY_TIME, &StorageEngine::mandate_acts_time_key(100, "ev-1"))
            .unwrap()
            .is_none()); // time key gone too
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 201); // past newest evicted (200)
        // Tick 2: evict the next 2 (ev-3@300, ev-4@400).
        assert_eq!(engine.evict_oldest_acts(2).unwrap(), 2);
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 401);
        assert_eq!(
            engine.list_acts_for_mandate(&mand, None, 10).unwrap().0,
            vec!["ev-5"]
        );
    ...
    #[test]
    fn evict_acts_over_budget_unlimited_and_under_budget_are_noops() {
        // B5: budget 0 = unlimited (archive) → never evicts; a budget above the
        // act mass → under budget → no eviction. Neither ever rejects ingest.
        use crate::mandate::MandateActEntry;
        let (engine, _dir) = test_engine();
        let mand = "a".repeat(64);
        let agent = "bb".repeat(32);
        engine.put_record("nb-1", &test_record("nb-1")).unwrap();
        engine
            .put_mandate_act("nb-1", &MandateActEntry::new(&mand, &agent, 1, None))
            .unwrap();
        assert_eq!(engine.evict_acts_over_budget(0, 5000).unwrap(), 0); // unlimited
        assert_eq!(engine.evict_acts_over_budget(u64::MAX, 5000).unwrap(), 0); // under budget
        assert!(engine.get_mandate_act("nb-1").is_some());
    }

    #[test]
    fn acts_coverage_floor_monotone_single_writer() {
        // B5 (B4 fix): the floor never regresses; advancing to ≤ current is a no-op.
        let (engine, _dir) = test_engine();
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 0); // absent = 0
        assert_eq!(engine.advance_acts_coverage_floor(100).unwrap(), 100);
        assert_eq!(engine.advance_acts_coverage_floor(50).unwrap(), 100); // regress ignored
        assert_eq!(engine.advance_acts_coverage_floor(100).unwrap(), 100); // equal = no-op
        assert_eq!(engine.advance_acts_coverage_floor(200).unwrap(), 200);
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 200);
    }

    #[test]
    fn acts_coverage_floor_init_policy() {
        // B5: boot init writes now_ms on a DB missing the key (pins B1b — safe
        // direction for upgraded seed + virgin join); genesis-ceremony forces 0.
        let (engine, _dir) = test_engine();
        assert!(engine
            .get_cf_raw(CF_METADATA, ACTS_COVERAGE_FLOOR_KEY.as_bytes())
            .unwrap()
            .is_none());
        engine.ensure_acts_coverage_floor_initialized().unwrap();
        let f = engine.acts_coverage_floor_ms().unwrap();
        assert!(f > 1_600_000_000_000, "init must be now_ms, got {f}");
        // Idempotent: a second init does not move it.
        engine.ensure_acts_coverage_floor_initialized().unwrap();
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), f);
        // Genesis force → 0; a later boot init leaves it 0 (key already present).
        engine.force_acts_coverage_floor_genesis().unwrap();
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 0);
        engine.ensure_acts_coverage_floor_initialized().unwrap();
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 0);
    }

    #[test]
    fn acts_coverage_floor_state_distinguishes_uninitialized_from_genesis() {
        // B5 post-ship review CRITICAL: the HONEST state reader must NOT collapse
        // "uninitialized" (absent/malformed key) into the written-0 that means
        // genesis full coverage — that collapse is the fail-open that reopens the
        // false-negative. acts_coverage_floor_ms keeps its 0-for-math delegation.
        let (engine, _dir) = test_engine();
        assert_eq!(engine.acts_coverage_floor_state().unwrap(), None); // absent → uninitialized
        assert_eq!(engine.acts_coverage_floor_ms().unwrap(), 0); // delegate still 0 for math

        // Genesis-0 (written 0) is distinguishable from uninitialized.
        engine.force_acts_coverage_floor_genesis().unwrap();
        assert_eq!(engine.acts_coverage_floor_state().unwrap(), Some(0));

        // Boot init → Some(now_ms).
        let (engine2, _dir2) = test_engine();
        engine2.ensure_acts_coverage_floor_initialized().unwrap();
        assert!(matches!(
            engine2.acts_coverage_floor_state().unwrap(),
            Some(v) if v > 1_600_000_000_000
        ));

        // Malformed value (wrong length) → None (fail-closed, not a garbage floor).
        let (engine3, _dir3) = test_engine();
        engine3
            .put_cf_raw(CF_METADATA, ACTS_COVERAGE_FLOOR_KEY.as_bytes(), b"bad")
            .unwrap();
        assert_eq!(engine3.acts_coverage_floor_state().unwrap(), None);
    }
    ...
    fn test_record(id: &str) -> ValidationRecord {
        {
            ...
            __r.id = id.to_string();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = sha3_256(id.as_bytes()).to_vec();
            __r.creator_public_key = vec![0xAA; 1952];
            __r.timestamp = 1700000000.0;
            __r.parents = vec![];
            __r.classification = Classification::Public;
            __r.metadata = BTreeMap::new();
    ...
    #[test]
    fn seal_record_ids_at_zone_epoch_enumerates_all_disc5_rivals() {
        let (engine, _dir) = test_engine();
        let epoch = 77u64;
        let zone = "default";

        // Empty slot → empty vec (an orphaned marker's "no seal here" case).
        assert!(engine.seal_record_ids_at_zone_epoch(epoch, zone).is_empty());

        // Store two RIVAL seals at the SAME (zone, epoch) — DISC-5 keeps both
        // (Phase-2 writes every parsed seal before the C2 canonicality decision).
        let store = |eng: &StorageEngine, id: &str| {
            let rec = test_record(id);
            let mut key = Vec::new();
            key.extend_from_slice(&epoch.to_be_bytes());
            key.extend_from_slice(zone.as_bytes());
            key.push(0u8);
            key.extend_from_slice(rec.id.as_bytes());
            eng.put_record_with_pk_zone(
                &rec.id,
                &rec,
                "idh",
                &rec.creator_public_key,
        ...
        let a = store(&engine, "seal-aaa");
        let b = store(&engine, "seal-bbb");

        let mut got = engine.seal_record_ids_at_zone_epoch(epoch, zone);
        got.sort();
        let mut want = vec![a, b];
        want.sort();
        assert_eq!(got, want, "both DISC-5 rivals at the slot must enumerate");

        // Prefix boundary: a different epoch or zone slot returns nothing —
        // the 0x00 terminator makes "default" not match a "default-x" zone.
        assert!(engine.seal_record_ids_at_zone_epoch(epoch + 1, zone).is_empty());
        assert!(engine.seal_record_ids_at_zone_epoch(epoch, "other").is_empty());
    }

    #[test]
    fn arm_rotation_seal_marker_for_test_is_scan_visible() {
        let (engine, _dir) = test_engine();
        assert!(engine.scan_rotation_seal_markers().is_empty());
        engine.arm_rotation_seal_marker_for_test("default", 9).unwrap();
        engine.arm_rotation_seal_marker_for_test("default", 4).unwrap();
        // Ascending-epoch order within a zone (the §3-3 sweep order).
        assert_eq!(
            engine.scan_rotation_seal_markers(),
            vec![("default".to_string(), 4), ("default".to_string(), 9)]
        );
        engine.delete_rotation_seal_marker("default", 4).unwrap();
        assert_eq!(
            engine.scan_rotation_seal_markers(),
    ...
    fn test_record_delete() {
        ...
        engine.delete_record("delete-test", DeleteIntent::GcPrune).unwrap();
        assert!(!engine.record_exists("delete-test").unwrap());
    }
    ...
    fn delete_record_removes_cf_dag_row() {
        ...
        engine.delete_record("dag-leak-child", DeleteIntent::GcPrune).unwrap();
        assert!(
    ...
    fn record_entries_page_deleted_live_cursor_resumes_next_surviving() {
        ...
        engine.delete_record("gc-b", DeleteIntent::GcPrune).unwrap();
        let p = engine
    ...
    #[test]
    #[cfg(feature = "node-core")]
    fn f2_r2_rebuild_streaming_skips_tombstoned_governance_op() {
        // F2 R2: the full-rebuild governance loop EXCLUDES a tombstoned governance op
        // from the tally (degenerate-B — a moderated Propose/Vote shouldn't count).
        // The LEDGER-op loop still applies unconditionally (R1/semantic A); only the
        // governance loop skips. Governance state is out-of-seal (silent divergence),
        // so this KAT is the ONLY guard against a regression here. Differential: a
        // non-tombstoned Propose IS tallied (baseline), a tombstoned one is NOT.
        // See internal design notes (R2).
        use crate::accounting::governance::{propose_metadata, ProposalCategory};
        use crate::accounting::types::{mint_metadata, stake_metadata, StakePurpose, BASE_UNITS_PER_BEAT};
        ...
        // Genesis (== the fixed test identity) mints itself a balance, stakes half of
        // it for governance (> MIN_PROPOSAL_STAKE = 1000 beat), then proposes → the
        // gov stake makes create_proposal succeed, so a NON-tombstoned Propose lands.
        let amt = 4_000u64 * BASE_UNITS_PER_BEAT;
        put(&engine, "mint-gov", 1_700_000_001.0, mint_metadata(amt, &gen_hash, "test"));
        put(&engine, "stake-gov", 1_700_000_002.0, stake_metadata(amt / 2, &StakePurpose::Governance));
        put(&engine, "propose-1", 1_700_000_003.0,
        ...
        // Tombstone the Propose record (a `tombstone:{id}` CF_METADATA key), rebuild again.
        engine
        ...
        // The ledger arm is unaffected — the (non-tombstoned) mint/stake still applied.
        assert!(
            tombed.accounts.contains_key(&gen_hash),
            "ledger ops (mint/stake) still apply under R1 — only governance skips"
        );
    }

    #[test]
    #[cfg(feature = "node-core")]
    fn rebuild_ledger_streaming_revives_tombstoned_ledger_op() {
        // CHARACTERIZATION of the F2 bug — see
        // internal design notes.
        //
        // A Public record carrying a ledger op that is tombstoned-first is
        // SUPPRESSED on the live ledger (ingest.rs:2752 early-returns before the
        // ledger-apply block at :2893), but rebuild_ledger_streaming filters ONLY
        // on Classification::Public (rocks.rs:6674) and never consults the
        // `tombstone:{id}` marker — so the boot/sync rebuild RE-APPLIES the op,
        // diverging from the live ledger it is meant to reconstruct.
        //
        // This pins the CURRENT (buggy) behavior: alice IS credited by the rebuild
        // despite the tombstone. When the F2 semantics fix lands (propagation-only:
        // apply-when-tombstoned in ingest; OR revocation: skip-in-fold +
        // reverse-in-live), flip these assertions — this becomes the red→green
        // regression anchor. Do NOT "fix" it with a tombstone skip in the fold
        // alone: that breaks the apply-then-tombstone ordering (brief §3).
        let (engine, _dir) = test_engine();
        let gen_hash = crate::accounting::types::creator_identity_hash(&test_record("gh"));
        let alice = "alice-identity";

        insert_mint(&engine, "mint-tomb", 1_700_000_001.0, "1000", alice);
        // Tombstone the mint exactly as state.rs:6140 mark_tombstoned does — a
        // `tombstone:{id}` key in CF_METADATA. The record stays Public in CF_RECORDS.
        engine
            .put_cf_raw(
                crate::storage::rocks::CF_METADATA,
                b"tombstone:mint-tomb",
                br#"{"tombstone_id":"t1","reason":"characterization"}"#,
            )
            .unwrap();
        ...
        assert_eq!(
            state.accounts.get(alice).map(|a| a.available).unwrap_or(0),
            1000,
            "F2: rebuild revives the tombstoned mint (live path suppresses it → divergence)"
        );
        assert_eq!(
            state.total_supply, 1000,
            "F2: the revived mint also inflates the rebuilt total_supply"
        );
    }
    ...
    #[test]
    #[cfg(feature = "node-core")]
    fn incremental_ledger_replay_also_revives_tombstoned_op() {
        // F2 SIBLING PATH (brief §6): incremental_ledger_replay (the checkpoint
        // fast-path) shares rebuild_ledger_streaming's `Public`-only filter and
        // likewise never consults the `tombstone:{id}` marker — so it too RE-APPLIES
        // a tombstoned op the live ingest path suppressed. Executed here so the
        // "confirm at build time" note in the brief is no longer a hand-wave.
        let (engine, _dir) = test_engine();
        let gen_hash = crate::accounting::types::creator_identity_hash(&test_record("gh"));
        insert_mint(&engine, "mint-tomb-inc", 1_700_000_001.0, "1000", "alice-identity");
        engine
            .put_cf_raw(
                crate::storage::rocks::CF_METADATA,
                b"tombstone:mint-tomb-inc",
                br#"{"tombstone_id":"t1","reason":"characterization"}"#,
            )
            .unwrap();
        ...
        assert_eq!(
            ledger.accounts.get("alice-identity").map(|a| a.available).unwrap_or(0),
            1000,
            "F2: incremental_ledger_replay also revives the tombstoned op (fixable inline — has storage access)"
        );
    }
