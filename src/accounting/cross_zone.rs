...
/// A pending cross-zone transfer (stored in RocksDB pending_xzone CF).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CrossZoneState {
    ...
    /// Re-derive the per-status pending counters from
    /// `self.pending`. One-time O(n) sweep — call from boot recovery
    /// after deserializing a ledger snapshot that predates these counters
    /// (counters default to 0, must be re-counted to the live state). Idempotent
    /// at steady state (same input → same output).
    ...
    ///
    /// Calling this with `committee_size = 0` is a no-op: it leaves the
    /// transfer in the legacy "inclusion-only" path (`claim_transfer` does
    /// not enforce finality). To actually enable enforcement, supply a
    /// nonzero `committee_size` AND enough signers to satisfy quorum.
