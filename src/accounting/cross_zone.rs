...
mod tests {
    ...
    // ── Merkle-brief pre-work P-A + P-C (2026-08-19, non-forking pins) ──

    /// P-A: the input dedup-by-leaf-hash in `build_committee_proofs` is
    /// LOAD-BEARING against the CVE-2012-2459 duplicate-leaf shape — this fold
    /// pads odd levels by DUPLICATING the last element, so `[a,b,c]` and
    /// `[a,b,c,c]` are structurally the same tree. The dedup makes that
    /// ambiguity semantically inert: a PK listed twice IS one member, both
    /// inputs are the same committee, same root, and the duplicated PK's
    /// proof still verifies. If a refactor ever drops the dedup, this test
    /// names the CVE class it re-opens.
    #[test]
    fn committee_dedup_is_load_bearing_against_duplicate_leaf_ambiguity() {
        let a = vec![0xA1u8; 1952];
        let b = vec![0xB2u8; 1952];
        let c = vec![0xC3u8; 1952];
        let (root_odd, proofs_odd) = build_committee_proofs(&[a.clone(), b.clone(), c.clone()]);
        let (root_dup, proofs_dup) =
            build_committee_proofs(&[a.clone(), b.clone(), c.clone(), c.clone()]);
        assert_eq!(
            root_odd, root_dup,
            "duplicate PK collapses via dedup — same committee, same root (ambiguity inert)"
        );
    ...
    /// (`network::sync::MerkleTree::root`) PROMOTES the odd element unchanged,
    /// this committee fold DUPLICATES it. For any ≥3-element odd set the two
    /// produce DIFFERENT roots over the same leaves. Pinned so nobody
    /// "unifies" them casually: each is signed into different artifact classes
    /// (seals vs finality witnesses), and either change is fold-fork-class
    /// (see internal design notes).
    ///
    /// Gated on `node-core` because it compares against the seal fold, which
    /// lives in the feature-gated `network` module — same pattern as
    /// `build_committee_proofs_root_matches_committee_hash_from_pks` below.
    /// (Ungated it broke the public mirror's default-features all-targets CI
    /// legs on 2026-08-19 — no local leg compiles the test tree without
    /// `node`, so the mirror caught it first.)
    #[cfg(feature = "node-core")]
    #[test]
    fn seal_fold_and_committee_fold_diverge_on_odd_sets_by_design() {
        // Same three leaves through both disciplines, minus the leaf-tag
        // difference: feed both folds the SAME 32-byte leaf values.
        let leaves: Vec<[u8; 32]> = vec![
            sha3_256(b"leaf-1"),
            sha3_256(b"leaf-2"),
            sha3_256(b"leaf-3"),
        ];
        // Committee-style fold (duplicate-odd), inlined to isolate the rule:
        ...
        let committee_style_root = level[0];
        let seal_style_root = crate::network::sync::MerkleTree::root(&leaves);
        assert_ne!(
            committee_style_root, seal_style_root,
            "duplicate-odd vs promote-odd MUST diverge on odd sets — two disciplines, two artifact classes"
        );
    }
