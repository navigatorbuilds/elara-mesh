...
pub fn create_ledger_record_with_nonce(
    ...
    create_ledger_record_with_nonce_bound(identity, parents, metadata, nonce, None)
}

/// T65 D2 bridge — the network-bound construction path. `network_id =
/// Some(non-empty)` inserts the `network_id` metadata key BEFORE the content
/// preimage is computed, so the binding is covered by the content hash, the
/// v2 preimage that verifiers recompute (`expected_body` checks in
/// reward.rs), AND the signature. This is why ledger records cannot use the
/// record crate's post-create `bind_network_id()` — by then the preimage has
/// already been hashed over a map without the key. Additive twin of
/// `create_ledger_record_with_nonce` so the 25+ existing call sites keep
/// compiling unchanged and migrate individually behind
/// `bind_network_id_in_metadata` (T65 D2 ruling: no one-sweep migration).
pub fn create_ledger_record_with_nonce_bound(
    identity: &crate::identity::Identity,
    parents: Vec<String>,
...
mod tests {
    ...
    fn make_ledger_record(
    ...
    ) -> ValidationRecord {
        {
            ...
            __r.id = "test-001".into();
            __r.version = crate::wire::CURRENT_SIGNING_VERSION;
            __r.content_hash = sha3_256(b"test").to_vec();
            __r.creator_public_key = dummy_pk();
    ...
    /// T65 D2 — the bound construction path: `Some(net)` puts `network_id`
    /// INSIDE the content preimage (so the recompute gate passes), `None` is
    /// byte-compatible with the legacy fn, and binding post-create is exactly
    /// the tamper the gate rejects — which is why the helper exists.
    #[test]
    fn create_ledger_record_bound_binds_inside_preimage_none_is_legacy() {
        let id = v2_test_identity();

        let bound = create_ledger_record_with_nonce_bound(
            &id,
            vec![],
            transfer_metadata(42, "recipient_x", None),
            3,
            Some("testnet"),
        )
        .unwrap();
        assert_eq!(
            ...
            "bound path must stamp the key"
        );
        assert!(
            verify_ledger_content_hash_v2(&bound).is_ok(),
            "the binding must be inside the recomputed preimage"
        );
        let sig_ok = crate::identity::Identity::verify(
            &bound.signable_bytes(),
            bound.signature.as_ref().expect("builder signs"),
            &bound.creator_public_key,
        )
        .unwrap_or(false);
        assert!(sig_ok, "signature must cover the bound signable bytes");

        // None == the legacy fn, byte-for-byte (same nonce, same metadata).
        let legacy = create_ledger_record_with_nonce(
            &id,
            vec![],
            transfer_metadata(42, "recipient_x", None),
            3,
        ...
        .unwrap();
        let via_none = create_ledger_record_with_nonce_bound(
            &id,
            vec![],
            transfer_metadata(42, "recipient_x", None),
            3,
        ...
        .unwrap();
        assert!(!via_none.metadata.contains_key("network_id"));
        assert_eq!(
            legacy.content_hash, via_none.content_hash,
            "None must be indistinguishable from the legacy path"
        );

        // Empty string is a no-op, not an empty binding.
        let empty = create_ledger_record_with_nonce_bound(
            &id,
            vec![],
            transfer_metadata(42, "recipient_x", None),
            3,
        ...
        .unwrap();
        assert!(!empty.metadata.contains_key("network_id"));

        // The trap the bound path exists to avoid: binding AFTER create is
        // tampering as far as the recompute gate is concerned.
        let mut post_hoc = create_ledger_record_with_nonce(
            &id,
            vec![],
            transfer_metadata(42, "recipient_x", None),
            3,
        ...
        post_hoc
            .metadata
            .insert("network_id".into(), serde_json::json!("testnet"));
        assert!(
            verify_ledger_content_hash_v2(&post_hoc).is_err(),
            "post-create binding must fail the gate — bind at construction"
        );
    }
