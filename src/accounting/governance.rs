...
pub struct GovernableParams {
    /// Per-identity propagation rate limit (records/hour).
    pub propagation_rate_limit_per_hour: u64,
    /// Epoch seal interval in seconds.
    pub epoch_seal_interval_secs: f64,
    /// Witness reward per attestation (base units, 10^9/beat; `_micros` name is legacy).
    pub witness_reward_micros: u64,
    /// Record retention before GC (seconds, 0 = infinite).
    pub record_retention_secs: f64,
    /// Stake-gated throughput ratio: base units (10^9/beat) of stake per daily record (economics §9.4).
