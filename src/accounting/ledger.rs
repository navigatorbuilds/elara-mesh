...
pub struct LedgerState {
    ...
    ///
    /// WIRED (2026-07-19): `record_pool_disbursement` is now called from the
    /// `apply_op` WitnessReward arm passing `record.timestamp` — the sealed,
    /// signature-bound value (identical per-record on every node), NOT a per-node
    /// wall-clock (the C11 hazard). This field is NOT in the account-SMT leaf
    /// (network/account_merkle.rs) nor the snapshot checksum (network/snapshot.rs),
    /// and `apply_op` never READS it for any accept/reject decision — it is read
    /// only by the single genesis authority's own advisory throttle budget
    /// (reward.rs::distribute_rewards). Consequences: (i) a per-node divergence
    /// here can NEVER fork the chain; (ii) the tuple is nonetheless NOT guaranteed
    /// bit-identical across nodes — two independent witness_reward records that
    /// straddle a 30-day rollover can finalize in different relative orders on
    /// different nodes, a benign divergence that biases toward MORE accumulated
    /// `disbursed` (more throttling), never less. See internal design notes.
    #[serde(default)]
...
impl LedgerState {
    ...
    /// (the same class fixed for `pool_cap`). Read author-side (advisory) by the
    /// genesis authority in reward.rs::distribute_rewards with its wall-clock `now`
    /// — acceptable because only that single node reads this budget. The paired
    /// WRITE that MUST use a sealed timestamp is `record_pool_disbursement` in
    /// apply_op; see this field's struct doc for the full determinism argument.
    ...
    pub fn record_pool_disbursement(&mut self, amount: u64, now: f64) {
        ...
        if now - window_start >= MONTH_SECS {
            ...
            // saturating: matches the file-wide convention (sibling reward
            // mutations all saturate) and closes the arithmetic class now that
            // this fn has a production caller (apply_op) — it had none at the
            // 2026-06 overflow sweep, so it was never covered then.
            self.pool_disbursed_window = (window_start, disbursed.saturating_add(amount));
        }
    }
...
pub fn apply_op(
    ...
    match op {
        ...
        ParsedLedgerOp::WitnessReward { amount, from, to, record_id: _ } => {
            ...
            // Consensus-side accounting for the monthly-disbursement throttle
            // (author-side enforcement in reward.rs::distribute_rewards). Passes the
            // record's OWN signature-bound timestamp (identical per-record on every
            // node), NEVER a per-node wall-clock. This can never fork the chain:
            // pool_disbursed_window is in no consensus root (not the account-SMT
            // leaf, not the snapshot checksum) and apply_op never reads it — only the
            // genesis authority reads its own copy, for the advisory budget. (The
            // tuple is not guaranteed bit-identical across nodes when independent
            // rewards straddle a window rollover in different finality orders —
            // benign, biases toward more throttling.) Deduped by applied_record_ids
            // above, so each reward is counted exactly once. See the field's doc.
            state.record_pool_disbursement(*amount, record.timestamp);
...
mod tests {
    ...
    fn make_record_with_pk(
    ...
    ) -> ValidationRecord {
        {
            ...
            __r.creator_public_key = pk.to_vec();
            __r.timestamp = ts;
            __r.parents = vec![];
    ...
    #[test]
    fn witness_reward_records_pool_disbursement_for_monthly_throttle() {
        // Pins the consensus-side accounting the monthly-disbursement throttle
        // depends on: applying a witness_reward through apply_op must advance
        // `pool_disbursed_window` by the reward amount, keyed to the reward
        // record's OWN sealed timestamp — never a per-node wall-clock — so the
        // 30-day window boundary is identical (fork-free) on every node.
        let genesis_hash = identity_hash(&genesis_pk());
        let witness_hash = identity_hash(&witness_pk());
        ...
        // The pool drew exactly 1 beat AND the disbursement window recorded that
        // same beat — proof apply_op wires record_pool_disbursement (previously
        // dormant: zero production callers).
        assert_eq!(ledger.conservation_pool, 99 * BASE_UNITS_PER_BEAT);
        assert_eq!(
            ledger.pool_disbursed_window.1,
            BASE_UNITS_PER_BEAT,
            "apply_op must record a witness reward as a pool disbursement (monthly-throttle accounting)"
        );
    }
