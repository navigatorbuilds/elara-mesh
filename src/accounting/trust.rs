...
//!
//! **Trust tiers** (§9.3; daily figures = the ENFORCED constants below — a stale
//! "10/day" doc figure lived here for months while `TIER_0_DAILY` was 20, flagged
//! in round2-findings-2026-07-03 §Tier0 and finally fixed 2026-08-19):
//! - Tier 0 (New): identity created, 20/day, no witness, no earn
...
//!
//! **Entropy throttle** (`daily_limit_for`): entropy in [0.3, 0.6) HALVES the tier's
//! daily limit — so a fresh burst-emitting identity typically sees an effective
//! **10/day** (20/2), which is exactly what the live ingest refusal reports
//! ("identity limited to 10/day"). Both figures are real: 20 is the tier constant,
//! 10 is the throttled effective cap.
//!
...
pub enum TrustTier {
    /// New identity: `TIER_0_DAILY` (20) records/day — halved to 10 while
    /// entropy sits in the throttle band — no witness, no earn.
    Tier0,
...
impl EntropyEngine {
    ...
    /// Tier/entropy daily LIMIT for an UNSTAKED identity — the limit only, no
    /// count read or mutation (the authoritative count lives in
    /// `NodeState::daily_caps`, enforced non-skippably at ingest). Unknown
    /// identity → `TIER_0_DAILY` (strictest headroom). Mirrors the unstaked branch
    /// of `check_submission_with_continuity` so the two never drift.
    pub fn daily_limit_for(&self, identity: &str, now: f64, continuity_score: f64) -> u32 {
        let Some(profile) = self.profiles.get(identity) else {
            return TIER_0_DAILY;
        };
        let daily_limit = profile.tier_with_continuity(now, Some(continuity_score)).daily_limit();
        let entropy = profile.entropy();
        if (ENTROPY_THROTTLE..ENTROPY_FULL_ACCESS).contains(&entropy) {
            daily_limit / 2
        } else {
            daily_limit
        }
...
mod tests {
    ...
    #[test]
    fn daily_limit_for_unknown_is_strictest_and_reads_known_profile() {
        let mut e = EntropyEngine::new();
        // Unknown identity → strictest (fail-closed) cap — the flood-skip fallback
        // the ingest gate uses when the trust lock is contended.
        assert_eq!(e.daily_limit_for("nobody", 1000.0, 1.0), TIER_0_DAILY);
        // A known fresh profile is Tier 0: its limit is positive and ≤ TIER_0_DAILY
        // (the entropy-throttle band may halve it), proving daily_limit_for reads
        // the profile rather than always returning the fallback.
        e.profiles.insert("fresh".into(), EntropyProfile::new(1000.0));
        let got = e.daily_limit_for("fresh", 1000.0, 1.0);
        assert!(got > 0 && got <= TIER_0_DAILY, "fresh Tier-0 profile limit was {got}");
    }
