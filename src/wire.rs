...
pub fn decode_metadata_binary(reader: &mut WireReader) -> Result<std::collections::BTreeMap<String, serde_json::Value>> {
    ...
    if count > MAX_METADATA_ENTRIES {
        return Err(ElaraError::Wire(format!("too many metadata entries: {count} (max {MAX_METADATA_ENTRIES})")));
    }
    let mut map = std::collections::BTreeMap::new();
    for _ in 0..count {
        let key_bytes = reader.read_u8_prefixed()?;
        let key = std::str::from_utf8(key_bytes)
            .map_err(|e| ElaraError::Wire(format!("invalid metadata key: {e}")))?
            .to_string();
    ...
    match tag {
        ...
        META_INT => {
            ...
            let arr = <[u8; 8]>::try_from(bytes)
                .map_err(|_| ElaraError::Wire("int tag: expected 8 bytes".to_string()))?;
            Ok(serde_json::json!(i64::from_be_bytes(arr)))
        }
        META_FLOAT => {
            let bytes = reader.read_bytes(8)?;
            let arr = <[u8; 8]>::try_from(bytes)
                .map_err(|_| ElaraError::Wire("float tag: expected 8 bytes".to_string()))?;
        ...
        META_STRING => {
            ...
            let s = std::str::from_utf8(s_bytes)
                .map_err(|e| ElaraError::Wire(format!("invalid metadata string: {e}")))?
                .to_string();
        ...
        META_OBJECT => {
            ...
            for _ in 0..count {
                ...
                let k = std::str::from_utf8(k_bytes)
                    .map_err(|e| ElaraError::Wire(format!("invalid metadata obj key: {e}")))?
                    .to_string();
        ...
        _ => Err(ElaraError::Wire(format!("unknown metadata type tag: {tag}"))),
    }
}
