...
//! 6. Propagation filtering — DESIGNED but NOT YET IMPLEMENTED (see the F2 NOTE below).
//!
//! F2 NOTE (2026-07-20): feature 6 above is aspirational — no gate consults the
//! tombstone marker on the gossip/serve path (`is_tombstoned` has one production
//! consumer: the ingest ledger gate), so a tombstoned record still propagates and
//! serves. Content defense today = the structural byte caps (#2) + the tombstone flag,
//! NOT propagation suppression. The fix is owner-gated + fork-risky (the naive gate
//! starves peer sync) — see internal design notes.
...
const ALLOWED_KEYS: &[&str] = &[
    // T65 D2 — network binding of the record's SIGNED content (emitted by
    // `create_ledger_record_with_nonce_bound` / `bind_network_id`, gated off
    // by default behind `bind_network_id_in_metadata`). Registered here as a
    // producer-side schema key; ingest REQUIRING it is a schema-version
    // decision that waits for the v6 flag day (T65 D4 step 3), per the
    // "consensus-interpreted keys need version discipline" rule below.
    "network_id",
...
mod tests {
    ...
    /// T65 D2 — `network_id` is a registered producer-side key: a bound
    /// record admits cleanly through the metadata gate, on its own and next
    /// to ledger keys.
    #[test]
    fn t65_network_id_key_is_registered_and_admitted() {
        assert!(ALLOWED_KEYS.contains(&"network_id"), "registered in the schema registry");
        let m = meta(&[
            ("beat_op", "transfer"),
            ("beat_amount", "1000000"),
            ("beat_to", "abc123def456"),
    ...
    fn test_all_record_builders_pass_allowlist() {
        ...
        // drift on the keys slice a3's producer (drand_fetch.rs) emits on
        // fetcher-enabled nodes.
    ...
    fn test_meta_test_covers_all_builders_in_src() {
        ...
        const NOT_BUILDERS: &[&str] = &[
            ...
            // of a map (extractor, returns Option<DrandPulse>). Slices
            // (a2)/(a3) wired the drand_* keys onto seals; those KEYS are
            // allowlisted in ALLOWED_KEYS and pinned by the populated-pulse
            // guard above (the keys, not these two fns, are what flows to
            // the validator).
            "write_metadata",
            "from_metadata",
