...
mod pyo3_bindings {
    ...
    pub fn dict_to_record(dict: &Bound<'_, PyDict>) -> PyResult<record::ValidationRecord> {
        ...
        // Reconstruction from a Python dict (non_exhaustive: shell + assignments;
        // unset fields keep the shell's wire-neutral defaults).
        let mut rec = record::ValidationRecord::fixture_shell();
        rec.id = id;
        rec.version = version;
        rec.content_hash = content_hash;
        rec.creator_public_key = creator_public_key;
        rec.timestamp = timestamp;
        rec.parents = parents;
        rec.classification = classification;
        rec.metadata = metadata;
        rec.signature = signature;
        rec.sphincs_signature = sphincs_signature;
        rec.zk_proof = zk_proof;
        rec.itc_stamp = itc_stamp;
        rec.zone_refs = zone_refs;
        rec.creator_sphincs_pk = creator_sphincs_pk;
        rec.sig_algorithm = crypto::ALG_DILITHIUM3;
        rec.sphincs_algorithm = sphincs_algorithm;
        Ok(rec)
    }
