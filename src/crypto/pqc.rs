...
// Canonical algorithm IDs live in elara-record::pqc — the wire crate owns the
...
mod tests {
    ...
    /// T63 KAT FREEZE GENERATOR — run ONCE against pre-change code:
    ///   cargo test --lib kat_freeze_generator_t63 -- --ignored --nocapture
    /// Writes `crates/elara-record/tests/kat/frozen_preimages_t63.json`: frozen
    /// v4/v5 `signable_bytes` / `record_hash` / wire vectors (synthetic v4+v5
    /// with fresh keypairs, + real dual-sig v5 chain records lifted from the
    /// public receipt envelopes' wire hex). The pinning test in elara-record
    /// then proves v≤5 preimages BYTE-IDENTICAL across any signable_bytes()
    /// change (the T63 v6 domain-tag gate precondition, T65 :269-271).
    ///
    /// ⚠ NEVER regenerate to "fix" a pinning-test mismatch — a mismatch IS the
    /// alarm firing (a v≤5 preimage moved = every existing signature breaks).
    /// Regenerate only to ADD vectors, from a binary whose pinning test is green.
    ///
    /// v1-v3 synthetics deliberately absent: `to_bytes()` emits binary metadata
    /// unconditionally (record.rs R3-8 slice 1) while `from_bytes()` reads v≤3
    /// metadata as JSON — that dead re-emission path (no v≤3 records exist
    /// post-ceremony) cannot round-trip and is queued as its own item, not
    /// silently altered under T63.
    #[test]
    #[ignore]
    fn kat_freeze_generator_t63() {
        use crate::record::{Classification, ValidationRecord};
        use std::collections::BTreeMap;
        use std::path::Path;

        let manifest = env!("CARGO_MANIFEST_DIR");
        let out_dir = Path::new(manifest).join("crates/elara-record/tests/kat");
        std::fs::create_dir_all(&out_dir).expect("mkdir kat");
        let out_path = out_dir.join("frozen_preimages_t63.json");

        let mut vectors: Vec<serde_json::Value> = Vec::new();

        // A vector is only frozen after the generator itself proves, at freeze
        // time, that decode→re-encode reproduces the input bytes and that every
        // present signature verifies over the recomputed preimage.
        let mut freeze = |name: &str, source: &str, wire: Vec<u8>, assert_reencode: bool| {
            let rec = ValidationRecord::from_bytes(&wire).expect("decode at freeze");
            let signable = rec.signable_bytes();
            let hash = rec.record_hash();
            if assert_reencode {
                assert_eq!(rec.to_bytes(), wire, "{name}: re-encode must reproduce wire bytes");
            }
            let sig = rec.signature.as_ref().expect("frozen vectors are signed");
            assert!(
                dilithium3_verify(&signable, sig, &rec.creator_public_key).unwrap(),
                "{name}: dilithium must verify at freeze"
            );
            let expect_sphincs = match (&rec.sphincs_signature, &rec.creator_sphincs_pk) {
                (Some(s), Some(pk)) => {
                    assert!(
                        sphincs_verify(&signable, s, pk).unwrap(),
                        "{name}: sphincs must verify at freeze"
                    );
                    true
                }
                _ => false,
            ...
            vectors.push(serde_json::json!({
                "name": name,
                "source": source,
                "frozen": "2026-08-18 pre-change (commit lineage: T63 verdict b0bf1803)",
                "wire_hex": hex::encode(&wire),
                "signable_hex": hex::encode(&signable),
                "record_hash_hex": hex::encode(hash),
                "expect_dilithium": true,
                "expect_sphincs": expect_sphincs,
                "assert_reencode": assert_reencode,
            }));
        };

        // ── Synthetic v4 (single-sig, parents + rich metadata, binary metadata path)
        let dil = dilithium3_keygen().unwrap();
        let mut meta = BTreeMap::new();
        meta.insert("kat".into(), serde_json::json!("t63"));
        meta.insert("n".into(), serde_json::json!(7));
        let mut v4 = ValidationRecord::create(
            b"T63 KAT frozen content v4",
            dil.public_key.clone(),
            vec!["t63-kat-parent-a".into(), "t63-kat-parent-b".into()],
            Classification::Public,
            Some(meta.clone()),
        );
        v4.version = 4; // pre-nonce formula: signable stops at metadata/zk
        v4.id = "t63-kat-synthetic-v4".into();
        v4.timestamp = 1_755_500_000.25;
        let sig4 = dilithium3_sign_with_pk(&v4.signable_bytes(), &dil.secret_key, &dil.public_key).unwrap();
        v4.signature = Some(sig4);
        freeze("synthetic-v4-single-sig", "generated pre-change, fresh keypair", v4.to_bytes(), true);

        // ── Synthetic v5 (dual-sig, nonce=7 pinning the v5 nonce bytes)
        let dil5 = dilithium3_keygen().unwrap();
        let sph5 = sphincs_keygen().unwrap();
        let mut v5 = ValidationRecord::create(
            b"T63 KAT frozen content v5",
            dil5.public_key.clone(),
            vec!["t63-kat-parent-a".into()],
            Classification::Public,
            Some(meta),
        );
        assert_eq!(v5.version, 5, "pre-change create() must stamp v5");
        v5.id = "t63-kat-synthetic-v5".into();
        v5.timestamp = 1_755_500_000.25;
        v5.nonce = 7;
        v5.creator_sphincs_pk = Some(sph5.public_key.clone());
        v5.sphincs_algorithm = Some(ALG_SPHINCS_SHA2_192F);
        let signable5 = v5.signable_bytes();
        v5.signature =
            Some(dilithium3_sign_with_pk(&signable5, &dil5.secret_key, &dil5.public_key).unwrap());
        v5.sphincs_signature =
            Some(sphincs_sign_with_pk(&signable5, &sph5.secret_key, &sph5.public_key).unwrap());
        freeze("synthetic-v5-dual-sig-nonce7", "generated pre-change, fresh keypairs", v5.to_bytes(), true);

        // ── Real v5 chain records: wire hex straight out of the public receipt
        //    envelopes (assembled by the node from /record/{id}/wire).
        for (name, file, leg) in [
            ("real-v5-act-019fe675", "site/receipts/019fe675-87c2-7b71-8363-7ea92a4fd4c9.receipt.json", "record"),
            ("real-v5-seal-for-019fe675", "site/receipts/019fe675-87c2-7b71-8363-7ea92a4fd4c9.receipt.json", "seal"),
            ("real-v5-act-019fc8c6", "site/receipts/019fc8c6-e2ad-7ec3-973a-282ee116700d.receipt.json", "record"),
        ] {
            let env_json: serde_json::Value = serde_json::from_str(
                &std::fs::read_to_string(Path::new(manifest).join(file)).expect("read receipt"),
            )
            .expect("parse receipt");
            let wire_hex = env_json["legs"][leg].as_str().expect("wire hex leg");
            let wire = hex::decode(wire_hex).expect("decode wire hex");
            freeze(name, &format!("live chain record via {file} leg={leg}"), wire, true);
        }

        assert!(vectors.len() >= 5, "expected >=5 frozen vectors");
        let doc = serde_json::json!({
            "comment": "T63 frozen preimage KATs. DO NOT regenerate on mismatch — see kat_freeze_generator_t63 rustdoc.",
            "vectors": vectors,
        });
        std::fs::write(&out_path, serde_json::to_string_pretty(&doc).unwrap()).expect("write kat");
        println!("froze {} vectors -> {}", doc["vectors"].as_array().unwrap().len(), out_path.display());
    }

    /// T63 Phase C — one-shot generator for the committed v6 conformance
    /// fixture `examples/verify/sample-record-v6.wire`: a SIGNED v6 record
    /// (dual-sig, nonce, network binding) that the public reference tools
    /// (decode_record.py, elara-verify, conformance vectors) exercise the v6
    /// path against BEFORE any node emits v6 (decode-everywhere-first).
    ///
    /// Reproducibility model = same as the KAT freeze: the generator runs
    /// ONCE with a fresh keypair and the OUTPUT file is the frozen artifact —
    /// never regenerate over an existing fixture (a changed fixture invalidates
    /// published conformance vectors; the drift guard exists to catch exactly
    /// that). Freeze-time proofs: re-encode reproduces bytes; both signatures
    /// verify over the TAGGED preimage; the preimage leads with
    /// ELARA_RECORD_V1; decode round-trips the network binding.
    ///
    ///   cargo test --lib v6_fixture_generator_t63 -- --ignored --nocapture
    #[test]
    #[ignore]
    fn v6_fixture_generator_t63() {
        use crate::record::{Classification, ValidationRecord, DOMAIN_TAG_RECORD_V1};
        use std::collections::BTreeMap;
        use std::path::Path;

        let manifest = env!("CARGO_MANIFEST_DIR");
        let out_path = Path::new(manifest).join("examples/verify/sample-record-v6.wire");
        assert!(
            !out_path.exists(),
            "fixture already frozen at {} — never regenerate over it",
            out_path.display()
        );

        let dil = dilithium3_keygen().unwrap();
        let sph = sphincs_keygen().unwrap();
        let mut meta = BTreeMap::new();
        meta.insert("fixture".into(), serde_json::json!("t63-v6-conformance"));
        meta.insert("purpose".into(), serde_json::json!("public v6 decode/verify specimen"));
        let mut rec = ValidationRecord::create(
            b"T63 v6 conformance fixture content",
            dil.public_key.clone(),
            vec!["t63-v6-parent-a".into()],
            Classification::Public,
            Some(meta),
        );
        // Manual specimen construction — production emission stays v5 until
        // the Phase E flip; a fixture may stamp v6 deliberately.
        rec.version = 6;
        rec.network_id = "elara-conformance-v6".into();
        rec.id = "t63-v6-conformance-fixture".into();
        rec.timestamp = 1_755_540_000.5;
        rec.nonce = 42; // pins the v6 wire tail order: nonce THEN network_id
        rec.creator_sphincs_pk = Some(sph.public_key.clone());
        rec.sphincs_algorithm = Some(ALG_SPHINCS_SHA2_192F);
        let signable = rec.signable_bytes();
        assert!(
            signable.starts_with(DOMAIN_TAG_RECORD_V1),
            "v6 preimage must lead with the record domain tag"
        );
        rec.signature =
            Some(dilithium3_sign_with_pk(&signable, &dil.secret_key, &dil.public_key).unwrap());
        rec.sphincs_signature =
            Some(sphincs_sign_with_pk(&signable, &sph.secret_key, &sph.public_key).unwrap());

        let wire = rec.to_bytes();
        let back = ValidationRecord::from_bytes(&wire).expect("v6 fixture must decode");
        assert_eq!(back.to_bytes(), wire, "re-encode must reproduce fixture bytes");
        assert_eq!(back.network_id, "elara-conformance-v6");
        assert_eq!(back.nonce, 42);
        assert!(dilithium3_verify(&back.signable_bytes(), back.signature.as_ref().unwrap(), &back.creator_public_key).unwrap());
        assert!(sphincs_verify(&back.signable_bytes(), back.sphincs_signature.as_ref().unwrap(), back.creator_sphincs_pk.as_ref().unwrap()).unwrap());

        std::fs::write(&out_path, &wire).expect("write v6 fixture");
        println!("froze v6 fixture -> {} ({} bytes)", out_path.display(), wire.len());
        println!("record_hash: {}", hex::encode(back.record_hash()));
        println!("signable len: {} (tag+netid prefix {})", signable.len(), 15 + 2 + 20);
    }
}
