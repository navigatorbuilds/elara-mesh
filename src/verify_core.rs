//! verify_core — the pure, node-free, wasm-portable verification LOGIC behind
//! the `elara-verify` CLI (`src/bin/elara_verify.rs`).
//!
//! This module exists so a future WASM/browser build can call the IDENTICAL
//! verify logic with no drift: the binary is a thin shell that reads files,
//! parses CLI flags, and renders prose/JSON, but every cryptographic/structural
//! CHECK lives here. It deliberately references only pure-Rust deps the moved
//! code already used (`crate::record::ValidationRecord`, `crate::crypto::*`,
//! `crate::light_verify`, `elara_smt`, `serde_json`, `hex`) — NOT `std::process`,
//! `clap`, `std::fs`, or any node-gated symbol — so it compiles for
//! `wasm32-unknown-unknown` under default features.
//!
...
use crate::crypto::pqc::{dilithium3_verify, sphincs_verify};
use crate::light_verify::verify_seal_record_against_anchor;
