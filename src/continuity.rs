...
mod tests {
    ...
    // ── T61: continuity persistence (internal design notes §2.2) ──
    // ContinuityState was never persisted or rebuilt, so `first_seen` re-anchored
    // to process start and the score tracked NODE UPTIME, not identity history —
    // live logs read continuity=0.00 on 96 of 96 samples, making Tier 1
    // (needs >= 0.2) structurally unreachable. These pin the fix.

    #[test]
    fn t61_continuity_state_survives_serde_round_trip_with_history() {
        let start = 1_700_000_000.0;
        let mut st = ContinuityState::new();
        // 20 days of daily activity.
        for d in 0..20 {
            st.record_activity("ops-identity", start + (d as f64) * SECS_PER_DAY);
        }
        let now = start + 20.0 * SECS_PER_DAY;
        let live_score = st.score("ops-identity", now);
        assert!(live_score > 0.0, "sanity: accumulated history must score above zero");

        let bytes = serde_json::to_vec(&st).expect("serialize");
        let restored: ContinuityState = serde_json::from_slice(&bytes).expect("deserialize");

        assert_eq!(restored.identity_count(), 1);
        assert!(
            (restored.score("ops-identity", now) - live_score).abs() < 1e-9,
            "restored score must equal the live score — this is what makes the tier ladder reachable"
        );
        assert!(
            restored.score("ops-identity", now) >= 0.2,
            "20 days of daily activity must clear the Tier-1 continuity gate (>= 0.2) after restore"
        );
    }

    #[test]
    fn t61_unseen_identity_still_scores_zero_after_restore() {
        // The permissive direction must NOT change: an identity with no history
        // scores 0.0 whether or not a snapshot was loaded.
        let mut st = ContinuityState::new();
        st.record_activity("someone-else", 1_700_000_000.0);
        let bytes = serde_json::to_vec(&st).unwrap();
        let restored: ContinuityState = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(
            restored.score("never-seen", 1_700_000_000.0 + 40.0 * SECS_PER_DAY),
            0.0,
            "unknown identity must still score 0.0 — persistence must not invent credit"
        );
    }

    #[test]
    fn t61_rebuild_from_records_anchors_to_oldest_record_not_process_start() {
        // The boot rebuild feeds record_activity in timestamp order, exactly as
        // live ingest does. Replaying an identity's history must yield the same
        // state as having been online for it — that equivalence IS the fix.
        let first_record = 1_700_000_000.0;
        let now = first_record + 15.0 * SECS_PER_DAY;

        let mut online = ContinuityState::new();
        let mut rebuilt = ContinuityState::new();
        for d in 0..15 {
            let ts = first_record + (d as f64) * SECS_PER_DAY;
            online.record_activity("ident", ts);
            rebuilt.record_activity("ident", ts);
        }

        assert!(
            (rebuilt.score("ident", now) - online.score("ident", now)).abs() < 1e-9,
            "rebuilt-from-records score must equal the online score"
        );
        // And the fresh-process behaviour it replaces: anchoring at `now` scores 0.0.
        let mut fresh = ContinuityState::new();
        fresh.record_activity("ident", now);
        assert_eq!(fresh.score("ident", now), 0.0,
            "pre-T61 behaviour (anchor at process start) scored exactly 0.0 — the bug being fixed");
    }
