...
mod tests {
    ...
    /// The OFFLINE VERIFIER (`elara-verify` — the shipped CLI + browser-wasm
    /// artifact) is a DISTINCT code path from `DamVm::insert`, and it is what a
    /// reviewer actually runs. It must reject a *downgraded* Profile-A record —
    /// one where a single signature is forged — or the public "dual-signed: a
    /// break of one leaves the other standing" claim is false. Both halves are
    /// pinned so the guarantee is an AND (both required), never an OR (either
    /// accepted): forging one leg fails ONLY that leg (`signable_bytes` excludes
    /// the signatures, so the untouched leg still verifies), and the FAIL
    /// dominates the verdict via `Verdict::of`.
    #[test]
    fn offline_verifier_rejects_dual_sig_downgrade() {
        use elara_verify::{verify_record, Check, Status, Verdict};

        let (_vm, id, _dir) = setup_vm_profile_a();
        let mut rec = ValidationRecord::create(
            b"downgrade-resistance fixture",
            id.public_key.clone(),
            vec![],
            Classification::Public,
            None,
        ...
        let run = |r: &ValidationRecord| -> (Verdict, Vec<Check>) {
            let mut checks = Vec::new();
            verify_record(r, None, "", &mut checks);
            (Verdict::of(&checks), checks)
        };
        let leg = |checks: &[Check], name: &str| {
            checks.iter().find(|c| c.name == name).map(|c| c.status)
        };

        // Happy path: a genuinely valid dual-signed record verifies, BOTH legs proven.
        let (verdict, checks) = run(&rec);
        assert_eq!(verdict, Verdict::Verified, "valid Profile-A record must verify");
        assert_eq!(leg(&checks, "signature"), Some(Status::Pass));
        assert_eq!(leg(&checks, "profile"), Some(Status::Pass));

        // Forge SPHINCS+ only → ONLY the "profile" leg fails; Dilithium3 unaffected.
        let mut bad_sphincs = rec.clone();
        bad_sphincs.sphincs_signature.as_mut().unwrap()[0] ^= 0xFF;
        let (verdict, checks) = run(&bad_sphincs);
        assert_eq!(verdict, Verdict::Failed, "forged SPHINCS+ must FAIL the verifier");
        assert_eq!(leg(&checks, "signature"), Some(Status::Pass), "Dilithium3 leg must be untouched");
        assert_eq!(leg(&checks, "profile"), Some(Status::Fail), "SPHINCS+ leg must fail closed");

        // Forge Dilithium3 only → the mirror image: ONLY the "signature" leg fails.
        let mut bad_dilithium = rec.clone();
        bad_dilithium.signature.as_mut().unwrap()[0] ^= 0xFF;
        let (verdict, checks) = run(&bad_dilithium);
        assert_eq!(verdict, Verdict::Failed, "forged Dilithium3 must FAIL the verifier");
        assert_eq!(leg(&checks, "profile"), Some(Status::Pass), "SPHINCS+ leg must be untouched");
        assert_eq!(leg(&checks, "signature"), Some(Status::Fail), "Dilithium3 leg must fail closed");
    }
