...
//! integration test in `src/network/account_merkle.rs`. It MUST be run in
//! release — in the default debug profile it is ~20× slower and the throughput
//! assertion is meaningless:
//! `cargo test --features node --lib --release -- --ignored smt_soak`.
//!
//! ⚠ WHICH PATH THESE MEASURE (R30, 2026-07-26). `flush_dirty` has **no
//! production callers** — it is test/bench-only. Production runs
//! `snapshot_dirty` + `apply_snapshot` (see `sync.rs` bootstrap and
//! `ingest.rs` witness-SMT), a pair introduced by DISC-8 specifically to move
//! the O(dirty × log N) walk off `ledger.write().await`. The `flush_dirty`
//! benches below were written when it WAS the production path and were never
//! repointed, so on their own they say nothing about live cost.
//! `bench_smt_apply_snapshot_batch` measures the path the node actually runs
//! and is directly comparable to `bench_smt_flush_batch` (same N, same
//! rebuild-per-iteration). Whether the older benches should be retargeted or
//! retired is a queued AUDIT-FIRST decision — do not delete them on sight.

use criterion::{criterion_group, criterion_main, BenchmarkId, Criterion, Throughput};

use elara_runtime::crypto::hash::sha3_256;
use elara_runtime::network::account_merkle::{
    apply_snapshot, flush_dirty, hash_account_state, snapshot_dirty, verify_proof,
    AccountStateSMT,
};
...
/// THE PRODUCTION PATH (R30): `snapshot_dirty` + `apply_snapshot`, the pair the
/// node actually runs on every bootstrap and witness-SMT update.
///
/// Deliberately mirrors `bench_smt_flush_batch` exactly — same N values, same
/// rebuild-per-iteration so the tree starts empty and the full insert path is
/// measured — so the two are directly comparable and the cost of the DISC-8
/// offload is readable as a delta rather than guessed at. Both halves are timed
/// together, because together they are what `flush_dirty` does in one call.
fn bench_smt_apply_snapshot_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("smt_apply_snapshot_batch");
    for &n in &[100u64, 1_000, 10_000] {
        group.throughput(Throughput::Elements(n));
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, &n| {
            b.iter_custom(|iters| {
...
/// R35 (audit Stage 0): steady-state variant of the PRODUCTION path — tree
/// already populated, every account re-dirtied in place, timing
/// `snapshot_dirty` + `apply_snapshot` only. `bench_smt_apply_snapshot_batch`
/// above measures COLD inserts into an empty tree; production epochs re-touch
/// existing accounts, which is this shape.
fn bench_smt_apply_snapshot_reflow(c: &mut Criterion) {
    let mut group = c.benchmark_group("smt_apply_snapshot_reflow");
...
criterion_group!(
    ...
    bench_smt_apply_snapshot_batch,
    bench_smt_apply_snapshot_reflow,
