...
fn bench_ledger_derivation(c: &mut Criterion) {
    ...
    for &count in &[100, 1000, 10000] {
        ...
        let mint_rec = {
            ...
            __r.id = "mint-0".into();
            __r.version = 1;
            __r.content_hash = vec![0; 32];
            __r.creator_public_key = genesis.public_key.clone();
            __r.timestamp = 0.0;
            __r.parents = vec![];
            __r.classification = Classification::Public;
            __r.metadata = mint_meta.clone();
            __r.signature = None;
        ...
        for i in 0..count {
            ...
            let rec = {
                let mut __r = ValidationRecord::fixture_shell();
                __r.id = format!("xfer-{i}");
                __r.version = 1;
                __r.content_hash = vec![0; 32];
                __r.creator_public_key = alice.public_key.clone();
                __r.timestamp = (i + 1) as f64;
                __r.parents = vec![];
                __r.classification = Classification::Public;
                __r.metadata = meta;
                __r.signature = None;
