...
# Bounded window: drop harvested files whose act left the newest-N set. Runs
# even when the harvest branch above was SKIPPED: a skipped harvest writes no
# browser_verify flags into this snapshot, so files left on disk are envelopes
# the page no longer references — dead weight the mirror would keep shipping
# (and a stale-envelope trap if a reader deep-links one). Snapshot flags and
# on-disk files must leave this script consistent on every path.
