...
/// Wire format version — the DECODE CEILING (highest version `read_header`
/// accepts), NOT the version fresh records are stamped with (that is
/// [`CURRENT_SIGNING_VERSION`]; the split is T65 D4 Step 2).
...
/// (T65 D4 Step 2: "decode-capable everywhere before emitting anywhere; Seed
/// flips last"). Raising THIS to 6 is the flag-day flip and gets its own
...
/// metadata that to_bytes could never re-emit — silent round-trip corruption),
/// and no stored or frozen artifact is below v4 (KAT floor = v4; live store
/// verified v5-only at the frontier, earliest record 2026-07-04 vs v5 emission
/// since 2026-04-16). The JSON metadata branch was deleted in the SAME commit —
/// the two moves are one atomic invariant. The further 4→5 raise is a separate
...
mod tests {
    ...
    fn batch_b_wire_module_constants_pinned_to_literal_values() {
        ...
        // clippy::assertions_on_constants exists to flag in ordinary code.
        #[allow(clippy::assertions_on_constants)]
        {
            assert!(
                CURRENT_SIGNING_VERSION <= WIRE_VERSION,
                "emission version must never exceed the decode ceiling"
            );
        }

        // WIRE_VERSION_MIN — backward-compat floor for read_header. CONSCIOUS
        // EDIT 2026-08-18 (T63 flag-day verdict, Phase B): 1 → 4, retiring the
        // v1-v3 decode path together with its T80 JSON-metadata asymmetry.
        // This pin is a deliberate tripwire: any future raise (4→5 is the
        // queued T65 D4 Step 3 pass) must edit this line consciously, in the
        // same commit as the constant, with the store histogram in hand.
        assert_eq!(WIRE_VERSION_MIN, 4);

        // MAX_NETWORK_ID_LEN — v6 network_id decode cap.
        assert_eq!(MAX_NETWORK_ID_LEN, 64);
