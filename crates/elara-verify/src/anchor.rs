...
/// What object a verified drand pulse's not-before actually bounds. This is
/// load-bearing for honest claims (crown-F1): a pulse EMBEDDED IN A SEAL bounds
/// the seal's own creation, but a pulse an existed-by ANCHOR fetched fresh at
/// stamping time (`scripts/elara-epoch-anchor.sh` pulls `/public/latest`) bounds
/// only the ANCHOR's construction — the seal it commits to already existed, so
/// an anchor's pulse is NOT a lower bound on the seal's existence. Conflating the
/// two overclaimed the seal's floor by the seal→anchor latency (10.5 min in the
/// shipped sample). The leg is shared, so the subject must be passed in.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PulseSubject {
...
impl PulseSubject {
    /// Subject clause opening the detail line — who cites the round (neutral,
    /// so it reads correctly in the forged-signature Fail arm too).
    fn cites(self) -> &'static str {
        match self {
            Self::Seal => "the seal cites",
...
/// `loe_param_conflict` fail-closed gate). `subject` picks whether the
/// verified bound is attributed to the seal or (only) the anchor (crown-F1).
pub fn drand_not_before_leg(
    obj: &Value,
        ...
        Some(r) if r > 0 => {
            ...
            let (status, detail) = match bls {
                ...
                DrandBls::Failed(why) => (
                    Status::Fail,
                    format!(
                        "{} {beacon_label} round {r} but its drand BLS signature DOES \
...
pub fn seal_drand_leg(wire: &[u8], checks: &mut Vec<Check>) {
    if let Ok(rec) = ValidationRecord::from_bytes(wire) {
        if rec.metadata.contains_key("drand_round") {
            ...
            drand_not_before_leg(&drand_obj, PulseSubject::Seal, checks);
        }
    }
}
...
pub fn verify_anchor_obj(obj: &Value, checks: &mut Vec<Check>) -> Result<AnchorSummary, String> {
    ...
    // through one implementation). Subject = Anchor: the artifact's pulse is one
    // the anchor fetched fresh at stamping time, so it bounds the anchor's
    // freshness, NOT the seal's existence (crown-F1).
