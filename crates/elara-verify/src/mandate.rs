...
impl MandateScope {
    ...
    /// Lowercase every op/zone token. The matchers (`allows_op` /
    /// `allows_zone`) are exact, and the canonical op vocabulary
...
impl MandateRecord {
    ...
    /// **Naming caveat (corrected 2026-07-29 — the previous wording was false).**
    /// This comment used to say *"The principal's signature (the carrier record's
    /// signature) is over these bytes"*. It is not, and nothing in the tree ever
    /// verifies a signature over them: the ONLY caller of this function is
    /// [`Self::mandate_id`]. The carrier record's Dilithium3 signature covers the
    /// record's own `signable_bytes()` (which includes the mandate JSON as signed
    /// metadata) — not this preimage. The real principal binding is an equality
    /// check at ingest: the carrier's creator identity MUST equal
    /// `principal_identity_hash` (`src/network/mandate_node.rs:126-129`), which is
    /// what makes a mandate unforgeable by a third party.
    ///
    /// So read "signing bytes" as **id-derivation bytes with signing-grade
    /// discipline** (domain-separated, length-prefixed, canonically ordered) — the
    /// discipline is real and worth keeping, because it is what makes `mandate_id`
    /// a stable content address. Just do not infer a verified signature from the
    /// name. [`Self::mandate_id`] is `sha3` of these bytes.
    ///
    /// ⚠ Do not confuse this with its same-named neighbour
    /// `RealmCert::canonical_signing_bytes` (`src/network/realm.rs:90`), which IS
    /// genuinely signed and verified (`realm.rs:130` signs, `realm.rs:188`
    /// dilithium3-verifies). The identical method name across the two types is the
    /// likely origin of the false claim corrected above — anyone grepping the name
    /// finds a real sign/verify pair and assumes it applies here.
...
pub enum MandateFlag {
    ...
    NotYetValid = 7,
    /// The sub-delegation chain could not be verified on this node: an ancestor is
    /// missing (sync lag or a fabricated lineage no honest node will hold),
    /// malformed/cross-network, or a genealogy link (`child.principal ==
    /// parent.agent`) is broken. Honest "not verified" — never a false `Valid` or
    /// `NoChain`. Live verdict from `walk_chain`.
    UnverifiedChain = 8,
    /// The sub-delegation chain exceeded a depth cap — the global
    /// `MANDATE_MAX_CHAIN_DEPTH`, or an ancestor's own `sub_delegation_max_depth`
    /// (the minimum down the chain binds). Live verdict from `walk_chain`.
    DepthExceeded = 9,
    /// A hop's scope (op/zone/amount or validity window) broadened its parent's,
    /// violating monotone narrowing — a forged privilege escalation through an
    /// intermediate. Live verdict from `walk_chain`.
    ScopeBroadened = 10,
    /// RESERVED (v1): a revocation signed by a party not authorized to revoke — a
    /// property of a revocation record, not an act, so it is never returned by the
    /// act verifier (revocation authority is enforced at read time by the resolver
    /// lookup key).
