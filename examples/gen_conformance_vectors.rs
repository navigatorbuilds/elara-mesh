...
fn main() {
    ...
    let seal = fs::read("examples/verify/epoch-41340-zone-0.seal.wire").expect(
        "run from the repo root: examples/verify/epoch-41340-zone-0.seal.wire not found",
    );
    ...
    let wire_v6 = fs::read("examples/verify/sample-record-v6.wire").expect(
        "run from the repo root: examples/verify/sample-record-v6.wire not found \
         (frozen by `cargo test --lib v6_fixture_generator_t63 -- --ignored`)",
    );
    let set = elara_runtime::conformance::generate_vector_set(&wire, &seal, &anchor, &wire_v6)
        .expect("derive conformance vectors from the committed sample/seal/anchor/v6 inputs");
