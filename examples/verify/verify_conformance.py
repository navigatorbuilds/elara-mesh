...
def reproduce(vec: dict) -> str:
    ...
    if prim == "domain-separation":
        # T63 v6 — recompute the byte prefix that leads every v6 signing
        # preimage from the vector's OWN inputs: ASCII tag ‖ u16-BE len ‖
        # network_id. Independent of any wire file; a wrong prefix silently
        # corrupts every v6 record_hash while leaving v5 correct — this vector
        # is the cheap standalone check for that.
        tag = inp["tag_ascii"].encode("ascii")
        nid = inp["network_id"].encode("ascii")
        return (tag + len(nid).to_bytes(2, "big") + nid).hex()
