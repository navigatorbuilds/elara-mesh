...
```
✗ FAILED — 1 of 4 checks failed (signature); the evidence is forged, tampered, or inconsistent — do not rely on it.
...
Exit code `1`. It names **exactly** the one check that broke (`signature`) and
...
`epoch-41340-zone-0.json` is an epoch-anchor artifact. It carries the drand
beacon round **and the beacon's BLS signature**, so the anchor's freshness is
verifiable, not just cited. Beside it are the proofs it needs, all offline:

| File | What it proves |
|------|----------------|
| `epoch-41340-zone-0.json.ots`   | OpenTimestamps: a SHA-256 path committing the seal into a **Bitcoin block's** merkle root |
| `btc-header-957487.txt`         | the archived 80-byte header of that block — so the existed-by check needs no Bitcoin node and no calendar server |
| `epoch-41340-zone-0.json.tsr`   | an RFC-3161 timestamp token — an *independent* TSA witness, **not checked by `elara-verify`** (verify separately with `openssl ts -verify`) |
| `epoch-41340-zone-0.json.qtsr`  | an **eIDAS-qualified** timestamp token — a statutory-grade witness, **not checked by `elara-verify`** (verify with an eIDAS validator) |
...
seal's floor would over-tighten the seal's existence by that ~10½-minute gap; the
verifier does not.
...
> locally — verifies as **PARTIAL** (exit 3): the anchor's drand freshness holds,
...
```bash
$V examples/verify/sample-record.wire --wire \
   --anchor examples/verify/epoch-41340-zone-0.json
...
elara-verify --account-inclusion account-proof.json \
  --seal epoch-41340-zone-0.seal.wire \
  --trusted-anchor "$(cat zone-0-anchor-pubkey.hex)" \
  --expected-hash 826306639200879beac7fc073166d18b968ad73756bbeefe836bdd60b557d3b7
```
...
epoch**, so this chain reproduces against any live node.
...
`sample-receipt.json` here carries the same record and epoch-41340 seal as the
legs above:
...
`elara-verify --inclusion` command checks against a sealed root. It is a
...
anchor-signed epoch seal (`epoch-41340-zone-0.seal.wire` — the same epoch
...
the real bundled epoch seal. It needs a PQ library, so without `python-oqs`/liboqs it
...
Harvested from the live Elara testnet as fixed snapshots that stay valid
...
- **The seal (legs 5–6):** the zone-0 **epoch-41340** seal
  ...
  (`zone-0-anchor-pubkey.hex`). This seal carries its **own embedded drand
  pulse** (round 6276475, published 2026-07-10 23:14:30 UTC — the producer ran
  the opt-in `drand_pulse_enabled` fetcher), distinct from the anchor's pulse
  below: two independently fetched rounds minutes apart, each the freshest its
  producer had when it acted — which is what genuine live fetching looks like
  (copied values would match exactly). To see the seal's own drand leg pass
  with no anchor at all:
  `elara-verify --seal epoch-41340-zone-0.seal.wire --trusted-anchor "$(cat zone-0-anchor-pubkey.hex)" --expected-hash 826306639200879beac7fc073166d18b968ad73756bbeefe836bdd60b557d3b7`
  — the same check `verify.sh` runs inside leg 5.
- **The anchor (legs 2–4, 0d, 0e):** `epoch-41340-zone-0.json` — the
  existed-by artifact for **that same seal**: published against drand round
...
naming a *different* seal fails `seal↔anchor` (exit 1) rather than borrowing
the bracket.

(An earlier bundle vintage — epoch 3217 of the pre-re-genesis chain,
Bitcoin block 953657 — was superseded here on 2026-07-11 but is not
weakened by retirement: "those seal bytes existed by block 953657" is a
timeless statement, its pin remains compiled into the verifier, and the
archived bundle stays verifiable forever from git history.)
