...
What it checks (all offline — no node, no calendar server, no network syscall):
  ...
  1. ARTIFACT BINDING   ``SHA-256(epoch-3217-zone-0.json)`` equals the digest the
                        ``.ots`` proof commits to — so the proof is for *this*
                        anchor (which names seal ``c2bc324c…``), not some other file.
...
# examples/verify demo seal (epoch 3217, zone 0; the seal verify.sh legs 1-4 use).
ANCHOR_JSON = HERE / "epoch-3217-zone-0.json"
OTS_PROOF = HERE / "epoch-3217-zone-0.json.ots"
