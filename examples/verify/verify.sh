...
# confirmed (block 953657 archived), so the demo REQUIRES exit 0 — a 3 here means
...
SEAL_WIRE="$HERE/epoch-8219-zone-0.seal.wire"
ANCHOR_PK="$(cat "$HERE/zone-0-anchor-pubkey.hex")"
SEAL_HASH="d0742b057c228305f1ae36e60230dd68fdbbef65ac339eea2963ca44ad3e6f04"
...
# The Rust elara-verify legs 1-4 below prove the trustless time bracket (drand
# not-before + Bitcoin existed-by). This leg re-derives the *upper* (Bitcoin)
...
# ── 0e. drand not-before — independent BLS verification ──────────────────────
# The companion to leg 0d: it re-derives the *lower* (drand not-before) bound of
# the trustless bracket in a second, non-Rust toolchain — py_ecc.bls (the Ethereum
# Foundation's pure-Python BLS12-381 reference, independent of the Rust drand-verify
# backend) verifies the League-of-Entropy beacon signature over the chained-beacon
# message against a key PINNED in the script. Fail-closed (exit 1) on a bad sig /
# substituted key; graceful-skip (exit 3) when no BLS library is installed (most
# boxes — `pip install py_ecc` enables it), never a fake green. Together 0d+0e put
# BOTH ends of the time bracket in an independent toolchain.
if [ -n "$PY" ] && [ -f "$HERE/verify_drand.py" ]; then
  echo
  echo "── 0e. drand not-before — independent BLS verify (py_ecc) ────────────"
  "$PY" "$HERE/verify_drand.py"; drand_rc=$?
  if [ "$drand_rc" -eq 0 ]; then
    echo "  → PASS (py_ecc independently reproduces the drand not-before bound)"
...
run "2. anchor — fully time-bracketed: a BLS-verified drand not-before below and a Bitcoin existed-by above, both proven offline (this sample is confirmed in block 953657; a still-pending anchor would exit 3, not 0)" \
    --anchor "$HERE/epoch-3217-zone-0.json"
...
run_expect_partial "3. record + anchor together — two INDEPENDENT proofs, no binding claimed: both facts are proven but the record↔seal link is ⚠ UNPROVEN, so the combined verdict is honestly PARTIAL (exit 3), never an implied chain-VERIFIED; --inclusion + --seal bind the chain (leg 5)" \
    "$HERE/sample-record.wire" --wire --anchor "$HERE/epoch-3217-zone-0.json"
...
# anchor JSON is present. The verifier keeps the trustless drand not-before (✓) and
# marks the Bitcoin upper bound ⚠ UNPROVEN — PARTIAL (exit 3), never a false VERIFIED.
PARTIAL_DIR="$(mktemp -d)"
trap 'rm -rf "$PARTIAL_DIR"' EXIT
cp "$HERE/epoch-3217-zone-0.json" "$PARTIAL_DIR/"   # NB: the .ots is deliberately NOT copied
run_expect_partial "4. honest failure by design — the SAME anchor with its Bitcoin existed-by proof withheld: the verifier keeps the trustless drand not-before but marks the upper bound ⚠ UNPROVEN (exit 3 PARTIAL), it does NOT fake a green ✓" \
    --anchor "$PARTIAL_DIR/epoch-3217-zone-0.json"
...
# re-harvested 2026-06-16 from a node re-genesis'd onto the new construction. This
# seal is validator-signed (PQ trust chain via --trusted-anchor), NOT Bitcoin-
# anchored like the epoch-3217 seal that legs 1-4 use — leg 5's verdict makes no
# Bitcoin claim. The legs run only when account-proof.json carries the compressed-
...
if grep -q '"present"' "$HERE/account-proof.json"; then
  ...
  account_skipped=1
  echo
  echo "── 5 & 6. account-chain legs — SKIPPED (bundle pending re-genesis) ──"
  echo "  account-proof.json predates the 256-bit identity-bound compressed"
  echo "  account-SMT. The Bitcoin-anchored"
  echo "  account bundle will be re-harvested + re-anchored after the testnet"
  echo "  re-genesis. The record + anchor legs above are unaffected."
fi
