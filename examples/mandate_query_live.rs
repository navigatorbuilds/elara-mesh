...
use elara_runtime::mandate_sdk::{
    Coverage, CoverageBasis, MandateActVerdict, MandateDetail, MandateQueryClient, MandateSdkError,
};
...
fn coverage_note(c: Coverage) -> String {
    match c {
        Coverage::Authoritative => {
            "authoritative (full-history node — answer is complete)".to_string()
        }
        Coverage::Incomplete { basis, acts_complete_from_ms } => {
            let reason = match basis {
                CoverageBasis::SnapshotBaseline => "snapshot follower",
                CoverageBasis::PrunedOrUpgradeBaseline => "acts pruned / pre-upgrade baseline",
                CoverageBasis::ZoneSubset => "zone-scoped node (omits out-of-zone acts)",
                CoverageBasis::StorageError => "a storage read faulted",
                CoverageBasis::Unknown => "incomplete (unrecognized reason — newer node)",
                _ => "incomplete",
            };
            match acts_complete_from_ms {
                Some(floor) => format!(
                    "⚠ {reason} — a negative is authoritative only for acts at/after {floor} ms; \
                     below that, query a full-history node"
                ),
                None => format!(
                    "⚠ {reason} — a negative may be a false negative; query a full-history node"
                ),
            }
        }
        // `Coverage` is #[non_exhaustive]: a newer node may introduce a coverage
        // shape this client predates — treat it as incomplete, never authoritative.
        _ => "⚠ coverage shape unrecognized (newer node) — treat as incomplete".to_string(),
    }
}
...
fn print_verdict(label: &str, v: &MandateActVerdict) {
    ...
    match v {
        ...
        MandateActVerdict::NotAMandateAct { coverage } => {
            println!("     · not a mandate act (AUTHORITATIVE on this node)");
            println!("       • {}", coverage_note(*coverage));
        }
        MandateActVerdict::UnknownOutsideCoverage { coverage } => {
            println!("     ? UNKNOWN — absent, but this node's act-index coverage is INCOMPLETE for it");
            println!("       — NOT a confident 'not a mandate act'. {}", coverage_note(*coverage));
        }
        MandateActVerdict::RefutedForClaim { coverage, claimed_ms } => {
            println!("     ✗ REFUTED for claimed time {claimed_ms} ms — absent within this node's covered window");
            println!("       • {}", coverage_note(*coverage));
        }
