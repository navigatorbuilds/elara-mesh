...
- **Public dev-net soak** — external machines joining over the public
  ...
  join: 2026-07-02).
- **Drand time-bounds, network-default** — the beacon fetcher has shipped
  (opt-in, `drand_pulse_enabled`) and the offline verifier validates
  pulse-carrying seals today (a production sample ships in `examples/verify/`);
  the missing half is making it the *network default*, so every seal — not
  just an enabled operator's — carries the verifiable *not-before* bound.
