...
mod tests {
    ...
    // ── Tampered-but-well-formed SIGNED records FAIL CLOSED (CLI-parity) ─────
    // The `scripts/verify-fuzz-fail-closed.sh` negative contract, proven at the
    // shipping wasm artifact: a record that PARSES cleanly but whose signature
    // no longer authenticates its bytes must grade FAILED through the browser
    // export — never a VERIFIED it cannot prove, never a soft PARTIAL that hides
    // a forgery. The happy-path tests above lock the same fixture to VERIFIED,
    // so a flip to FAILED here is a true negative, not a broken fixture. Three
    // distinct attack classes: forged signature, forged content, swapped
    // creator identity. Without these, the CLI has an end-to-end fail-closed
    // probe the browser widget lacks — and the widget is the public exhibit.

    #[test]
    fn wasm_shell_rejects_a_signed_record_with_a_corrupted_signature() {
        let (mut rec, _) = signed_record(b"authentic payload");
        // Baseline: the untampered fixture MUST verify (else the tamper is vacuous).
        let ok = verify_receipt_inner(&serde_json::to_string(&rec).expect("json"), "");
        assert_eq!(ok.verdict, "VERIFIED", "fixture must verify before tampering");
        // Flip one byte of the Dilithium3 (ML-DSA-65) signature.
        rec.signature.as_mut().expect("signed fixture")[0] ^= 0xFF;
        let r = verify_receipt_inner(&serde_json::to_string(&rec).expect("json"), "");
        assert_eq!(
            r.verdict, "FAILED",
        ...
        assert!(
            !r.checks.iter().any(|c| c.name == "signature" && c.status == "pass"),
            "signature check must not read pass on a mangled signature",
        );
    }

    #[test]
    fn wasm_shell_rejects_a_signed_record_with_forged_content() {
        // content_hash is inside signable_bytes(), so mutating it after signing
        // breaks the signature — the "swap the validated payload, keep the sig"
        // forgery (CLI T2 --expect-hash analog).
        let (mut rec, _) = signed_record(b"authentic payload");
        rec.content_hash[0] ^= 0xFF;
        let r = verify_receipt_inner(&serde_json::to_string(&rec).expect("json"), "");
        assert_eq!(
            r.verdict, "FAILED",
            "forged content must FAIL closed: {}",
            r.headline
        );
        assert!(
            !r.checks.iter().any(|c| c.name == "signature" && c.status == "pass"),
            "signature must not verify over a mutated content_hash",
        );
    }

    #[test]
    fn wasm_shell_rejects_a_signed_record_with_a_swapped_creator_key() {
        // creator_public_key is both the verify key AND inside signable_bytes():
        // presenting a DIFFERENT valid identity's key can never authenticate a
        // signature made by the original signer (CLI T5 --expect-identity analog).
        let (mut rec, _signer) = signed_record(b"authentic payload");
        let intruder = Identity::generate(EntityType::Device, CryptoProfile::ProfileB)
            .expect("generate intruder identity");
        rec.creator_public_key = intruder.public_key.clone();
        let r = verify_receipt_inner(&serde_json::to_string(&rec).expect("json"), "");
        assert_eq!(
            r.verdict, "FAILED",
            "creator-key swap must FAIL closed: {}",
            r.headline
        );
        assert!(
            !r.checks.iter().any(|c| c.name == "signature" && c.status == "pass"),
            "signature must not verify under a swapped creator key",
        );
    }
}
