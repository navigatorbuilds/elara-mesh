...
notarized into Bitcoin — not the seal's birth. The upper bound is the one claim
about the seal: it **existed by** the Bitcoin block. The seal's own not-before is
an earlier, separate bound, proven from the drand pulse the seal *itself* embeds
(the `--seal` leg — see "The full chain"). Pairing a `<record>` with `--anchor`
verifies both as independent facts; binding the record *into* that seal needs
`--inclusion` + `--seal`. The verifier prints a NOTE saying so when a record and an
anchor are checked together without that binding.

(A signature-less legacy artifact shows the freshness bound as `(reference)`
instead, and the verdict names which — the bound is never claimed stronger than
what was checked.)
