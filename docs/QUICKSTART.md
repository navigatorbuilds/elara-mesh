...
> **Here to give an AI agent a signed, revocable mandate with receipts?**
> That flow — your own chain, your agent, issue → act → check → revoke →
> verify offline — is [`docs/QUICKSTART-ISSUER.md`](QUICKSTART-ISSUER.md).
...
- **Three free local ports.** A node binds more than the one you curl:

  | Port | What | Bound on |
  |---|---|---|
  | `9473` | HTTP API — the one every example below uses | your `--listen` address |
  | `9573` | PQ data channel (always HTTP **+ 100**) — all sync/snapshot/gossip | your `--listen` address |
  | `9472` | internal data plane (admin + node-to-node control) | `127.0.0.1` only |

  If any of them is already taken the node exits immediately with
  `fatal: Network error: failed to bind … Address already in use`. Override with
  `--listen <addr>` (moves 9473 and, with it, the PQ port) and
  `ELARA_DATA_PLANE_LISTEN=127.0.0.1:<port>`.
...
#
# Recommended FIRST: encrypt the key at rest. Without this the node prints
#   "WARNING: no ELARA_IDENTITY_PASSPHRASE set — saving identity in PLAINTEXT"
# and stores your secret keys unencrypted. Set it before generating, and keep it
# — the identity cannot be recovered without it.
export ELARA_IDENTITY_PASSPHRASE='<a strong passphrase you will not lose>'
...
**Straight after startup you will see something else** — `{"status":"warming"}`
(readiness `orange`) with an **empty** `checks` array, because the health probes
have not run yet. That is normal; give it up to a minute and re-curl.

Once warm, a **solo** node reports `{"status":"degraded",...}` (readiness
`yellow`), and that is **expected** — the only failing check is `peers` ("no
connected peers (running solo)"); the rest read `ok`. `"status":"healthy"` requires connected
