...
- **G5 — realm domain-separator in the signed pre-image (closes A8).** A
  ...
  **SEVERITY ANNOTATION (2026-08-02, follow-up audit — the deferral above STANDS,
  ...
  *ingested* untagged — they are also *co-signed* untagged.** The node exposes a
  witness path that verifies a caller's signature over `signable_bytes()` and then
  signs that same preimage with the node's own identity key. Because the preimage
  carries no network id (the very gap G5 exists to close), such a co-signature is
  not scoped to the network that produced it. So the residual is not merely
  "foreign records ingest as inert spam but never settle" (A8, still accurate) —
  it also includes an externally presentable co-signature that no longer carries
  its own context.

  This is **not** reachable by the transport-level network header: that header is
  client-supplied and is not consulted on the co-signing path at all. It is
  therefore direct evidence that network identity must live in the *signed
  preimage*, and it strengthens the case for the `version >= 6` domain-tag branch
  specified above as the only real fix. Detailed analysis, affected paths and the
  implementation ruling are tracked internally with the T65 work item; this note
  records the severity change against the June decision, which is what a reader of
  *this* document needs.
