...
method+path with `elara-cli`. Requests without a valid signed header receive
HTTP 422.
...
# Per call: sign the exact method + path, then send the header.
...
Read-only diagnostic endpoints (`/admin/gc` GET, `/admin/dag_check`, `/admin/fork_check`, `/admin/revocations`, `/admin/key_rotations`, `/admin/witness_liveness`, `/admin/sunset`) do not require auth.
