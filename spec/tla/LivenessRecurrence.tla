...
(*                                                                         *)
(* THE MECHANISM UNDER TEST: the CHAINED BEACON                            *)
(* chained_beacon(prev_seal_hash, epoch, zone) (aggregator.rs), a SHA3-256 *)
(* hash, not a VRF. The proposer rank ladder is re-derived every epoch off *)
(* the PREVIOUS epoch's seal hash, which is not known until that epoch     *)
(* seals. THIS MODEL ASSUMES the beacon is not steerable. The shipped      *)
(* beacon meets that only while the previous sealer is honest: a sealer    *)
(* chooses fields of its own seal, so a Byzantine sealer can search them   *)
(* to influence the next epoch's rank order (planned fix: an unsteerable   *)
(* beacon). Liveness.tla bounds the worst SINGLE epoch; this model proves  *)
(* the worst case cannot RECUR indefinitely under a re-randomizing beacon, *)
(* and FAILS (non-vacuously) under a grindable / adversary-pinned beacon.  *)
(*                                                                         *)
...
(* adversary does NOT choose (the modelling assumption stated above).      *)
...
(*       the headline contribution: it machine-checks that a               *)
(*       re-randomizing beacon is what keeps the protocol on its fast      *)
(*       path.                                                             *)
(*                                                                         *)
...
(* This is faithful to is_zone_stuck (aggregator.rs): escalation unlocks      *)
...
(***************************************************************************)
(* The chained beacon advances. Once the epoch sealed, the chain moves to  *)
