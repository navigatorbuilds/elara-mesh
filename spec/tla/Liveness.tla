...
(* leader+view-change:                                                      *)
(*                                                                         *)
(*   1. PROPOSAL — a hash-stake RANK LADDER. proposer_rank (aggregator.rs: *)
(*      290) ranks identities by hash(beacon||zone||id)/isqrt(stake);      *)
...
(* eventually seals, []<>sealed — rests on the CHAINED BEACON              *)
(* chained_beacon(prev_seal_hash, epoch, zone) (aggregator.rs): the        *)
(* proposer ranks RE-RANDOMIZE every epoch off the previous seal hash. The *)
(* worst-case single epoch modelled here is an upper bound on per-epoch    *)
(* stall; cross-epoch []<>sealed (every epoch seals, AND the FAST local    *)
(* path recurs under a beacon that re-randomizes ranks) is machine-checked *)
(* in LivenessRecurrence.tla (Phase E.3) — see that module + the README.   *)
(* That model ASSUMES the beacon is not steerable; the shipped beacon      *)
(* meets this only while the previous sealer is honest, since a Byzantine  *)
(* sealer can search its own seal's fields to influence the next epoch's   *)
(* rank order.                                                             *)
(*                                                                         *)
...
CONSTANTS
    Ranks,               \* set of proposer ranks (naturals 0..R-1), hash-stake ordered
