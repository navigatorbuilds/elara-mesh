...
(*                                                                         *)
(* REFINEMENT MAP (TLA+ operator  <->  Rust function, file + symbol):      *)
(*   CorrQ          <-> ConsensusState::correlation_weighted_q  consensus.rs      *)
(*   IndependenceQ  <-> ConsensusState::independence_q          consensus.rs      *)
(*   EffStakeQ      <-> ConsensusState::effective_stake_q       consensus.rs      *)
(*   DiverseSettled <-> ConsensusState::diverse_threshold_met_q consensus.rs      *)
(*                      reached via   is_settled_diverse        consensus.rs      *)
...
(***************************************************************************)
(* Fixed-point constants: ALPHA_Q / BETA_Q / GAMMA_Q at consensus.rs.         *)
...
\* correlation_weighted_q(a, b, gamma_q)   (consensus.rs).
...
\* independence_q(a, S)   (consensus.rs):  d_q = Q^2 \div (Q + corr_sum).
\* `\div` is integer floor division, matching Rust's u128 `/`.
IndependenceQ(a, S) == (Q * Q) \div (Q + CorrSumQ(a, S))

\* effective_stake_q(S)   (consensus.rs): Sum over w in S of stake[w]*d_q(w,S).
...
\* diverse_threshold_met_q(eff_q, eligible)   (consensus.rs):
\* settle iff  3*eff_q >= 2*eligible*Q.  Reached via is_settled_diverse (consensus.rs).
