...
(* `elara_xzone_sealed_locked_past_expiry_count` (network/server/mod.rs:7759, *)
(* fed by sealed_locked_past_expiry_count cross_zone.rs:1111), which exists *)
...
(*     (REAP_HORIZON_SECS, compute_stale_reap_batch cross_zone.rs:1041 /    *)
(*     apply_reap_batch (:1074)) eventually refunds the lock. The code is   *)
(*     explicit that without a quorum a sealed transfer "stays Locked       *)
(*     indefinitely" (epoch.rs:7282, cross_zone.rs:899) — so the real       *)
...
(*                        pending_abort_candidates cross_zone.rs:857 /      *)
(*                        emitter try_sign_xzone_abort epoch.rs:7563). gst  *)
...
(*                        even with no GST.                                  *)
(*   refunded/Refunded<-> apply_reap_batch (cross_zone.rs:1074) sets status *)
...
(* abstracts the BEST-EFFORT retry-on-next-tick emitter (epoch.rs:7285 uses *)
