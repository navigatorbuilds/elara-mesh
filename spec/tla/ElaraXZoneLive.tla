...
(* `elara_xzone_sealed_locked_past_expiry_count` (network/server/mod.rs,      *)
(* fed by sealed_locked_past_expiry_count cross_zone.rs), which exists      *)
(* precisely to detect stuck-sealed transfers.                              *)
(*                                                                         *)
...
(*     (REAP_HORIZON_SECS, compute_stale_reap_batch in cross_zone.rs)       *)
(*     eventually refunds via apply_reap_batch (cross_zone.rs). The code is *)
(*     explicit that without a quorum a sealed transfer "stays Locked       *)
(*     indefinitely" — epoch_seal_loop in epoch.rs — so the real            *)
(*     never-stuck-FOREVER guarantee is the reaper:                         *)
(*     apply_reap_batch in cross_zone.rs bounds the stuck window at ~30     *)
(*     days, NOT the abort quorum. Omitting the reaper would model a        *)
(*     path the code treats as best-effort while ignoring the path the      *)
(*     code actually relies on; LiveBackstop closes that gap.               *)
(*                                                                         *)
...
(*                                                                         *)
(* REFINEMENT MAP (TLA+ <-> Rust) — apply_reap_batch in cross_zone.rs,      *)
(* epoch_seal_loop in epoch.rs:                                             *)
(*   Quorum(S)        <-> count-based 2/3 gate shared by                    *)
(*                        verify_finality_quorum and verify_abort_quorum.   *)
...
(*                        pending_abort_candidates cross_zone.rs /          *)
(*                        emitter try_sign_xzone_abort epoch.rs). gst       *)
(*                        abstracts gossip delivery.                        *)
(*   SignClaim/Abort  <-> a committee member adding its attestation.        *)
(*   ExpireWindow     <-> wall clock passing expires_at                     *)
(*                        (CLAIM_TIMEOUT_SECS, cross_zone.rs). NOT gst-gated*)
(*                        (time elapses regardless of synchrony).           *)
(*   ReapWindow/Reap  <-> the 30-day stale-reap, quorum-free, gated on      *)
(*                        expires_at + REAP_HORIZON_SECS (cross_zone.rs).   *)
(*                        NOT gst-gated — this is why LiveBackstop holds     *)
(*                        even with no GST.                                  *)
(*   refunded/Refunded<-> apply_reap_batch (cross_zone.rs) sets status      *)
(*                        Refunded — the SEALED 30-day reap terminal; the   *)
(*                        unsealed passive-24h refund path is out of scope. *)
(*                                                                         *)
(* FIDELITY of the fairness (honest-claims, audited 2026-06-28): honest WF  *)
(* abstracts the BEST-EFFORT retry-on-next-tick emitter — epoch_seal_loop   *)
(* in epoch.rs uses try_read/try_lock and "re-attempts on the next tick",   *)
(* which under a fair scheduler eventually wins its locks — NOT a           *)
(* "deterministic per-tick emission". Non-seal-eligible committee members   *)
(* rely on gossip relay rather than self-emission; the model abstracts      *)
(* both as the per-honest                                                   *)
